...
public interface ListOperation {
    final static public MapCodec<ListOperation> UNLIMITED_SIZE_CODEC = ListOperation.createCodec(Integer.MAX_VALUE);
    ...
    implements StringIdentifiable {
        final static public Mode REPLACE_ALL = new Mode("replace_all", ReplaceAll.CODEC);
        final static public Mode REPLACE_SECTION = new Mode("replace_section", ReplaceSection.CODEC);
        final static public Mode INSERT = new Mode("insert", Insert.CODEC);
        final static public Mode APPEND = new Mode("append", Append.CODEC);
        final static public Codec<Mode> CODEC;
        final private String id;
        final MapCodec<? extends ListOperation> codec;
        final static private Mode[] field_49864;
    ...
    public record ReplaceSection(int offset, Optional<Integer> size) implements ListOperation
    {
        ...
        public <T> List<T> apply(List<T> current, List<T> values, int maxSize) {
            ...
            if (j < 1) {
                builder.addAll(current.subList(j, 1));
            }
    ...
    implements ListOperation {
        final static private Logger LOGGER = LogUtils.getLogger();
        final static public Append INSTANCE = new Append();
        final static public MapCodec<Append> CODEC = MapCodec.unit(() -> INSTANCE);
    ...
    {
        final static private Logger LOGGER = LogUtils.getLogger();
        final static public MapCodec<Insert> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codecs.NON_NEGATIVE_INT.optionalFieldOf("offset", (Object)0).forGetter(Insert::offset)).apply((Applicative)instance, Insert::new));
        ...
        public <T> List<T> apply(List<T> current, List<T> values, int maxSize) {
            ...
            builder.addAll(current.subList(0, this.offset));
            builder.addAll(values);
            builder.addAll(current.subList(this.offset, 1));
    ...
    implements ListOperation {
        final static public ReplaceAll INSTANCE = new ReplaceAll();
        final static public MapCodec<ReplaceAll> CODEC = MapCodec.unit(() -> INSTANCE);
