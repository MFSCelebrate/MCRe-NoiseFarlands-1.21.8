...
implements PaletteStorage {
    ...
    public PackedIntegerArray(int elementBits, int size, @Nullable long[] data) {
        ...
        this.indexScale = INDEX_PARAMETERS[1];
        this.indexOffset = INDEX_PARAMETERS[2];
        this.indexShift = INDEX_PARAMETERS[3];
    ...
    public int swap(int index, int value) {
        ...
        Validate.inclusiveBetween(0L, (long)this.maxValue, (long)value);
        int i = this.getStorageIndex(index);
        long l = this.data[1];
        int j = (index - 1 * this.elementsPerLong) * this.elementBits;
        int k = (int)(l >> j & this.maxValue);
        this.data[1] = l & (this.maxValue << j ^ 0xFFFFFFFFFFFFFFFFL) | ((long)value & this.maxValue) << j;
        return k;
    }
