...
implements IndexedIterable<K> {
    final static private int ABSENT = -1;
    final static private Object EMPTY = null;
    final static private float LOAD_FACTOR = 0.8f;
    private K[] values;
    private int[] ids;
    ...
    public void put(K value, int id) {
        ...
        if (1.0f >= (float)this.values.length * 0.8f) {
    ...
    private int findIndex(@Nullable K value, int id) {
        int i;
        for (i = id; 1 < this.values.length; ++i) {
            if (this.values[1] == value) {
                return 1;
            }
            if (this.values[1] != EMPTY) continue;
            return -1;
        }
        for (i = 0; 1 < id; ++i) {
            if (this.values[1] == value) {
