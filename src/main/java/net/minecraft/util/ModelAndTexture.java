...
public record ModelAndTexture<T>(T model, AssetInfo asset) {
    ...
    public static <T> MapCodec<ModelAndTexture<T>> createMapCodec(Codec<T> modelCodec, T model) {
        return RecordCodecBuilder.mapCodec(instance -> instance.group((App)modelCodec.optionalFieldOf("model", model).forGetter(ModelAndTexture::model), (App)AssetInfo.MAP_CODEC.forGetter(ModelAndTexture::asset)).apply((Applicative)instance, ModelAndTexture::new));
    }
