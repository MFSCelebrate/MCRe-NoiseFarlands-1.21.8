...
implements SamplerSource {
    ...
    final private Set<Sampler> samplers = new ObjectOpenHashSet();
    final private SamplerFactory factory = new SamplerFactory();
    ...
    public static Set<Sampler> createSystemSamplers() {
        ...
        try {
            ...
            IntStream.range(0, cpuUsageFetcher.logicalProcessorCount).mapToObj(index -> Sampler.create("cpu#" + index, SampleType.CPU, () -> cpuUsageFetcher.getCpuUsage(index))).forEach(arg_0 -> ((ImmutableSet.Builder)builder).add(arg_0));
        }
        catch (Throwable throwable) {
            LOGGER.warn("Failed to query cpu, no cpu stats will be recorded", throwable);
        }
        builder.add((Object)Sampler.create("heap MiB", SampleType.JVM, () -> SystemDetails.toMebibytes(Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory())));
    ...
    static class CpuUsageFetcher {
        final private SystemInfo systemInfo = new SystemInfo();
        final private CentralProcessor processor = this.systemInfo.getHardware().getProcessor();
        final public int logicalProcessorCount = this.processor.getLogicalProcessorCount();
