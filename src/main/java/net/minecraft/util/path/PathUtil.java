...
public class PathUtil {
    final static private Pattern FILE_NAME_WITH_COUNT = Pattern.compile("(<name>.*) \\((<count>\\d*)\\)", 66);
    final static private int MAX_NAME_LENGTH = 255;
    final static private Pattern RESERVED_WINDOWS_NAMES = Pattern.compile(".*\\.|(?:COM|CLOCK\\$|CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])(?:\\..*)?", 2);
    final static private Pattern VALID_FILE_NAME = Pattern.compile("[-._a-z0-9]+");
    ...
    public static String getNextUniqueName(Path path, String name, String extension) throws IOException {
        if (!PathUtil.isNotReservedWindowsName((String)(name = PathUtil.replaceInvalidChars((String)name)))) {
    ...
    public static boolean isAllowedName(Path path) {
        for (Path path2 : path) {
            if (PathUtil.isNotReservedWindowsName(path2.toString())) continue;
            return false;
        }
        return true;
    }
