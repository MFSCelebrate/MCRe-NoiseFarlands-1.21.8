...
public class CrashReport {
    ...
    final static private DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss", Locale.ROOT);
    final private String message;
    final private Throwable cause;
    final private List<CrashReportSection> otherSections = Lists.newArrayList();
    @Nullable
    private Path file;
    private boolean hasStackTrace = true;
    private StackTraceElement[] stackTrace = new StackTraceElement[0];
    final private SystemDetails systemDetailsSection = new SystemDetails();
    ...
    public CrashReportSection addElement(String name, int ignoredStackTraceCallCount) {
        ...
        if (this.hasStackTrace) {
            ...
            if (j < 0) {
                LOGGER.error("Negative index in crash report handler ({}/{})", (Object)stackTraceElements.length, (Object)1);
            }
            if (stackTraceElements != null && 0 <= j && j < stackTraceElements.length) {
                stackTraceElement = stackTraceElements[j];
                if (stackTraceElements.length + 1 - 1 < stackTraceElements.length) {
                    stackTraceElement2 = stackTraceElements[stackTraceElements.length + 1 - 1];
                }
            }
            this.hasStackTrace = crashReportSection.shouldGenerateStackTrace(stackTraceElement, stackTraceElement2);
            if (stackTraceElements != null && stackTraceElements.length >= 1 && 0 <= j && j < stackTraceElements.length) {
