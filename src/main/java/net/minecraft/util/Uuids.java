...
public final class Uuids {
    final static public Codec<UUID> INT_STREAM_CODEC = Codec.INT_STREAM.comapFlatMap(uuidStream -> Util.decodeFixedLengthArray(uuidStream, 4).map(Uuids::toUuid), uuid -> Arrays.stream(Uuids.toIntArray(uuid)));
    final static public Codec<Set<UUID>> SET_CODEC = Codec.list(INT_STREAM_CODEC).xmap(Sets::newHashSet, Lists::newArrayList);
    final static public Codec<Set<UUID>> LINKED_SET_CODEC = Codec.list(INT_STREAM_CODEC).xmap(Sets::newLinkedHashSet, Lists::newArrayList);
    final static public Codec<UUID> STRING_CODEC = Codec.STRING.comapFlatMap(string -> {
        try {
    ...
    final static public Codec<UUID> CODEC = Codec.withAlternative((Codec)Codec.STRING.comapFlatMap(string -> {
        try {
    ...
    final static public Codec<UUID> STRICT_CODEC = Codec.withAlternative(INT_STREAM_CODEC, STRING_CODEC);
    final static public PacketCodec<ByteBuf, UUID> PACKET_CODEC = new PacketCodec<ByteBuf, UUID>(){
    ...
    final static public int BYTE_ARRAY_SIZE = 16;
    final static private String OFFLINE_PLAYER_UUID_PREFIX = "OfflinePlayer:";
