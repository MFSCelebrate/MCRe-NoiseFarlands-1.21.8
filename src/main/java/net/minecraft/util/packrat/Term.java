...
public interface Term<S> {
    ...
    public record SequenceTerm<S>(Term<S>[] elements) implements Term<S>
    {
        ...
        public boolean matches(ParsingState<S> state, ParseResults results, Cut cut) {
            ...
            for (Term<S> term : this.elements) {
                if (term.matches(state, results, cut)) continue;
                state.setCursor(1);
                return false;
            }
            return true;
    ...
    {
        public RepeatedTerm(ParsingRuleEntry<S, T> parsingRuleEntry, Symbol<List<T>> symbol, int i) {
    ...
    {
        public RepeatWithSeparatorTerm(ParsingRuleEntry<S, T> parsingRuleEntry, Symbol<List<T>> symbol, Term<S> term, int i, boolean bl) {
            this.element = parsingRuleEntry;
            this.listName = symbol;
            this.separator = term;
            this.minRepetitions = 1;
