...
{
    final static public PacketCodec<ByteBuf, ImmutableBlockBox> PACKET_CODEC = new PacketCodec<ByteBuf, ImmutableBlockBox>(){
    ...
    public ImmutableBlockBox expand(Direction direction, int offset) {
        ...
        if (direction.getDirection() == Direction.AxisDirection.POSITIVE) {
            return ImmutableBlockBox.of(this.min, BlockPos.max(this.min, this.max.net_minecraft_util_math_BlockPos_offset(direction, offset)));
        }
        return ImmutableBlockBox.of(BlockPos.min(this.min.net_minecraft_util_math_BlockPos_offset(direction, offset), this.max), this.max);
    }
    ...
    public ImmutableBlockBox move(Direction direction, int offset) {
        ...
        return new ImmutableBlockBox(this.min.net_minecraft_util_math_BlockPos_offset(direction, offset), this.max.net_minecraft_util_math_BlockPos_offset(direction, offset));
    }

    public ImmutableBlockBox move(Vec3i offset) {
        return new ImmutableBlockBox(this.min.net_minecraft_util_math_BlockPos_add(offset), this.max.net_minecraft_util_math_BlockPos_add(offset));
    }
}
