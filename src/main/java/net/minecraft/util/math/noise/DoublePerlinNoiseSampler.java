...
public class DoublePerlinNoiseSampler {
    final static private double DOMAIN_SCALE = 1.0181268882175227;
    final static private double field_31703 = 0.3333333333333333;
    final private double amplitude;
    final private OctavePerlinNoiseSampler firstSampler;
    final private OctavePerlinNoiseSampler secondSampler;
    final private double maxValue;
    final private NoiseParameters parameters;
    ...
    private DoublePerlinNoiseSampler(Random random, NoiseParameters parameters, boolean modern) {
        ...
        if (modern) {
            this.firstSampler = OctavePerlinNoiseSampler.create(random, 1, doubleList);
            this.secondSampler = OctavePerlinNoiseSampler.create(random, 1, doubleList);
        } else {
            this.firstSampler = OctavePerlinNoiseSampler.createLegacy(random, 1, doubleList);
            this.secondSampler = OctavePerlinNoiseSampler.createLegacy(random, 1, doubleList);
        }
    ...
    extends Record {
        ...
        final static public Codec<NoiseParameters> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.INT.fieldOf("firstOctave").forGetter(NoiseParameters::firstOctave), (App)Codec.DOUBLE.listOf().fieldOf("amplitudes").forGetter(NoiseParameters::amplitudes)).apply((Applicative)instance, NoiseParameters::new));
        final static public Codec<RegistryEntry<NoiseParameters>> REGISTRY_ENTRY_CODEC = RegistryElementCodec.of(RegistryKeys.NOISE_PARAMETERS, CODEC);
