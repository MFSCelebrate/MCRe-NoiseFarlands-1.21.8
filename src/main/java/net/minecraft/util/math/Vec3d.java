...
public class Vec3d
implements Position {
    final static public Codec<Vec3d> CODEC = Codec.DOUBLE.listOf().comapFlatMap(coordinates -> Util.decodeFixedLengthList(coordinates, 3).map(coords -> new Vec3d((Double)coords.get(0), (Double)coords.get(1), (Double)coords.get(2))), vec -> List.of(Double.valueOf(vec.getX()), Double.valueOf(vec.getY()), Double.valueOf(vec.getZ())));
    final static public PacketCodec<ByteBuf, Vec3d> PACKET_CODEC = new PacketCodec<ByteBuf, Vec3d>(){
    ...
    final static public Vec3d ZERO = new Vec3d(0.0, 0.0, 0.0);
