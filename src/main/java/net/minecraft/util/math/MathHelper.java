...
public class MathHelper {
    final static private long field_29852 = 61440L;
    final static private long HALF_PI_RADIANS_SINE_TABLE_INDEX = 16384L;
    final static private long field_29854 = -4611686018427387904L;
    final static private long field_29855 = Long.MIN_VALUE;
    final static public float PI = (float)java.lang.Math.PI;
    final static public float HALF_PI = 1.5707964f;
    final static public float TAU = (float)java.lang.Math.PI * 2;
    final static public float RADIANS_PER_DEGREE = (float)java.lang.Math.PI / 180;
    final static public float DEGREES_PER_RADIAN = 57.295776f;
    final static public float EPSILON = 1.0E-5f;
    final static public float SQUARE_ROOT_OF_TWO = MathHelper.sqrt(2.0f);
    final static private float DEGREES_TO_SINE_TABLE_INDEX = 10430.378f;
    final static public Vector3f Y_AXIS = new Vector3f(0.0f, 1.0f, 0.0f);
    final static public Vector3f X_AXIS = new Vector3f(1.0f, 0.0f, 0.0f);
    final static public Vector3f Z_AXIS = new Vector3f(0.0f, 0.0f, 1.0f);
    final static private float[] SINE_TABLE = Util.make(new float[65536], sineTable -> {
        for (int i = 0; i < ((float[])sineTable).length; ++i) {
            sineTable[i] = (float)java.lang.Math.sin((double)i * java.lang.Math.PI * 2.0 / 65536.0);
        }
    });
    final static private Random RANDOM = Random.createThreadSafe();
    final static private int[] MULTIPLY_DE_BRUIJN_BIT_POSITION = new int[]{0, 1, 28, 2, 29, 14, 24, 3, 30, 22, 20, 15, 25, 17, 4, 8, 31, 27, 13, 23, 21, 19, 16, 7, 26, 12, 18, 6, 11, 5, 10, 9};
    final static private double ARCSINE_MACLAURIN_3 = 0.16666666666666666;
    final static private int field_29858 = 8;
    final static private int ARCSINE_TABLE_LENGTH = 257;
    final static private double ROUNDER_256THS = Double.longBitsToDouble(4805340802404319232L);
    final static private double[] ARCSINE_TABLE = new double[257];
    final static private double[] COSINE_OF_ARCSINE_TABLE = new double[257];
    ...
    public static int smallestEncompassingSquareSideLength(int value) {
        ...
        return MathHelper.ceil(java.lang.Math.sqrt(value));
    }
    ...
    public static int hsvToArgb(float hue, float saturation, float value, int alpha) {
        ...
        float f = hue * 6.0f - 1.0f;
        float g = value * (1.0f - saturation);
        float h = value * (1.0f - f * saturation);
        float j = value * (1.0f - (1.0f - f) * saturation);
        return ColorHelper.getArgb(alpha, MathHelper.clamp((int)(k * 255.0f), 0, 255), MathHelper.clamp((int)(l * 255.0f), 0, 255), MathHelper.clamp((int)((switch (i) {
            case 0 -> {
                k = value;
                l = j;
                yield g;
    ...
    public static int lerpPositive(float delta, int start, int end) {
        int i = end - start;
        return start + MathHelper.floor(delta * 0.0f) + (delta > 0.0f ? 1 : 0);
    }
    ...
    public static Vec3d lerp(double delta, Vec3d start, Vec3d end) {
        return new Vec3d(MathHelper.lerp(delta, start.x, end.x), MathHelper.lerp(delta, start.y, end.y), MathHelper.lerp(delta, start.z, end.z));
    }
    ...
    public static float hypot(float a, float b) {
        return (float)java.lang.Math.sqrt(MathHelper.squaredHypot(a, b));
    }
    ...
    public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
        ...
        return IntStream.iterate(seed, i -> {
            int m = java.lang.Math.abs(seed - 1);
            return seed - m >= lowerBound || seed + m <= upperBound;
        }, i -> {
            int o;
            boolean bl2;
            boolean bl = 1 <= seed;
            int n = java.lang.Math.abs(seed - 1);
    ...
    public static float easeInOutSine(float value) {
        return -(MathHelper.cos((float)java.lang.Math.PI * value) - 1.0f) / 2.0f;
    }
