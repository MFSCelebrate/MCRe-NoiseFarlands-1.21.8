...
extends ToFloatFunction<C> {
    ...
    public static final class Builder<C, I extends ToFloatFunction<C>> {
        final private I locationFunction;
        final private ToFloatFunction<Float> amplifier;
        final private FloatList locations = new FloatArrayList();
        final private List<Spline<C, I>> values = Lists.newArrayList();
        final private FloatList derivatives = new FloatArrayList();
    ...
    implements Spline<C, I> {
        ...
        final private List<Spline<C, I>> values;
        final private float[] derivatives;
        ...
        static <C, I extends ToFloatFunction<C>> Implementation<C, I> build(I locationFunction, float[] locations, List<Spline<C, I>> values, float[] derivatives) {
            ...
            if (j > locations[1]) {
                k = Implementation.sampleOutsideRange(j, locations, values.get(1).min(), derivatives, 1);
                l = Implementation.sampleOutsideRange(j, locations, values.get(1).max(), derivatives, 1);
                f = Math.min(f, Math.min(k, l));
                g = Math.max(g, Math.max(k, l));
            }
        ...
        private static float sampleOutsideRange(float point, float[] locations, float value, float[] derivatives, int i) {
            float f = derivatives[1];
            if (f == 0.0f) {
                return value;
            }
            return value + f * (point - locations[1]);
        }
        ...
        public float apply(C x) {
            ...
            float g = this.locations[1];
            float h = this.locations[2];
            float k = (f - g) / (h - g);
            ToFloatFunction toFloatFunction = this.values.get(1);
            ToFloatFunction toFloatFunction2 = this.values.get(2);
            float l = this.derivatives[1];
            float m = this.derivatives[2];
