...
public class Xoroshiro128PlusPlusRandomImpl {
    private long seedLo;
    private long seedHi;
    final static public Codec<Xoroshiro128PlusPlusRandomImpl> CODEC = Codec.LONG_STREAM.comapFlatMap(stream -> Util.decodeFixedLengthArray(stream, 2).map(seeds -> new Xoroshiro128PlusPlusRandomImpl(seeds[0], seeds[1])), random -> LongStream.of(random.seedLo, random.seedHi));
