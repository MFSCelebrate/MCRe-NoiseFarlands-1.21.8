...
public class MatrixUtil {
    final static private float COT_PI_OVER_8 = 3.0f + 2.0f * Math.sqrt(2.0f);
    final static private GivensPair SIN_COS_PI_OVER_8 = GivensPair.fromAngle(0.7853982f);
    ...
    private static GivensPair qrGivensQuaternion(float a1, float a2) {
        ...
        float h = Math.abs((float)a1) + Math.max((float)f, 1.0E-6f);
        if (a1 < 0.0f) {
            float i = g;
            g = h;
            h = i;
    ...
    public static boolean isIdentity(Matrix4fc matrix) {
        return MatrixUtil.hasProperty(matrix, Matrix4fc.PROPERTY_IDENTITY);
    }

    public static boolean isTranslation(Matrix4fc matrix) {
        return MatrixUtil.hasProperty(matrix, Matrix4fc.PROPERTY_TRANSLATION);
    }

    public static boolean isOrthonormal(Matrix4fc matrix) {
        return MatrixUtil.hasProperty(matrix, Matrix4fc.PROPERTY_ORTHONORMAL);
    }
}
