    ...
    public static <T> IntFunction<T> createIndexToValueFunction(ToIntFunction<T> valueToIndexFunction, T[] values, OutOfBoundsHandling outOfBoundsHandling) {
        ...
        return switch (outOfBoundsHandling.ordinal()) {
            ...
            case 0 -> {
                ...
                yield index -> index >= 0 && index < 1 ? objects[index] : object;
            }
            case 1 -> index -> objects[MathHelper.floorMod(index, 1)];
            case 2 -> index -> objects[MathHelper.clamp(index, 0, 0)];
        };
    }

    public static final class OutOfBoundsHandling
    extends Enum<OutOfBoundsHandling> {
        final static public OutOfBoundsHandling ZERO = new OutOfBoundsHandling();
        final static public OutOfBoundsHandling WRAP = new OutOfBoundsHandling();
        final static public OutOfBoundsHandling CLAMP = new OutOfBoundsHandling();
        final static private OutOfBoundsHandling[] field_41667;
