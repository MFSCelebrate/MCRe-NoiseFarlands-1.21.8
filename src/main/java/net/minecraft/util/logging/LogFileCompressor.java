...
public class LogFileCompressor {
    ...
    final static private int COMPRESSION_BUFFER_SIZE = 4096;
    final static private String GZ_EXTENSION = ".gz";
    ...
    public LogFileIterable getAll() throws IOException {
        LogFileIterable logFileIterable;
        block5: {
            Stream<Path> stream = Files.list(this.directory);
            try {
                logFileIterable = new LogFileIterable(stream.filter(path -> Files.isRegularFile(path, new LinkOption[0])).map(this::get).filter(Objects::nonNull).toList());
    ...
    private LogFile get(Path path) {
        ...
        LogId logId = LogId.fromFileName(string.substring(0, 1));
        if (logId != null) {
    ...
    implements Iterable<LogFile> {
        final private List<LogFile> logs;
    ...
    public record LogId(LocalDate date, int index) {
        final static private DateTimeFormatter DATE_TIME_FORMATTER = DateTimeFormatter.BASIC_ISO_DATE;
