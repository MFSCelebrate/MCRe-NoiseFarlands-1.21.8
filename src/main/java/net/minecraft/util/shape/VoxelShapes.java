...
public final class VoxelShapes {
    final static public double MIN_SIZE = 1.0E-7;
    final static public double field_31881 = 1.0E-6;
    final static private VoxelShape FULL_CUBE = Util.make(() -> {
        BitSetVoxelSet voxelSet = new BitSetVoxelSet(1, 1, 1);
        ((VoxelSet)voxelSet).set(0, 0, 0);
        return new SimpleVoxelShape(voxelSet);
    });
    final static private Vec3d BLOCK_CENTER = new Vec3d(0.5, 0.5, 0.5);
    final static public VoxelShape UNBOUNDED = VoxelShapes.cuboid(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY);
    final static private VoxelShape EMPTY = new ArrayVoxelShape((VoxelSet)new BitSetVoxelSet(0, 0, 0), (DoubleList)new DoubleArrayList(new double[]{0.0}), (DoubleList)new DoubleArrayList(new double[]{0.0}), (DoubleList)new DoubleArrayList(new double[]{0.0}));
    ...
    private static boolean matchesAnywhere(PairList mergedX, PairList mergedY, PairList mergedZ, VoxelSet shape1, VoxelSet shape2, BooleanBiFunction predicate) {
        return !mergedX.forEachPair((x1, x2, index1) -> mergedY.forEachPair((y1, y2, index2) -> mergedZ.forEachPair((z1, z2, index3) -> !predicate.apply(shape1.inBoundsAndContains(1, y1, z1), shape2.inBoundsAndContains(x2, y2, z2)))));
    }
    ...
    public static boolean isSideCovered(VoxelShape shape, VoxelShape neighbor, Direction direction) {
        ...
        return DoubleMath.fuzzyEquals((double)voxelShape.getMax(axis), 1.0, 1.0E-7) && DoubleMath.fuzzyEquals((double)voxelShape2.getMin(axis), 0.0, 1.0E-7) && !VoxelShapes.matchesAnywhere(new SlicedVoxelShape(voxelShape, axis, voxelShape.voxels.getSize(axis) - 1), new SlicedVoxelShape(voxelShape2, axis, 0), booleanBiFunction);
    }
    ...
    public static boolean adjacentSidesCoverSquare(VoxelShape one, VoxelShape two, Direction direction) {
        ...
        if (!DoubleMath.fuzzyEquals((double)voxelShape.getMax(axis), 1.0, 1.0E-7)) {
            voxelShape = VoxelShapes.empty();
        }
        if (!DoubleMath.fuzzyEquals((double)voxelShape2.getMin(axis), 0.0, 1.0E-7)) {
    ...
    public static VoxelShape transform(VoxelShape shape, DirectionTransformation transformation, Vec3d anchor) {
        ...
        return new ArrayVoxelShape(voxelSet, VoxelShapes.transform(doubleList, bl4, anchor.getComponentAlongAxis(axis), anchor.x), VoxelShapes.transform(doubleList2, bl5, anchor.getComponentAlongAxis(axis2), anchor.y), VoxelShapes.transform(doubleList3, bl6, anchor.getComponentAlongAxis(axis3), anchor.z));
    }
