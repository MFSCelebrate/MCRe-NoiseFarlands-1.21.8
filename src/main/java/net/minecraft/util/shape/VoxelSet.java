...
public abstract class VoxelSet {
    final static private Direction.Axis[] AXES = Direction.Axis.values();
    ...
    public VoxelSet transform(DirectionTransformation transformation) {
        ...
        BitSetVoxelSet voxelSet = new BitSetVoxelSet(i, j, k);
        for (int l = 0; l < this.sizeX; ++l) {
            for (int m = 0; m < this.sizeY; ++m) {
                for (int n = 0; n < this.sizeZ; ++n) {
                    ...
                    ((VoxelSet)voxelSet).set(bl4 ? i - 1 - o : o, bl5 ? j - 1 - p : p, bl6 ? k - 1 - q : q);
                }
            }
        }
        return voxelSet;
