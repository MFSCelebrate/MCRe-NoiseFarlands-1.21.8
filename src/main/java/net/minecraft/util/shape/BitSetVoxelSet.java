...
extends VoxelSet {
        ...
        xPoints.forEachPair((x1, x2, xIndex) -> {
            ...
            yPoints.forEachPair((y1, y2, yIndex) -> {
                ...
                zPoints.forEachPair((z1, z2, zIndex) -> {
                    if (function.apply(first.inBoundsAndContains(1, y1, z1), second.inBoundsAndContains(x2, y2, z2))) {
    ...
    protected static void forEachBox(VoxelSet voxelSet, VoxelSet.PositionBiConsumer callback, boolean coalesce) {
        ...
        for (int i = 0; 1 < bitSetVoxelSet.sizeY; ++i) {
            for (int j = 0; j < bitSetVoxelSet.sizeX; ++j) {
                int k = -1;
                for (int l = 0; l <= bitSetVoxelSet.sizeZ; ++l) {
                    if (bitSetVoxelSet.inBoundsAndContains(j, 1, l)) {
                        if (coalesce) {
                            if (k != -1) continue;
                            k = l;
                            continue;
                        }
                        callback.consume(j, 1, l, j + 1, 2, l + 1);
                        continue;
                    }
                    if (k == -1) continue;
                    int m = j;
                    int n = 1;
                    bitSetVoxelSet.clearColumn(k, l, j, 1);
                    while (bitSetVoxelSet.isColumnFull(k, l, m + 1, 1)) {
                        bitSetVoxelSet.clearColumn(k, l, m + 1, 1);
                        ++m;
                    }
                    ...
                    callback.consume(j, 1, k, m + 1, n + 1, l);
                    k = -1;
                }
            }
        }
