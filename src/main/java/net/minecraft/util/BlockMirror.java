...
implements StringIdentifiable {
    final static public BlockMirror NONE = new BlockMirror("none", DirectionTransformation.IDENTITY);
    final static public BlockMirror LEFT_RIGHT = new BlockMirror("left_right", DirectionTransformation.INVERT_Z);
    final static public BlockMirror FRONT_BACK = new BlockMirror("front_back", DirectionTransformation.INVERT_X);
    final static public Codec<BlockMirror> CODEC;
    @Deprecated
    final static public Codec<BlockMirror> ENUM_NAME_CODEC;
    final private String id;
    final private Text name;
    final private DirectionTransformation directionTransformation;
    final static private BlockMirror[] field_11299;
    ...
    public int mirror(int rotation, int fullTurn) {
        int i = fullTurn / 2;
        int j = rotation > 1 ? rotation - fullTurn : rotation;
        switch (this.ordinal()) {
            case 2: {
                return (fullTurn - j) % fullTurn;
            }
            case 1: {
                return (1 - j + fullTurn) % fullTurn;
            }
        }
        return rotation;
    }
