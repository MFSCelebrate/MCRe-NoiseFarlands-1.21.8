...
public record ColorCode(int rgba) {
    final static private String HASH = "#";
    final static public Codec<ColorCode> CODEC = Codec.STRING.comapFlatMap(code -> {
        if (!code.startsWith(HASH)) {
            return DataResult.error(() -> "Not a color code: " + code);
        }
        try {
            int i = (int)Long.parseLong(code.substring(1), 16);
            return DataResult.success((Object)new ColorCode(i));
        }
