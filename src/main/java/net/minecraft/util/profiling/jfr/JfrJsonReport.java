...
public class JfrJsonReport {
    final static private String BYTES_PER_SECOND = "bytesPerSecond";
    final static private String COUNT = "count";
    final static private String DURATION_NANOS_TOTAL = "durationNanosTotal";
    final static private String TOTAL_BYTES = "totalBytes";
    final static private String COUNT_PER_SECOND = "countPerSecond";
    ...
    private JsonElement collectStructureGenSection(List<StructureGenerationSample> structureGenerationSamples) {
        ...
        structureGenerationSamples.stream().collect(Collectors.groupingBy(StructureGenerationSample::structureName)).forEach((name, samples) -> {
            ...
            jsonObject22.addProperty("name", name);
            LongRunningSampleStatistics longRunningSampleStatistics2 = LongRunningSampleStatistics.fromSamples(samples);
            jsonObject22.addProperty(COUNT, (Number)longRunningSampleStatistics2.count());
            jsonObject22.addProperty(DURATION_NANOS_TOTAL, (Number)longRunningSampleStatistics2.totalDuration().toNanos());
            jsonObject22.addProperty("durationNanosAvg", (Number)(longRunningSampleStatistics2.totalDuration().toNanos() / (long)longRunningSampleStatistics2.count()));
            ...
            longRunningSampleStatistics2.quantiles().forEach((integer, double_) -> jsonObject3.addProperty("p" + integer, (Number)double_));
            Function<StructureGenerationSample, JsonElement> function = sample -> {
                ...
                jsonObject.addProperty("success", Boolean.valueOf(sample.success()));
                return jsonObject;
            };
            jsonObject.add("fastest", function.apply((StructureGenerationSample)longRunningSampleStatistics.fastestSample()));
            jsonObject.add("slowest", function.apply((StructureGenerationSample)longRunningSampleStatistics.slowestSample()));
            jsonObject.add("secondSlowest", (JsonElement)(longRunningSampleStatistics.secondSlowestSample() != null ? function.apply((StructureGenerationSample)longRunningSampleStatistics.secondSlowestSample()) : JsonNull.INSTANCE));
        });
        return jsonObject;
    }
    ...
    private <T> JsonElement collectPacketSection(NetworkIoStatistics<T> statistics, BiConsumer<T, JsonObject> callback) {
        ...
        statistics.getTopContributors().forEach(topContributor -> {
            ...
            jsonObject.addProperty("averageSize", (Number)Float.valueOf(packetStatistics.getAverageSize()));
        });
        return jsonObject;
    }
