...
public record GcHeapSummarySample(Instant time, long heapUsed, SummaryType summaryType) {
    ...
    private static double getAllocatedBytesPerSecond(List<GcHeapSummarySample> samples) {
        ...
        Duration duration = Duration.between(samples.get(1).time, samples.get((int)(samples.size() - 1)).time);
    ...
    extends Enum<SummaryType> {
        final static public SummaryType BEFORE_GC = new SummaryType();
        final static public SummaryType AFTER_GC = new SummaryType();
        final static private SummaryType[] field_34445;
