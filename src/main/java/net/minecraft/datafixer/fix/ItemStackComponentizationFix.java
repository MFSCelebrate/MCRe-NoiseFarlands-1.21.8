...
extends DataFix {
    final static private int HIDE_ENCHANTMENTS_FLAG = 1;
    final static private int HIDE_MODIFIERS_FLAG = 2;
    final static private int HIDE_UNBREAKABLE_FLAG = 4;
    final static private int HIDE_CAN_DESTROY_FLAG = 8;
    final static private int HIDE_CAN_PLACE_FLAG = 16;
    final static private int HIDE_ADDITIONAL_FLAG = 32;
    final static private int HIDE_DYED_FLAG = 64;
    final static private int HIDE_UPGRADE_FLAG = 128;
    final static private Set<String> POTION_ITEM_IDS = Set.of("minecraft:potion", "minecraft:splash_potion", "minecraft:lingering_potion", "minecraft:tipped_arrow");
    final static private Set<String> ENTITY_BUCKET_ITEM_IDS = Set.of("minecraft:pufferfish_bucket", "minecraft:salmon_bucket", "minecraft:cod_bucket", "minecraft:tropical_fish_bucket", "minecraft:axolotl_bucket", "minecraft:tadpole_bucket");
    final static private List<String> RELEVANT_ENTITY_NBT_KEYS = List.of("NoAI", "Silent", "NoGravity", "Glowing", "Invulnerable", "Health", "Age", "Variant", "HuntingCooldown", "BucketVariantTag");
    final static private Set<String> BOOLEAN_BLOCK_STATE_PROPERTIES = Set.of("attached", "bottom", "conditional", "disarmed", "drag", "enabled", "extended", "eye", "falling", "hanging", "has_bottle_0", "has_bottle_1", "has_bottle_2", "has_record", "has_book", "inverted", "in_wall", "lit", "locked", "occupied", "open", "persistent", "powered", "short", "signal_fire", "snowy", "triggered", "unstable", "waterlogged", "berries", "bloom", "shrieking", "can_summon", "up", "down", "north", "east", "south", "west", "slot_0_occupied", "slot_1_occupied", "slot_2_occupied", "slot_3_occupied", "slot_4_occupied", "slot_5_occupied", "cracked", "crafting");
    ...
    private static Dynamic<?> fixDisplay(StackData data, Dynamic<?> dynamic, int hideFlags) {
        ...
        if (optional.isPresent() || bl) {
            Dynamic dynamic2 = dynamic.emptyMap().set("rgb", dynamic.createInt(optional.orElse(10511680).intValue()));
            if (bl) {
    ...
    private static <T> Dynamic<T> fixBlockEntityData(StackData data, Dynamic<T> dynamic, String blockEntityId) {
        ...
        return switch (blockEntityId) {
            ...
            case "minecraft:shulker_box", "minecraft:chest", "minecraft:trapped_chest", "minecraft:furnace", "minecraft:ender_chest", "minecraft:dispenser", "minecraft:dropper", "minecraft:brewing_stand", "minecraft:hopper", "minecraft:barrel", "minecraft:smoker", "minecraft:blast_furnace", "minecraft:campfire", "minecraft:chiseled_bookshelf", "minecraft:crafter" -> {
                List list = dynamic.get("Items").asList(itemsDynamic -> itemsDynamic.emptyMap().set("slot", itemsDynamic.createInt(itemsDynamic.get("Slot").asByte(0) & 0xFF)).set("item", itemsDynamic.remove("Slot")));
                if (!list.isEmpty()) {
    ...
    private static void fixEnchantments(StackData data, Dynamic<?> dynamic, String nbtKey, String componentId, boolean hideInTooltip) {
        ...
        if (!list.isEmpty() || hideInTooltip) {
            ...
            for (Pair pair : list) {
                dynamic3 = dynamic3.set((String)pair.getFirst(), dynamic.createInt(((Integer)pair.getSecond()).intValue()));
            }
    ...
    private static void fixFireworks(StackData data) {
        data.applyFixer("Fireworks", true, fireworksDynamic -> {
            ...
            data.setComponent("minecraft:fireworks", fireworksDynamic.emptyMap().set("explosions", fireworksDynamic.createList(stream)).set("flight_duration", fireworksDynamic.createByte(1)));
    ...
    static class StackData {
        ...
        final private Dynamic<?> leftoverNbt;
        Dynamic<?> nbt;
