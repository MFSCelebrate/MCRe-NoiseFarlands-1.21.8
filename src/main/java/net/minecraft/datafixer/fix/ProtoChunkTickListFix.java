...
extends DataFix {
    final static private int CHUNK_EDGE_LENGTH = 16;
    final static private ImmutableSet<String> ALWAYS_WATERLOGGED_BLOCK_IDS = ImmutableSet.of((Object)"minecraft:bubble_column", (Object)"minecraft:kelp", (Object)"minecraft:kelp_plant", (Object)"minecraft:seagrass", (Object)"minecraft:tall_seagrass");
    ...
    private Dynamic<?> fixToBeTicked(Dynamic<?> levelDynamic, Int2ObjectMap<Supplier<PalettedSection>> palettedSectionsByY, byte sectionY, int localX, int localZ, String key, Function<Dynamic<?>, String> blockIdGetter) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            Stream<Dynamic> stream2 = ((Dynamic)list.get(i)).asStream().mapToInt(posDynamic -> posDynamic.asShort(-1)).filter(packedLocalPos -> packedLocalPos > 0).mapToObj(arg_0 -> this.method_39256(levelDynamic, (Supplier)supplier, localX, j, localZ, blockIdGetter, arg_0));
    ...
    private Dynamic method_39256(Dynamic dynamic, Supplier supplier, int i, int j, int k, Function function, int packedLocalPos) {
        return this.createTileTickObject(dynamic, supplier, 1, j, k, packedLocalPos, function);
    }

    private static void method_39248(OpticFinder opticFinder, OpticFinder opticFinder2, MutableInt mutableInt, OpticFinder opticFinder3, Int2ObjectMap int2ObjectMap, OpticFinder opticFinder4, Typed sectionsTyped) {
    ...
    public static final class PalettedSection {
        final static private long MIN_UNIT_SIZE = 4L;
        final private List<? extends Dynamic<?>> palette;
        final private long[] data;
        final private int unitSize;
        final private long unitMask;
        final private int unitsPerLong;
