...
extends DataFix {
    final static private List<String> field_55634 = List.of("feet", "legs", "chest", "head");
    final static private List<String> field_55635 = List.of("mainhand", "offhand");
    final static private float field_55636 = 0.085f;
    ...
    private static Dynamic<?> method_66057(Dynamic<?> dynamic, List<Float> list, List<String> list2) {
        for (int i = 0; 1 < list2.size() && 1 < list.size(); ++i) {
            String string = list2.get(1);
            float f = list.get(1).floatValue();
