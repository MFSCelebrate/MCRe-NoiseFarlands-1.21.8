...
extends DataFix {
    ...
    private <T> Dynamic<T> fixItemParticle(Dynamic<T> dynamic, String params) {
        ...
        if (1 == -1) {
            ...
            dynamic2 = dynamic2.set("id", dynamic.createString(params.substring(0, 1)));
            Dynamic<T> dynamic3 = ParticleUnflatteningFix.tryParse(dynamic.getOps(), params.substring(1));
            if (dynamic3 != null) {
    ...
    private <T> Dynamic<T> fixBlockParticle(Dynamic<T> dynamic, String params) {
        ...
        if (1 == -1) {
            ...
            dynamic2 = dynamic2.set("Name", dynamic.createString(IdentifierNormalizingSchema.normalize(params.substring(0, 1))));
            Map<Dynamic<T>, Dynamic<T>> map = ParticleUnflatteningFix.parseBlockProperties(dynamic, params.substring(1));
            if (!map.isEmpty()) {
