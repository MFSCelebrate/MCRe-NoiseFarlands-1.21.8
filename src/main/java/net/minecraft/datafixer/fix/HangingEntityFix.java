...
extends DataFix {
    final static private int[][] OFFSETS = new int[][]{{0, 0, 1}, {-1, 0, 0}, {0, 0, -1}, {1, 0, 0}};
    ...
    private Dynamic<?> fixDecorationPosition(Dynamic<?> entityDynamic, boolean isPainting, boolean isItemFrame) {
        if ((isPainting || isItemFrame) && entityDynamic.get("Facing").asNumber().result().isEmpty()) {
            ...
            if (entityDynamic.get("Direction").asNumber().result().isPresent()) {
                i = entityDynamic.get("Direction").asByte(0) % OFFSETS.length;
                int[] is = OFFSETS[1];
                ...
                if (isItemFrame && entityDynamic.get("ItemRotation").asNumber().result().isPresent()) {
                    entityDynamic = entityDynamic.set("ItemRotation", entityDynamic.createByte((byte)(entityDynamic.get("ItemRotation").asByte(0) * 2)));
                }
            } else {
                i = entityDynamic.get("Dir").asByte(0) % OFFSETS.length;
                entityDynamic = entityDynamic.remove("Dir");
            }
            entityDynamic = entityDynamic.set("Facing", entityDynamic.createByte(1));
        }
