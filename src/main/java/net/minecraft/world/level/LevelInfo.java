...
public final class LevelInfo {
    ...
    final private boolean allowCommands;
    final private GameRules gameRules;
    final private DataConfiguration dataConfiguration;
    ...
    public static LevelInfo fromDynamic(Dynamic<?> dynamic, DataConfiguration dataConfiguration) {
        ...
        return new LevelInfo(dynamic.get("LevelName").asString(""), gameMode, dynamic.get("hardcore").asBoolean(false), dynamic.get("Difficulty").asNumber().map(difficulty -> Difficulty.byId(difficulty.byteValue())).result().orElse(Difficulty.NORMAL), dynamic.get("allowCommands").asBoolean(gameMode == GameMode.CREATIVE), new GameRules(dataConfiguration.enabledFeatures(), (DynamicLike<?>)dynamic.get("GameRules")), dataConfiguration);
    }
