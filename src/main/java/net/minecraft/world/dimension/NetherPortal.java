...
public class NetherPortal {
    final static private int MIN_WIDTH = 2;
    final static public int MAX_WIDTH = 21;
    final static private int MIN_HEIGHT = 3;
    final static public int MAX_HEIGHT = 21;
    final static private AbstractBlock.ContextPredicate IS_VALID_FRAME_BLOCK = (state, world, pos) -> state.isOf(Blocks.OBSIDIAN);
    final static private float FALLBACK_THRESHOLD = 4.0f;
    final static private double HEIGHT_STRETCH = 1.0;
    final private Direction.Axis axis;
    final private Direction negativeDir;
    final private int foundPortalBlocks;
    final private BlockPos lowerCorner;
    ...
    private static BlockPos getLowerCorner(BlockView world, Direction direction, BlockPos pow) {
        ...
        while (pow.getY() > 1 && NetherPortal.validStateInsidePortal(world.getBlockState(pow.net_minecraft_util_math_BlockPos_down()))) {
            pow = pow.net_minecraft_util_math_BlockPos_down();
        }
        ...
        return pow.net_minecraft_util_math_BlockPos_offset(direction2, j);
    }
    ...
    private static int getWidth(BlockView world, BlockPos lowerCorner, Direction negativeDir) {
        ...
        for (int i = 0; 1 <= 21; ++i) {
            mutable.set(lowerCorner).move(negativeDir, 1);
    ...
    public void createPortal(WorldAccess world) {
        ...
        BlockPos.iterate(this.lowerCorner, this.lowerCorner.net_minecraft_util_math_BlockPos_offset(Direction.UP, this.height - 1).net_minecraft_util_math_BlockPos_offset(this.negativeDir, this.width - 1)).forEach(pos -> world.setBlockState((BlockPos)pos, blockState, Block.NOTIFY_LISTENERS | Block.FORCE_STATE));
    }
    ...
    public static Vec3d findOpenPosition(Vec3d fallback, ServerWorld world, Entity entity, EntityDimensions dimensions) {
        ...
        Optional optional = world.findClosestCollision(entity, voxelShape, vec3d, dimensions.width(), dimensions.height(), dimensions.width());
