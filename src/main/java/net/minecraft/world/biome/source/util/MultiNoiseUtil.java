...
public class MultiNoiseUtil {
    final static private boolean field_34477 = false;
    final static private float TO_LONG_FACTOR = 10000.0f;
    @VisibleForTesting
    final static protected int HYPERCUBE_DIMENSION = 7;
    ...
    public record ParameterRange(long min, long max) {
        final static public Codec<ParameterRange> CODEC = Codecs.createCodecForPairObject(Codec.floatRange(-2.0f, 2.0f), "min", "max", (min, max) -> {
    ...
    public record MultiNoiseSampler(DensityFunction temperature, DensityFunction humidity, DensityFunction continentalness, DensityFunction erosion, DensityFunction depth, DensityFunction weirdness, List<NoiseHypercube> spawnTarget) {
        public NoiseValuePoint sample(int x, int y, int z) {
            ...
            DensityFunction.UnblendedNoisePos unblendedNoisePos = new DensityFunction.UnblendedNoisePos(1, j, k);
    ...
    static class FittestPositionFinder {
        final static private long field_54705 = 2048L;
        Result bestResult;
    ...
    public static class Entries<T> {
        final private List<Pair<NoiseHypercube, T>> entries;
        final private SearchTree<T> tree;
    ...
    protected static final class SearchTree<T> {
        final static private int MAX_NODES_FOR_SIMPLE_TREE = 6;
        final private TreeNode<T> firstNode;
        final private ThreadLocal<TreeLeafNode<T>> previousResultNode = new ThreadLocal();
        ...
        static abstract class TreeNode<T> {
            final protected ParameterRange[] parameters;
