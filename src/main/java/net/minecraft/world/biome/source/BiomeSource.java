...
implements BiomeSupplier {
    final static public Codec<BiomeSource> CODEC = Registries.BIOME_SOURCE.getCodec().dispatchStable(BiomeSource::getCodec, Function.identity());
    final private Supplier<Set<RegistryEntry<Biome>>> biomes = Suppliers.memoize(() -> (Set)this.biomeStream().distinct().collect(ImmutableSet.toImmutableSet()));
            ...
            while (p <= o) {
                ...
                for (int q = -o; q <= o; q += blockCheckInterval) {
                    ...
                    if (pair == null || random.nextInt(m + 1) == 0) {
                        ...
                        if (bl) {
                            return Pair.of((Object)blockPos, registryEntry);
                        }
                        pair = Pair.of((Object)blockPos, registryEntry);
                    }
                    ++m;
                }
