...
public class MultiNoiseBiomeSourceParameterList {
    final static public Codec<MultiNoiseBiomeSourceParameterList> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Preset.CODEC.fieldOf("preset").forGetter(multiNoiseBiomeSourceParameterList -> multiNoiseBiomeSourceParameterList.preset), RegistryOps.getEntryLookupCodec(RegistryKeys.BIOME)).apply((Applicative)instance, MultiNoiseBiomeSourceParameterList::new));
    final static public Codec<RegistryEntry<MultiNoiseBiomeSourceParameterList>> REGISTRY_CODEC = RegistryElementCodec.of(RegistryKeys.MULTI_NOISE_BIOME_SOURCE_PARAMETER_LIST, CODEC);
    final private Preset preset;
    final private MultiNoiseUtil.Entries<RegistryEntry<Biome>> entries;
    ...
    extends Record {
        ...
        final static public Preset NETHER = new Preset(Identifier.ofVanilla("nether"), new BiomeSourceFunction(){
        ...
        final static public Preset OVERWORLD = new Preset(Identifier.ofVanilla("overworld"), new BiomeSourceFunction(){
        ...
        final static Map<Identifier, Preset> BY_IDENTIFIER = Stream.of(NETHER, OVERWORLD).collect(Collectors.toMap(Preset::id, preset -> preset));
        final static public Codec<Preset> CODEC = Identifier.CODEC.flatXmap(id -> Optional.ofNullable(BY_IDENTIFIER.get(id)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown preset: " + String.valueOf(id))), preset -> DataResult.success((Object)preset.id));
