...
public class Heightmap {
    ...
    final static Predicate<BlockState> NOT_AIR = state -> !state.isAir();
    final static Predicate<BlockState> SUFFOCATES = AbstractBlock.AbstractBlockState::blocksMovement;
    final private PaletteStorage storage;
    final private Predicate<BlockState> blockPredicate;
    ...
    public Heightmap(Chunk chunk, Type type) {
        ...
        this.storage = new PackedIntegerArray(1, 256);
    }
    ...
    public void setTo(Chunk chunk, Type type, long[] values) {
        ...
        LOGGER.warn("Ignoring heightmap data for chunk " + String.valueOf(chunk.getPos()) + ", size does not match; expected: " + ls.length + ", got: " + values.length);
    ...
    implements StringIdentifiable {
        final static public Type WORLD_SURFACE_WG = new Type(0, "WORLD_SURFACE_WG", Purpose.WORLDGEN, NOT_AIR);
        final static public Type WORLD_SURFACE = new Type(1, "WORLD_SURFACE", Purpose.CLIENT, NOT_AIR);
        final static public Type OCEAN_FLOOR_WG = new Type(2, "OCEAN_FLOOR_WG", Purpose.WORLDGEN, SUFFOCATES);
        final static public Type OCEAN_FLOOR = new Type(3, "OCEAN_FLOOR", Purpose.LIVE_WORLD, SUFFOCATES);
        final static public Type MOTION_BLOCKING = new Type(4, "MOTION_BLOCKING", Purpose.CLIENT, state -> state.blocksMovement() || !state.getFluidState().isEmpty());
        final static public Type MOTION_BLOCKING_NO_LEAVES = new Type(5, "MOTION_BLOCKING_NO_LEAVES", Purpose.CLIENT, state -> (state.blocksMovement() || !state.getFluidState().isEmpty()) && !(state.getBlock() instanceof LeavesBlock));
        final static public Codec<Type> CODEC;
        final static private IntFunction<Type> INDEX_MAPPER;
        final static public PacketCodec<ByteBuf, Type> PACKET_CODEC;
        final private int index;
        final private String id;
        final private Purpose purpose;
        final private Predicate<BlockState> blockPredicate;
        final static private Type[] field_13199;
    ...
    extends Enum<Purpose> {
        final static public Purpose WORLDGEN = new Purpose();
        final static public Purpose LIVE_WORLD = new Purpose();
        final static public Purpose CLIENT = new Purpose();
        final static private Purpose[] field_13208;
