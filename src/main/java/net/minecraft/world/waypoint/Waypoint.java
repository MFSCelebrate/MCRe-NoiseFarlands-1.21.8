...
public interface Waypoint {
    final static public int DEFAULT_PLAYER_RANGE = 60000000;
    final static public EntityAttributeModifier DISABLE_TRACKING = new EntityAttributeModifier(Identifier.ofVanilla("waypoint_transmit_range_hide"), -1.0, EntityAttributeModifier.Operation.ADD_MULTIPLIED_TOTAL);
    ...
    public static class Config {
        final static public Codec<Config> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RegistryKey.createCodec(WaypointStyles.REGISTRY).fieldOf("style").forGetter(config -> config.style), (App)Codecs.RGB.optionalFieldOf("color").forGetter(config -> config.color)).apply((Applicative)instance, Config::new));
        final static public PacketCodec<ByteBuf, Config> PACKET_CODEC = PacketCodec.tuple(RegistryKey.createPacketCodec(WaypointStyles.REGISTRY), config -> config.style, PacketCodecs.optional(PacketCodecs.RGB), config -> config.color, Config::new);
        final static public Config DEFAULT = new Config();
        ...
        public Config withTeamColorOf(LivingEntity entity) {
            ...
            if (registryKey == this.style && optional.isEmpty()) {
                return this;
            }
            return new Config(registryKey, optional);
        }
