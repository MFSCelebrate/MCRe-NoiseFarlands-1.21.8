...
implements SpecialSpawner {
    final static private int DEFAULT_SPAWN_TIMER = 1200;
    final static public int DEFAULT_SPAWN_DELAY = 24000;
    final static private int MIN_SPAWN_CHANCE = 25;
    final static private int MAX_SPAWN_CHANCE = 75;
    final static private int DEFAULT_SPAWN_CHANCE = 25;
    final static private int field_30635 = 10;
    final static private int field_30636 = 10;
    final private Random random = Random.create();
    final private ServerWorldProperties properties;
    ...
    private BlockPos getNearbySpawnPos(WorldView world, BlockPos pos, int range) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            BlockPos blockPos2 = new BlockPos(j, l = world.getTopY(Heightmap.Type.WORLD_SURFACE, j, k = pos.getZ() + this.random.nextInt(range * 2) - range), 1);
            if (!spawnLocation.isSpawnPositionOk(world, blockPos2, EntityType.WANDERING_TRADER)) continue;
            blockPos = blockPos2;
            break;
        }
        return blockPos;
    ...
    private boolean doesNotSuffocateAt(BlockView world, BlockPos pos) {
        for (BlockPos blockPos : BlockPos.iterate(pos, pos.net_minecraft_util_math_BlockPos_add(1, 2, 1))) {
