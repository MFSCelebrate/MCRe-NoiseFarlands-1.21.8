...
BiomeAccess.Storage {
    ...
    default public int getTopY(Heightmap.Type heightmap, BlockPos pos) {
    ...
    default public Chunk getChunk(BlockPos pos) {
        return this.net_minecraft_world_chunk_Chunk_getChunk(ChunkSectionPos.getSectionCoord(pos.getX()), ChunkSectionPos.getSectionCoord(pos.getZ()));
    }

    default public Chunk net_minecraft_world_chunk_Chunk_getChunk(int chunkX, int chunkZ) {
    ...
    default public boolean isRegionLoaded(int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
        if (maxY < this.getBottomY() || minY > this.getTopYInclusive()) {
            return false;
        }
    ...
    default public <T> RegistryWrapper<T> createCommandRegistryWrapper(RegistryKey<? extends Registry<? extends T>> registryRef) {
        RegistryWrapper.Impl registry = this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow((RegistryKey)registryRef);
