...
public record OrderedTick<T>(T type, BlockPos pos, long triggerTick, TickPriority priority, long subTickOrder) {
    final static public Comparator<OrderedTick<?>> TRIGGER_TICK_COMPARATOR = (first, second) -> {
    ...
    final static public Comparator<OrderedTick<?>> BASIC_COMPARATOR = (first, second) -> {
    ...
    final static public Hash.Strategy<OrderedTick<?>> HASH_STRATEGY = new Hash.Strategy<OrderedTick<?>>(){
