...
implements QueryableTickScheduler<T> {
    final static private Comparator<ChunkTickScheduler<?>> COMPARATOR = (a, b) -> OrderedTick.BASIC_COMPARATOR.compare(a.peekNextTick(), b.peekNextTick());
    final private LongPredicate tickingFutureReadyPredicate;
    final private Long2ObjectMap<ChunkTickScheduler<T>> chunkTickSchedulers = new Long2ObjectOpenHashMap();
    final private Long2LongMap nextTriggerTickByChunkPos = (Long2LongMap)Util.make(new Long2LongOpenHashMap(), map -> map.defaultReturnValue(Long.MAX_VALUE));
    final private Queue<ChunkTickScheduler<T>> tickableChunkTickSchedulers = new PriorityQueue(COMPARATOR);
    final private Queue<OrderedTick<T>> tickableTicks = new ArrayDeque<OrderedTick<T>>();
    final private List<OrderedTick<T>> tickedTicks = new ArrayList<OrderedTick<T>>();
    final private Set<OrderedTick<?>> copiedTickableTicksList = new ObjectOpenCustomHashSet(OrderedTick.HASH_STRATEGY);
    final private BiConsumer<ChunkTickScheduler<T>, OrderedTick<T>> queuedTickConsumer = (chunkTickScheduler, tick) -> {
    ...
    public void scheduleTicks(WorldTickScheduler<T> scheduler, BlockBox box, Vec3i offset) {
        ...
        list.forEach(tick -> this.scheduleTick(new OrderedTick(tick.type(), tick.pos().net_minecraft_util_math_BlockPos_add(offset), tick.triggerTick(), tick.priority(), tick.subTickOrder() - l + m + 1L)));
    }
