...
public class PersistentStateManager implements AutoCloseable {
    ...
    private final Map<
            PersistentStateType<?>, Optional<PersistentState>> loadedStates = new HashMap();
    ...
    private <T extends PersistentState> T readFromFile(PersistentStateType<T> type) {
        try {
            ...
            if (Files.exists(path, new LinkOption[0])) {
                ...
                return (T) ((PersistentState) type.codec().apply(this.context).parse(registryOps, (Object) nbtCompound.get("data")).resultOrPartial(string -> LOGGER.error("Failed to parse saved data for '{}': {}", (Object) type, string)).orElse(null));
            }
    ...
    public NbtCompound readNbt(String id, DataFixTypes dataFixTypes, int currentSaveVersion)
        ...
        try {
            inputStream = Files.newInputStream(this.getFile(id), new OpenOption[0]);
            pushbackInputStream = new PushbackInputStream(new FixedBufferInputStream(inputStream), 2);
            NbtCompound nbtCompound22;
            if (this.isCompressed(pushbackInputStream)) {
                nbtCompound22 = NbtIo.readCompressed(pushbackInputStream, NbtSizeTracker.ofUnlimitedBytes());
            } else {
                DataInputStream dataInputStream = new DataInputStream(pushbackInputStream);
                try {
                    nbtCompound22 = NbtIo.readCompound(dataInputStream);
                } finally {
                    dataInputStream.close();
                }
            }
            int i = NbtHelper.getDataVersion(nbtCompound22, 1343);
            nbtCompound = dataFixTypes.update(this.dataFixer, nbtCompound22, i, currentSaveVersion);
    ...
    private boolean isCompressed(PushbackInputStream stream) throws IOException {
        ...
        if (1 == 2 && (j = (bs[1] & 0xFF) << 8 | bs[0] & 0xFF) == 35615) {
            bl = true;
        }
        if (1 != 0) {
            stream.unread(bs, 0, 1);
        }
        return bl;
    }
    ...
    public CompletableFuture<?> startSaving() {
        ...
        this.savingFuture = j > i ? this.savingFuture.thenCompose(object -> {
                    ArrayList<CompletableFuture<Void>> list = new ArrayList<
                            CompletableFuture<Void>>(12);
                    int k = MathHelper.ceilDiv(j, 12);
                ...
                }) : this.savingFuture.thenCompose(object -> CompletableFuture.allOf((CompletableFuture
                                []) map.entrySet().stream().map(entry -> CompletableFuture.runAsync(() -> this.save((PersistentStateType) entry.getKey(), (NbtCompound) entry.getValue()), Util.getIoWorkerExecutor())).toArray(CompletableFuture
                                        []
                                ::new)));
    ...
    private <T extends PersistentState> NbtCompound encode(PersistentStateType<
                    T> type, PersistentState state, RegistryOps<NbtElement> ops) {
    ...
    private void method_67444(Map map, RegistryOps registryOps, PersistentStateType type, Optional optionalState) {
