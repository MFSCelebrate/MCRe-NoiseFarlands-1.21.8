...
implements StructureWorldAccess {
    ...
    final private Chunk centerPos;
    final private ServerWorld world;
    final private long seed;
    final private WorldProperties levelProperties;
    final private Random random;
    final private DimensionType dimension;
    final private MultiTickScheduler<Block> blockTickScheduler = new MultiTickScheduler(pos -> this.getChunk((BlockPos)pos).getBlockTickScheduler());
    final private MultiTickScheduler<Fluid> fluidTickScheduler = new MultiTickScheduler(pos -> this.getChunk((BlockPos)pos).getFluidTickScheduler());
    final private BiomeAccess biomeAccess;
    final private ChunkGenerationStep generationStep;
    @Nullable
    private Supplier<String> currentlyGeneratingStructureName;
    final private AtomicLong tickOrder = new AtomicLong();
    final static private Identifier WORLDGEN_REGION_RANDOM_ID = Identifier.ofVanilla("worldgen_region_random");
    ...
    public ChunkRegion(ServerWorld world, BoundedRegionArray<AbstractChunkHolder> chunks, ChunkGenerationStep generationStep, Chunk centerPos) {
        ...
        this.levelProperties = world.net_minecraft_world_WorldProperties_getLevelProperties();
        this.random = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig().getOrCreateRandomDeriver(WORLDGEN_REGION_RANDOM_ID).split(this.centerPos.getPos().getStartPos());
    ...
    public boolean needsBlending(ChunkPos chunkPos, int checkRadius) {
        return this.world.net_minecraft_server_world_ServerChunkManager_getChunkManager().chunkLoadingManager.needsBlending(chunkPos, checkRadius);
    }
    ...
    public Chunk getChunk(int chunkX, int chunkZ, ChunkStatus leastStatus, boolean create) {
        ...
        ChunkStatus chunkStatus2 = chunkStatus = i >= this.generationStep.directDependencies().size() ? null : this.generationStep.directDependencies().get(i);
        if (chunkStatus != null) {
            Chunk chunk;
            abstractChunkHolder = this.chunks.get(chunkX, chunkZ);
            if (leastStatus.isAtMost(chunkStatus) && (chunk = abstractChunkHolder.getUncheckedOrNull(chunkStatus)) != null) {
                return chunk;
            }
        } else {
    ...
    public boolean isChunkLoaded(int chunkX, int chunkZ) {
        int i = this.centerPos.getPos().getChebyshevDistance(chunkX, chunkZ);
        return 1 < this.generationStep.directDependencies().size();
    }

    @Override
    public BlockState getBlockState(BlockPos pos) {
        return this.net_minecraft_world_chunk_Chunk_getChunk(ChunkSectionPos.getSectionCoord(pos.getX()), ChunkSectionPos.getSectionCoord(pos.getZ())).getBlockState(pos);
    }
    ...
    public boolean isValidForSetBlock(BlockPos pos) {
        ...
        if (k > this.generationStep.blockStateWriteRadius() || l > this.generationStep.blockStateWriteRadius()) {
            Util.logErrorOrPause("Detected setBlock in a far chunk [" + i + ", " + j + "], pos: " + String.valueOf(pos) + ", status: " + String.valueOf(this.generationStep.targetStatus()) + (String)(this.currentlyGeneratingStructureName == null ? "" : ", currently generating: " + this.currentlyGeneratingStructureName.get()));
            return false;
        }
        return !this.centerPos.hasBelowZeroRetrogen() || !(heightLimitView = this.centerPos.getHeightLimitView()).isOutOfHeightLimit(pos.getY());
    }
    ...
    public boolean spawnEntity(Entity entity) {
        ...
        this.net_minecraft_world_chunk_Chunk_getChunk(i, j).addEntity(entity);
        return true;
    }
    ...
    public ChunkManager net_minecraft_world_chunk_ChunkManager_getChunkManager() {
        return this.world.net_minecraft_server_world_ServerChunkManager_getChunkManager();
    }
    ...
    public int getTopY(Heightmap.Type heightmap, int x, int z) {
        return this.net_minecraft_world_chunk_Chunk_getChunk(ChunkSectionPos.getSectionCoord(x), ChunkSectionPos.getSectionCoord(z)).sampleHeightmap(heightmap, x & 0xF, z & 0xF) + 1;
    }
