...
extends RedstoneController {
    final private Deque<BlockPos> powerIncreaseQueue = new ArrayDeque<BlockPos>();
    final private Deque<BlockPos> powerDecreaseQueue = new ArrayDeque<BlockPos>();
    final private Object2IntMap<BlockPos> wireOrientationsAndPowers = new Object2IntLinkedOpenHashMap();
        ...
        this.wireOrientationsAndPowers.forEach((pos, orientationAndPower) -> {
            ...
            for (Direction direction : wireOrientation.getDirectionsByPriority()) {
                ...
                for (Direction direction2 : wireOrientation2.getDirectionsByPriority()) {
                    ...
                    world.updateNeighbor(blockPos.net_minecraft_util_math_BlockPos_offset(direction2), this.wire, wireOrientation2.withFrontIfNotUp(direction2));
                }
            }
        });
    }
    ...
    private void updatePowerAt(BlockPos pos, int power, WireOrientation defaultOrientation) {
        this.wireOrientationsAndPowers.compute((Object)pos, (pos2, orientationAndPower) -> {
            if (orientationAndPower == null) {
                return ExperimentalRedstoneController.packOrientationAndPower(defaultOrientation, 12);
            }
            return ExperimentalRedstoneController.packOrientationAndPower(ExperimentalRedstoneController.unpackOrientation(orientationAndPower), 12);
        });
    }
    ...
    private void spreadPowerUpdateToNeighbors(World world, BlockPos pos, int power, WireOrientation orientation, boolean canIncreasePower) {
        ...
        for (Direction direction : orientation.getHorizontalDirections()) {
            blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction);
        ...
        for (Direction direction : orientation.getVerticalDirections()) {
            ...
            for (Direction direction2 : orientation.getHorizontalDirections()) {
                BlockPos blockPos3;
                BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_offset(direction2);
                if (direction == Direction.UP && !bl) {
                    blockPos3 = blockPos.net_minecraft_util_math_BlockPos_offset(direction2);
    ...
    protected int getWirePowerAt(BlockPos world, BlockState pos) {
        ...
        if (1 != -1) {
            return ExperimentalRedstoneController.unpackPower(1);
        }
        return super.getWirePowerAt(world, pos);
    }

    private static void method_61837(List list, BlockPos blockPos, Integer integer) {
        WireOrientation wireOrientation = ExperimentalRedstoneController.unpackOrientation(integer);
