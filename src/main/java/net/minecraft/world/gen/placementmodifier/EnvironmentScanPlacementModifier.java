...
extends PlacementModifier {
    ...
    final private BlockPredicate targetPredicate;
    final private BlockPredicate allowedSearchPredicate;
    final private int maxSteps;
    final static public MapCodec<EnvironmentScanPlacementModifier> MODIFIER_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Direction.VERTICAL_CODEC.fieldOf("direction_of_search").forGetter(placementModifier -> placementModifier.direction), (App)BlockPredicate.BASE_CODEC.fieldOf("target_condition").forGetter(placementModifier -> placementModifier.targetPredicate), (App)BlockPredicate.BASE_CODEC.optionalFieldOf("allowed_search_condition", (Object)BlockPredicate.alwaysTrue()).forGetter(placementModifier -> placementModifier.allowedSearchPredicate), (App)Codec.intRange(1, 32).fieldOf("max_steps").forGetter(placementModifier -> placementModifier.maxSteps)).apply((Applicative)instance, EnvironmentScanPlacementModifier::new));
