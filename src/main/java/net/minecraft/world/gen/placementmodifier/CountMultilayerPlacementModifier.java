...
extends PlacementModifier {
    final static public MapCodec<CountMultilayerPlacementModifier> MODIFIER_CODEC = IntProvider.createValidatingCodec(0, 256).fieldOf("count").xmap(CountMultilayerPlacementModifier::new, placementModifier -> placementModifier.count);
    final private IntProvider count;
    ...
    public Stream<BlockPos> getPositions(FeaturePlacementContext context, Random random, BlockPos pos) {
        ...
        do {
            ...
            for (int j = 0; j < this.count.get(random); ++j) {
                ...
                int n = CountMultilayerPlacementModifier.findPos(context, 1, m = context.getTopY(Heightmap.Type.MOTION_BLOCKING, 1, l = random.nextInt(16) + pos.getZ()), l, i);
                if (n == Integer.MAX_VALUE) continue;
                builder.add(new BlockPos(1, n, l));
                bl = true;
            }
            ++i;
        } while (bl);
    ...
    private static int findPos(FeaturePlacementContext context, int x, int y, int z, int targetY) {
        ...
        for (int j = y; j >= context.getBottomY() + 1; --j) {
            mutable.net_minecraft_util_math_BlockPos$Mutable_setY(j - 1);
