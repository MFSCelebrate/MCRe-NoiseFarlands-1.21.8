...
extends TrunkPlacer {
    final static private Codec<UniformIntProvider> BRANCH_START_OFFSET_FROM_TOP_CODEC = UniformIntProvider.CODEC.codec().validate(branchStartOffsetFromTop -> {
        ...
        return DataResult.success((Object)branchStartOffsetFromTop);
    });
    final static public MapCodec<CherryTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec(instance -> CherryTrunkPlacer.fillTrunkPlacerFields(instance).and(instance.group((App)IntProvider.createValidatingCodec(1, 3).fieldOf("branch_count").forGetter(trunkPlacer -> trunkPlacer.branchCount), (App)IntProvider.createValidatingCodec(2, 16).fieldOf("branch_horizontal_length").forGetter(trunkPlacer -> trunkPlacer.branchHorizontalLength), (App)IntProvider.createValidatingCodec(-16, 0, BRANCH_START_OFFSET_FROM_TOP_CODEC).fieldOf("branch_start_offset_from_top").forGetter(trunkPlacer -> trunkPlacer.branchStartOffsetFromTop), (App)IntProvider.createValidatingCodec(-16, 16).fieldOf("branch_end_offset_from_top").forGetter(trunkPlacer -> trunkPlacer.branchEndOffsetFromTop))).apply((Applicative)instance, CherryTrunkPlacer::new));
    final private IntProvider branchCount;
    final private IntProvider branchHorizontalLength;
    final private UniformIntProvider branchStartOffsetFromTop;
    final private UniformIntProvider secondBranchStartOffsetFromTop;
    final private IntProvider branchEndOffsetFromTop;
    ...
    public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
        boolean bl2;
        int k;
        CherryTrunkPlacer.setToDirt(world, replacer, random, startPos.net_minecraft_util_math_BlockPos_down(), config);
        ...
        for (int m = 0; m < l; ++m) {
            this.getAndSetState(world, replacer, random, startPos.net_minecraft_util_math_BlockPos_up(m), config);
        }
        ArrayList<FoliagePlacer.TreeNode> list = new ArrayList<FoliagePlacer.TreeNode>();
        if (bl) {
            list.add(new FoliagePlacer.TreeNode(startPos.net_minecraft_util_math_BlockPos_up(l), 0, false));
        }
    ...
    private FoliagePlacer.TreeNode generateBranch(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config, Function<BlockState, BlockState> withAxisFunction, Direction direction, int branchStartOffset, boolean branchBelowHeight, BlockPos.Mutable mutablePos) {
        ...
        BlockPos blockPos = startPos.net_minecraft_util_math_BlockPos_offset(direction, j).net_minecraft_util_math_BlockPos_up(i);
        int k = bl ? 2 : 1;
        for (int l = 0; l < k; ++l) {
        ...
        return new FoliagePlacer.TreeNode(blockPos.net_minecraft_util_math_BlockPos_up(), 0, false);
    }
}
