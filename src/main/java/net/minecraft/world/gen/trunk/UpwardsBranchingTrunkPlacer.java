...
extends TrunkPlacer {
    final static public MapCodec<UpwardsBranchingTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec(instance -> UpwardsBranchingTrunkPlacer.fillTrunkPlacerFields(instance).and(instance.group((App)IntProvider.POSITIVE_CODEC.fieldOf("extra_branch_steps").forGetter(trunkPlacer -> trunkPlacer.extraBranchSteps), (App)Codec.floatRange(0.0f, 1.0f).fieldOf("place_branch_per_log_probability").forGetter(trunkPlacer -> Float.valueOf(trunkPlacer.placeBranchPerLogProbability)), (App)IntProvider.NON_NEGATIVE_CODEC.fieldOf("extra_branch_length").forGetter(trunkPlacer -> trunkPlacer.extraBranchLength), (App)RegistryCodecs.entryList(RegistryKeys.BLOCK).fieldOf("can_grow_through").forGetter(trunkPlacer -> trunkPlacer.canGrowThrough))).apply((Applicative)instance, UpwardsBranchingTrunkPlacer::new));
    final private IntProvider extraBranchSteps;
    final private float placeBranchPerLogProbability;
    final private IntProvider extraBranchLength;
    final private RegistryEntryList<Block> canGrowThrough;
    ...
    private void generateExtraBranch(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, TreeFeatureConfig config, List<FoliagePlacer.TreeNode> nodes, BlockPos.Mutable pos, int yOffset, Direction direction, int length, int steps) {
        ...
        if (i - yOffset > 1) {
            ...
            nodes.add(new FoliagePlacer.TreeNode(blockPos.net_minecraft_util_math_BlockPos_down(2), 0, false));
        }
    }
