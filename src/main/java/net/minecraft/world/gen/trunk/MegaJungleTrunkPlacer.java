...
extends GiantTrunkPlacer {
    final static public MapCodec<MegaJungleTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec(instance -> MegaJungleTrunkPlacer.fillTrunkPlacerFields(instance).apply((Applicative)instance, MegaJungleTrunkPlacer::new));
    ...
    public List<FoliagePlacer.TreeNode> generate(TestableWorld world, BiConsumer<BlockPos, BlockState> replacer, Random random, int height, BlockPos startPos, TreeFeatureConfig config) {
        ...
        for (int i = height - 2 - random.nextInt(4); i > height / 2; i -= 2 + random.nextInt(4)) {
            ...
            for (int l = 0; l < 5; ++l) {
                ...
                BlockPos blockPos = startPos.net_minecraft_util_math_BlockPos_add(j, i - 3 + l / 2, k);
                this.getAndSetState(world, replacer, random, blockPos, config);
            }
            list.add(new FoliagePlacer.TreeNode(startPos.net_minecraft_util_math_BlockPos_add(j, i, k), -2, false));
        }
        return list;
    }
}
