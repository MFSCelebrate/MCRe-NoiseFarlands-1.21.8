...
extends TreeDecorator {
    final static public MapCodec<CreakingHeartTreeDecorator> CODEC = Codec.floatRange(0.0f, 1.0f).fieldOf("probability").xmap(CreakingHeartTreeDecorator::new, treeDecorator -> Float.valueOf(treeDecorator.probability));
    ...
    public void generate(TreeDecorator.Generator generator) {
        ...
        Optional<BlockPos> optional = list2.stream().filter(pos -> {
            for (Direction direction : Direction.values()) {
                if (generator.matches(pos.net_minecraft_util_math_BlockPos_offset(direction), state -> state.isIn(BlockTags.LOGS))) continue;
                return false;
            }
            return true;
        }).findFirst();
