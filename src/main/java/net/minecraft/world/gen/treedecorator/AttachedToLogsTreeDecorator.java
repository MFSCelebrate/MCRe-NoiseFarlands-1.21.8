...
extends TreeDecorator {
    final static public MapCodec<AttachedToLogsTreeDecorator> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.floatRange(0.0f, 1.0f).fieldOf("probability").forGetter(treeDecorator -> Float.valueOf(treeDecorator.probability)), (App)BlockStateProvider.TYPE_CODEC.fieldOf("block_provider").forGetter(treeDecorator -> treeDecorator.blockProvider), (App)Codecs.nonEmptyList(Direction.CODEC.listOf()).fieldOf("directions").forGetter(treeDecorator -> treeDecorator.directions)).apply((Applicative)instance, AttachedToLogsTreeDecorator::new));
    final private float probability;
    final private BlockStateProvider blockProvider;
