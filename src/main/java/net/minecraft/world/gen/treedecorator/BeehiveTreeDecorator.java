...
extends TreeDecorator {
    final static public MapCodec<BeehiveTreeDecorator> CODEC = Codec.floatRange(0.0f, 1.0f).fieldOf("probability").xmap(BeehiveTreeDecorator::new, decorator -> Float.valueOf(decorator.probability));
    final static private Direction BEE_NEST_FACE = Direction.SOUTH;
    final static private Direction[] GENERATE_DIRECTIONS = (Direction[])Direction.Type.HORIZONTAL.stream().filter(direction -> direction != BEE_NEST_FACE.getOpposite()).toArray(Direction[]::new);
    ...
    public void generate(TreeDecorator.Generator generator) {
        ...
        Optional<BlockPos> optional = list3.stream().filter(pos -> generator.isAir((BlockPos)pos) && generator.isAir(pos.net_minecraft_util_math_BlockPos_offset(BEE_NEST_FACE))).findFirst();
        if (optional.isEmpty()) {
            return;
        }
