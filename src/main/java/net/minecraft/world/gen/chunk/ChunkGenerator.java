...
public abstract class ChunkGenerator {
    final static public Codec<ChunkGenerator> CODEC = Registries.CHUNK_GENERATOR.getCodec().dispatchStable(ChunkGenerator::getCodec, Function.identity());
    final protected BiomeSource biomeSource;
    final private Supplier<List<PlacedFeatureIndexer.IndexedFeatures>> indexedFeaturesListSupplier;
    final private Function<RegistryEntry<Biome>, GenerationSettings> generationSettingsGetter;
    ...
    public Pair<BlockPos, RegistryEntry<Structure>> locateStructure(ServerWorld world, RegistryEntryList<Structure> structures, BlockPos center, int radius, boolean skipReferencedStructures) {
        StructurePlacementCalculator structurePlacementCalculator = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getStructurePlacementCalculator();
    ...
    private Pair<BlockPos, RegistryEntry<Structure>> locateConcentricRingsStructure(Set<RegistryEntry<Structure>> structures, ServerWorld world, StructureAccessor structureAccessor, BlockPos center, boolean skipReferencedStructures, ConcentricRingsStructurePlacement placement) {
        List<ChunkPos> list = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getStructurePlacementCalculator().getPlacementPositions(placement);
        if (list == null) {
    ...
    public void generateFeatures(StructureWorldAccess world, Chunk chunk, StructureAccessor structureAccessor) {
        ...
        RegistryWrapper.Impl registry = world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.STRUCTURE);
        ...
        int i = list.size();
        try {
            RegistryWrapper.Impl registry2 = world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.PLACED_FEATURE);
    ...
    public void addStructureReferences(StructureWorldAccess world, StructureAccessor structureAccessor, Chunk chunk) {
        ...
        for (int n = j - 8; n <= j + 8; ++n) {
            for (int o = k - 8; o <= k + 8; ++o) {
                ...
                for (StructureStart structureStart : world.net_minecraft_world_chunk_Chunk_getChunk(n, o).getStructureStarts().values()) {
                    try {
    ...
    private static String method_38271(Registry registry, PlacedFeature placedFeature) {
        return registry.getKey(placedFeature).map(Object::toString).orElseGet(placedFeature::toString);
    }

    private static void method_39788(IntSet intSet, PlacedFeatureIndexer.IndexedFeatures indexedFeatures, PlacedFeature feature) {
        intSet.add(indexedFeatures.indexMapping().applyAsInt(feature));
    }

    private static String method_38272(Registry registry, Structure structure) {
        return registry.getKey(structure).map(Object::toString).orElseGet(structure::toString);
    }

    private static void method_39787(StructureWorldAccess structureWorldAccess, Set set, ChunkPos pos) {
        Chunk chunk = structureWorldAccess.net_minecraft_world_chunk_Chunk_getChunk(pos.x, pos.z);
