...
extends ChunkGenerator {
    ...
    private void populateBiomes(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(chunkx -> this.createChunkNoiseSampler((Chunk)chunkx, structureAccessor, blender, noiseConfig));
    ...
    public VerticalBlockSample getColumnSample(int x, int z, HeightLimitView world, NoiseConfig noiseConfig) {
        ...
        this.sampleHeightmap(world, noiseConfig, x, z, (MutableObject<VerticalBlockSample>)mutableObject, null);
    ...
    private OptionalInt sampleHeightmap(HeightLimitView world, NoiseConfig noiseConfig, int x, int z, @Nullable MutableObject<VerticalBlockSample> columnSample, @Nullable Predicate<BlockState> stopPredicate) {
        ...
        if (columnSample == null) {
            ...
            columnSample.setValue((Object)new VerticalBlockSample(j, blockStates));
        }
    ...
    public void buildSurface(ChunkRegion region, StructureAccessor structures, NoiseConfig noiseConfig, Chunk chunk) {
        ...
        this.buildSurface(chunk, heightContext, noiseConfig, structures, region.getBiomeAccess(), (Registry<Biome>)region.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME), Blender.getBlender(region));
    }
    ...
    public void carve(ChunkRegion chunkRegion, long seed, NoiseConfig noiseConfig, BiomeAccess biomeAccess, StructureAccessor structureAccessor, Chunk chunk) {
        ...
        ChunkNoiseSampler chunkNoiseSampler = chunk.getOrCreateChunkNoiseSampler(chunkx -> this.createChunkNoiseSampler((Chunk)chunkx, structureAccessor, Blender.getBlender(chunkRegion), noiseConfig));
        ...
        for (int j = -8; j <= 8; ++j) {
            for (int k = -8; k <= 8; ++k) {
                ...
                Chunk chunk2 = chunkRegion.net_minecraft_world_chunk_Chunk_getChunk(chunkPos2.x, chunkPos2.z);
                GenerationSettings generationSettings = chunk2.getOrCreateGenerationSettings(() -> this.getGenerationSettings(this.biomeSource.getBiome(BiomeCoords.fromBlock(chunkPos2.getStartX()), 0, BiomeCoords.fromBlock(chunkPos2.getStartZ()), noiseConfig.getMultiNoiseSampler())));
    ...
    public CompletableFuture<Chunk> populateNoise(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            Chunk chunk2;
            int l = chunk.getSectionIndex(1 * generationShapeConfig.verticalCellBlockCount() - 1 + i);
            ...
            try {
                chunk2 = this.populateNoise(blender, structureAccessor, noiseConfig, chunk, 1, 1);
            }
                    ...
                    for (int s = l - 1; s >= 0; --s) {
                        ...
                        for (int w = 0; w < k; ++w) {
                            ...
                            for (int z = 0; z < k; ++z) {
                                ...
                                if ((blockState = this.getBlockState(chunkNoiseSampler, x, t, aa, blockState)) == AIR || SharedConstants.isOutsideGenerationArea(chunk.getPos())) continue;
