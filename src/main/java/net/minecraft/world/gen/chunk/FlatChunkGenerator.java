...
extends ChunkGenerator {
    final static public MapCodec<FlatChunkGenerator> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)FlatChunkGeneratorConfig.CODEC.fieldOf("settings").forGetter(FlatChunkGenerator::getConfig)).apply((Applicative)instance, instance.stable(FlatChunkGenerator::new)));
    final private FlatChunkGeneratorConfig config;
    ...
    public StructurePlacementCalculator createStructurePlacementCalculator(RegistryWrapper<StructureSet> structureSetRegistry, NoiseConfig noiseConfig, long seed) {
        Stream stream = this.config.getStructureOverrides().map(RegistryEntryList::stream).orElseGet(() -> structureSetRegistry.streamEntries().map(structureEntry -> structureEntry));
