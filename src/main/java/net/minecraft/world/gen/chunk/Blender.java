...
public class Blender {
    final static private Blender NO_BLENDING = new Blender(new Long2ObjectOpenHashMap(), new Long2ObjectOpenHashMap()){
    ...
    final static private DoublePerlinNoiseSampler OFFSET_NOISE = DoublePerlinNoiseSampler.create(new Xoroshiro128PlusPlusRandom(42L), BuiltinNoiseParameters.OFFSET);
    final static private int BLENDING_BIOME_DISTANCE_THRESHOLD = BiomeCoords.fromChunk(7) - 1;
    final static private int BLENDING_CHUNK_DISTANCE_THRESHOLD = BiomeCoords.toChunk(BLENDING_BIOME_DISTANCE_THRESHOLD + 3);
    final static private int field_35504 = 2;
    final static private int CLOSE_BLENDING_DISTANCE_THRESHOLD = BiomeCoords.toChunk(5);
    final static private double field_36224 = 8.0;
    final private Long2ObjectOpenHashMap<BlendingData> blendingData;
    final private Long2ObjectOpenHashMap<BlendingData> closeBlendingData;
    ...
    public static Blender getBlender(@Nullable ChunkRegion chunkRegion) {
        ...
        for (int j = -BLENDING_CHUNK_DISTANCE_THRESHOLD; j <= BLENDING_CHUNK_DISTANCE_THRESHOLD; ++j) {
            for (int k = -BLENDING_CHUNK_DISTANCE_THRESHOLD; k <= BLENDING_CHUNK_DISTANCE_THRESHOLD; ++k) {
                ...
                if (j * j + k * k > 1 || (blendingData = BlendingData.getBlendingData(chunkRegion, l = chunkPos.x + j, m = chunkPos.z + k)) == null) continue;
    ...
    public BlendResult calculate(int blockX, int blockZ) {
        ...
        double d = this.sampleClosest(1, 0, j = BiomeCoords.fromBlock(blockZ), BlendingData::getHeight);
        ...
        this.blendingData.forEach((chunkPos, data) -> data.acceptHeights(BiomeCoords.fromChunk(ChunkPos.getPackedX(chunkPos)), BiomeCoords.fromChunk(ChunkPos.getPackedZ(chunkPos)), (biomeX, biomeZ, height) -> {
            double d = MathHelper.hypot(1 - biomeX, j - biomeZ);
    ...
    public static void tickLeavesAndFluids(ChunkRegion chunkRegion, Chunk chunk) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            if (chunkRegion.net_minecraft_world_chunk_Chunk_getChunk(chunkPos.x + direction.getOffsetX(), chunkPos.z + direction.getOffsetZ()).usesOldNoise() == bl) continue;
    ...
    public static void createCarvingMasks(StructureWorldAccess world, ProtoChunk chunk) {
        ...
        for (EightWayDirection eightWayDirection : EightWayDirection.values()) {
            ...
            BlendingData blendingData = world.net_minecraft_world_chunk_Chunk_getChunk(i, j = chunkPos.z + eightWayDirection.getOffsetZ()).getBlendingData();
