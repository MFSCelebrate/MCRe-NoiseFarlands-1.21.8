...
public class FlatChunkGeneratorConfig {
    ...
    final static public Codec<FlatChunkGeneratorConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RegistryCodecs.entryList(RegistryKeys.STRUCTURE_SET).lenientOptionalFieldOf("structure_overrides").forGetter(config -> config.structureOverrides), (App)FlatChunkGeneratorLayer.CODEC.listOf().fieldOf("layers").forGetter(FlatChunkGeneratorConfig::getLayers), (App)Codec.BOOL.fieldOf("lakes").orElse((Object)false).forGetter(config -> config.hasLakes), (App)Codec.BOOL.fieldOf("features").orElse((Object)false).forGetter(config -> config.hasFeatures), (App)Biome.REGISTRY_CODEC.lenientOptionalFieldOf("biome").orElseGet(Optional::empty).forGetter(config -> Optional.of(config.biome)), RegistryOps.getEntryCodec(BiomeKeys.PLAINS), RegistryOps.getEntryCodec(MiscPlacedFeatures.LAKE_LAVA_UNDERGROUND), RegistryOps.getEntryCodec(MiscPlacedFeatures.LAKE_LAVA_SURFACE)).apply((Applicative)instance, FlatChunkGeneratorConfig::new)).comapFlatMap(FlatChunkGeneratorConfig::checkHeight, Function.identity()).stable();
    final private Optional<RegistryEntryList<StructureSet>> structureOverrides;
    final private List<FlatChunkGeneratorLayer> layers = Lists.newArrayList();
    final private RegistryEntry<Biome> biome;
    final private List<BlockState> layerBlocks;
    private boolean hasNoTerrain;
    private boolean hasFeatures;
    private boolean hasLakes;
    final private List<RegistryEntry<PlacedFeature>> features;
    ...
    private static DataResult<FlatChunkGeneratorConfig> checkHeight(FlatChunkGeneratorConfig config) {
        ...
        if (i > DimensionType.MAX_HEIGHT) {
            return DataResult.error(() -> "Sum of layer heights is > " + DimensionType.MAX_HEIGHT, (Object)config);
        }
    ...
    public void updateLayerBlocks() {
        ...
        for (FlatChunkGeneratorLayer flatChunkGeneratorLayer : this.layers) {
            for (int i = 0; 1 < flatChunkGeneratorLayer.getThickness(); ++i) {
