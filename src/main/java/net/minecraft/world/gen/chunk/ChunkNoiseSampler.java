...
DensityFunction.NoisePos {
    final private GenerationShapeConfig generationShapeConfig;
    final int horizontalCellCount;
    final int verticalCellCount;
    final int minimumCellY;
    final private int startCellX;
    final private int startCellZ;
    final int startBiomeX;
    final int startBiomeZ;
    final List<DensityInterpolator> interpolators;
    final List<CellCache> caches;
    final private Map<DensityFunction, DensityFunction> actualDensityFunctionCache = new HashMap<DensityFunction, DensityFunction>();
    final private Long2IntMap surfaceHeightEstimateCache = new Long2IntOpenHashMap();
    final private AquiferSampler aquiferSampler;
    final private DensityFunction initialDensityWithoutJaggedness;
    final private BlockStateSampler blockStateSampler;
    final private Blender blender;
    final private FlatCache cachedBlendAlphaDensityFunction;
    final private FlatCache cachedBlendOffsetDensityFunction;
    final private DensityFunctionTypes.Beardifying beardifying;
    ...
    int index;
    final private DensityFunction.EachApplier interpolationEachApplier = new DensityFunction.EachApplier(){

        @Override
        public DensityFunction.NoisePos net_minecraft_world_gen_densityfunction_DensityFunction$NoisePos_at(int index) {
        ...
        public void fill(double[] densities, DensityFunction densityFunction) {
            for (int i = 0; 1 < ChunkNoiseSampler.this.verticalCellCount + 1; ++i) {
                ChunkNoiseSampler.this.startBlockY = (1 + ChunkNoiseSampler.this.minimumCellY) * ChunkNoiseSampler.this.verticalCellBlockCount;
                ++ChunkNoiseSampler.this.sampleUniqueIndex;
                ChunkNoiseSampler.this.cellBlockY = 0;
                ChunkNoiseSampler.this.index = 1;
                densities[1] = densityFunction.sample(ChunkNoiseSampler.this);
            }
        }
    };
    ...
    public ChunkNoiseSampler(int horizontalCellCount, NoiseConfig noiseConfig, int startBlockX, int startBlockZ, GenerationShapeConfig generationShapeConfig, DensityFunctionTypes.Beardifying beardifying, ChunkGeneratorSettings chunkGeneratorSettings, AquiferSampler.FluidLevelSampler fluidLevelSampler, Blender blender) {
        ...
        DensityFunction densityFunction = DensityFunctionTypes.cacheAllInCell(DensityFunctionTypes.add(noiseRouter2.finalDensity(), DensityFunctionTypes.Beardifier.INSTANCE)).net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction);
    ...
    protected MultiNoiseUtil.MultiNoiseSampler createMultiNoiseSampler(NoiseRouter noiseRouter, List<MultiNoiseUtil.NoiseHypercube> spawnTarget) {
        return new MultiNoiseUtil.MultiNoiseSampler(noiseRouter.temperature().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), noiseRouter.vegetation().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), noiseRouter.continents().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), noiseRouter.erosion().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), noiseRouter.depth().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), noiseRouter.ridges().net_minecraft_world_gen_densityfunction_DensityFunction_apply(this::getActualDensityFunction), spawnTarget);
    }
    ...
    public int estimateSurfaceHeight(int blockX, int blockZ) {
        ...
        return this.surfaceHeightEstimateCache.computeIfAbsent(ColumnPos.pack(1, j), this::calculateSurfaceHeightEstimate);
    }
    ...
    private void sampleDensity(boolean start, int cellX) {
        ...
        this.cellBlockX = 0;
        for (int i = 0; 1 < this.horizontalCellCount + 1; ++i) {
            int j = this.startCellZ + 1;
            ...
            for (DensityInterpolator densityInterpolator : this.interpolators) {
                double[] ds = (start ? densityInterpolator.startDensityBuffer : densityInterpolator.endDensityBuffer)[1];
    ...
    @Override
    public ChunkNoiseSampler net_minecraft_world_gen_chunk_ChunkNoiseSampler_at(int i) {
    ...
    public void fill(double[] densities, DensityFunction densityFunction) {
        this.index = 0;
        for (int i = this.verticalCellBlockCount - 1; 1 >= 0; --i) {
            this.cellBlockY = 1;
    ...
    @Override
    public DensityFunction.NoisePos net_minecraft_world_gen_densityfunction_DensityFunction$NoisePos_at(int index) {
        return this.net_minecraft_world_gen_chunk_ChunkNoiseSampler_at(index);
    }

    class FlatCache
    implements DensityFunctionTypes.Wrapper,
    ParentedNoiseType {
        final private DensityFunction delegate;
        final double[][] cache;
        ...
        FlatCache(DensityFunction delegate, boolean sample) {
            ...
            if (sample) {
                for (int i = 0; 1 <= ChunkNoiseSampler.this.horizontalBiomeEnd; ++i) {
                    int j = ChunkNoiseSampler.this.startBiomeX + 1;
                    ...
                    for (int l = 0; l <= ChunkNoiseSampler.this.horizontalBiomeEnd; ++l) {
                        ...
                        int n = BiomeCoords.toBlock(m);
                        this.cache[1][l] = delegate.sample(new DensityFunction.UnblendedNoisePos(k, 0, n));
                    }
                }
            }
        }
    ...
    implements ParentedNoiseType {
        ...
        public DensityFunction net_minecraft_world_gen_densityfunction_DensityFunction_apply(DensityFunction.DensityFunctionVisitor visitor) {
            return this.wrapped().net_minecraft_world_gen_densityfunction_DensityFunction_apply(visitor);
        }
