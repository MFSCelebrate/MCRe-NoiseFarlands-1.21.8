...
public abstract class FoliagePlacer {
    final static public Codec<FoliagePlacer> TYPE_CODEC = Registries.FOLIAGE_PLACER_TYPE.getCodec().dispatch(FoliagePlacer::getType, FoliagePlacerType::getCodec);
    final protected IntProvider radius;
    final protected IntProvider offset;
    ...
    protected static <P extends FoliagePlacer> Products.P2<RecordCodecBuilder.Mu<P>, IntProvider, IntProvider> fillFoliagePlacerFields(RecordCodecBuilder.Instance<P> instance) {
        return instance.group((App)IntProvider.createValidatingCodec(0, 16).fieldOf("radius").forGetter(placer -> placer.radius), (App)IntProvider.createValidatingCodec(0, 16).fieldOf("offset").forGetter(placer -> placer.offset));
    }
    ...
    protected final void generateSquareWithHangingLeaves(TestableWorld world, BlockPlacer placer, Random random, TreeFeatureConfig config, BlockPos centerPos, int radius, int y, boolean giantTrunk, float hangingLeavesChance, float hangingLeavesExtensionChance) {
        ...
        int i = giantTrunk ? 1 : 0;
        BlockPos blockPos = centerPos.net_minecraft_util_math_BlockPos_down();
        BlockPos.Mutable mutable = new BlockPos.Mutable();
    ...
    public static final class TreeNode {
        final private BlockPos center;
        final private int foliageRadius;
        final private boolean giantTrunk;
