...
public abstract class Carver<C extends CarverConfig> {
    final static public Carver<CaveCarverConfig> CAVE = Carver.register("cave", new CaveCarver(CaveCarverConfig.CAVE_CODEC));
    final static public Carver<CaveCarverConfig> NETHER_CAVE = Carver.register("nether_cave", new NetherCaveCarver(CaveCarverConfig.CAVE_CODEC));
    final static public Carver<RavineCarverConfig> RAVINE = Carver.register("canyon", new RavineCarver(RavineCarverConfig.RAVINE_CODEC));
    final static protected BlockState AIR = Blocks.AIR.getDefaultState();
    final static protected BlockState CAVE_AIR = Blocks.CAVE_AIR.getDefaultState();
    final static protected FluidState WATER = Fluids.WATER.getDefaultState();
    final static protected FluidState LAVA = Fluids.LAVA.getDefaultState();
    protected Set<Fluid> carvableFluids = ImmutableSet.of((Object)Fluids.WATER);
    final private MapCodec<ConfiguredCarver<C>> codec;
        ...
        for (int r = k; r <= l; ++r) {
            ...
            for (int t = p; t <= q; ++t) {
                ...
                for (int v = o; v > m; --v) {
                    ...
                    if (skipPredicate.shouldSkip(context, g, w, h, v) || mask.get(r, v, t) && !Carver.isDebug(config)) continue;
                    mask.set(r, v, t);
                    mutable.set(s, v, u);
    ...
    protected boolean carveAtPoint(CarverContext context, C config, Chunk chunk, Function<BlockPos, RegistryEntry<Biome>> posToBiome, CarvingMask mask, BlockPos.Mutable pos, BlockPos.Mutable tmp, AquiferSampler aquiferSampler, MutableBoolean replacedGrassy) {
        ...
        if (!this.canAlwaysCarveBlock(config, blockState) && !Carver.isDebug(config)) {
            return false;
        }
        ...
        if (replacedGrassy.isTrue()) {
            ...
            if (chunk.getBlockState(tmp).isOf(Blocks.DIRT)) {
                context.applyMaterialRule(posToBiome, chunk, tmp, !blockState2.getFluidState().isEmpty()).ifPresent(state -> {
                    chunk.setBlockState(tmp, (BlockState)state);
