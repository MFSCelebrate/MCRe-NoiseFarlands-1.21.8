...
extends CarverConfig {
    final static public Codec<RavineCarverConfig> RAVINE_CODEC = RecordCodecBuilder.create(instance -> instance.group((App)CarverConfig.CONFIG_CODEC.forGetter(ravineCarverConfig -> ravineCarverConfig), (App)FloatProvider.VALUE_CODEC.fieldOf("vertical_rotation").forGetter(config -> config.verticalRotation), (App)Shape.CODEC.fieldOf("shape").forGetter(config -> config.shape)).apply((Applicative)instance, RavineCarverConfig::new));
    final public FloatProvider verticalRotation;
