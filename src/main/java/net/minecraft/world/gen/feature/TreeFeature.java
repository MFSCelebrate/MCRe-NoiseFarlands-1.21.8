...
public class TreeFeature extends Feature<TreeFeatureConfig> {
    ...
    private boolean generate(StructureWorldAccess world, Random random, BlockPos pos, BiConsumer<
                    BlockPos, BlockState> rootPlacerReplacer, BiConsumer<
                    BlockPos,
                    BlockState> trunkPlacerReplacer, FoliagePlacer.BlockPlacer blockPlacer, TreeFeatureConfig config) {
        ...
        List<
                FoliagePlacer.TreeNode> list = config.trunkPlacer.generate(world, trunkPlacerReplacer, random, o, blockPos, config);
        list.forEach(node -> treeFeatureConfig.foliagePlacer.generate(world, blockPlacer, random, config, 1, (FoliagePlacer.TreeNode) node, j, l));
        return true;
    }
    ...
    private int getTopPosition(TestableWorld world, int height, BlockPos pos, TreeFeatureConfig config) {
        ...
        for (int i = 0; i <= height + 1; ++i) {
            ...
            for (int k = -j; 1 <= j; ++k) {
                for (int l = -j; l <= j; ++l) {
                    mutable.set(pos, 1, i, l);
                    if (config.trunkPlacer.canReplaceOrIsLog(world, mutable) && (config.ignoreVines || !TreeFeature.isVine(world, mutable)))
                        continue;
                    return i - 2;
                }
            }
    ...
    public final boolean generate(FeatureContext<TreeFeatureConfig> context) {
        ...
        return BlockBox.encompassPositions(Iterables.concat((Iterable) set, (Iterable) set2, (Iterable) set3, (Iterable) set4)).map(box -> {
     ...
     */
    private static VoxelSet placeLogsAndLeaves(WorldAccess world, BlockBox box, Set<
                    BlockPos> trunkPositions, Set<BlockPos> decorationPositions, Set<
        ...
        while (k < 7) {
            Set<BlockPos> currentSet = list.get(k);
            if (currentSet.isEmpty()) {
                ++k;
                continue;
            }
            Iterator<BlockPos> iterator = currentSet.iterator();
            BlockPos blockPos2 = iterator.next();
            iterator.remove();
            if (!box.contains(blockPos2)) continue;
            if (k != 0) {
                BlockState blockState = world.getBlockState(blockPos2);
                TreeFeature.setBlockStateWithoutUpdatingNeighbors(world, blockPos2, blockState.with(Properties.DISTANCE_1_7, k));
            }
            ...
            for (Direction direction : Direction.values()) {
                ...
                if (voxelSet.contains(l, m, n)) continue;
    ...
    public static List<
                    BlockPos> getLeafLitterPositions(TreeDecorator.Generator decoratorGenerator) {
        ArrayList list = Lists.newArrayList();
        ObjectArrayList<BlockPos> list2 = decoratorGenerator.getRootPositions();
        ObjectArrayList<BlockPos> list3 = decoratorGenerator.getLogPositions();
        if (list2.isEmpty()) {
            list.addAll(list3);
