...
implements FeatureConfig {
    final static public Codec<FallenTreeFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BlockStateProvider.TYPE_CODEC.fieldOf("trunk_provider").forGetter(featureConfig -> featureConfig.trunkProvider), (App)IntProvider.createValidatingCodec(0, 16).fieldOf("log_length").forGetter(featureConfig -> featureConfig.logLength), (App)TreeDecorator.TYPE_CODEC.listOf().fieldOf("stump_decorators").forGetter(featureConfig -> featureConfig.stumpDecorators), (App)TreeDecorator.TYPE_CODEC.listOf().fieldOf("log_decorators").forGetter(featureConfig -> featureConfig.logDecorators)).apply((Applicative)instance, FallenTreeFeatureConfig::new));
    final public BlockStateProvider trunkProvider;
    final public IntProvider logLength;
    final public List<TreeDecorator> stumpDecorators;
    final public List<TreeDecorator> logDecorators;
    ...
    public static class Builder {
        final private BlockStateProvider trunkProvider;
        final private IntProvider logLength;
