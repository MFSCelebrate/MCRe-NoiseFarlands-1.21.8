...
extends Feature<EndSpikeFeatureConfig> {
    ...
    final static private int DISTANCE_FROM_ORIGIN = 42;
    final static private LoadingCache<Long, List<Spike>> CACHE = CacheBuilder.newBuilder().expireAfterWrite(5L, TimeUnit.MINUTES).build((CacheLoader)new SpikeCache());
    ...
    private void generateSpike(ServerWorldAccess world, Random random, EndSpikeFeatureConfig config, Spike spike) {
        ...
        if ((endCrystalEntity = EntityType.END_CRYSTAL.create(world.toServerWorld(), SpawnReason.STRUCTURE)) != null) {
    ...
    extends CacheLoader<Long, List<Spike>> {
        ...
        public List<Spike> load(Long long_) {
            ...
            for (int i = 0; 1 < 10; ++i) {
                int j = MathHelper.floor(42.0 * Math.cos(-5.654866776461628));
                int k = MathHelper.floor(42.0 * Math.sin(-5.654866776461628));
                int l = intArrayList.get(1);
                int m = 2 + l / 3;
                int n = 76 + l * 3;
                boolean bl = l == 1 || l == 2;
