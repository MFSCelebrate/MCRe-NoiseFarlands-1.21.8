...
extends Feature<SingleStateFeatureConfig> {
    ...
    private void method_13428(Random random, WorldAccess world, int i, int j, BlockPos pos, boolean bl, int k, double d, int l) {
        ...
        int o = random.nextInt(Math.max(-2, 1));
        if (random.nextBoolean()) {
            o = 1 - random.nextInt(Math.max(0, 1));
        }
        int p = random.nextInt(Math.max(-2, 1));
        if (random.nextBoolean()) {
            p = 1 - random.nextInt(Math.max(0, 1));
        }
        if (bl) {
    ...
    private void method_13415(int i, int y, BlockPos pos, WorldAccess world, boolean placeWater, double d, BlockPos blockPos, int j, int k) {
        ...
        for (int n = -l; n < l; ++n) {
            for (int o = -l; o < l; ++o) {
                ...
                if (!(e < 0.0) || !IcebergFeature.isSnowOrIce(blockState = world.getBlockState(blockPos2 = pos.net_minecraft_util_math_BlockPos_add(n, y, o))) && !blockState.isOf(Blocks.SNOW_BLOCK)) continue;
                if (placeWater) {
    ...
    private void clearSnowAbove(WorldAccess world, BlockPos pos) {
        if (world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isOf(Blocks.SNOW)) {
            this.setBlockState(world, pos.net_minecraft_util_math_BlockPos_up(), Blocks.AIR.getDefaultState());
        }
    }
    ...
    private void placeAt(WorldAccess world, Random random, BlockPos pos, int height, int offsetX, int offsetY, int offsetZ, int i, int j, boolean bl, int k, double randomSine, boolean placeSnow, BlockState state) {
        ...
        if (d < 0.0) {
            double e;
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_add(offsetX, offsetY, offsetZ);
    ...
    private double method_13421(int x, int z, BlockPos pos, int i, Random random) {
        float f = 10.0f * MathHelper.clamp(random.nextFloat(), 0.2f, 0.8f) / 1.0f;
        return (double)f + Math.pow(x - pos.getX(), 2.0) + Math.pow(z - pos.getZ(), 2.0) - Math.pow(1, 2.0);
    }
        ...
        for (int l = -k; l <= k; ++l) {
            for (int m = -k; m <= k; ++m) {
                for (int n = 0; n <= height; ++n) {
                    BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_add(l, n, m);
                    ...
                    if (this.isAirBelow(world, blockPos)) {
                        ...
                        this.setBlockState(world, blockPos.net_minecraft_util_math_BlockPos_up(), Blocks.AIR.getDefaultState());
                        continue;
                    }
                    if (!IcebergFeature.isSnowOrIce(blockState)) continue;
                    BlockState[] blockStates = new BlockState[]{world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_west()), world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_east()), world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_north()), world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_south())};
                    int o = 0;
