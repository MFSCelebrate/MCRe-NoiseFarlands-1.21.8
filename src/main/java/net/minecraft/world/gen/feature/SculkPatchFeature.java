...
extends Feature<SculkPatchFeatureConfig> {
    ...
    public boolean generate(FeatureContext<SculkPatchFeatureConfig> context) {
        ...
        for (l = 0; l < k; ++l) {
            BlockPos blockPos3 = blockPos.net_minecraft_util_math_BlockPos_add(random.nextInt(5) - 2, 0, random.nextInt(5) - 2);
            if (!structureWorldAccess.getBlockState(blockPos3).isAir() || !structureWorldAccess.getBlockState(blockPos3.net_minecraft_util_math_BlockPos_down()).isSideSolidFullSquare(structureWorldAccess, blockPos3.net_minecraft_util_math_BlockPos_down(), Direction.UP)) continue;
