...
extends Feature<DefaultFeatureConfig> {
    ...
    protected boolean generateCoralPiece(WorldAccess world, Random random, BlockPos pos, BlockState state) {
        ...
        if (random.nextFloat() < 0.25f) {
            Registries.BLOCK.getRandomEntry(BlockTags.CORALS, random).map(RegistryEntry::value).ifPresent(block -> world.setBlockState(blockPos, block.getDefaultState(), Block.NOTIFY_LISTENERS));
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            BlockPos blockPos2;
            if (!(random.nextFloat() < 0.2f) || !world.getBlockState(blockPos2 = pos.net_minecraft_util_math_BlockPos_offset(direction)).isOf(Blocks.WATER)) continue;
