...
implements FeatureConfig {
    final static public Codec<VegetationPatchFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)TagKey.codec(RegistryKeys.BLOCK).fieldOf("replaceable").forGetter(config -> config.replaceable), (App)BlockStateProvider.TYPE_CODEC.fieldOf("ground_state").forGetter(config -> config.groundState), (App)PlacedFeature.REGISTRY_CODEC.fieldOf("vegetation_feature").forGetter(config -> config.vegetationFeature), (App)VerticalSurfaceType.CODEC.fieldOf("surface").forGetter(config -> config.surface), (App)IntProvider.createValidatingCodec(1, 128).fieldOf("depth").forGetter(config -> config.depth), (App)Codec.floatRange(0.0f, 1.0f).fieldOf("extra_bottom_block_chance").forGetter(config -> Float.valueOf(config.extraBottomBlockChance)), (App)Codec.intRange(1, 256).fieldOf("vertical_range").forGetter(config -> config.verticalRange), (App)Codec.floatRange(0.0f, 1.0f).fieldOf("vegetation_chance").forGetter(config -> Float.valueOf(config.vegetationChance)), (App)IntProvider.VALUE_CODEC.fieldOf("xz_radius").forGetter(config -> config.horizontalRadius), (App)Codec.floatRange(0.0f, 1.0f).fieldOf("extra_edge_column_chance").forGetter(config -> Float.valueOf(config.extraEdgeColumnChance))).apply((Applicative)instance, VegetationPatchFeatureConfig::new));
    final public TagKey<Block> replaceable;
    final public BlockStateProvider groundState;
    final public RegistryEntry<PlacedFeature> vegetationFeature;
    final public VerticalSurfaceType surface;
    final public IntProvider depth;
    final public float extraBottomBlockChance;
    final public int verticalRange;
    final public float vegetationChance;
    final public IntProvider horizontalRadius;
    final public float extraEdgeColumnChance;
