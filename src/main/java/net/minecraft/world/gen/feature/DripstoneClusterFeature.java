...
extends Feature<DripstoneClusterFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DripstoneClusterFeatureConfig> context) {
        ...
        for (int l = -j; l <= j; ++l) {
            for (int m = -k; m <= k; ++m) {
                ...
                BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(l, 0, m);
    ...
    private boolean canWaterSpawn(StructureWorldAccess world, BlockPos pos) {
        ...
        if (world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).getFluidState().isIn(FluidTags.WATER)) {
            return false;
        }
        for (Direction direction : Direction.Type.HORIZONTAL) {
            if (this.isStoneOrWater(world, pos.net_minecraft_util_math_BlockPos_offset(direction))) continue;
            return false;
        }
        return this.isStoneOrWater(world, pos.net_minecraft_util_math_BlockPos_down());
    }
    ...
    private double dripstoneChance(int radiusX, int radiusZ, int localX, int localZ, DripstoneClusterFeatureConfig config) {
        ...
        int k = Math.min(i, j);
        return MathHelper.clampedMap(1, 0.0f, config.maxDistanceFromCenterAffectingChanceOfDripstoneColumn, config.chanceOfDripstoneColumnAtMaxDistanceFromCenter, 1.0f);
    }
