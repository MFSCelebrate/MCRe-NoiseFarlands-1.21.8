...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER)) {
            ...
            for (int l = 0; l <= k; ++l) {
                if (structureWorldAccess.getBlockState(blockPos2).isOf(Blocks.WATER) && structureWorldAccess.getBlockState(blockPos2.net_minecraft_util_math_BlockPos_up()).isOf(Blocks.WATER) && blockState2.canPlaceAt(structureWorldAccess, blockPos2)) {
                    if (l == k) {
                    ...
                    BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_down();
                    if (!blockState.canPlaceAt(structureWorldAccess, blockPos3) || structureWorldAccess.getBlockState(blockPos3.net_minecraft_util_math_BlockPos_down()).isOf(Blocks.KELP)) break;
                ...
                blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_up();
            }
        }
        return i > 0;
    }
