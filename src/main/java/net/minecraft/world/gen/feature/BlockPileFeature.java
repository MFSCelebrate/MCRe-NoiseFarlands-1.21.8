...
extends Feature<BlockPileFeatureConfig> {
    ...
    public boolean generate(FeatureContext<BlockPileFeatureConfig> context) {
        ...
        for (BlockPos blockPos2 : BlockPos.iterate(blockPos.net_minecraft_util_math_BlockPos_add(-i, 0, -j), blockPos.net_minecraft_util_math_BlockPos_add(i, 1, j))) {
            int l;
    ...
    private void addPileBlock(WorldAccess world, BlockPos pos, Random random, BlockPileFeatureConfig config) {
        if (world.isAir(pos) && this.canPlace(world, pos, random)) {
            world.setBlockState(pos, config.stateProvider.get(random, pos), Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
        }
    }
}
