...
extends Feature<ReplaceBlobsFeatureConfig> {
    ...
    public boolean generate(FeatureContext<ReplaceBlobsFeatureConfig> context) {
        ...
        BlockPos blockPos = ReplaceBlobsFeature.moveDownToTarget(structureWorldAccess, context.getOrigin().mutableCopy().clamp(Direction.Axis.Y, structureWorldAccess.getBottomY() + 1, structureWorldAccess.getTopYInclusive()), block);
        if (blockPos == null) {
            return false;
        }
