...
implements FeatureConfig {
    final static public Codec<FossilFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Identifier.CODEC.listOf().fieldOf("fossil_structures").forGetter(config -> config.fossilStructures), (App)Identifier.CODEC.listOf().fieldOf("overlay_structures").forGetter(config -> config.overlayStructures), (App)StructureProcessorType.REGISTRY_CODEC.fieldOf("fossil_processors").forGetter(config -> config.fossilProcessors), (App)StructureProcessorType.REGISTRY_CODEC.fieldOf("overlay_processors").forGetter(config -> config.overlayProcessors), (App)Codec.intRange(0, 7).fieldOf("max_empty_corners_allowed").forGetter(config -> config.maxEmptyCorners)).apply((Applicative)instance, FossilFeatureConfig::new));
    final public List<Identifier> fossilStructures;
    final public List<Identifier> overlayStructures;
    final public RegistryEntry<StructureProcessorList> fossilProcessors;
    final public RegistryEntry<StructureProcessorList> overlayProcessors;
    final public int maxEmptyCorners;
