...
public record BlockColumnFeatureConfig(List<Layer> layers, Direction direction, BlockPredicate allowedPlacement, boolean prioritizeTip) implements FeatureConfig
{
    ...
    public record Layer(IntProvider height, BlockStateProvider state) {
        final static public Codec<Layer> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)IntProvider.NON_NEGATIVE_CODEC.fieldOf("height").forGetter(Layer::height), (App)BlockStateProvider.TYPE_CODEC.fieldOf("provider").forGetter(Layer::state)).apply((Applicative)instance, Layer::new));
    }
}
