...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        blockPos = blockPos.net_minecraft_util_math_BlockPos_up(random.nextInt(4));
        int i = random.nextInt(4) + 7;
        int j = i / 4 + random.nextInt(2);
        if (j > 1 && random.nextInt(60) == 0) {
            blockPos = blockPos.net_minecraft_util_math_BlockPos_up(10 + random.nextInt(30));
        }
        for (k = 0; k < i; ++k) {
            ...
            for (int m = -l; m <= l; ++m) {
                ...
                for (int n = -l; n <= l; ++n) {
                    ...
                    BlockState blockState = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_add(m, k, n));
                    if (blockState.isAir() || IceSpikeFeature.isSoil(blockState) || blockState.isOf(Blocks.SNOW_BLOCK) || blockState.isOf(Blocks.ICE)) {
                        this.setBlockState(structureWorldAccess, blockPos.net_minecraft_util_math_BlockPos_add(m, k, n), Blocks.PACKED_ICE.getDefaultState());
                    }
                    if (k == 0 || l <= 1 || !(blockState = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_add(m, -k, n))).isAir() && !IceSpikeFeature.isSoil(blockState) && !blockState.isOf(Blocks.SNOW_BLOCK) && !blockState.isOf(Blocks.ICE)) continue;
                    this.setBlockState(structureWorldAccess, blockPos.net_minecraft_util_math_BlockPos_add(m, -k, n), Blocks.PACKED_ICE.getDefaultState());
                }
            }
        }
        ...
        for (int o = -k; o <= k; ++o) {
            for (l = -k; l <= k; ++l) {
                ...
                BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(o, -1, l);
                int p = 50;
                ...
                while (blockPos2.getY() > 50 && ((blockState2 = structureWorldAccess.getBlockState(blockPos2)).isAir() || IceSpikeFeature.isSoil(blockState2) || blockState2.isOf(Blocks.SNOW_BLOCK) || blockState2.isOf(Blocks.ICE) || blockState2.isOf(Blocks.PACKED_ICE))) {
                    ...
                    blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_down();
                    if (--p > 0) continue;
                    blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_down(random.nextInt(5) + 1);
                    p = random.nextInt(5);
                }
            }
        }
