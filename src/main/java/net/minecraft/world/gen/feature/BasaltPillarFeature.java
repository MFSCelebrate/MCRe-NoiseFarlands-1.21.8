...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        if (!structureWorldAccess.isAir(blockPos) || structureWorldAccess.isAir(blockPos.net_minecraft_util_math_BlockPos_up())) {
            return false;
        }
        ...
        for (int i = -3; i < 4; ++i) {
            for (int j = -3; j < 4; ++j) {
                ...
                mutable3.set(mutable.net_minecraft_util_math_BlockPos_add(i, 0, j));
                int l = 3;
