...
extends Feature<DefaultFeatureConfig> {
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        BlockState blockState = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_up());
    ...
    private void generateVinesInArea(WorldAccess world, Random random, BlockPos pos) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (!world.isAir(mutable) || !(blockState = world.getBlockState((BlockPos)mutable.net_minecraft_util_math_Vec3i_up())).isOf(Blocks.NETHERRACK) && !blockState.isOf(Blocks.NETHER_WART_BLOCK)) continue;
    ...
    public static void generateVineColumn(WorldAccess world, Random random, BlockPos.Mutable pos, int length, int minAge, int maxAge) {
        for (int i = 0; i <= length; ++i) {
            if (world.isAir(pos)) {
                if (i == length || !world.isAir((BlockPos)pos.net_minecraft_util_math_Vec3i_down())) {
