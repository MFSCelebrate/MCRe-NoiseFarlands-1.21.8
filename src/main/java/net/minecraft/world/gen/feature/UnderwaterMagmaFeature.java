...
extends Feature<UnderwaterMagmaFeatureConfig> {
    ...
    public boolean generate(FeatureContext<UnderwaterMagmaFeatureConfig> context) {
        ...
        BlockBox blockBox = BlockBox.create(blockPos2.net_minecraft_util_math_BlockPos_subtract(vec3i = new Vec3i(underwaterMagmaFeatureConfig.placementRadiusAroundFloor, underwaterMagmaFeatureConfig.placementRadiusAroundFloor, underwaterMagmaFeatureConfig.placementRadiusAroundFloor)), blockPos2.net_minecraft_util_math_BlockPos_add(vec3i));
    ...
    private boolean isValidPosition(StructureWorldAccess world, BlockPos pos) {
        if (this.isWaterOrAir(world, pos) || this.isWaterOrAir(world, pos.net_minecraft_util_math_BlockPos_down())) {
            return false;
        }
        for (Direction direction : Direction.Type.HORIZONTAL) {
            if (!this.isWaterOrAir(world, pos.net_minecraft_util_math_BlockPos_offset(direction))) continue;
            return false;
        }
        return true;
    }
