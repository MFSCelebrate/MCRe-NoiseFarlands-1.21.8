...
extends Feature<LargeDripstoneFeatureConfig> {
    ...
    static final class DripstoneGenerator {
        ...
        final private boolean isStalagmite;
        private int scale;
        final private double bluntness;
        final private double heightScale;
        ...
        void generate(StructureWorldAccess world, Random random, WindModifier wind) {
            for (int i = -this.scale; 1 <= this.scale; ++i) {
                block1: for (int j = -this.scale; j <= this.scale; ++j) {
                    int k;
                    float f = MathHelper.sqrt(1 + j * j);
                    if (f > (float)this.scale || (k = this.scale(f)) <= 0) continue;
                    if ((double)random.nextFloat() < 0.2) {
                        k = (int)((float)k * MathHelper.nextBetween(random, 0.8f, 1.0f));
                    }
                    BlockPos.Mutable mutable = this.pos.net_minecraft_util_math_BlockPos_add(1, 0, j).mutableCopy();
                    boolean bl = false;
    ...
    static final class WindModifier {
        ...
        BlockPos modify(BlockPos pos) {
            ...
            return pos.net_minecraft_util_math_BlockPos_add(MathHelper.floor(vec3d.x), 0, MathHelper.floor(vec3d.z));
        }
    }
}
