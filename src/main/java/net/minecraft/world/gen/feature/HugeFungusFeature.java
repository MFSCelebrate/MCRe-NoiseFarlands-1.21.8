...
extends Feature<HugeFungusFeatureConfig> {
    final static private float field_31507 = 0.06f;
    ...
    public boolean generate(FeatureContext<HugeFungusFeatureConfig> context) {
        ...
        BlockState blockState = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_down());
        ...
        structureWorldAccess.setBlockState(blockPos, Blocks.AIR.getDefaultState(), Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
        ...
        for (int j = -i; j <= i; ++j) {
            for (int k = -i; k <= i; ++k) {
                ...
                for (int l = 0; l < stemHeight; ++l) {
                    ...
                    if (!HugeFungusFeature.isReplaceable(world, mutable, config, true)) continue;
                    if (config.planted) {
                        if (!world.getBlockState((BlockPos)mutable.net_minecraft_util_math_Vec3i_down()).isAir()) {
        ...
        for (int k = j = hatHeight - i; k <= hatHeight; ++k) {
            ...
            for (int m = -l; m <= l; ++m) {
                for (int n2 = -l; n2 <= l; ++n2) {
                    ...
                    if (!HugeFungusFeature.isReplaceable(world, mutable, config, false)) continue;
                    if (config.planted && !world.getBlockState((BlockPos)mutable.net_minecraft_util_math_Vec3i_down()).isAir()) {
                        world.breakBlock(mutable, true);
                    }
                    if (bl6) {
                        if (bl4) continue;
    ...
    private void placeWithOptionalVines(WorldAccess world, Random random, BlockPos pos, BlockState state, boolean vines) {
        if (world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isOf(state.getBlock())) {
    ...
    private static void generateVines(BlockPos pos, WorldAccess world, Random random) {
        ...
        int j = 23;
        int k = 25;
        WeepingVinesFeature.generateVineColumn(world, random, mutable, 1, 23, 25);
    }
}
