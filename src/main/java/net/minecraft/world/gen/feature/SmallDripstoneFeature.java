...
extends Feature<SmallDripstoneFeatureConfig> {
    ...
    public boolean generate(FeatureContext<SmallDripstoneFeatureConfig> context) {
        ...
        BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(optional.get().getOpposite());
        SmallDripstoneFeature.generateDripstoneBlocks(worldAccess, random, blockPos2, smallDripstoneFeatureConfig);
        int i = random.nextFloat() < smallDripstoneFeatureConfig.chanceOfTallerDripstone && DripstoneHelper.canGenerate(worldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_offset(optional.get()))) ? 2 : 1;
    ...
    private static Optional<Direction> getDirection(WorldAccess world, BlockPos pos, Random random) {
        boolean bl = DripstoneHelper.canReplace(world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()));
        boolean bl2 = DripstoneHelper.canReplace(world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()));
        if (bl && bl2) {
    ...
    private static void generateDripstoneBlocks(WorldAccess world, Random random, BlockPos pos, SmallDripstoneFeatureConfig config) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(Direction.random(random));
            DripstoneHelper.generateDripstoneBlock(world, blockPos2);
            if (random.nextFloat() > config.chanceOfSpreadRadius3) continue;
            BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(Direction.random(random));
