...
implements FeatureConfig {
    final static public Codec<EndGatewayFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BlockPos.CODEC.optionalFieldOf("exit").forGetter(config -> config.exitPos), (App)Codec.BOOL.fieldOf("exact").forGetter(config -> config.exact)).apply((Applicative)instance, EndGatewayFeatureConfig::new));
    final private Optional<BlockPos> exitPos;
