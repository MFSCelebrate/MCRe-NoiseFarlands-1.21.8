...
extends Feature<Config> {
    final static private BlockState CAVE_AIR = Blocks.CAVE_AIR.getDefaultState();
    ...
    public boolean generate(FeatureContext<Config> context) {
        ...
        blockPos = blockPos.net_minecraft_util_math_BlockPos_down(4);
        ...
        for (s = 0; s < 16; ++s) {
            for (t = 0; t < 16; ++t) {
                for (int u = 0; u < 8; ++u) {
                    ...
                    if (!bl) continue;
                    BlockState blockState2 = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_add(s, u, t));
                    if (u >= 4 && blockState2.isLiquid()) {
                        return false;
                    }
                    if (u >= 4 || blockState2.isSolid() || structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_add(s, u, t)) == blockState) continue;
                    return false;
                }
            }
        }
        for (s = 0; s < 16; ++s) {
            for (t = 0; t < 16; ++t) {
                for (int u = 0; u < 8; ++u) {
                    BlockPos blockPos2;
                    if (!bls[(s * 16 + t) * 8 + u] || !this.canReplace(structureWorldAccess.getBlockState(blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(s, u, t)))) continue;
                    boolean bl2 = u >= 4;
            ...
            for (t = 0; t < 16; ++t) {
                for (int u = 0; u < 16; ++u) {
                    for (int v = 0; v < 8; ++v) {
                        ...
                        if (!bl2 || v >= 4 && random.nextInt(2) == 0 || !(blockState4 = structureWorldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_add(t, v, u))).isSolid() || blockState4.isIn(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE)) continue;
                        BlockPos blockPos3 = blockPos.net_minecraft_util_math_BlockPos_add(t, v, u);
        ...
        if (blockState.getFluidState().isIn(FluidTags.WATER)) {
            for (t = 0; t < 16; ++t) {
                for (int u = 0; u < 16; ++u) {
                    int v = 4;
                    BlockPos blockPos4 = blockPos.net_minecraft_util_math_BlockPos_add(t, 4, u);
    ...
    {
        final static public Codec<Config> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BlockStateProvider.TYPE_CODEC.fieldOf("fluid").forGetter(Config::fluid), (App)BlockStateProvider.TYPE_CODEC.fieldOf("barrier").forGetter(Config::barrier)).apply((Applicative)instance, Config::new));
    }
}
