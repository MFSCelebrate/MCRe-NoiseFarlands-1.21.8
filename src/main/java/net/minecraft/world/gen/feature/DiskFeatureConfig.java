...
{
    final static public Codec<DiskFeatureConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)PredicatedStateProvider.CODEC.fieldOf("state_provider").forGetter(DiskFeatureConfig::stateProvider), (App)BlockPredicate.BASE_CODEC.fieldOf("target").forGetter(DiskFeatureConfig::target), (App)IntProvider.createValidatingCodec(0, 8).fieldOf("radius").forGetter(DiskFeatureConfig::radius), (App)Codec.intRange(0, 4).fieldOf("half_height").forGetter(DiskFeatureConfig::halfHeight)).apply((Applicative)instance, DiskFeatureConfig::new));
