...
extends Feature<DefaultFeatureConfig> {
    ...
    final static private EntityType<?>[] MOB_SPAWNER_ENTITIES = new EntityType[]{EntityType.SKELETON, EntityType.ZOMBIE, EntityType.ZOMBIE, EntityType.SPIDER};
    final static private BlockState AIR = Blocks.CAVE_AIR.getDefaultState();
    ...
    public boolean generate(FeatureContext<DefaultFeatureConfig> context) {
        ...
        for (s = 1; s <= l; ++s) {
            for (t = -1; t <= 4; ++t) {
                for (u = p; u <= q; ++u) {
                    blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(s, t, u);
                    ...
                    if (s != 1 && s != l && u != p && u != q || t != 0 || !structureWorldAccess.isAir(blockPos2) || !structureWorldAccess.isAir(blockPos2.net_minecraft_util_math_BlockPos_up())) continue;
                    ++r;
                }
            }
        }
        ...
        for (s = 1; s <= l; ++s) {
            for (t = 3; t >= -1; --t) {
                for (u = p; u <= q; ++u) {
                    ...
                    if (s == 1 || t == -1 || u == p || s == l || t == 4 || u == q) {
                        if (blockPos2.getY() >= structureWorldAccess.getBottomY() && !structureWorldAccess.getBlockState(blockPos2.net_minecraft_util_math_BlockPos_down()).isSolid()) {
        ...
        block6: for (s = 0; s < 2; ++s) {
            for (t = 0; t < 3; ++t) {
                ...
                for (Direction direction : Direction.Type.HORIZONTAL) {
                    if (!structureWorldAccess.getBlockState(blockPos3.net_minecraft_util_math_BlockPos_offset(direction)).isSolid()) continue;
                    ++x;
                }
                if (x != 1) continue;
