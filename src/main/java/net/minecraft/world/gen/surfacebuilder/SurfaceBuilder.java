...
public class SurfaceBuilder {
    final static private BlockState WHITE_TERRACOTTA = Blocks.WHITE_TERRACOTTA.getDefaultState();
    final static private BlockState ORANGE_TERRACOTTA = Blocks.ORANGE_TERRACOTTA.getDefaultState();
    final static private BlockState TERRACOTTA = Blocks.TERRACOTTA.getDefaultState();
    final static private BlockState YELLOW_TERRACOTTA = Blocks.YELLOW_TERRACOTTA.getDefaultState();
    final static private BlockState BROWN_TERRACOTTA = Blocks.BROWN_TERRACOTTA.getDefaultState();
    final static private BlockState RED_TERRACOTTA = Blocks.RED_TERRACOTTA.getDefaultState();
    final static private BlockState LIGHT_GRAY_TERRACOTTA = Blocks.LIGHT_GRAY_TERRACOTTA.getDefaultState();
    final static private BlockState PACKED_ICE = Blocks.PACKED_ICE.getDefaultState();
    final static private BlockState SNOW_BLOCK = Blocks.SNOW_BLOCK.getDefaultState();
    final private BlockState defaultState;
    final private int seaLevel;
    final private BlockState[] terracottaBands;
    final private DoublePerlinNoiseSampler terracottaBandsOffsetNoise;
    final private DoublePerlinNoiseSampler badlandsPillarNoise;
    final private DoublePerlinNoiseSampler badlandsPillarRoofNoise;
    final private DoublePerlinNoiseSampler badlandsSurfaceNoise;
    final private DoublePerlinNoiseSampler icebergPillarNoise;
    final private DoublePerlinNoiseSampler icebergPillarRoofNoise;
    final private DoublePerlinNoiseSampler icebergSurfaceNoise;
    final private RandomSplitter randomDeriver;
    final private DoublePerlinNoiseSampler surfaceNoise;
    final private DoublePerlinNoiseSampler surfaceSecondaryNoise;
    ...
    public SurfaceBuilder(NoiseConfig noiseConfig, BlockState defaultState, int seaLevel, RandomSplitter randomDeriver) {
        ...
        this.terracottaBands = SurfaceBuilder.createTerracottaBands(randomDeriver.split(Identifier.ofVanilla("clay_bands")));
    ...
    public void buildSurface(NoiseConfig noiseConfig, BiomeAccess biomeAccess, Registry<Biome> biomeRegistry, boolean useLegacyRandom, HeightContext heightContext, final Chunk chunk, ChunkNoiseSampler chunkNoiseSampler, MaterialRules.MaterialRule materialRule) {
        ...
        BlockColumn blockColumn = new BlockColumn(){
            ...
            public BlockState getState(int y) {
                return chunk.getBlockState(mutable.net_minecraft_util_math_BlockPos$Mutable_setY(y));
            }
            ...
            public void setState(int y, BlockState state) {
                ...
                if (heightLimitView.isInHeightLimit(y)) {
                    chunk.setBlockState(mutable.net_minecraft_util_math_BlockPos$Mutable_setY(y), state);
        ...
        for (int k = 0; k < 16; ++k) {
            for (int l = 0; l < 16; ++l) {
                ...
                mutable.net_minecraft_util_math_BlockPos$Mutable_setX(m).net_minecraft_util_math_BlockPos$Mutable_setZ(n);
    ...
    public Optional<BlockState> applyMaterialRule(MaterialRules.MaterialRule rule, CarverContext context, Function<BlockPos, RegistryEntry<Biome>> posToBiome, Chunk chunk, ChunkNoiseSampler chunkNoiseSampler, BlockPos pos, boolean hasFluid) {
        MaterialRules.MaterialRuleContext materialRuleContext = new MaterialRules.MaterialRuleContext(this, context.getNoiseConfig(), chunk, chunkNoiseSampler, posToBiome, (Registry<Biome>)context.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME), context);
    ...
    private void placeIceberg(int minY, Biome biome, BlockColumn column, BlockPos.Mutable mutablePos, int x, int z, int surfaceY) {
        ...
        for (int o = Math.max(surfaceY, (int)k + 1); o >= minY; --o) {
            if (!(column.getState(o).isAir() && o < (int)k && random.nextDouble() > 0.01) && (!column.getState(o).isOf(Blocks.WATER) || o <= (int)j || o >= this.seaLevel || j == 0.0 || !(random.nextDouble() > 0.15))) continue;
            if (n <= l && o > m) {
    ...
    protected BlockState getTerracottaBlock(int x, int y, int z) {
        ...
        return this.terracottaBands[(y + 1 + this.terracottaBands.length) % this.terracottaBands.length];
    }
}
