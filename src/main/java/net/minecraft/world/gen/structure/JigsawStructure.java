...
extends Structure {
    final static public DimensionPadding DEFAULT_DIMENSION_PADDING = DimensionPadding.NONE;
    final static public StructureLiquidSettings DEFAULT_LIQUID_SETTINGS = StructureLiquidSettings.APPLY_WATERLOGGING;
    final static public int MAX_SIZE = 128;
    final static public int field_49155 = 0;
    final static public int MAX_GENERATION_DEPTH = 20;
    final static public MapCodec<JigsawStructure> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(JigsawStructure.configCodecBuilder(instance), (App)StructurePool.REGISTRY_CODEC.fieldOf("start_pool").forGetter(structure -> structure.startPool), (App)Identifier.CODEC.optionalFieldOf("start_jigsaw_name").forGetter(structure -> structure.startJigsawName), (App)Codec.intRange(0, 20).fieldOf("size").forGetter(structure -> structure.size), (App)HeightProvider.CODEC.fieldOf("start_height").forGetter(structure -> structure.startHeight), (App)Codec.BOOL.fieldOf("use_expansion_hack").forGetter(structure -> structure.useExpansionHack), (App)Heightmap.Type.CODEC.optionalFieldOf("project_start_to_heightmap").forGetter(structure -> structure.projectStartToHeightmap), (App)Codec.intRange(1, 128).fieldOf("max_distance_from_center").forGetter(structure -> structure.maxDistanceFromCenter), (App)Codec.list(StructurePoolAliasBinding.CODEC).optionalFieldOf("pool_aliases", List.of()).forGetter(structure -> structure.poolAliasBindings), (App)DimensionPadding.CODEC.optionalFieldOf("dimension_padding", (Object)DEFAULT_DIMENSION_PADDING).forGetter(structure -> structure.dimensionPadding), (App)StructureLiquidSettings.codec.optionalFieldOf("liquid_settings", (Object)DEFAULT_LIQUID_SETTINGS).forGetter(jigsawStructure -> jigsawStructure.liquidSettings)).apply((Applicative)instance, JigsawStructure::new)).validate(JigsawStructure::validate);
    final private RegistryEntry<StructurePool> startPool;
    final private Optional<Identifier> startJigsawName;
    final private int size;
    final private HeightProvider startHeight;
    final private boolean useExpansionHack;
    final private Optional<Heightmap.Type> projectStartToHeightmap;
    final private int maxDistanceFromCenter;
    final private List<StructurePoolAliasBinding> poolAliasBindings;
    final private DimensionPadding dimensionPadding;
    final private StructureLiquidSettings liquidSettings;
    ...
    private static DataResult<JigsawStructure> validate(JigsawStructure structure) {
        ...
        if (structure.maxDistanceFromCenter + 12 > 128) {
    ...
    public Optional<Structure.StructurePosition> getStructurePosition(Structure.Context context) {
        ...
        BlockPos blockPos = new BlockPos(chunkPos.getStartX(), 1, chunkPos.getStartZ());
