...
public final class NoiseConfig {
    ...
    final private RegistryEntryLookup<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersRegistry;
    final private NoiseRouter noiseRouter;
    final private MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler;
    final private SurfaceBuilder surfaceBuilder;
    final private RandomSplitter aquiferRandomDeriver;
    final private RandomSplitter oreRandomDeriver;
    final private Map<RegistryKey<DoublePerlinNoiseSampler.NoiseParameters>, DoublePerlinNoiseSampler> noises;
    final private Map<Identifier, RandomSplitter> randomDerivers;
    ...
    private NoiseConfig(ChunkGeneratorSettings chunkGeneratorSettings, RegistryEntryLookup<DoublePerlinNoiseSampler.NoiseParameters> noiseParametersLookup, final long seed) {
        ...
        implements DensityFunction.DensityFunctionVisitor {
            final private Map<DensityFunction, DensityFunction> cache = new HashMap<DensityFunction, DensityFunction>();
        ...
        DensityFunction.DensityFunctionVisitor densityFunctionVisitor = new DensityFunction.DensityFunctionVisitor(this){
            final private Map<DensityFunction, DensityFunction> unwrapped = new HashMap<DensityFunction, DensityFunction>();
        ...
        this.multiNoiseSampler = new MultiNoiseUtil.MultiNoiseSampler(this.noiseRouter.temperature().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), this.noiseRouter.vegetation().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), this.noiseRouter.continents().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), this.noiseRouter.erosion().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), this.noiseRouter.depth().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), this.noiseRouter.ridges().net_minecraft_world_gen_densityfunction_DensityFunction_apply(densityFunctionVisitor), chunkGeneratorSettings.spawnTarget());
    }
