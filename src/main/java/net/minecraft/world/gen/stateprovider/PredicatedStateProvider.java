...
public record PredicatedStateProvider(BlockStateProvider fallback, List<Rule> rules) {
    ...
    public record Rule(BlockPredicate ifTrue, BlockStateProvider then) {
        final static public Codec<Rule> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BlockPredicate.BASE_CODEC.fieldOf("if_true").forGetter(Rule::ifTrue), (App)BlockStateProvider.TYPE_CODEC.fieldOf("then").forGetter(Rule::then)).apply((Applicative)instance, Rule::new));
    }
}
