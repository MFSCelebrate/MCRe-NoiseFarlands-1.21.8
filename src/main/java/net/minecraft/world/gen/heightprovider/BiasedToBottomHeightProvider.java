...
extends HeightProvider {
    final static public MapCodec<BiasedToBottomHeightProvider> BIASED_TO_BOTTOM_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)YOffset.OFFSET_CODEC.fieldOf("min_inclusive").forGetter(provider -> provider.minOffset), (App)YOffset.OFFSET_CODEC.fieldOf("max_inclusive").forGetter(provider -> provider.maxOffset), (App)Codec.intRange(1, Integer.MAX_VALUE).optionalFieldOf("inner", (Object)1).forGetter(provider -> provider.inner)).apply((Applicative)instance, BiasedToBottomHeightProvider::new));
    ...
    public int get(Random random, HeightContext context) {
        ...
        int k = random.nextInt(j - 1 - this.inner + 1);
        return random.nextInt(k + this.inner) + 1;
    }
