...
public class WorldPreset {
    final static public Codec<WorldPreset> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.unboundedMap(RegistryKey.createCodec(RegistryKeys.DIMENSION), DimensionOptions.CODEC).fieldOf("dimensions").forGetter(preset -> preset.dimensions)).apply((Applicative)instance, WorldPreset::new)).validate(WorldPreset::validate);
    final static public Codec<RegistryEntry<WorldPreset>> ENTRY_CODEC = RegistryElementCodec.of(RegistryKeys.WORLD_PRESET, CODEC);
    final private Map<RegistryKey<DimensionOptions>, DimensionOptions> dimensions;
