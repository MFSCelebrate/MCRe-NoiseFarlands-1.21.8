...
public class SpawnDensityCapper {
    final private Long2ObjectMap<List<ServerPlayerEntity>> chunkPosToMobSpawnablePlayers = new Long2ObjectOpenHashMap();
    final private Map<ServerPlayerEntity, DensityCap> playersToDensityCap = Maps.newHashMap();
    ...
    private List<ServerPlayerEntity> getMobSpawnablePlayers(ChunkPos chunkPos) {
        return (List)this.chunkPosToMobSpawnablePlayers.computeIfAbsent(chunkPos.toLong(), pos -> this.chunkLoadingManager.getPlayersWatchingChunk(chunkPos));
    }
    ...
    static class DensityCap {
        final private Object2IntMap<SpawnGroup> spawnGroupsToDensity = new Object2IntOpenHashMap(SpawnGroup.values().length);

        DensityCap() {
        }

        public void increaseDensity(SpawnGroup spawnGroup) {
            this.spawnGroupsToDensity.computeInt((Object)spawnGroup, (group, density) -> density == null ? 1 : density + 1);
        }
