...
public interface Vibrations {
    final static public List<RegistryKey<GameEvent>> RESONATIONS = List.of(GameEvent.RESONATE_1.registryKey(), GameEvent.RESONATE_2.registryKey(), GameEvent.RESONATE_3.registryKey(), GameEvent.RESONATE_4.registryKey(), GameEvent.RESONATE_5.registryKey(), GameEvent.RESONATE_6.registryKey(), GameEvent.RESONATE_7.registryKey(), GameEvent.RESONATE_8.registryKey(), GameEvent.RESONATE_9.registryKey(), GameEvent.RESONATE_10.registryKey(), GameEvent.RESONATE_11.registryKey(), GameEvent.RESONATE_12.registryKey(), GameEvent.RESONATE_13.registryKey(), GameEvent.RESONATE_14.registryKey(), GameEvent.RESONATE_15.registryKey());
    final static public int DEFAULT_FREQUENCY = 0;
    final static public ToIntFunction<RegistryKey<GameEvent>> FREQUENCIES = (ToIntFunction)Util.make(new Reference2IntOpenHashMap(), frequencies -> {
    ...
    public static interface Ticker {
        ...
        private static boolean areChunksTickingAround(World world, BlockPos pos) {
            ...
            for (int i = chunkPos.x - 1; 1 <= chunkPos.x + 1; ++i) {
                for (int j = chunkPos.z - 1; j <= chunkPos.z + 1; ++j) {
                    if (world.shouldTickBlocksInChunk(ChunkPos.toLong(1, j)) && world.net_minecraft_world_chunk_ChunkManager_getChunkManager().getWorldChunk(1, j) != null) continue;
                    return false;
                }
            }
            return true;
    ...
    implements GameEventListener {
        final private Vibrations receiver;
    ...
    public static final class ListenerData {
        static public Codec<ListenerData> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Vibration.CODEC.lenientOptionalFieldOf("event").forGetter(listenerData -> Optional.ofNullable(listenerData.vibration)), (App)VibrationSelector.CODEC.fieldOf("selector").forGetter(ListenerData::getSelector), (App)Codecs.NON_NEGATIVE_INT.fieldOf("event_delay").orElse((Object)0).forGetter(ListenerData::getDelay)).apply((Applicative)instance, (vibration, selector, delay) -> new ListenerData(vibration.orElse(null), (VibrationSelector)selector, (int)delay, true)));
        final static public String LISTENER_NBT_KEY = "listener";
        @Nullable
        Vibration vibration;
        private int delay;
