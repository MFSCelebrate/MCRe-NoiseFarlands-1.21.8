...
public record Vibration(RegistryEntry<GameEvent> gameEvent, float distance, Vec3d pos, @Nullable UUID uuid, @Nullable UUID projectileOwnerUuid, @Nullable Entity entity) {
    final static public Codec<Vibration> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)GameEvent.CODEC.fieldOf("game_event").forGetter(Vibration::gameEvent), (App)Codec.floatRange(0.0f, Float.MAX_VALUE).fieldOf("distance").forGetter(Vibration::distance), (App)Vec3d.CODEC.fieldOf("pos").forGetter(Vibration::pos), (App)Uuids.INT_STREAM_CODEC.lenientOptionalFieldOf("source").forGetter(vibration -> Optional.ofNullable(vibration.uuid())), (App)Uuids.INT_STREAM_CODEC.lenientOptionalFieldOf("projectile_owner").forGetter(vibration -> Optional.ofNullable(vibration.projectileOwnerUuid()))).apply((Applicative)instance, (event, distance, pos, uuid, projectileOwnerUuid) -> new Vibration((RegistryEntry<GameEvent>)event, distance.floatValue(), (Vec3d)pos, uuid.orElse(null), projectileOwnerUuid.orElse(null))));
    ...
    private static UUID getOwnerUuid(@Nullable Entity entity) {
        ...
        if (entity instanceof ProjectileEntity && (projectileEntity = (ProjectileEntity)entity).net_minecraft_entity_Entity_getOwner() != null) {
            return projectileEntity.net_minecraft_entity_Entity_getOwner().getUuid();
        }
        return null;
    }
