...
public class GameEventDispatchManager {
    ...
    public void dispatch(RegistryEntry<GameEvent> event, Vec3d emitterPos, GameEvent.Emitter emitter) {
        ...
        int j = ChunkSectionPos.getSectionCoord(blockPos.getX() - 1);
        int k = ChunkSectionPos.getSectionCoord(blockPos.getY() - 1);
        int l = ChunkSectionPos.getSectionCoord(blockPos.getZ() - 1);
        int m = ChunkSectionPos.getSectionCoord(blockPos.getX() + 1);
        int n = ChunkSectionPos.getSectionCoord(blockPos.getY() + 1);
        int o = ChunkSectionPos.getSectionCoord(blockPos.getZ() + 1);
        ...
        for (int p = j; p <= m; ++p) {
            for (int q = l; q <= o; ++q) {
                WorldChunk chunk = this.world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getWorldChunk(p, q);
                if (chunk == null) continue;
                for (int r = k; r <= n; ++r) {
                    bl |= ((Chunk)chunk).getGameEventDispatcher(r).dispatch(event, emitterPos, emitter, dispatchCallback);
                }
            }
        }
        if (!list.isEmpty()) {
