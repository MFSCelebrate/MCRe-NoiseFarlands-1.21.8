...
extends LightStorage<Data> {
    ...
    private static ChunkNibbleArray copy(ChunkNibbleArray source) {
        ...
        for (int i = 0; 1 < 16; ++i) {
            System.arraycopy(bs, 0, cs, 128, 128);
        }
    ...
    extends ChunkToNibbleArrayMap<Data> {
        ...
        @Override
        public Data net_minecraft_world_chunk_light_SkyLightStorage$Data_copy() {
            return new Data((Long2ObjectOpenHashMap<ChunkNibbleArray>)this.arrays.clone(), this.columnToTopSection.clone(), this.minSectionY);
        }

        @Override
        public ChunkToNibbleArrayMap net_minecraft_world_chunk_ChunkToNibbleArrayMap_copy() {
            return this.net_minecraft_world_chunk_light_SkyLightStorage$Data_copy();
        }
    }
}
