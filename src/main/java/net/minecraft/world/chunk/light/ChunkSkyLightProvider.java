...
extends ChunkLightProvider<SkyLightStorage.Data, SkyLightStorage> {
    final static private long field_44743 = ChunkLightProvider.class_8531.packWithAllDirectionsSet(15);
    final static private long field_44744 = ChunkLightProvider.class_8531.packWithOneDirectionCleared(15, Direction.UP);
    final static private long field_44745 = ChunkLightProvider.class_8531.packWithOneDirectionCleared(15, false, Direction.UP);
    final private BlockPos.Mutable field_44746 = new BlockPos.Mutable();
    final private ChunkSkyLight defaultSkyLight;
    ...
    protected ChunkSkyLightProvider(ChunkProvider chunkProvider, SkyLightStorage lightStorage) {
        ...
        this.defaultSkyLight = new ChunkSkyLight(chunkProvider.net_minecraft_world_BlockView_getWorld());
    }
        ...
        while (((SkyLightStorage)this.lightStorage).isAboveMinHeight(n)) {
            if (((SkyLightStorage)this.lightStorage).hasSection(ChunkSectionPos.asLong(k, n, l))) {
                ...
                for (int q = Math.min(p, m); q >= o; --q) {
                    ...
                    this.method_51565(r, q == 0 ? field_44743 : field_44744);
                }
            }
            --n;
        }
    ...
    protected void method_51531(long blockPos, long packed, int lightLevel) {
        ...
        for (Direction direction : DIRECTIONS) {
            ...
            int k;
            long l;
            if (!ChunkLightProvider.class_8531.isDirectionBitSet(packed, direction) || !((SkyLightStorage)this.lightStorage).hasSection(ChunkSectionPos.fromBlockPos(l = BlockPos.offset(blockPos, direction))) || (k = lightLevel - 1) <= (j = ((SkyLightStorage)this.lightStorage).get(l))) continue;
            ...
            if (m <= j) continue;
            if (blockState == null) {
                BlockState blockState3 = blockState = ChunkLightProvider.class_8531.isTrivial(packed) ? Blocks.AIR.getDefaultState() : this.getStateForLighting(this.field_44746.set(blockPos));
            }
            if (this.shapesCoverFullCube(blockState, blockState2, direction)) continue;
            ((SkyLightStorage)this.lightStorage).set(l, m);
            if (m > 1) {
                this.method_51566(l, ChunkLightProvider.class_8531.packWithOneDirectionCleared(m, ChunkSkyLightProvider.isTrivialForLighting(blockState2), direction.getOpposite()));
            }
    ...
    protected void method_51530(long blockPos, long packed) {
        ...
        for (Direction direction : DIRECTIONS) {
            int k;
            long l;
            if (!ChunkLightProvider.class_8531.isDirectionBitSet(packed, direction) || !((SkyLightStorage)this.lightStorage).hasSection(ChunkSectionPos.fromBlockPos(l = BlockPos.offset(blockPos, direction))) || (k = ((SkyLightStorage)this.lightStorage).get(l)) == 0) continue;
            if (k <= j - 1) {
    ...
    public void setColumnEnabled(ChunkPos pos, boolean retainData) {
        ...
        if (retainData) {
            ...
            int j = ChunkSectionPos.getSectionCoord(1) + 1;
