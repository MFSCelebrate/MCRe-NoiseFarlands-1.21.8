...
public class ChunkSkyLight {
    final static private int field_44711 = 16;
    final static public int field_44710 = Integer.MIN_VALUE;
    final private int minY;
    final private PaletteStorage palette;
    final private BlockPos.Mutable reusableBlockPos1 = new BlockPos.Mutable();
    final private BlockPos.Mutable reusableBlockPos2 = new BlockPos.Mutable();
    ...
    private int calculateSurfaceY(Chunk chunk, int topSectionIndex, int localX, int localZ) {
        ...
        for (int j = topSectionIndex; j >= 0; --j) {
            ...
            if (chunkSection.isEmpty()) {
                ...
                mutable.net_minecraft_util_math_BlockPos$Mutable_setY(ChunkSectionPos.getBlockCoord(k));
                mutable2.net_minecraft_util_math_BlockPos$Mutable_setY(mutable.getY() - 1);
                continue;
            }
            for (k = 15; k >= 0; --k) {
                BlockState blockState2 = chunkSection.getBlockState(localX, k, localZ);
                if (ChunkSkyLight.faceBlocksLight(blockState, blockState2)) {
                    return mutable.getY();
                }
    ...
    private boolean isSkyLightAccessible(BlockView blockView, int packedIndex, int value, BlockPos upperPos, BlockState upperState, BlockPos lowerPos, BlockState lowerState) {
        int i = upperPos.getY();
        if (ChunkSkyLight.faceBlocksLight(upperState, lowerState)) {
            if (i > value) {
    ...
    private int locateLightBlockingBlockBelow(BlockView blockView, BlockPos pos, BlockState blockState) {
        ...
        while (mutable2.getY() >= this.minY) {
            ...
            if (ChunkSkyLight.faceBlocksLight(blockState2, blockState3)) {
                return mutable.getY();
            }
