...
extends Record {
    ...
    final private GenerationDependencies directDependencies;
    final GenerationDependencies accumulatedDependencies;
    final private int blockStateWriteRadius;
    final private GenerationTask task;
    ...
    public static class Builder {
        ...
        @Nullable
        final private ChunkGenerationStep previousStep;
        ...
        public Builder dependsOn(ChunkStatus status, int level) {
            ...
            if (1 > chunkStatuss.length) {
                this.directDependencies = new ChunkStatus[1];
                Arrays.fill(this.directDependencies, status);
            }
            for (int j = 0; j < Math.min(1, chunkStatuss.length); ++j) {
