...
public record SerializedChunk(Registry<Biome> biomeRegistry, ChunkPos chunkPos, int minSectionY, long lastUpdateTime, long inhabitedTime, ChunkStatus chunkStatus, @Nullable BlendingData.Serialized blendingData, @Nullable BelowZeroRetrogen belowZeroRetrogen, UpgradeData upgradeData, @Nullable long[] carvingMask, Map<Heightmap.Type, long[]> heightmaps, Chunk.TickSchedulers packedTicks, ShortList[] postProcessingSections, boolean lightCorrect, List<SectionData> sectionData, List<NbtCompound> entities, List<NbtCompound> blockEntities, NbtCompound structureData) {
    final static private Codec<PalettedContainer<BlockState>> CODEC = PalettedContainer.createPalettedContainerCodec(Block.STATE_IDS, BlockState.CODEC, PalettedContainer.PaletteProvider.BLOCK_STATE, Blocks.AIR.getDefaultState());
    final static private Codec<List<Tick<Block>>> BLOCK_TICKS_CODEC = Tick.createCodec(Registries.BLOCK.getCodec()).listOf();
    final static private Codec<List<Tick<Fluid>>> FLUID_TICKS_CODEC = Tick.createCodec(Registries.FLUID.getCodec()).listOf();
    final static private Logger LOGGER = LogUtils.getLogger();
    final static private String UPGRADE_DATA_KEY = "UpgradeData";
    final static private String BLOCK_TICKS = "block_ticks";
    final static private String FLUID_TICKS = "fluid_ticks";
    final static public String X_POS_KEY = "xPos";
    final static public String Z_POS_KEY = "zPos";
    final static public String HEIGHTMAPS_KEY = "Heightmaps";
    final static public String IS_LIGHT_ON_KEY = "isLightOn";
    final static public String SECTIONS_KEY = "sections";
    final static public String BLOCK_LIGHT_KEY = "BlockLight";
    final static public String SKY_LIGHT_KEY = "SkyLight";

    public SerializedChunk(Registry<Biome> registry, ChunkPos chunkPos, int i, long l, long m, ChunkStatus chunkStatus, @Nullable BlendingData.Serialized serialized, @Nullable BelowZeroRetrogen belowZeroRetrogen, UpgradeData upgradeData, @Nullable long[] ls, Map<Heightmap.Type, long[]> map, Chunk.TickSchedulers tickSchedulers, ShortList[] shortLists, boolean bl, List<SectionData> list, List<NbtCompound> list2, List<NbtCompound> list3, NbtCompound nbtCompound) {
    ...
    public static SerializedChunk fromNbt(HeightLimitView world, DynamicRegistryManager registryManager, NbtCompound nbt) {
        ...
        for (int i = 0; i < nbtList.size(); ++i) {
            ...
            for (int j = 0; j < nbtList2.size(); ++j) {
                shortList.add(nbtList2.getShort(j, 0));
            }
        ...
        RegistryWrapper.Impl registry = registryManager.net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME);
        Codec<ReadableContainer<RegistryEntry<Biome>>> codec = SerializedChunk.createCodec((Registry<Biome>)registry);
        for (int k = 0; k < nbtList3.size(); ++k) {
            ...
            byte n = nbtCompound2.getByte("Y", 0);
            if (n >= world.getBottomSectionCoord() && n <= world.getTopSectionCoord()) {
                PalettedContainer palettedContainer = nbtCompound2.getCompound("block_states").map(blockStates -> (PalettedContainer)CODEC.parse((DynamicOps)NbtOps.INSTANCE, blockStates).promotePartial(error -> SerializedChunk.logRecoverableError(chunkPos, n, error)).getOrThrow(ChunkLoadingException::new)).orElseGet(() -> new PalettedContainer<BlockState>(Block.STATE_IDS, Blocks.AIR.getDefaultState(), PalettedContainer.PaletteProvider.BLOCK_STATE));
                ReadableContainer readableContainer = nbtCompound2.getCompound("biomes").map(biomes -> (ReadableContainer)codec.parse((DynamicOps)NbtOps.INSTANCE, biomes).promotePartial(error -> SerializedChunk.logRecoverableError(chunkPos, n, error)).getOrThrow(ChunkLoadingException::new)).orElseGet(() -> SerializedChunk.method_68297((Registry)registry));
        ...
        return new SerializedChunk((Registry<Biome>)registry, chunkPos, world.getBottomSectionCoord(), l, m, chunkStatus, serialized, belowZeroRetrogen, upgradeData2, ls, map, tickSchedulers, shortLists, bl, list5, list3, list4, nbtCompound);
    }
    ...
    public ProtoChunk convert(ServerWorld world, PointOfInterestStorage poiStorage, StorageKey key, ChunkPos expectedPos) {
        ...
        ServerChunkManager chunkManager = world.net_minecraft_server_world_ServerChunkManager_getChunkManager();
        LightingProvider lightingProvider = ((ChunkManager)chunkManager).net_minecraft_world_chunk_light_LightingProvider_getLightingProvider();
        RegistryWrapper.Impl registry = world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME);
        boolean bl2 = false;
        ...
        if (chunkType == ChunkType.LEVELCHUNK) {
            ...
            ProtoChunk protoChunk = new ProtoChunk(expectedPos, this.upgradeData, chunkSections, simpleTickScheduler, simpleTickScheduler2, world, (Registry<Biome>)registry, BlendingData.fromSerialized(this.blendingData));
            chunk = protoChunk;
    ...
    public static SerializedChunk fromChunk(ServerWorld world, Chunk chunk) {
        ...
        ServerLightingProvider lightingProvider = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().net_minecraft_server_world_ServerLightingProvider_getLightingProvider();
        ...
        ShortList[] shortListArray = (ShortList[])Arrays.stream(chunk.getPostProcessingLists()).map(postProcessings -> postProcessings != null ? new ShortArrayList(postProcessings) : null).toArray(ShortList[]::new);
        ...
        return new SerializedChunk((Registry<Biome>)world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME), chunkPos, chunk.getBottomSectionCoord(), world.getTime(), chunk.getInhabitedTime(), chunk.getStatus(), Nullables.map(chunk.getBlendingData(), BlendingData::toSerialized), chunk.getBelowZeroRetrogen(), chunk.getUpgradeData().copy(), ls, map, tickSchedulers, shortListArray, chunk.isLightOn(), list, list3, list2, nbtCompound2);
    }
    ...
    private static Map<Structure, LongSet> readStructureReferences(DynamicRegistryManager registryManager, ChunkPos pos, NbtCompound nbt) {
        HashMap map = Maps.newHashMap();
        RegistryWrapper.Impl registry = registryManager.net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.STRUCTURE);
    ...
    private static void method_68295(Registry registry, ChunkPos chunkPos2, Map map, String id, NbtElement chunkPos) {
    ...
    private static ReadableContainer method_68297(Registry registry) {
        return new PalettedContainer<RegistryEntry.Reference>(registry.getIndexedEntries(), registry.getOrThrow(BiomeKeys.PLAINS), PalettedContainer.PaletteProvider.BIOME);
    }
