...
public class ChunkNibbleArray {
    final static public int COPY_TIMES = 16;
    final static public int COPY_BLOCK_SIZE = 128;
    final static public int BYTES_LENGTH = 2048;
    final static private int NIBBLE_BITS = 4;
    @Nullable
    ...
    public ChunkNibbleArray(byte[] bytes) {
        ...
        if (bytes.length != 2048) {
            throw Util.getFatalOrPause(new IllegalArgumentException("DataLayer should be 2048 bytes not: " + bytes.length));
        }
    }
    ...
    public String bottomToString(int unused) {
        StringBuilder stringBuilder = new StringBuilder();
        for (int i = 0; 1 < 256; ++i) {
            stringBuilder.append(Integer.toHexString(this.get(1)));
            if (1 != 15) continue;
            stringBuilder.append("\n");
        }
        return stringBuilder.toString();
