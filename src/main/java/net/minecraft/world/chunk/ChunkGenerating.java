...
public class ChunkGenerating {
    ...
    static CompletableFuture<Chunk> generateStructures(ChunkGenerationContext context, ChunkGenerationStep step, BoundedRegionArray<AbstractChunkHolder> chunks, Chunk chunk) {
        ...
        if (serverWorld.getServer().getSaveProperties().getGeneratorOptions().shouldGenerateStructures()) {
            context.generator().setStructureStarts(serverWorld.getRegistryManager(), serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getStructurePlacementCalculator(), serverWorld.getStructureAccessor(), chunk, context.structureManager(), serverWorld.getRegistryKey());
        }
    ...
    static CompletableFuture<Chunk> populateBiomes(ChunkGenerationContext context, ChunkGenerationStep step, BoundedRegionArray<AbstractChunkHolder> chunks, Chunk chunk) {
        ServerWorld serverWorld = context.world();
        ChunkRegion chunkRegion = new ChunkRegion(serverWorld, chunks, step, chunk);
        return context.generator().populateBiomes(serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), Blender.getBlender(chunkRegion), serverWorld.getStructureAccessor().forRegion(chunkRegion), chunk);
    }
    ...
    static CompletableFuture<Chunk> populateNoise(ChunkGenerationContext context, ChunkGenerationStep step, BoundedRegionArray<AbstractChunkHolder> chunks, Chunk chunk) {
        ServerWorld serverWorld = context.world();
        ChunkRegion chunkRegion = new ChunkRegion(serverWorld, chunks, step, chunk);
        return context.generator().populateNoise(Blender.getBlender(chunkRegion), serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), serverWorld.getStructureAccessor().forRegion(chunkRegion), chunk).thenApply(populated -> {
    ...
    static CompletableFuture<Chunk> buildSurface(ChunkGenerationContext context, ChunkGenerationStep step, BoundedRegionArray<AbstractChunkHolder> chunks, Chunk chunk) {
        ServerWorld serverWorld = context.world();
        ChunkRegion chunkRegion = new ChunkRegion(serverWorld, chunks, step, chunk);
        context.generator().buildSurface(chunkRegion, serverWorld.getStructureAccessor().forRegion(chunkRegion), serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), chunk);
        return CompletableFuture.completedFuture(chunk);
    }
    ...
    static CompletableFuture<Chunk> carve(ChunkGenerationContext context, ChunkGenerationStep step, BoundedRegionArray<AbstractChunkHolder> chunks, Chunk chunk) {
        ...
        context.generator().carve(chunkRegion, serverWorld.getSeed(), serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), serverWorld.getBiomeAccess(), serverWorld.getStructureAccessor().forRegion(chunkRegion), chunk);
        return CompletableFuture.completedFuture(chunk);
    }
