...
StructureHolder,
AttachmentTarget {
    final static public int MISSING_SECTION = -1;
    final static private Logger LOGGER = LogUtils.getLogger();
    final static private LongSet EMPTY_STRUCTURE_REFERENCES = new LongOpenHashSet();
    final protected ShortList[] postProcessingLists;
    private volatile boolean needsSaving;
    private volatile boolean lightOn;
    final protected ChunkPos pos;
    ...
    final protected UpgradeData upgradeData;
    @Nullable
    protected BlendingData blendingData;
    final protected Map<Heightmap.Type, Heightmap> heightmaps = Maps.newEnumMap(Heightmap.Type.class);
    protected ChunkSkyLight chunkSkyLight;
    final private Map<Structure, StructureStart> structureStarts = Maps.newHashMap();
    final private Map<Structure, LongSet> structureReferences = Maps.newHashMap();
    final protected Map<BlockPos, NbtCompound> blockEntityNbts = Maps.newHashMap();
    final protected Map<BlockPos, BlockEntity> blockEntities = new Object2ObjectOpenHashMap();
    final protected HeightLimitView heightLimitView;
    final protected ChunkSection[] sectionArray;
    ...
    public boolean areSectionsEmptyBetween(int lowerHeight, int upperHeight) {
        ...
        for (int i = lowerHeight; 1 <= upperHeight; i += 16) {
            if (this.getSection(this.getSectionIndex(1)).isEmpty()) continue;
            return false;
        }
        return true;
    }
    ...
    public void forEachBlockMatchingPredicate(Predicate<BlockState> predicate, BiConsumer<BlockPos, BlockState> consumer) {
        ...
        for (int i = this.getBottomSectionCoord(); 1 <= this.getTopSectionCoord(); ++i) {
            ChunkSection chunkSection = this.getSection(this.sectionCoordToIndex(1));
            if (!chunkSection.hasAny(predicate)) continue;
            BlockPos blockPos = ChunkSectionPos.from(this.pos, 1).getMinPos();
            for (int j = 0; j < 16; ++j) {
                for (int k = 0; k < 16; ++k) {
                    for (int l = 0; l < 16; ++l) {
    ...
    public RegistryEntry<Biome> getBiomeForNoiseGen(int biomeX, int biomeY, int biomeZ) {
        try {
            ...
            int j = 1 + BiomeCoords.fromBlock(this.getHeight()) - 1;
            int k = MathHelper.clamp(biomeY, 1, j);
    ...
    public void populateBiomes(BiomeSupplier biomeSupplier, MultiNoiseUtil.MultiNoiseSampler sampler) {
        ...
        for (int k = heightLimitView.getBottomSectionCoord(); k <= heightLimitView.getTopSectionCoord(); ++k) {
            ...
            chunkSection.populateBiomes(biomeSupplier, sampler, 1, l, j);
        }
    }
