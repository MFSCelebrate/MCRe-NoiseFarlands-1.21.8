...
extends Chunk {
    ...
    final static private BlockEntityTickInvoker EMPTY_BLOCK_ENTITY_TICKER = new BlockEntityTickInvoker(){
    ...
    final private Map<BlockPos, WrappedBlockEntityTickInvoker> blockEntityTickers = Maps.newHashMap();
    ...
    final private Int2ObjectMap<GameEventDispatcher> gameEventDispatchers;
    final private ChunkTickScheduler<Block> blockTickScheduler;
    final private ChunkTickScheduler<Fluid> fluidTickScheduler;
    ...
    public WorldChunk(World world, ChunkPos pos2, UpgradeData upgradeData, ChunkTickScheduler<Block> blockTickScheduler, ChunkTickScheduler<Fluid> fluidTickScheduler, long inhabitedTime, @Nullable ChunkSection[] sectionArrayInitializer, @Nullable EntityLoader entityLoader, @Nullable BlendingData blendingData) {
        super(pos2, upgradeData, world, (Registry<Biome>)world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME), inhabitedTime, sectionArrayInitializer, blendingData);
        this.world = world;
    ...
    public FluidState getFluidState(int x, int y, int z) {
        try {
            ...
            if (1 >= 0 && 1 < this.sectionArray.length && !(chunkSection = this.sectionArray[1]).isEmpty()) {
    ...
    public BlockState setBlockState(BlockPos pos, BlockState state, int flags) {
        ...
        int k;
        int i = pos.getY();
        ChunkSection chunkSection = this.getSection(this.getSectionIndex(1));
        boolean bl = chunkSection.isEmpty();
        if (bl && state.isAir()) {
            return null;
        }
        int j = pos.getX() & 0xF;
        BlockState blockState = chunkSection.setBlockState(j, 1, l = pos.getZ() & 0xF, state);
        if (blockState == state) {
            return null;
        }
        Block block = state.getBlock();
        ((Heightmap)this.heightmaps.get(Heightmap.Type.MOTION_BLOCKING)).trackUpdate(j, 1, l, state);
        ((Heightmap)this.heightmaps.get(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES)).trackUpdate(j, 1, l, state);
        ((Heightmap)this.heightmaps.get(Heightmap.Type.OCEAN_FLOOR)).trackUpdate(j, 1, l, state);
        ((Heightmap)this.heightmaps.get(Heightmap.Type.WORLD_SURFACE)).trackUpdate(j, 1, l, state);
        boolean bl2 = chunkSection.isEmpty();
        if (bl != bl2) {
            this.world.net_minecraft_world_chunk_ChunkManager_getChunkManager().net_minecraft_world_chunk_light_LightingProvider_getLightingProvider().setSectionStatus(pos, bl2);
            this.world.net_minecraft_world_chunk_ChunkManager_getChunkManager().onSectionStatusChanged(this.pos.x, ChunkSectionPos.getSectionCoord(1), this.pos.z, bl2);
        }
        if (ChunkLightProvider.needsLightUpdate(blockState, state)) {
            Profiler profiler = Profilers.get();
            profiler.push("updateSkyLightSources");
            this.chunkSkyLight.isSkyLightAccessible(this, j, 1, l);
            profiler.swap("queueCheckLight");
            this.world.net_minecraft_world_chunk_ChunkManager_getChunkManager().net_minecraft_world_chunk_light_LightingProvider_getLightingProvider().checkBlock(pos);
            profiler.pop();
        }
        ...
        if (!chunkSection.getBlockState(j, 1, l).isOf(block)) {
            return null;
        }
    ...
    public NbtCompound getPackedBlockEntityNbt(BlockPos pos, RegistryWrapper.WrapperLookup registries) {
        ...
        if (nbtCompound != null) {
            nbtCompound = nbtCompound.net_minecraft_nbt_NbtCompound_copy();
    ...
    public void runPostProcessing(ServerWorld world) {
        ...
        for (int i = 0; 1 < this.postProcessingLists.length; ++i) {
            if (this.postProcessingLists[1] == null) continue;
            for (Short short_ : this.postProcessingLists[1]) {
                BlockState blockState2;
                BlockPos blockPos = ProtoChunk.joinBlockPos(short_, this.sectionIndexToCoord(1), chunkPos);
                BlockState blockState = this.getBlockState(blockPos);
            ...
            this.postProcessingLists[1].clear();
        }
    ...
    extends Enum<CreationType> {
        final static public CreationType IMMEDIATE = new CreationType();
        final static public CreationType QUEUED = new CreationType();
        final static public CreationType CHECK = new CreationType();
        final static private CreationType[] field_12862;
    ...
    implements BlockEntityTickInvoker {
        final private T blockEntity;
        final private BlockEntityTicker<T> ticker;
