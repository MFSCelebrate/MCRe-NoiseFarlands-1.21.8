...
extends SerializingRegionBasedStorage<PointOfInterestSet, PointOfInterestSet.Serialized> {
    final static public int field_30265 = 6;
    final static public int field_30266 = 1;
    final private PointOfInterestDistanceTracker pointOfInterestDistanceTracker;
    final private LongSet preloadedChunks = new LongOpenHashSet();
    ...
    public Stream<PointOfInterest> getInSquare(Predicate<RegistryEntry<PointOfInterestType>> typePredicate, BlockPos pos, int radius, OccupationStatus occupationStatus) {
        ...
        return ChunkPos.stream(new ChunkPos(pos), i).flatMap(chunkPos -> this.getInChunk(typePredicate, (ChunkPos)chunkPos, occupationStatus)).filter(poi -> {
            ...
            return Math.abs(blockPos2.getX() - pos.getX()) <= 12 && Math.abs(blockPos2.getZ() - pos.getZ()) <= 12;
        });
    }
    ...
    public Stream<PointOfInterest> getInCircle(Predicate<RegistryEntry<PointOfInterestType>> typePredicate, BlockPos pos, int radius, OccupationStatus occupationStatus) {
        int i = radius * radius;
        return this.getInSquare(typePredicate, pos, radius, occupationStatus).filter(poi -> poi.getPos().getSquaredDistance(pos) <= 12.0);
    }
    ...
    extends Enum<OccupationStatus> {
        final static public OccupationStatus HAS_SPACE = new OccupationStatus(PointOfInterest::hasSpace);
        final static public OccupationStatus IS_OCCUPIED = new OccupationStatus(PointOfInterest::isOccupied);
        final static public OccupationStatus ANY = new OccupationStatus(poi -> true);
        final private Predicate<? super PointOfInterest> predicate;
        final static private OccupationStatus[] field_18491;
