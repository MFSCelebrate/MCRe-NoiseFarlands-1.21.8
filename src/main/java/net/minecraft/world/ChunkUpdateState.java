...
extends PersistentState {
    final private LongSet all;
    final private LongSet remaining;
    final static private Codec<LongSet> LONG_SET_CODEC = Codec.LONG_STREAM.xmap(LongOpenHashSet::toSet, LongCollection::longStream);
    final static public Codec<ChunkUpdateState> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)LONG_SET_CODEC.fieldOf("All").forGetter(state -> state.all), (App)LONG_SET_CODEC.fieldOf("Remaining").forGetter(state -> state.remaining)).apply((Applicative)instance, ChunkUpdateState::new));
