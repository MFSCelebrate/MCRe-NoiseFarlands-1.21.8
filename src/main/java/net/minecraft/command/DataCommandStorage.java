...
public class DataCommandStorage {
    final static private String COMMAND_STORAGE_PREFIX = "command_storage_";
    final private Map<String, PersistentState> storages = new HashMap<String, PersistentState>();
    final private PersistentStateManager stateManager;
    ...
    extends net.minecraft.world.PersistentState {
        final static public Codec<PersistentState> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.unboundedMap(Codecs.IDENTIFIER_PATH, NbtCompound.CODEC).fieldOf("contents").forGetter(state -> state.map)).apply((Applicative)instance, PersistentState::new));
        final private Map<String, NbtCompound> map;
