...
public class SuggestionProviders {
    final static private Map<Identifier, SuggestionProvider<CommandSource>> REGISTRY = new HashMap<Identifier, SuggestionProvider<CommandSource>>();
    final static private Identifier ASK_SERVER_ID = Identifier.ofVanilla("ask_server");
    final static public SuggestionProvider<CommandSource> ASK_SERVER = SuggestionProviders.register(ASK_SERVER_ID, (SuggestionProvider<CommandSource>)((SuggestionProvider)(context, builder) -> ((CommandSource)context.getSource()).getCompletions(context)));
    final static public SuggestionProvider<CommandSource> AVAILABLE_SOUNDS = SuggestionProviders.register(Identifier.ofVanilla("available_sounds"), (SuggestionProvider<CommandSource>)((SuggestionProvider)(context, builder) -> CommandSource.suggestIdentifiers(((CommandSource)context.getSource()).getSoundIds(), builder)));
    final static public SuggestionProvider<CommandSource> SUMMONABLE_ENTITIES = SuggestionProviders.register(Identifier.ofVanilla("summonable_entities"), (SuggestionProvider<CommandSource>)((SuggestionProvider)(context, builder) -> CommandSource.suggestFromIdentifier(Registries.ENTITY_TYPE.stream().filter(entityType -> entityType.isEnabled(((CommandSource)context.getSource()).getEnabledFeatures()) && entityType.isSummonable()), builder, EntityType::getId, EntityType::getName)));
    ...
    implements SuggestionProvider<CommandSource> {
        final Identifier id;
        final private SuggestionProvider<CommandSource> provider;

        LocalProvider(Identifier identifier, SuggestionProvider<CommandSource> suggestionProvider) {
            this.id = identifier;
