...
implements ArgumentType<EntryPredicate<T>> {
    final static private Collection<String> EXAMPLES = Arrays.asList("foo", "foo:bar", "012", "#skeletons", "#minecraft:skeletons");
    final static private Dynamic2CommandExceptionType NOT_FOUND_EXCEPTION = new Dynamic2CommandExceptionType((tag, type) -> Text.stringifiedTranslatable("argument.resource_tag.not_found", tag, type));
    final static private Dynamic3CommandExceptionType WRONG_TYPE_EXCEPTION = new Dynamic3CommandExceptionType((tag, type, expectedType) -> Text.stringifiedTranslatable("argument.resource_tag.invalid_type", tag, type, expectedType));
    ...
    implements ArgumentSerializer<RegistryEntryPredicateArgumentType<T>, Properties> {
        ...
        @Override
        public Properties net_minecraft_command_argument_RegistryEntryPredicateArgumentType$Serializer$Properties_fromPacket(PacketByteBuf packetByteBuf) {
        ...
        public ArgumentSerializer.ArgumentTypeProperties net_minecraft_command_argument_serialize_ArgumentSerializer$ArgumentTypeProperties_fromPacket(PacketByteBuf buf) {
            return this.net_minecraft_command_argument_RegistryEntryPredicateArgumentType$Serializer$Properties_fromPacket(buf);
        }
