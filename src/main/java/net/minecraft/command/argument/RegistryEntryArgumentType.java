...
implements ArgumentType<RegistryEntry<T>> {
    final static private Collection<String> EXAMPLES = List.of("foo", "foo:bar", "012", "{}", "true");
    final static public DynamicCommandExceptionType FAILED_TO_PARSE_EXCEPTION = new DynamicCommandExceptionType(argument -> Text.stringifiedTranslatable("argument.resource_or_id.failed_to_parse", argument));
    final static public Dynamic2CommandExceptionType NO_SUCH_ELEMENT_EXCEPTION = new Dynamic2CommandExceptionType((key, registryRef) -> Text.stringifiedTranslatable("argument.resource_or_id.no_such_element", key, registryRef));
    final static public DynamicOps<NbtElement> OPS = NbtOps.INSTANCE;
    final private RegistryWrapper.WrapperLookup registries;
    final private Optional<? extends RegistryWrapper.Impl<T>> registry;
    final private Codec<T> entryCodec;
    final private PackratParser<EntryParser<T, NbtElement>> parser;
    final private RegistryKey<? extends Registry<T>> registryRef;
