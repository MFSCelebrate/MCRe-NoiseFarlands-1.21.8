...
implements ArgumentType<Identifier> {
    final static private Collection<String> EXAMPLES = Stream.of(World.OVERWORLD, World.NETHER).map(key -> key.getValue().toString()).collect(Collectors.toList());
    final static private DynamicCommandExceptionType INVALID_DIMENSION_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("argument.dimension.invalid", id));
