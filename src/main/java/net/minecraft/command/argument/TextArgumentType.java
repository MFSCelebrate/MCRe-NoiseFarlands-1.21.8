...
extends ParserBackedArgumentType<Text> {
    final static private Collection<String> EXAMPLES = Arrays.asList("\"hello world\"", "'hello world'", "\"\"", "{text:\"hello world\"}", "[\"\"]");
    final static public DynamicCommandExceptionType INVALID_COMPONENT_EXCEPTION = new DynamicCommandExceptionType(text -> Text.stringifiedTranslatable("argument.component.invalid", text));
    final static private DynamicOps<NbtElement> OPS = NbtOps.INSTANCE;
    final static private Parser<NbtElement> PARSER = SnbtParsing.createParser(OPS);
