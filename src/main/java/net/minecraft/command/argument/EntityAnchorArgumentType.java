...
implements ArgumentType<EntityAnchor> {
    final static private Collection<String> EXAMPLES = Arrays.asList("eyes", "feet");
    final static private DynamicCommandExceptionType INVALID_ANCHOR_EXCEPTION = new DynamicCommandExceptionType(name -> Text.stringifiedTranslatable("argument.anchor.invalid", name));
    ...
    public EntityAnchor net_minecraft_command_argument_EntityAnchorArgumentType$EntityAnchor_parse(StringReader stringReader) throws CommandSyntaxException {
    ...
    public Object java_lang_Object_parse(StringReader reader) throws CommandSyntaxException {
        return this.net_minecraft_command_argument_EntityAnchorArgumentType$EntityAnchor_parse(reader);
    }

    public static final class EntityAnchor
    extends Enum<EntityAnchor> {
        final static public EntityAnchor FEET = new EntityAnchor("feet", (pos, entity) -> pos);
        final static public EntityAnchor EYES = new EntityAnchor("eyes", (pos, entity) -> new Vec3d(pos.x, pos.y + (double)entity.getStandingEyeHeight(), pos.z));
        final static Map<String, EntityAnchor> ANCHORS;
        final private String id;
        final private BiFunction<Vec3d, Entity, Vec3d> offset;
        final static private EntityAnchor[] field_9850;
