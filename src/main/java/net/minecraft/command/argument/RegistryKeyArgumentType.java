...
implements ArgumentType<RegistryKey<T>> {
    ...
    final static private DynamicCommandExceptionType INVALID_FEATURE_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.place.feature.invalid", id));
    final static private DynamicCommandExceptionType INVALID_STRUCTURE_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.place.structure.invalid", id));
    final static private DynamicCommandExceptionType INVALID_JIGSAW_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.place.jigsaw.invalid", id));
    final static private DynamicCommandExceptionType RECIPE_NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("recipe.notFound", id));
    final static private DynamicCommandExceptionType ADVANCEMENT_NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("advancement.advancementNotFound", id));
    ...
    private static <T> Registry<T> getRegistry(CommandContext<ServerCommandSource> context, RegistryKey<? extends Registry<T>> registryRef) {
        return ((ServerCommandSource)context.getSource()).getServer().getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(registryRef);
    }
    ...
    implements ArgumentSerializer<RegistryKeyArgumentType<T>, Properties> {
        ...
        @Override
        public Properties net_minecraft_command_argument_RegistryKeyArgumentType$Serializer$Properties_fromPacket(PacketByteBuf packetByteBuf) {
        ...
        public ArgumentSerializer.ArgumentTypeProperties net_minecraft_command_argument_serialize_ArgumentSerializer$ArgumentTypeProperties_fromPacket(PacketByteBuf buf) {
            return this.net_minecraft_command_argument_RegistryKeyArgumentType$Serializer$Properties_fromPacket(buf);
        }
