...
implements ArgumentType<GameMode> {
    final static private Collection<String> EXAMPLES = Stream.of(GameMode.SURVIVAL, GameMode.CREATIVE).map(GameMode::getId).collect(Collectors.toList());
    final static private GameMode[] VALUES = GameMode.values();
    final static private DynamicCommandExceptionType INVALID_GAME_MODE_EXCEPTION = new DynamicCommandExceptionType(gameMode -> Text.stringifiedTranslatable("argument.gamemode.invalid", gameMode));

    public GameMode net_minecraft_world_GameMode_parse(StringReader stringReader) throws CommandSyntaxException {
    ...
    public Object java_lang_Object_parse(StringReader reader) throws CommandSyntaxException {
        return this.net_minecraft_world_GameMode_parse(reader);
    }
}
