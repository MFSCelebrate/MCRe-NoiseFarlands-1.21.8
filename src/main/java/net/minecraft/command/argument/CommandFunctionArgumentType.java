...
implements ArgumentType<FunctionArgument> {
    final static private Collection<String> EXAMPLES = Arrays.asList("foo", "foo:bar", "#foo");
    final static private DynamicCommandExceptionType UNKNOWN_FUNCTION_TAG_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("arguments.function.tag.unknown", id));
    final static private DynamicCommandExceptionType UNKNOWN_FUNCTION_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("arguments.function.unknown", id));

    public static CommandFunctionArgumentType commandFunction() {
        return new CommandFunctionArgumentType();
    }

    public FunctionArgument net_minecraft_command_argument_CommandFunctionArgumentType$FunctionArgument_parse(StringReader stringReader) throws CommandSyntaxException {
    ...
    public Object java_lang_Object_parse(StringReader reader) throws CommandSyntaxException {
        return this.net_minecraft_command_argument_CommandFunctionArgumentType$FunctionArgument_parse(reader);
    }
