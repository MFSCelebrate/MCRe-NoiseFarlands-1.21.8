...
implements ArgumentType<Collection<RegistryEntry.Reference<T>>> {
    final static private Collection<String> EXAMPLES = List.of("minecraft:*", "*:asset", "*");
    final static public Dynamic2CommandExceptionType NOT_FOUND_EXCEPTION = new Dynamic2CommandExceptionType((selector, registryRef) -> Text.stringifiedTranslatable("argument.resource_selector.not_found", selector, registryRef));
    final RegistryKey<? extends Registry<T>> registryRef;
    final private RegistryWrapper<T> registry;

    RegistrySelectorArgumentType(CommandRegistryAccess registries, RegistryKey<? extends Registry<T>> registryRef) {
        this.registryRef = registryRef;
        this.registry = registries.net_minecraft_registry_RegistryEntryLookup_getOrThrow(registryRef);
    }
    ...
    implements ArgumentSerializer<RegistrySelectorArgumentType<T>, Properties> {
        ...
        @Override
        public Properties net_minecraft_command_argument_RegistrySelectorArgumentType$Serializer$Properties_fromPacket(PacketByteBuf packetByteBuf) {
        ...
        public ArgumentSerializer.ArgumentTypeProperties net_minecraft_command_argument_serialize_ArgumentSerializer$ArgumentTypeProperties_fromPacket(PacketByteBuf buf) {
            return this.net_minecraft_command_argument_RegistrySelectorArgumentType$Serializer$Properties_fromPacket(buf);
        }
