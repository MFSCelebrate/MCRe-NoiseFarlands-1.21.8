...
public class EntitySelector {
    final static public int MAX_VALUE = Integer.MAX_VALUE;
    final static public BiConsumer<Vec3d, List<? extends Entity>> ARBITRARY = (pos, entities) -> {};
    final static private TypeFilter<Entity, ?> PASSTHROUGH_FILTER = new TypeFilter<Entity, Entity>(){
    ...
    final private boolean includesNonPlayers;
    final private boolean localWorldOnly;
    final private List<Predicate<Entity>> predicates;
    final private NumberRange.DoubleRange distance;
    final private Function<Vec3d, Vec3d> positionOffset;
    @Nullable
    final private Box box;
    final private BiConsumer<Vec3d, List<? extends Entity>> sorter;
    final private boolean senderOnly;
    @Nullable
    final private String playerName;
    @Nullable
    final private UUID uuid;
    final private TypeFilter<Entity, ?> entityFilter;
    final private boolean usesAt;
    ...
    public List<? extends Entity> getEntities(ServerCommandSource source) throws CommandSyntaxException {
        ...
        if (this.playerName != null) {
            ServerPlayerEntity serverPlayerEntity = source.getServer().net_minecraft_server_PlayerManager_getPlayerManager().getPlayer(this.playerName);
        ...
        if (this.uuid != null) {
            for (ServerWorld serverWorld : source.getServer().getWorlds()) {
                Entity entity = serverWorld.net_minecraft_entity_Entity_getEntity(this.uuid);
    ...
    public List<ServerPlayerEntity> getPlayers(ServerCommandSource source) throws CommandSyntaxException {
        ...
        if (this.uuid != null) {
            ServerPlayerEntity serverPlayerEntity = source.getServer().net_minecraft_server_PlayerManager_getPlayerManager().getPlayer(this.uuid);
            if (serverPlayerEntity == null) {
                return List.of();
            }
            return List.of(serverPlayerEntity);
        ...
        if (this.isLocalWorldOnly()) {
            ...
            for (ServerPlayerEntity serverPlayerEntity3 : source.getServer().net_minecraft_server_PlayerManager_getPlayerManager().getPlayerList()) {
    ...
    private Predicate<Entity> getPositionPredicate(Vec3d pos, @Nullable Box box, @Nullable FeatureSet enabledFeatures) {
        ...
        int i = (bl ? 1 : 0) + ((bl2 = box != null) ? 1 : 0) + ((bl3 = !this.distance.isDummy()) ? 1 : 0);
        if (i == 0) {
