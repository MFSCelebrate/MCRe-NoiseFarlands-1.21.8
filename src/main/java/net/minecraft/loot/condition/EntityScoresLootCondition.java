...
{
    final static public MapCodec<EntityScoresLootCondition> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.unboundedMap((Codec)Codec.STRING, BoundedIntUnaryOperator.CODEC).fieldOf("scores").forGetter(EntityScoresLootCondition::scores), (App)LootContext.EntityTarget.CODEC.fieldOf("entity").forGetter(EntityScoresLootCondition::entity)).apply((Applicative)instance, EntityScoresLootCondition::new));
    ...
    public Set<ContextParameter<?>> getAllowedParameters() {
        return (Set)Stream.concat(Stream.of(this.entity.getParameter()), this.scores.values().stream().flatMap(operator -> operator.getRequiredParameters().stream())).collect(ImmutableSet.toImmutableSet());
    }
    ...
    public boolean test(LootContext lootContext) {
        ...
        ServerScoreboard scoreboard = lootContext.getWorld().net_minecraft_scoreboard_ServerScoreboard_getScoreboard();
    ...
    implements LootCondition.Builder {
        final private ImmutableMap.Builder<String, BoundedIntUnaryOperator> scores = ImmutableMap.builder();
        final private LootContext.EntityTarget target;
        ...
        @Override
        public LootCondition net_minecraft_loot_condition_LootCondition_build() {
