...
implements LootFunction {
    final static public MapCodec<AndLootFunction> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)LootFunctionTypes.BASE_CODEC.listOf().fieldOf("functions").forGetter(function -> function.terms)).apply((Applicative)instance, AndLootFunction::new));
    final static public Codec<AndLootFunction> INLINE_CODEC = LootFunctionTypes.BASE_CODEC.listOf().xmap(AndLootFunction::new, function -> function.terms);
    final private List<LootFunction> terms;
    final private BiFunction<ItemStack, LootContext, ItemStack> applier;
