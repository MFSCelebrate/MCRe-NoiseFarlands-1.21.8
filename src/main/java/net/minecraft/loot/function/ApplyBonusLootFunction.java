...
extends ConditionalLootFunction {
    final static private Map<Identifier, Type> FACTORIES = Stream.of(BinomialWithBonusCount.TYPE, OreDrops.TYPE, UniformBonusCount.TYPE).collect(Collectors.toMap(Type::id, Function.identity()));
    final static private Codec<Type> TYPE_CODEC = Identifier.CODEC.comapFlatMap(id -> {
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        if (itemStack != null) {
            ...
            int j = this.formula.getValue(context.getRandom(), stack.getCount(), 1);
            stack.setCount(j);
        }
        return stack;
    }
    ...
    {
        final static public Codec<UniformBonusCount> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.INT.fieldOf("bonusMultiplier").forGetter(UniformBonusCount::bonusMultiplier)).apply((Applicative)instance, UniformBonusCount::new));
        final static public Type TYPE = new Type(Identifier.ofVanilla("uniform_bonus_count"), CODEC);
    ...
    {
        final static public Codec<OreDrops> CODEC = Codec.unit(OreDrops::new);
        final static public Type TYPE = new Type(Identifier.ofVanilla("ore_drops"), CODEC);
