...
extends ConditionalLootFunction {
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        Stream<RegistryEntry> stream = this.options.map(RegistryEntryList::stream).orElseGet(() -> context.getWorld().getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().map(Function.identity())).filter(entry -> !bl2 || ((Enchantment)entry.value()).isAcceptableItem(stack));
    ...
    public static Builder builder(RegistryWrapper.WrapperLookup registries) {
        return EnchantRandomlyLootFunction.create().options(registries.net_minecraft_registry_RegistryEntryLookup_getOrThrow(RegistryKeys.ENCHANTMENT).getOrThrow(EnchantmentTags.ON_RANDOM_LOOT));
    }
    ...
    extends ConditionalLootFunction.Builder<Builder> {
        ...
        @Override
        protected Builder net_minecraft_loot_function_EnchantRandomlyLootFunction$Builder_getThisBuilder() {
            return this;
        }
        ...
        protected ConditionalLootFunction.Builder net_minecraft_loot_function_ConditionalLootFunction$Builder_getThisBuilder() {
            return this.net_minecraft_loot_function_EnchantRandomlyLootFunction$Builder_getThisBuilder();
        }
    }
}
