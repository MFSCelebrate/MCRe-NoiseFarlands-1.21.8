...
extends SimpleTagProvider<Enchantment> {
    ...
    protected void createTooltipOrderTag(RegistryWrapper.WrapperLookup registries, RegistryKey<Enchantment> ... enchantments) {
        ...
        List list = registries.net_minecraft_registry_RegistryEntryLookup_getOrThrow(RegistryKeys.ENCHANTMENT).streamEntries().filter(entry -> !set.contains(entry.getKey().get())).map(RegistryEntry::getIdAsString).collect(Collectors.toList());
        if (!list.isEmpty()) {
