...
extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (result != null) {
            BlockPos blockPos2 = result.getFrontTopLeft().net_minecraft_util_math_BlockPos_add(-3, 0, -3);
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
                    BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_add(i, 0, j);
            ...
            world.syncGlobalEvent(WorldEvents.END_PORTAL_OPENED, blockPos2.net_minecraft_util_math_BlockPos_add(1, 0, 1), 0);
        }
        return ActionResult.SUCCESS;
    }
