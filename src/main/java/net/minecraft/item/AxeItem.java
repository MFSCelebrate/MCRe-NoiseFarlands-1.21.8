...
extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (playerEntity != null) {
            itemStack.damage(1, (LivingEntity)playerEntity, LivingEntity.getSlotForHand(context.getHand()));
        }
    ...
    private Optional<BlockState> tryStrip(World world, BlockPos pos, @Nullable PlayerEntity player, BlockState state) {
        ...
        if (optional2.isPresent()) {
            world.playSound((Entity)player, pos, SoundEvents.ITEM_AXE_SCRAPE, SoundCategory.BLOCKS, 1.0f, 1.0f);
        ...
        if (optional3.isPresent()) {
            world.playSound((Entity)player, pos, SoundEvents.ITEM_AXE_WAX_OFF, SoundCategory.BLOCKS, 1.0f, 1.0f);
