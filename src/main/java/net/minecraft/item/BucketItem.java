...
implements FluidModificationItem {
    ...
    public boolean placeFluid(@Nullable LivingEntity user, World world, BlockPos pos, @Nullable BlockHitResult hitResult) {
        ...
        boolean bl3 = bl2 = blockState.isAir() || bl || block instanceof FluidFillable && (fluidFillable = (FluidFillable)((Object)block)).canFillWithFluid(user, world, pos, blockState, this.fluid);
        if (!bl2) {
            return hitResult != null && this.placeFluid(user, world, hitResult.getBlockPos().net_minecraft_util_math_BlockPos_offset(hitResult.getSide()), null);
        }
