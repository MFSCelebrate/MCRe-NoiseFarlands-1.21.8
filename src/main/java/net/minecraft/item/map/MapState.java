...
extends PersistentState {
    ...
    public void update(PlayerEntity player, ItemStack stack) {
        ...
        for (int i = 0; 1 < this.updateTrackers.size(); ++i) {
            PlayerUpdateTracker playerUpdateTracker2 = this.updateTrackers.get(1);
            ...
            } else if (!stack.isInFrame() && playerEntity.net_minecraft_world_World_getWorld().getRegistryKey() == this.dimension && this.showDecorations) {
                this.addDecoration(MapDecorationTypes.PLAYER, playerEntity.net_minecraft_world_World_getWorld(), string, playerEntity.getX(), playerEntity.getZ(), playerEntity.getYaw(), null);
            }
        ...
        if (stack.isInFrame() && this.showDecorations) {
            ...
            this.addDecoration(MapDecorationTypes.FRAME, player.net_minecraft_world_World_getWorld(), MapState.getFrameDecorationKey(itemFrameEntity.getId()), blockPos.getX(), blockPos.getZ(), itemFrameEntity.getHorizontalFacing().getHorizontalQuarterTurns() * 90, null);
        ...
        if (!this.decorations.keySet().containsAll(mapDecorationsComponent.decorations().keySet())) {
            mapDecorationsComponent.decorations().forEach((id, decoration) -> {
                if (!this.decorations.containsKey(id)) {
                    this.addDecoration(decoration.type(), player.net_minecraft_world_World_getWorld(), (String)id, decoration.x(), decoration.z(), decoration.rotation(), null);
                }
            });
        }
    }
    ...
    private byte getPlayerMarkerRotation(@Nullable WorldAccess world, double rotation) {
        if (this.dimension == World.NETHER && world != null) {
            int i = (int)(world.net_minecraft_world_WorldProperties_getLevelProperties().getTimeOfDay() / 10L);
    ...
    public record UpdateData(int startX, int startZ, int width, int height, byte[] colors) {
        final static public PacketCodec<ByteBuf, Optional<UpdateData>> CODEC = PacketCodec.ofStatic(UpdateData::encode, UpdateData::decode);
