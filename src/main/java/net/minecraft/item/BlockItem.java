...
extends Item {
    ...
    public boolean shouldShowOperatorBlockWarnings(ItemStack stack, @Nullable PlayerEntity player) {
        ...
        if (player != null && player.getPermissionLevel() >= 2 && (nbtComponent = stack.get(DataComponentTypes.BLOCK_ENTITY_DATA)) != null) {
            BlockEntityType<?> blockEntityType = nbtComponent.getRegistryValueOfId(player.net_minecraft_world_World_getWorld().getRegistryManager(), RegistryKeys.BLOCK_ENTITY_TYPE);
