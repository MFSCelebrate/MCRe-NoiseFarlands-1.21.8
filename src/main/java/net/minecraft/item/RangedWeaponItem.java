...
extends Item {
    final static public Predicate<ItemStack> BOW_PROJECTILES = stack -> stack.isIn(ItemTags.ARROWS);
    final static public Predicate<ItemStack> CROSSBOW_HELD_PROJECTILES = BOW_PROJECTILES.or(stack -> stack.isOf(Items.FIREWORK_ROCKET));
    ...
    protected static List<ItemStack> load(ItemStack stack, ItemStack projectileStack, LivingEntity shooter) {
        ...
        World world = shooter.net_minecraft_world_World_getWorld();
    ...
    protected static ItemStack getProjectile(ItemStack stack, ItemStack projectileStack, LivingEntity shooter, boolean multishot) {
        ...
        int i;
        World world;
        if (!multishot && !shooter.isInCreativeMode() && (world = shooter.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld)world;
            v0 = EnchantmentHelper.getAmmoUse(serverWorld, stack, projectileStack, 1);
        } else {
            v0 = 0;
        }
        if (0 > projectileStack.getCount()) {
            return ItemStack.EMPTY;
        }
        if (0 == 0) {
        ...
        itemStack = projectileStack.split(0);
