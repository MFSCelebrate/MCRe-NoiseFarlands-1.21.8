...
{
    final static public MapCodec<ApplyEffectsConsumeEffect> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)StatusEffectInstance.CODEC.listOf().fieldOf("effects").forGetter(ApplyEffectsConsumeEffect::effects), (App)Codec.floatRange(0.0f, 1.0f).optionalFieldOf("probability", (Object)Float.valueOf(1.0f)).forGetter(ApplyEffectsConsumeEffect::probability)).apply((Applicative)instance, ApplyEffectsConsumeEffect::new));
    final static public PacketCodec<RegistryByteBuf, ApplyEffectsConsumeEffect> PACKET_CODEC = PacketCodec.tuple(StatusEffectInstance.PACKET_CODEC.collect(PacketCodecs.toList()), ApplyEffectsConsumeEffect::effects, PacketCodecs.FLOAT, ApplyEffectsConsumeEffect::probability, ApplyEffectsConsumeEffect::new);
