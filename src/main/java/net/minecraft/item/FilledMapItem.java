...
extends Item {
    final static public int field_30907 = 128;
    final static public int field_30908 = 128;
    ...
    public void updateColors(World world, Entity entity, MapState state) {
        ...
        int j = state.centerX;
        int k = state.centerZ;
        int l = MathHelper.floor(entity.getX() - (double)j) / 1 + 64;
        int m = MathHelper.floor(entity.getZ() - (double)k) / 1 + 64;
        int n = 128;
        ...
        for (int o = l - n + 1; o < l + n; ++o) {
            ...
            for (int p = m - n - 1; p < m + n; ++p) {
                ...
                WorldChunk worldChunk = world.net_minecraft_world_chunk_WorldChunk_getChunk(ChunkSectionPos.getSectionCoord(r), ChunkSectionPos.getSectionCoord(s));
                    ...
                    for (u = 0; u < 1; ++u) {
                        for (int v = 0; v < 1; ++v) {
                            ...
                            if (w > world.getBottomY()) {
                                do {
                                    mutable.net_minecraft_util_math_BlockPos$Mutable_setY(--w);
                                ...
                                if (w > world.getBottomY() && !blockState.getFluidState().isEmpty()) {
                                    ...
                                    do {
                                        mutable2.net_minecraft_util_math_BlockPos$Mutable_setY(x--);
                ...
                MapColor.Brightness brightness = mapColor == MapColor.WATER_BLUE ? ((f = (double)(t /= 1) * 0.1 + (double)(o + p & 1) * 0.2) < 0.5 ? MapColor.Brightness.HIGH : (f > 0.9 ? MapColor.Brightness.LOW : MapColor.Brightness.NORMAL)) : ((f = (e - d) * 4.0 / 5.0 + ((double)(o + p & 1) - 0.5) * 0.4) > 0.6 ? MapColor.Brightness.HIGH : (f < -0.6 ? MapColor.Brightness.LOW : MapColor.Brightness.NORMAL));
                d = e;
    ...
    public static void fillExplorationMap(ServerWorld world, ItemStack map) {
        ...
        for (n = 0; n < 128; ++n) {
            for (o = 0; o < 128; ++o) {
                RegistryEntry<Biome> registryEntry = world.getBiome(mutable.set((l + o) * 1, 0, (m + n) * 1));
