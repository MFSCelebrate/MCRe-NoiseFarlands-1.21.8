...
implements SignChangingItem {
    final static private Map<DyeColor, DyeItem> DYES = Maps.newEnumMap(DyeColor.class);
    ...
    public ActionResult useOnEntity(ItemStack stack, PlayerEntity user, LivingEntity entity, Hand hand) {
        ...
        if (entity instanceof SheepEntity && (sheepEntity = (SheepEntity)entity).isAlive() && !sheepEntity.isSheared() && sheepEntity.getColor() != this.color) {
            sheepEntity.net_minecraft_world_World_getWorld().playSoundFromEntity(user, sheepEntity, SoundEvents.ITEM_DYE_USE, SoundCategory.PLAYERS, 1.0f, 1.0f);
            if (!user.net_minecraft_world_World_getWorld().isClient) {
