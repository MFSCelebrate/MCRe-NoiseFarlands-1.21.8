...
public class CombinedDynamicRegistries<T> {
    final private List<T> types;
    final private List<DynamicRegistryManager.Immutable> registryManagers;
    final private DynamicRegistryManager.Immutable combinedRegistryManager;
    ...
    public CombinedDynamicRegistries<T> with(T type, List<DynamicRegistryManager.Immutable> registryManagers) {
        int i = this.getIndex(type);
        if (registryManagers.size() > this.registryManagers.size() - 1) {
