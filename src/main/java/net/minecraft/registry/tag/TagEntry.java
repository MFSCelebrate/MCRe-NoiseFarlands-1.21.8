...
public class TagEntry {
    final static private Codec<TagEntry> ENTRY_CODEC = RecordCodecBuilder.create((T instance) -> instance.group((App)Codecs.TAG_ENTRY_ID.fieldOf("id").forGetter(TagEntry::getIdForCodec), (App)Codec.BOOL.optionalFieldOf("required", (Object)true).forGetter(entry -> entry.required)).apply((Applicative)instance, TagEntry::new));
    final static public Codec<TagEntry> CODEC = Codec.either(Codecs.TAG_ENTRY_ID, ENTRY_CODEC).xmap(either -> (TagEntry)either.map(id -> new TagEntry((Codecs.TagEntryId)id, true), entry -> entry), entry -> entry.required ? Either.left((Object)entry.getIdForCodec()) : Either.right((Object)entry));
    final private Identifier id;
    final private boolean tag;
    ...
    public static interface ValueGetter<T> {
        @Nullable
        public T direct(Identifier var1, boolean var2);
