...
extends StructureProcessor {
    final static public MapCodec<BlockRotStructureProcessor> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)RegistryCodecs.entryList(RegistryKeys.BLOCK).optionalFieldOf("rottable_blocks").forGetter(processor -> processor.rottableBlocks), (App)Codec.floatRange(0.0f, 1.0f).fieldOf("integrity").forGetter(processor -> Float.valueOf(processor.integrity))).apply((Applicative)instance, BlockRotStructureProcessor::new));
    final private Optional<RegistryEntryList<Block>> rottableBlocks;
