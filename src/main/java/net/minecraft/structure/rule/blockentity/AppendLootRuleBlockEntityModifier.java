...
implements RuleBlockEntityModifier {
    final static public MapCodec<AppendLootRuleBlockEntityModifier> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)LootTable.TABLE_KEY.fieldOf("loot_table").forGetter(modifier -> modifier.lootTable)).apply((Applicative)instance, AppendLootRuleBlockEntityModifier::new));
    final private RegistryKey<LootTable> lootTable;
    ...
    public NbtCompound modifyBlockEntityNbt(Random random, @Nullable NbtCompound nbt) {
        NbtCompound nbtCompound = nbt == null ? new NbtCompound() : nbt.net_minecraft_nbt_NbtCompound_copy();
