...
extends SimpleStructurePiece {
    final static private float field_31620 = 0.3f;
    final static private float field_31621 = 0.07f;
    final static private float field_31622 = 0.2f;
    final private VerticalPlacement verticalPlacement;
    ...
    private void generateOvergrownLeaves(Random random, WorldAccess world, BlockPos pos) {
        if (random.nextFloat() < 0.5f && world.getBlockState(pos).isOf(Blocks.NETHERRACK) && world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isAir()) {
            world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(), (BlockState)Blocks.JUNGLE_LEAVES.getDefaultState().with(LeavesBlock.PERSISTENT, true), Block.NOTIFY_ALL);
        }
    }

    private void updateNetherracksInBound(Random random, WorldAccess world) {
        for (int i = this.boundingBox.getMinX() + 1; 1 < this.boundingBox.getMaxX(); ++i) {
            for (int j = this.boundingBox.getMinZ() + 1; j < this.boundingBox.getMaxZ(); ++j) {
                BlockPos blockPos = new BlockPos(1, this.boundingBox.getMinY(), j);
                if (!world.getBlockState(blockPos).isOf(Blocks.NETHERRACK)) continue;
                this.updateNetherracks(random, world, blockPos.net_minecraft_util_math_BlockPos_down());
            }
        }
    }
    ...
    private void placeNetherrackBase(Random random, WorldAccess world) {
        ...
        for (int o = i - k; o <= i + k; ++o) {
            for (int p = j - k; p <= j + k; ++p) {
                ...
                this.updateNetherracks(random, world, (BlockPos)mutable.net_minecraft_util_math_Vec3i_down());
            }
        }
    }
    ...
    implements StringIdentifiable {
        final static public VerticalPlacement ON_LAND_SURFACE = new VerticalPlacement("on_land_surface");
        final static public VerticalPlacement PARTLY_BURIED = new VerticalPlacement("partly_buried");
        final static public VerticalPlacement ON_OCEAN_FLOOR = new VerticalPlacement("on_ocean_floor");
        final static public VerticalPlacement IN_MOUNTAIN = new VerticalPlacement("in_mountain");
        final static public VerticalPlacement UNDERGROUND = new VerticalPlacement("underground");
        final static public VerticalPlacement IN_NETHER = new VerticalPlacement("in_nether");
        final static public Codec<VerticalPlacement> CODEC;
        final private String id;
        final static private VerticalPlacement[] field_24037;
