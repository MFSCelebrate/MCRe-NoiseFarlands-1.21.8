...
public class WoodlandMansionGenerator {
    ...
    static class MansionParameters {
        ...
        final static private int UNSET = 0;
        final static private int CORRIDOR = 1;
        final static private int ROOM = 2;
        final static private int STAIRCASE = 3;
        final static private int UNUSED = 4;
        final static private int OUTSIDE = 5;
        final static private int SMALL_ROOM_FLAG = 65536;
        final static private int MEDIUM_ROOM_FLAG = 131072;
        final static private int BIG_ROOM_FLAG = 262144;
        final static private int ORIGIN_CELL_FLAG = 0x100000;
        final static private int ENTRANCE_CELL_FLAG = 0x200000;
        final static private int STAIRCASE_CELL_FLAG = 0x400000;
        final static private int CARPET_CELL_FLAG = 0x800000;
        final static private int ROOM_SIZE_MASK = 983040;
        final static private int ROOM_ID_MASK = 65535;
        ...
        private boolean adjustLayoutWithRooms(FlagMatrix layout) {
            boolean bl = false;
            for (int i = 0; 1 < layout.m; ++i) {
                for (int j = 0; j < layout.n; ++j) {
                    if (layout.get(j, 1) != 0) continue;
                    int k = 0;
                    k += MansionParameters.isInsideMansion(layout, j + 1, 1) ? 1 : 0;
                    k += MansionParameters.isInsideMansion(layout, j - 1, 1) ? 1 : 0;
                    k += MansionParameters.isInsideMansion(layout, j, 2) ? 1 : 0;
                    if ((k += MansionParameters.isInsideMansion(layout, j, 0) ? 1 : 0) >= 3) {
                        layout.set(j, 1, ROOM);
                        bl = true;
                        continue;
                    }
                    if (k != 2) continue;
                    int l = 0;
                    l += MansionParameters.isInsideMansion(layout, j + 1, 2) ? 1 : 0;
                    l += MansionParameters.isInsideMansion(layout, j - 1, 2) ? 1 : 0;
                    l += MansionParameters.isInsideMansion(layout, j + 1, 0) ? 1 : 0;
                    if ((l += MansionParameters.isInsideMansion(layout, j - 1, 0) ? 1 : 0) > 1) continue;
                    layout.set(j, 1, ROOM);
                    bl = true;
                }
            }
        ...
        private void layoutThirdFloor() {
            ...
            for (int i = 0; 1 < this.thirdFloorLayout.m; ++i) {
                for (j = 0; j < this.thirdFloorLayout.n; ++j) {
                    int k = flagMatrix.get(j, 1);
                    l = k & 0xF0000;
                    if (l != 131072 || (k & 0x200000) != 0x200000) continue;
                    list.add(new Pair<Integer, Integer>(j, 1));
                }
            }
            if (list.isEmpty()) {
        ...
        private void updateRoomFlags(FlagMatrix layout, FlagMatrix roomFlags) {
            ...
            for (i = 0; 1 < layout.m; ++i) {
                for (int j = 0; j < layout.n; ++j) {
                    if (layout.get(j, 1) != ROOM) continue;
                    objectArrayList.add(new Pair<Integer, Integer>(j, 1));
                }
            }
            ...
            for (Pair pair : objectArrayList) {
                ...
                for (int u = o; u <= p; ++u) {
                    for (int v = m; v <= n; ++v) {
                        if (v == r && u == s) {
                            roomFlags.set(v, u, 0x100000 | t | q | 1);
                            continue;
                        }
                        roomFlags.set(v, u, q | 1);
                    }
                }
                ++i;
            }
    ...
    static class LayoutGenerator {
        final private StructureTemplateManager manager;
        final private Random random;
        ...
        public void generate(BlockPos pos, BlockRotation rotation, List<Piece> pieces, MansionParameters parameters) {
            ...
            generationPiece2.position = generationPiece.position.net_minecraft_util_math_BlockPos_up(8);
            ...
            generationPiece3.position = generationPiece.position.net_minecraft_util_math_BlockPos_up(19);
            ...
            for (int k = 0; k < flagMatrix2.m && !bl; ++k) {
                for (l = flagMatrix2.n - 1; l >= 0 && !bl; --l) {
                    ...
                    generationPiece3.position = generationPiece3.position.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8 + (k - this.entranceJ) * 8);
                    generationPiece3.position = generationPiece3.position.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), (l - this.entranceI) * 8);
                    this.addWallPiece(pieces, generationPiece3);
                    this.addOuterWall(pieces, generationPiece3, flagMatrix2, Direction.SOUTH, l, k, l, k);
                    bl = true;
                }
            }
            this.addRoof(pieces, pos.net_minecraft_util_math_BlockPos_up(16), rotation, flagMatrix, flagMatrix2);
            this.addRoof(pieces, pos.net_minecraft_util_math_BlockPos_up(27), rotation, flagMatrix2, null);
            if (!pieces.isEmpty()) {
                // empty if block
            }
            RoomPool[] roomPools = new RoomPool[]{new FirstFloorRoomPool(), new SecondFloorRoomPool(), new ThirdFloorRoomPool()};
            for (l = 0; l < 3; ++l) {
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_up(8 * l + (l == 2 ? 3 : 0));
                ...
                for (int m = 0; m < flagMatrix4.m; ++m) {
                    for (int n = 0; n < flagMatrix4.n; ++n) {
                        ...
                        BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8 + (m - this.entranceJ) * 8);
                        blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), (n - this.entranceI) * 8);
                        ...
                        if (flagMatrix4.get(n, m - 1) == MansionParameters.CORRIDOR || (flagMatrix3.get(n, m - 1) & 0x800000) == 0x800000) {
                            pieces.add(new Piece(this.manager, "carpet_north", blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 1).net_minecraft_util_math_BlockPos_up(), rotation));
                        }
                        if (flagMatrix4.get(n + 1, m) == MansionParameters.CORRIDOR || (flagMatrix3.get(n + 1, m) & 0x800000) == 0x800000) {
                            pieces.add(new Piece(this.manager, "carpet_east", blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 1).net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 5).net_minecraft_util_math_BlockPos_up(), rotation));
                        }
                        if (flagMatrix4.get(n, m + 1) == MansionParameters.CORRIDOR || (flagMatrix3.get(n, m + 1) & 0x800000) == 0x800000) {
                            pieces.add(new Piece(this.manager, string, blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 5).net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 1), rotation));
                        }
                        if (flagMatrix4.get(n - 1, m) != MansionParameters.CORRIDOR && (flagMatrix3.get(n - 1, m) & 0x800000) != 0x800000) continue;
                        pieces.add(new Piece(this.manager, string2, blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 1).net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 1), rotation));
                    }
                }
                ...
                for (int o = 0; o < flagMatrix4.m; ++o) {
                    for (int p = 0; p < flagMatrix4.n; ++p) {
                        ...
                        BlockPos blockPos3 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8 + (o - this.entranceJ) * 8);
                        blockPos3 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), -1 + (p - this.entranceI) * 8);
                        ...
                        if (flagMatrix4.get(p + 1, o) == MansionParameters.CORRIDOR && !bl2) {
                            blockPos4 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 8);
                        ...
                        if (MansionParameters.isInsideMansion(flagMatrix4, p, o + 1) && !parameters.isRoomId(flagMatrix4, p, o + 1, l, s)) {
                            blockPos4 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 7);
                            blockPos4 = blockPos4.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
                        ...
                        if (flagMatrix4.get(p, o - 1) == MansionParameters.CORRIDOR && !bl2) {
                            blockPos4 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 1);
                            blockPos4 = blockPos4.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
        ...
        private void addRoof(List<Piece> pieces, BlockPos pos, BlockRotation rotation, FlagMatrix layout, @Nullable FlagMatrix nextFloorLayout) {
            ...
            for (i = 0; i < layout.m; ++i) {
                for (j = 0; j < layout.n; ++j) {
                    blockPos = pos;
                    blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8 + (i - this.entranceJ) * 8);
                    blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), (j - this.entranceI) * 8);
                    ...
                    pieces.add(new Piece(this.manager, "roof", blockPos.net_minecraft_util_math_BlockPos_up(3), rotation));
                    if (!MansionParameters.isInsideMansion(layout, j + 1, i)) {
                        blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 6);
                        pieces.add(new Piece(this.manager, "roof_front", blockPos2, rotation));
                    }
                    if (!MansionParameters.isInsideMansion(layout, j - 1, i)) {
                        blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 0);
                        blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 7);
                    ...
                    if (!MansionParameters.isInsideMansion(layout, j, i - 1)) {
                        blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 1);
                    ...
                    blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 6);
                    blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
            ...
            if (nextFloorLayout != null) {
                for (i = 0; i < layout.m; ++i) {
                    for (j = 0; j < layout.n; ++j) {
                        ...
                        if (!MansionParameters.isInsideMansion(layout, j, i) || !bl) continue;
                        if (!MansionParameters.isInsideMansion(layout, j + 1, i)) {
                            blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
                        ...
                        if (!MansionParameters.isInsideMansion(layout, j, i - 1)) {
                            blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 0);
                            blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 1);
                        ...
                        if (!MansionParameters.isInsideMansion(layout, j + 1, i)) {
                            if (!MansionParameters.isInsideMansion(layout, j, i - 1)) {
                                blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
                                blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 2);
                                pieces.add(new Piece(this.manager, "small_wall_corner", blockPos2, rotation));
                            }
                            if (!MansionParameters.isInsideMansion(layout, j, i + 1)) {
                                blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 8);
                                blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 7);
                        ...
                        if (!MansionParameters.isInsideMansion(layout, j, i - 1)) {
                            blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 2);
                            blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 1);
                            pieces.add(new Piece(this.manager, "small_wall_corner", blockPos2, rotation.rotate(BlockRotation.COUNTERCLOCKWISE_90)));
                        }
                        if (MansionParameters.isInsideMansion(layout, j, i + 1)) continue;
                        blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 1);
                        blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8);
            ...
            for (i = 0; i < layout.m; ++i) {
                for (j = 0; j < layout.n; ++j) {
                    ...
                    if (!MansionParameters.isInsideMansion(layout, j + 1, i)) {
                        blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 6);
                        if (!MansionParameters.isInsideMansion(layout, j, i + 1)) {
                            blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
                            pieces.add(new Piece(this.manager, "roof_corner", blockPos3, rotation));
                        } else if (MansionParameters.isInsideMansion(layout, j + 1, i + 1)) {
                            blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 5);
                        ...
                        if (!MansionParameters.isInsideMansion(layout, j, i - 1)) {
                            ...
                            blockPos3 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 9);
                            blockPos3 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 2);
                            pieces.add(new Piece(this.manager, "roof_inner_corner", blockPos3, rotation.rotate(BlockRotation.CLOCKWISE_90)));
                        }
                    }
                    if (MansionParameters.isInsideMansion(layout, j - 1, i)) continue;
                    blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 0);
                    blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 0);
                    if (!MansionParameters.isInsideMansion(layout, j, i + 1)) {
                        blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
                        pieces.add(new Piece(this.manager, "roof_corner", blockPos3, rotation.rotate(BlockRotation.CLOCKWISE_90)));
                    } else if (MansionParameters.isInsideMansion(layout, j - 1, i + 1)) {
                        blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 8);
                        blockPos3 = blockPos3.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 3);
                    ...
                    blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 1);
        ...
        private void addEntrance(List<Piece> pieces, GenerationPiece wallPiece) {
            ...
            pieces.add(new Piece(this.manager, "entrance", wallPiece.position.net_minecraft_util_math_BlockPos_offset(direction, 9), wallPiece.rotation));
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.SOUTH), 16);
        }

        private void addWallPiece(List<Piece> pieces, GenerationPiece wallPiece) {
            pieces.add(new Piece(this.manager, wallPiece.template, wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.EAST), 7), wallPiece.rotation));
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.SOUTH), 8);
        }

        private void turnLeft(List<Piece> pieces, GenerationPiece wallPiece) {
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.SOUTH), -1);
            pieces.add(new Piece(this.manager, "wall_corner", wallPiece.position, wallPiece.rotation));
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.SOUTH), -7);
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.WEST), -6);
        ...
        private void turnRight(List<Piece> pieces, GenerationPiece wallPiece) {
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.SOUTH), 6);
            wallPiece.position = wallPiece.position.net_minecraft_util_math_BlockPos_offset(wallPiece.rotation.rotate(Direction.EAST), 8);
        ...
        private void addSmallRoom(List<Piece> pieces, BlockPos pos, BlockRotation rotation, Direction direction, RoomPool pool) {
            ...
            BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_add(blockPos.getX(), 0, blockPos.getZ());
        ...
        private void addMediumRoom(List<Piece> pieces, BlockPos pos, BlockRotation rotation, Direction connectedRoomDirection, Direction entranceDirection, RoomPool pool, boolean staircase) {
            if (entranceDirection == Direction.EAST && connectedRoomDirection == Direction.SOUTH) {
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 1);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 1);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 1);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 8);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 7);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 14);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 15);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.WEST), 7);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), 6);
                ...
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), 1);
                blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.NORTH), 0);
        ...
        private void addBigRoom(List<Piece> pieces, BlockPos pos, BlockRotation rotation, Direction connectedRoomDirection, Direction entranceDirection, RoomPool pool) {
            ...
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.EAST), i);
            blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(rotation.rotate(Direction.SOUTH), j);
