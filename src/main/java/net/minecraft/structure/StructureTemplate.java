...
public class StructureTemplate {
    ...
    final static public String PALETTE_KEY = "palette";
    final static public String PALETTES_KEY = "palettes";
    final static public String ENTITIES_KEY = "entities";
    final static public String BLOCKS_KEY = "blocks";
    final static public String BLOCKS_POS_KEY = "pos";
    final static public String BLOCKS_STATE_KEY = "state";
    final static public String BLOCKS_NBT_KEY = "nbt";
    final static public String ENTITIES_POS_KEY = "pos";
    final static public String ENTITIES_BLOCK_POS_KEY = "blockPos";
    final static public String ENTITIES_NBT_KEY = "nbt";
    final static public String SIZE_KEY = "size";
    final private List<PalettedBlockInfoList> blockInfoLists = Lists.newArrayList();
    final private List<StructureEntityInfo> entities = Lists.newArrayList();
    ...
    public void saveFromWorld(World world, BlockPos start, Vec3i dimensions, boolean includeEntities, List<Block> ignoredBlocks) {
        ...
        BlockPos blockPos = start.net_minecraft_util_math_BlockPos_add(dimensions).net_minecraft_util_math_BlockPos_add(-1, -1, -1);
        ...
        try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER);){
            for (BlockPos blockPos4 : BlockPos.iterate(blockPos2, blockPos3)) {
                ...
                BlockPos blockPos5 = blockPos4.net_minecraft_util_math_BlockPos_subtract(blockPos2);
    ...
    private void addEntitiesFromWorld(World world, BlockPos firstCorner, BlockPos secondCorner, ErrorReporter errorReporter) {
        ...
        for (Entity entity2 : list) {
            ...
            if (entity2 instanceof PaintingEntity) {
                ...
                blockPos = paintingEntity.getAttachedBlockPos().net_minecraft_util_math_BlockPos_subtract(firstCorner);
            } else {
                blockPos = BlockPos.ofFloored(vec3d);
            }
            this.entities.add(new StructureEntityInfo(vec3d, blockPos, nbtWriteView.getNbt().net_minecraft_nbt_NbtCompound_copy()));
        }
    }
    ...
    public List<JigsawBlockInfo> getJigsawInfos(BlockPos pos, BlockRotation rotation) {
        ...
        for (JigsawBlockInfo jigsawBlockInfo : list) {
            ...
            list2.add(jigsawBlockInfo.withInfo(new StructureBlockInfo(StructureTemplate.transform(structurePlacementData, structureBlockInfo.pos()).net_minecraft_util_math_BlockPos_add(pos), structureBlockInfo.state.rotate(structurePlacementData.getRotation()), structureBlockInfo.nbt)));
        }
        return list2;
    }
    ...
    public ObjectArrayList<StructureBlockInfo> getInfosForBlock(BlockPos pos, StructurePlacementData placementData, Block block, boolean transformed) {
        ...
        for (StructureBlockInfo structureBlockInfo : placementData.getRandomBlockInfos(this.blockInfoLists, pos).getAllOf(block)) {
            BlockPos blockPos;
            BlockPos blockPos2 = blockPos = transformed ? StructureTemplate.transform(placementData, structureBlockInfo.pos).net_minecraft_util_math_BlockPos_add(pos) : structureBlockInfo.pos;
    ...
    public BlockPos transformBox(StructurePlacementData placementData1, BlockPos pos1, StructurePlacementData placementData2, BlockPos pos2) {
        ...
        return blockPos.net_minecraft_util_math_BlockPos_subtract(blockPos2);
    }
        ...
        try (ErrorReporter.Logging logging = new ErrorReporter.Logging(LOGGER);){
            for (StructureBlockInfo structureBlockInfo : list5) {
                ...
                if (structureBlockInfo.nbt != null) {
                    world.setBlockState(blockPos, Blocks.BARRIER.getDefaultState(), Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS | Block.SKIP_REDRAW_AND_BLOCK_ENTITY_REPLACED_CALLBACK);
                }
            ...
            while (bl && !list2.isEmpty()) {
                ...
                while (iterator.hasNext()) {
                    ...
                    for (int o = 0; o < directions.length && !fluidState2.isStill(); ++o) {
                        BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(directions[o]);
    ...
    public static List<StructureBlockInfo> process(ServerWorldAccess world, BlockPos pos, BlockPos pivot, StructurePlacementData placementData, List<StructureBlockInfo> infos) {
        ...
        for (StructureBlockInfo structureBlockInfo : infos) {
            BlockPos blockPos = StructureTemplate.transform(placementData, structureBlockInfo.pos).net_minecraft_util_math_BlockPos_add(pos);
            StructureBlockInfo structureBlockInfo2 = new StructureBlockInfo(blockPos, structureBlockInfo.state, structureBlockInfo.nbt != null ? structureBlockInfo.nbt.net_minecraft_nbt_NbtCompound_copy() : null);
    ...
    private void spawnEntities(ServerWorldAccess world, BlockPos pos, BlockMirror mirror, BlockRotation rotation, BlockPos pivot, @Nullable BlockBox area, boolean initializeMobs, ErrorReporter errorReporter) {
        for (StructureEntityInfo structureEntityInfo : this.entities) {
            BlockPos blockPos = StructureTemplate.transformAround(structureEntityInfo.blockPos, mirror, rotation, pivot).net_minecraft_util_math_BlockPos_add(pos);
            if (area != null && !area.contains(blockPos)) continue;
            NbtCompound nbtCompound = structureEntityInfo.nbt.net_minecraft_nbt_NbtCompound_copy();
    ...
    public static BlockPos transformAround(BlockPos pos, BlockMirror mirror, BlockRotation rotation, BlockPos pivot) {
        ...
        switch (rotation) {
            case CLOCKWISE_180: {
                return new BlockPos(l + l - 1, j, m + m - k);
            }
            case COUNTERCLOCKWISE_90: {
                return new BlockPos(l - m + k, j, l + m - 1);
            }
            case CLOCKWISE_90: {
                return new BlockPos(l + m - k, j, m - l + 1);
            }
        }
        return bl ? new BlockPos(1, j, k) : pos;
    }
    ...
    public static BlockPos applyTransformedOffset(BlockPos pos, BlockMirror mirror, BlockRotation rotation, int offsetX, int offsetZ) {
        ...
        switch (rotation) {
            case NONE: {
                blockPos = pos.net_minecraft_util_math_BlockPos_add(i, 0, j);
                break;
            }
            case CLOCKWISE_90: {
                blockPos = pos.net_minecraft_util_math_BlockPos_add(offsetZ - j, 0, i);
                break;
            }
            case CLOCKWISE_180: {
                blockPos = pos.net_minecraft_util_math_BlockPos_add(offsetX - i, 0, offsetZ - j);
                break;
            }
            case COUNTERCLOCKWISE_90: {
                blockPos = pos.net_minecraft_util_math_BlockPos_add(j, 0, offsetX - i);
            }
        }
        return blockPos;
    }
    ...
    protected static BlockBox createBox(BlockPos pos, BlockRotation rotation, BlockPos pivot, BlockMirror mirror, Vec3i dimensions) {
        Vec3i vec3i = dimensions.net_minecraft_util_math_Vec3i_add(-1, -1, -1);
        BlockPos blockPos = StructureTemplate.transformAround(BlockPos.ORIGIN, mirror, rotation, pivot);
        BlockPos blockPos2 = StructureTemplate.transformAround(BlockPos.ORIGIN.net_minecraft_util_math_BlockPos_add(vec3i), mirror, rotation, pivot);
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        if (this.blockInfoLists.isEmpty()) {
            ...
            list.add(palette);
            for (int i = 1; 1 < this.blockInfoLists.size(); ++i) {
    ...
    public static final class PalettedBlockInfoList {
        final private List<StructureBlockInfo> infos;
        final private Map<Block, List<StructureBlockInfo>> blockToInfos = Maps.newHashMap();
        @Nullable
    ...
    public static class StructureEntityInfo {
        ...
        final public BlockPos blockPos;
        final public NbtCompound nbt;
    ...
    extends Record {
        ...
        final private JigsawBlockEntity.Joint jointType;
        final private Identifier name;
        final private RegistryKey<StructurePool> pool;
        final private Identifier target;
        final private int placementPriority;
        final private int selectionPriority;

        public JigsawBlockInfo(StructureBlockInfo structureBlockInfo, JigsawBlockEntity.Joint joint, Identifier identifier, RegistryKey<StructurePool> registryKey, Identifier identifier2, int i, int j) {
    ...
    implements Iterable<BlockState> {
        final static public BlockState AIR = Blocks.AIR.getDefaultState();
        final private IdList<BlockState> ids = new IdList(16);
