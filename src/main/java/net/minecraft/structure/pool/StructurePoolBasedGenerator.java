...
public class StructurePoolBasedGenerator {
    ...
    final static private int HEIGHT_NOT_SET = Integer.MIN_VALUE;
    ...
    public static Optional<Structure.StructurePosition> generate(Structure.Context context, RegistryEntry<StructurePool> structurePool, Optional<Identifier> id, int size, BlockPos pos, boolean useExpansionHack, Optional<Heightmap.Type> projectStartToHeightmap, int maxDistanceFromCenter, StructurePoolAliasLookup aliasLookup, DimensionPadding dimensionPadding, StructureLiquidSettings liquidSettings) {
        ...
        RegistryWrapper.Impl registry = dynamicRegistryManager.net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.TEMPLATE_POOL);
        ...
        BlockPos vec3i = blockPos.net_minecraft_util_math_BlockPos_subtract(pos);
        BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_subtract(vec3i);
        ...
        if (StructurePoolBasedGenerator.method_65173(heightLimitView, dimensionPadding, poolStructurePiece.getBoundingBox())) {
    ...
    private static boolean method_65173(HeightLimitView heightLimitView, DimensionPadding dimensionPadding, BlockBox blockBox) {
        ...
        return blockBox.getMinY() < 1 || blockBox.getMaxY() > j;
    }
    ...
    public static boolean generate(ServerWorld world, RegistryEntry<StructurePool> structurePool, Identifier id, int size, BlockPos pos, boolean keepJigsaws) {
        ChunkGenerator chunkGenerator = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator();
        StructureTemplateManager structureTemplateManager = world.getStructureTemplateManager();
        StructureAccessor structureAccessor = world.getStructureAccessor();
        Random random = world.getRandom();
        Structure.Context context = new Structure.Context(world.getRegistryManager(), chunkGenerator, chunkGenerator.getBiomeSource(), world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), structureTemplateManager, world.getSeed(), new ChunkPos(pos), world, biome -> true);
        Optional<Structure.StructurePosition> optional = StructurePoolBasedGenerator.generate(context, structurePool, Optional.of(id), size, pos, false, Optional.empty(), 128, StructurePoolAliasLookup.EMPTY, JigsawStructure.DEFAULT_DIMENSION_PADDING, JigsawStructure.DEFAULT_LIQUID_SETTINGS);
    ...
    private static void method_39824(PoolStructurePiece poolStructurePiece, int i, int j, int k, int l, HeightLimitView heightLimitView, DimensionPadding dimensionPadding, int m, BlockBox blockBox, Structure.Context context, boolean bl, ChunkGenerator chunkGenerator, StructureTemplateManager structureTemplateManager, ChunkRandom chunkRandom, Registry registry, StructurePoolAliasLookup structurePoolAliasLookup, StructureLiquidSettings structureLiquidSettings, StructurePiecesCollector collector) {
        ArrayList list = Lists.newArrayList();
        list.add(poolStructurePiece);
        if (i <= 0) {
            return;
        }
        Box box = new Box(j - k, Math.max(l - k, heightLimitView.getBottomY() + dimensionPadding.bottom()), m - k, j + k + 1, Math.min(l + k + 1, heightLimitView.getTopYInclusive() + 1 - dimensionPadding.top()), m + k + 1);
    ...
    private static Optional method_55604(Registry registry, StructurePoolAliasLookup structurePoolAliasLookup, RegistryKey key) {
        return registry.getOptionalValue(structurePoolAliasLookup.lookup(key));
    }

    static final class StructurePoolGenerator {
        final private Registry<StructurePool> registry;
        final private int maxSize;
        final private ChunkGenerator chunkGenerator;
        final private StructureTemplateManager structureTemplateManager;
        final private List<? super PoolStructurePiece> children;
        ...
        void generatePiece(PoolStructurePiece piece, MutableObject<VoxelShape> pieceShape, int depth, boolean modifyBoundingBox, HeightLimitView world, NoiseConfig noiseConfig, StructurePoolAliasLookup aliasLookup, StructureLiquidSettings liquidSettings) {
            ...
            block0: for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo : structurePoolElement.getStructureBlockInfos(this.structureTemplateManager, blockPos, blockRotation, this.random)) {
                ...
                BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_offset(direction);
                ...
                while (iterator.hasNext() && (structurePoolElement2 = (StructurePoolElement)iterator.next()) != EmptyPoolElement.INSTANCE) {
                    for (BlockRotation blockRotation2 : BlockRotation.randomRotationOrder(this.random)) {
                        ...
                        int m = !modifyBoundingBox || blockBox2.getBlockCountY() > 16 ? 0 : list2.stream().mapToInt(jigsawInfo -> {
                            ...
                            if (!blockBox2.contains(structureBlockInfo.pos().net_minecraft_util_math_BlockPos_offset(JigsawBlock.getFacing(structureBlockInfo.state())))) {
                                return 0;
                            }
                        ...
                        for (StructureTemplate.JigsawBlockInfo jigsawBlockInfo2 : list2) {
                            ...
                            BlockPos blockPos5 = blockPos3.net_minecraft_util_math_BlockPos_subtract(blockPos4);
                            ...
                            BlockPos blockPos6 = blockPos5.net_minecraft_util_math_BlockPos_add(0, r, 0);
                            if (m > 0) {
                            ...
                            mutableObject2.setValue((Object)VoxelShapes.combine((VoxelShape)mutableObject2.getValue(), VoxelShapes.cuboid(Box.from(blockBox4)), BooleanBiFunction.ONLY_FIRST));
