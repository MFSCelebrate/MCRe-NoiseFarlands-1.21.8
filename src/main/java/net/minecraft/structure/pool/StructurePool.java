...
public class StructurePool {
    final static private int DEFAULT_Y = Integer.MIN_VALUE;
    final static private MutableObject<Codec<RegistryEntry<StructurePool>>> FALLBACK = new MutableObject();
    final static public Codec<StructurePool> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.lazyInitialized(() -> FALLBACK.getValue()).fieldOf("fallback").forGetter(StructurePool::getFallback), (App)Codec.mapPair((MapCodec)StructurePoolElement.CODEC.fieldOf("element"), (MapCodec)Codec.intRange(1, 150).fieldOf("weight")).codec().listOf().fieldOf("elements").forGetter(pool -> pool.elementWeights)).apply((Applicative)instance, StructurePool::new));
    final static public Codec<RegistryEntry<StructurePool>> REGISTRY_CODEC = Util.make(RegistryElementCodec.of(RegistryKeys.TEMPLATE_POOL, CODEC), arg_0 -> FALLBACK.setValue(arg_0));
    final private List<Pair<StructurePoolElement, Integer>> elementWeights;
    final private ObjectArrayList<StructurePoolElement> elements;
    final private RegistryEntry<StructurePool> fallback;
    ...
    implements StringIdentifiable {
        final static public Projection TERRAIN_MATCHING = new Projection("terrain_matching", (ImmutableList<StructureProcessor>)ImmutableList.of((Object)new GravityStructureProcessor(Heightmap.Type.WORLD_SURFACE_WG, -1)));
        final static public Projection RIGID = new Projection("rigid", (ImmutableList<StructureProcessor>)ImmutableList.of());
        final static public StringIdentifiable.EnumCodec<Projection> CODEC;
        final private String id;
        final private ImmutableList<StructureProcessor> processors;
        final static private Projection[] field_16683;
