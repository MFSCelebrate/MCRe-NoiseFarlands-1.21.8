...
extends StructurePoolElement {
    final static private Comparator<StructureTemplate.JigsawBlockInfo> JIGSAW_BLOCK_INFO_COMPARATOR = Comparator.comparingInt(StructureTemplate.JigsawBlockInfo::selectionPriority).reversed();
    final static private Codec<Either<Identifier, StructureTemplate>> LOCATION_CODEC = Codec.of(SinglePoolElement::encodeLocation, (Decoder)Identifier.CODEC.map(Either::left));
    final static public MapCodec<SinglePoolElement> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(SinglePoolElement.locationGetter(), SinglePoolElement.processorsGetter(), SinglePoolElement.projectionGetter(), SinglePoolElement.overrideLiquidSettingsGetter()).apply((Applicative)instance, SinglePoolElement::new));
    final protected Either<Identifier, StructureTemplate> location;
    final protected RegistryEntry<StructureProcessorList> processors;
    final protected Optional<StructureLiquidSettings> overrideLiquidSettings;
    ...
    public boolean generate(StructureTemplateManager structureTemplateManager, StructureWorldAccess world, StructureAccessor structureAccessor, ChunkGenerator chunkGenerator, BlockPos pos, BlockPos pivot, BlockRotation rotation, BlockBox box, Random random, StructureLiquidSettings liquidSettings, boolean keepJigsaws) {
        ...
        StructureTemplate structureTemplate = this.getStructure(structureTemplateManager);
        if (structureTemplate.place(world, pos, pivot, structurePlacementData = this.createPlacementData(rotation, box, liquidSettings, keepJigsaws), random, Block.NOTIFY_LISTENERS | Block.FORCE_STATE)) {
