...
public record StructureSet(List<WeightedEntry> structures, StructurePlacement placement) {
    ...
    public record WeightedEntry(RegistryEntry<Structure> structure, int weight) {
        final static public Codec<WeightedEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Structure.ENTRY_CODEC.fieldOf("structure").forGetter(WeightedEntry::structure), (App)Codecs.POSITIVE_INT.fieldOf("weight").forGetter(WeightedEntry::weight)).apply((Applicative)instance, WeightedEntry::new));
    }
}
