...
extends Screen {
    ...
    final static private ToIntFunction<RegistryKey<World>> DIMENSION_COLORS = (ToIntFunction)Util.make(new Reference2IntOpenHashMap(), map -> {
    ...
    final private WorldUpdater updater;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        int l = k + 10;
        context.drawCenteredTextWithShadow(this.textRenderer, this.updater.getStatus(), this.width / 2, k - this.textRenderer.fontHeight - 2, Colors.LIGHT_GRAY);
        if (this.updater.getTotalChunkCount() > 0) {
            context.fill(i - 1, k - 1, j + 1, l + 1, Colors.BLACK);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.converted", this.updater.getUpgradedChunkCount()), i, 40, Colors.LIGHT_GRAY);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.skipped", this.updater.getSkippedChunkCount()), i, 40 + this.textRenderer.fontHeight + 3, Colors.LIGHT_GRAY);
            context.drawTextWithShadow(this.textRenderer, Text.translatable("optimizeWorld.info.total", this.updater.getTotalChunkCount()), i, 40 + (this.textRenderer.fontHeight + 3) * 2, Colors.LIGHT_GRAY);
            int m = 0;
            ...
            context.drawCenteredTextWithShadow(this.textRenderer, text, this.width / 2, k + 2 * this.textRenderer.fontHeight + 2, Colors.LIGHT_GRAY);
            context.drawCenteredTextWithShadow(this.textRenderer, text2, this.width / 2, k + (l - k) / 2 - this.textRenderer.fontHeight / 2, Colors.LIGHT_GRAY);
        }
    }
}
