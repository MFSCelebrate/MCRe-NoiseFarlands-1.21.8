...
extends Screen {
    final static private Text BUFFET_BIOME_TEXT = Text.translatable("createWorld.customize.buffet.biome").withColor(Colors.GRAY);
    final static private int field_49494 = 8;
    final private ThreePartsLayoutWidget layout = new ThreePartsLayoutWidget(this);
    final private Screen parent;
    final private Consumer<RegistryEntry<Biome>> onDone;
    ...
    public CustomizeBuffetLevelScreen(Screen parent, GeneratorOptionsHolder generatorOptionsHolder, Consumer<RegistryEntry<Biome>> onDone) {
        ...
        this.parent = parent;
        this.onDone = onDone;
        this.biomeRegistry = generatorOptionsHolder.getCombinedRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BIOME);
    ...
    extends AlwaysSelectedEntryListWidget<BuffetBiomeItem> {
        ...
        extends AlwaysSelectedEntryListWidget.Entry<BuffetBiomeItem> {
            ...
            public void render(DrawContext context, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean hovered, float tickProgress) {
                context.drawTextWithShadow(CustomizeBuffetLevelScreen.this.textRenderer, this.text, x + 5, y + 2, Colors.WHITE);
            }
