...
public class RecipeBookResults {
    final static public int field_32411 = 20;
    final static private ButtonTextures PAGE_FORWARD_TEXTURES = new ButtonTextures(Identifier.ofVanilla("recipe_book/page_forward"), Identifier.ofVanilla("recipe_book/page_forward_highlighted"));
    final static private ButtonTextures PAGE_BACKWARD_TEXTURES = new ButtonTextures(Identifier.ofVanilla("recipe_book/page_backward"), Identifier.ofVanilla("recipe_book/page_backward_highlighted"));
    final private List<AnimatedResultButton> resultButtons = Lists.newArrayListWithCapacity(20);
    @Nullable
    private AnimatedResultButton hoveredResultButton;
    final private RecipeAlternativesWidget alternatesWidget;
    private MinecraftClient client;
    final private RecipeBookWidget<?> recipeBookWidget;
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button, int areaLeft, int areaTop, int areaWidth, int areaHeight) {
        ...
        for (AnimatedResultButton animatedResultButton : this.resultButtons) {
            ...
            if (button == 0) {
                ...
                this.alternatesWidget.showAlternativesForResult(animatedResultButton.getResultCollection(), contextParameterMap, this.filteringCraftable, animatedResultButton.getX(), animatedResultButton.getY(), areaLeft + areaWidth / 2, areaTop + 13 + areaHeight / 2, animatedResultButton.getWidth());
            }
            return true;
        }
        return false;
