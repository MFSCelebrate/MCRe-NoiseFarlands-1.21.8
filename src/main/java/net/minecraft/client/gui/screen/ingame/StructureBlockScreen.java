...
extends Screen {
    final static private Text STRUCTURE_NAME_TEXT = Text.translatable("structure_block.structure_name");
    final static private Text POSITION_TEXT = Text.translatable("structure_block.position");
    final static private Text SIZE_TEXT = Text.translatable("structure_block.size");
    final static private Text INTEGRITY_TEXT = Text.translatable("structure_block.integrity");
    final static private Text CUSTOM_DATA_TEXT = Text.translatable("structure_block.custom_data");
    final static private Text INCLUDE_ENTITIES_TEXT = Text.translatable("structure_block.include_entities");
    final static private Text STRICT_TEXT = Text.translatable("structure_block.strict");
    final static private Text DETECT_SIZE_TEXT = Text.translatable("structure_block.detect_size");
    final static private Text SHOW_AIR_TEXT = Text.translatable("structure_block.show_air");
    final static private Text SHOW_BOUNDING_BOX_TEXT = Text.translatable("structure_block.show_boundingbox");
    final static private ImmutableList<StructureBlockMode> MODES = ImmutableList.copyOf((Object[])StructureBlockMode.values());
    final static private ImmutableList<StructureBlockMode> MODES_EXCEPT_DATA = (ImmutableList)MODES.stream().filter(mode -> mode != StructureBlockMode.DATA).collect(ImmutableList.toImmutableList());
    final private StructureBlockBlockEntity structureBlock;
    ...
    protected void init() {
        ...
        this.mirrorButton = this.addDrawableChild(CyclingButtonWidget.builder(BlockMirror::getName).values((BlockMirror[])BlockMirror.values()).omitKeyText().initially(this.mirror).build(this.width / 2 - 20, 185, 40, 20, Text.literal("MIRROR"), (button, mirror) -> this.structureBlock.setMirror((BlockMirror)mirror)));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (structureBlockMode == StructureBlockMode.LOAD || structureBlockMode == StructureBlockMode.SAVE) {
            ...
            context.drawTextWithShadow(this.textRenderer, INCLUDE_ENTITIES_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(INCLUDE_ENTITIES_TEXT), 150, Colors.LIGHT_GRAY);
        }
        ...
        if (structureBlockMode == StructureBlockMode.LOAD) {
            ...
            context.drawTextWithShadow(this.textRenderer, SHOW_BOUNDING_BOX_TEXT, this.width / 2 + 154 - this.textRenderer.getWidth(SHOW_BOUNDING_BOX_TEXT), 70, Colors.LIGHT_GRAY);
        }
        if (structureBlockMode == StructureBlockMode.DATA) {
