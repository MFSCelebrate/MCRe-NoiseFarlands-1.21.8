...
public class Bans {
    final static private Text TEMPORARY_TITLE = Text.translatable("gui.banned.title.temporary").formatted(Formatting.BOLD);
    final static private Text PERMANENT_TITLE = Text.translatable("gui.banned.title.permanent").formatted(Formatting.BOLD);
    final static public Text NAME_TITLE = Text.translatable("gui.banned.name.title").formatted(Formatting.BOLD);
    final static private Text SKIN_TITLE = Text.translatable("gui.banned.skin.title").formatted(Formatting.BOLD);
    final static private Text SKIN_DESCRIPTION = Text.translatable("gui.banned.skin.description", Text.of(Urls.JAVA_MODERATION));
    ...
    private static Text getReasonText(BanDetails banDetails) {
        ...
        if (StringUtils.isNumeric((CharSequence)string)) {
            ...
            BanReason banReason = BanReason.byId(1);
            MutableText text = banReason != null ? Texts.setStyleIfAbsent(banReason.getDescription().copy(), Style.EMPTY.withBold(true)) : (string2 != null ? Text.translatable("gui.banned.description.reason_id_message", 1, string2).formatted(Formatting.BOLD) : Text.translatable("gui.banned.description.reason_id", 1).formatted(Formatting.BOLD));
