...
extends AbstractClientPlayerEntity {
    ...
    final static private int field_32671 = 20;
    final static private int field_32672 = 600;
    final static private int field_32673 = 100;
    final static private float field_32674 = 0.6f;
    final static private double field_32675 = 0.35;
    final static private double MAX_SOFT_COLLISION_RADIANS = 0.13962633907794952;
    final static public float field_55135 = 0.2f;
    final public ClientPlayNetworkHandler networkHandler;
    final private StatHandler statHandler;
    final private ClientRecipeBook recipeBook;
    final private Cooldown itemDropCooldown = new Cooldown(20, 1280);
    final private List<ClientPlayerTickable> tickables = Lists.newArrayList();
    ...
    private void pushOutOfBlocks(double x, double z) {
        ...
        for (Direction direction2 : directions = new Direction[]{Direction.WEST, Direction.EAST, Direction.NORTH, Direction.SOUTH}) {
            ...
            if (!(h < f) || this.wouldCollideAt(blockPos.net_minecraft_util_math_BlockPos_offset(direction2))) continue;
            f = h;
    ...
    private boolean wouldCollideAt(BlockPos pos) {
        ...
        return this.net_minecraft_world_World_getWorld().canCollide(this, box2);
    }
    ...
    public void playSound(SoundEvent sound, float volume, float pitch) {
        this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), sound, this.getSoundCategory(), volume, pitch, false);
    }

    @Override
    public void playSoundToPlayer(SoundEvent sound, SoundCategory category, float volume, float pitch) {
        this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), sound, category, volume, pitch, false);
    }
    ...
    public void init() {
        ...
        if (this.net_minecraft_world_World_getWorld() != null) {
            for (double d = this.getY(); d > (double)this.net_minecraft_world_World_getWorld().getBottomY() && d <= (double)this.net_minecraft_world_World_getWorld().getTopYInclusive(); d += 1.0) {
                this.setPosition(this.getX(), d, this.getZ());
                if (this.net_minecraft_world_World_getWorld().isSpaceEmpty(this)) break;
            }
    ...
    protected void autoJump(float dx, float dz) {
        ...
        if (!blockState.getCollisionShape(this.net_minecraft_world_World_getWorld(), blockPos, shapeContext).isEmpty()) {
            return;
        }
        blockPos = blockPos.net_minecraft_util_math_BlockPos_up();
        BlockState blockState2 = this.net_minecraft_world_World_getWorld().getBlockState(blockPos);
        if (!blockState2.getCollisionShape(this.net_minecraft_world_World_getWorld(), blockPos, shapeContext).isEmpty()) {
            return;
        }
        float m = 7.0f;
        float n = 1.2f;
        ...
        Iterable iterable = this.net_minecraft_world_World_getWorld().getCollisions(this, box);
        ...
        while (iterator.hasNext()) {
            ...
            int s = 1;
            while ((float)s < n) {
                BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_up(s);
                BlockState blockState3 = this.net_minecraft_world_World_getWorld().getBlockState(blockPos3);
                VoxelShape voxelShape = blockState3.getCollisionShape(this.net_minecraft_world_World_getWorld(), blockPos3, shapeContext);
                if (!voxelShape.isEmpty() && (double)(r = (float)voxelShape.getMax(Direction.Axis.Y) + (float)blockPos3.getY()) - this.getY() > (double)n) {
                    return;
                }
                if (s > 1) {
                    blockPos = blockPos.net_minecraft_util_math_BlockPos_up();
                    BlockState blockState4 = this.net_minecraft_world_World_getWorld().getBlockState(blockPos);
                    if (!blockState4.getCollisionShape(this.net_minecraft_world_World_getWorld(), blockPos, shapeContext).isEmpty()) {
                        return;
                    }
                }
                ++s;
    ...
    protected boolean updateWaterSubmersionState() {
        ...
        if (!bl && bl2) {
            this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.AMBIENT_UNDERWATER_ENTER, SoundCategory.AMBIENT, 1.0f, 1.0f, false);
            this.client.getSoundManager().play(new AmbientSoundLoops.Underwater(this));
        }
        if (bl && !bl2) {
            this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.AMBIENT_UNDERWATER_EXIT, SoundCategory.AMBIENT, 1.0f, 1.0f, false);
        }
        return this.isSubmergedInWater;
    }
