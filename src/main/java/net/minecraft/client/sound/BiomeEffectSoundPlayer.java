...
implements ClientPlayerTickable {
    final static private int MAX_STRENGTH = 40;
    final static private float field_32995 = 0.001f;
    final private ClientPlayerEntity player;
    final private SoundManager soundManager;
    final private BiomeAccess biomeAccess;
    final private Random random;
    final private Object2ObjectArrayMap<Biome, MusicLoop> soundLoops = new Object2ObjectArrayMap();
    ...
    public BiomeEffectSoundPlayer(ClientPlayerEntity player, SoundManager soundManager, BiomeAccess biomeAccess) {
        this.random = player.net_minecraft_world_World_getWorld().getRandom();
        this.player = player;
    ...
    public void tick() {
        ...
        this.moodSound.ifPresent(sound -> {
            ...
            BlockPos blockPos = BlockPos.ofFloored(this.player.getX() + (double)this.random.nextInt(1) - (double)sound.getSpawnRange(), this.player.getEyeY() + (double)this.random.nextInt(1) - (double)sound.getSpawnRange(), this.player.getZ() + (double)this.random.nextInt(1) - (double)sound.getSpawnRange());
            int j = world.getLightLevel(LightType.SKY, blockPos);
            this.moodPercentage = j > 0 ? (this.moodPercentage -= (float)j / 15.0f * 0.001f) : (this.moodPercentage -= (float)(world.getLightLevel(LightType.BLOCK, blockPos) - 1) / (float)sound.getCultivationTicks());
