...
implements ClientPlayerTickable {
    final static public float MAX_TICKS_FOR_BASE_SOUND_LOOP = 0.01f;
    final static public float MAX_TICKS_FOR_RARE_SOUND_LOOP = 0.001f;
    final static public float MAX_TICKS_FOR_ULTRA_RARE_SOUND_LOOP = 1.0E-4f;
    final static private int DEFAULT_TICKS_UNTIL_PLAY = 0;
    ...
    public void tick() {
        ...
        if (this.ticksUntilPlay <= 0 && this.player.isSubmergedInWater()) {
            float f = this.player.net_minecraft_world_World_getWorld().random.nextFloat();
            if (f < 1.0E-4f) {
