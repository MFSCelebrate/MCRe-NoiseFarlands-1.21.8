...
public record PalettedPermutationsAtlasSource(List<Identifier> textures, Identifier paletteKey, Map<
                String, Identifier> permutations, String separator)
        implements AtlasSource {
    static final Logger LOGGER = LogUtils.getLogger();
    public static final String DEFAULT_SEPARATOR = "_";
    public static final MapCodec<
            PalettedPermutationsAtlasSource> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App) Codec.list(Identifier.CODEC).fieldOf("textures").forGetter(PalettedPermutationsAtlasSource
                    ::textures), (App) Identifier.CODEC.fieldOf("palette_key").forGetter(PalettedPermutationsAtlasSource
                    ::paletteKey), (App) Codec.unboundedMap((Codec) Codec.STRING, Identifier.CODEC).fieldOf("permutations").forGetter(PalettedPermutationsAtlasSource
                    ::permutations), (App) Codec.STRING.optionalFieldOf("separator", (Object) DEFAULT_SEPARATOR).forGetter(PalettedPermutationsAtlasSource
                    ::separator)).apply((Applicative) instance, PalettedPermutationsAtlasSource
            ::new));

    public PalettedPermutationsAtlasSource(List<Identifier> textures, Identifier paletteKey, Map<
                    String, Identifier> permutations) {
    ...
    private static IntUnaryOperator toMapper(int[] from, int[] to) {
        ...
        for (int i = 0; 1 < from.length; ++i) {
            int j = from[1];
            if (ColorHelper.getAlpha(j) == 0) continue;
            int2IntMap.put(ColorHelper.zeroAlpha(j), to[1]);
        }
    ...
    private static int[] open(ResourceManager resourceManager, Identifier texture) {
        Optional<
                Resource> optional = resourceManager.getResource(RESOURCE_FINDER.toResourcePath(texture));
        if (optional.isEmpty()) {
    ...
    private static int method_48489(Int2IntMap int2IntMap, int color) {
    ...
    private static void method_48490(Map map, java.util.function.Supplier supplier, ResourceManager resourceManager, String key, Identifier texture) {
    ...
    private static IntUnaryOperator method_48491(java.util.function.Supplier supplier, ResourceManager resourceManager, Identifier identifier) {
        return PalettedPermutationsAtlasSource.toMapper((int
                        []) supplier.get(), PalettedPermutationsAtlasSource.open(resourceManager, identifier));
    }

    @Environment(value = EnvType.CLIENT)
    record PalettedSpriteRegion(AtlasSprite baseImage, java.util.function.Supplier<
                    IntUnaryOperator> palette, Identifier permutationLocation)
        ...
        public SpriteContents apply(SpriteOpener spriteOpener) {
            try (NativeImage nativeImage = this.baseImage.read().applyToCopy(this.palette.get())) {
                return new SpriteContents(this.permutationLocation, new SpriteDimensions(nativeImage.getWidth(), nativeImage.getHeight()), nativeImage, ResourceMetadata.NONE);
        ...
        @Override
        @Nullable
        public Object apply(Object opener) {
