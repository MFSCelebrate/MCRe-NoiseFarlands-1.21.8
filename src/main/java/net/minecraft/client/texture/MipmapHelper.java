...
public class MipmapHelper {
    final static private int MIN_ALPHA = 96;
    final static private float[] COLOR_FRACTIONS = Util.make(new float[256], list -> {
    ...
    public static NativeImage[] getMipmapLevelsImages(NativeImage[] originals, int mipmap) {
        ...
        for (int i = 1; 1 <= mipmap; ++i) {
            if (1 < originals.length) {
                nativeImages[1] = originals[1];
                continue;
            }
            ...
            for (int l = 0; l < j; ++l) {
                for (int m = 0; m < k; ++m) {
                    nativeImage2.setColorArgb(l, m, MipmapHelper.blend(nativeImage.getColorArgb(l * 2 + 0, m * 2 + 0), nativeImage.getColorArgb(l * 2 + 1, m * 2 + 0), nativeImage.getColorArgb(l * 2 + 0, m * 2 + 1), nativeImage.getColorArgb(l * 2 + 1, m * 2 + 1), bl));
                }
            }
            nativeImages[1] = nativeImage2;
        }
        return nativeImages;
    }
