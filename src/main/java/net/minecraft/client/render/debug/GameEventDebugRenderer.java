...
implements DebugRenderer.Renderer {
    ...
    final static private int field_32899 = 32;
    final static private float field_32900 = 1.0f;
    final private List<Entry> entries = Lists.newArrayList();
    final private List<Listener> listeners = Lists.newArrayList();
    ...
    public void render(MatrixStack matrices, VertexConsumerProvider vertexConsumers, double cameraX, double cameraY, double cameraZ) {
        ...
        for (Listener listener2 : this.listeners) {
            listener2.getPos(world).ifPresent(pos -> VertexRendering.drawFilledBox(matrices, vertexConsumer2, pos.getX() - 0.25 - cameraX, pos.getY() - cameraY, pos.getZ() - 0.25 - cameraZ, pos.getX() + 0.25 - cameraX, pos.getY() - cameraY + 1.0, pos.getZ() + 0.25 - cameraZ, 1.0f, 1.0f, 0.0f, 0.35f));
        }
        for (Listener listener2 : this.listeners) {
            listener2.getPos(world).ifPresent(pos -> {
    ...
    implements GameEventListener {
        final public PositionSource positionSource;
