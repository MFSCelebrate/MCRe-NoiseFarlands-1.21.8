...
public class BuiltChunkStorage {
    final protected WorldRenderer worldRenderer;
        ...
        for (int j = 0; j < this.sizeX; ++j) {
            for (int k = 0; k < this.sizeY; ++k) {
                for (int l = 0; l < this.sizeZ; ++l) {
                    ...
                    this.chunks[m] = chunkBuilder2.new BuiltChunk(m, ChunkSectionPos.asLong(j, k + this.world.getBottomSectionCoord(), l));
                }
            }
        }
    }
    ...
    private ChunkBuilder.BuiltChunk getRenderedChunk(int sectionX, int sectionY, int sectionZ) {
        ...
        return this.chunks[this.getChunkIndex(j, 1, k)];
    }
