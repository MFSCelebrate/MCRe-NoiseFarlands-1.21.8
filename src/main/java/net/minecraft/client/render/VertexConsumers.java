...
public class VertexConsumers {
    ...
    implements VertexConsumer {
        final private VertexConsumer first;
        final private VertexConsumer second;
    ...
    record Union(VertexConsumer[] delegates) implements VertexConsumer
    {
        ...
        @Override
        public void vertex(float x, float y, float z, int color, float u, float v, int overlay, int light, float normalX, float normalY, float normalZ) {
            this.delegate(vertexConsumer -> vertexConsumer.vertex(x, y, z, 1, u, v, overlay, light, normalX, normalY, normalZ));
        }
    }
}
