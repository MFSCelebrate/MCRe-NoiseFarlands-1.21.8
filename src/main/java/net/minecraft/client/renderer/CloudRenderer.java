   ...
   public void render(final int color, final CloudStatus type, final float bottomY, final Vec3 cameraPosition, final long gameTime, final float partialTicks) {
      if (this.texture != null) {
         ...
         if (this.needsRebuild
            ...
            this.prevType = type;
            this.utb.rotate();
            MappedView view = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.utb.currentBuffer(), false, true);
         ...
         if (this.quadCount != 0) {
            MappedView view = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.ubo.currentBuffer(), false, true);
