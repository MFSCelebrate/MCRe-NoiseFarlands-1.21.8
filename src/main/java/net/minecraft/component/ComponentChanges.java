...
public final class ComponentChanges {
    final static public ComponentChanges EMPTY = new ComponentChanges(Reference2ObjectMaps.emptyMap());
    final static public Codec<ComponentChanges> CODEC = Codec.dispatchedMap(Type.CODEC, Type::getValueCodec).xmap(changes -> {
        if (changes.isEmpty()) {
            return EMPTY;
        }
    ...
    final static public PacketCodec<RegistryByteBuf, ComponentChanges> PACKET_CODEC = ComponentChanges.createPacketCodec(new PacketCodecFunction(){

        public <T> PacketCodec<RegistryByteBuf, T> apply(ComponentType<T> componentType) {
            return componentType.getPacketCodec().cast();
        }
    });
    final static public PacketCodec<RegistryByteBuf, ComponentChanges> LENGTH_PREPENDED_PACKET_CODEC = ComponentChanges.createPacketCodec(new PacketCodecFunction(){
    ...
    final static private String REMOVE_PREFIX = "!";
    ...
    public static class Builder {
        final private Reference2ObjectMap<ComponentType<?>, Optional<?>> changes = new Reference2ObjectArrayMap();
    ...
    public record AddedRemovedPair(ComponentMap added, Set<ComponentType<?>> removed) {
        final static public AddedRemovedPair EMPTY = new AddedRemovedPair(ComponentMap.EMPTY, Set.of());
    }

    record Type(ComponentType<?> type, boolean removed) {
        final static public Codec<Type> CODEC = Codec.STRING.flatXmap(id -> {
