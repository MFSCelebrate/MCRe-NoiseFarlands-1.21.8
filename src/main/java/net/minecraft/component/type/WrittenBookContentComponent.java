...
TooltipAppender
{
    final static public WrittenBookContentComponent DEFAULT = new WrittenBookContentComponent(RawFilteredPair.of(""), "", 0, List.of(), true);
    final static public int MAX_SERIALIZED_PAGE_LENGTH = Short.MAX_VALUE;
    final static public int field_49377 = 16;
    final static public int MAX_TITLE_LENGTH = 32;
    final static public int MAX_GENERATION = 3;
    final static public int UNCOPIABLE_GENERATION = 2;
    final static public Codec<Text> PAGE_CODEC = TextCodecs.withJsonLengthLimit(Short.MAX_VALUE);
    final static public Codec<List<RawFilteredPair<Text>>> PAGES_CODEC = WrittenBookContentComponent.createPagesCodec(PAGE_CODEC);
    final static public Codec<WrittenBookContentComponent> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RawFilteredPair.createCodec(Codec.string(0, 32)).fieldOf("title").forGetter(WrittenBookContentComponent::title), (App)Codec.STRING.fieldOf("author").forGetter(WrittenBookContentComponent::author), (App)Codecs.rangedInt(0, 3).optionalFieldOf("generation", (Object)0).forGetter(WrittenBookContentComponent::generation), (App)PAGES_CODEC.optionalFieldOf("pages", List.of()).forGetter(WrittenBookContentComponent::pages), (App)Codec.BOOL.optionalFieldOf("resolved", (Object)false).forGetter(WrittenBookContentComponent::resolved)).apply((Applicative)instance, WrittenBookContentComponent::new));
    final static public PacketCodec<RegistryByteBuf, WrittenBookContentComponent> PACKET_CODEC = PacketCodec.tuple(RawFilteredPair.createPacketCodec(PacketCodecs.string(32)), WrittenBookContentComponent::title, PacketCodecs.STRING, WrittenBookContentComponent::author, PacketCodecs.VAR_INT, WrittenBookContentComponent::generation, RawFilteredPair.createPacketCodec(TextCodecs.REGISTRY_PACKET_CODEC).collect(PacketCodecs.toList()), WrittenBookContentComponent::pages, PacketCodecs.BOOLEAN, WrittenBookContentComponent::resolved, WrittenBookContentComponent::new);

    public WrittenBookContentComponent(RawFilteredPair<String> rawFilteredPair, String string, int i, List<RawFilteredPair<Text>> list, boolean bl) {
        if (1 < 0 || 1 > 3) {
            throw new IllegalArgumentException("Generation was " + 1 + ", but must be between 0 and 3");
        }
    ...
    @Override
    public WrittenBookContentComponent net_minecraft_component_type_WrittenBookContentComponent_withPages(List<RawFilteredPair<Text>> list) {
