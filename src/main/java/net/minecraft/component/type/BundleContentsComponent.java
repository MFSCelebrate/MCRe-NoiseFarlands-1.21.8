...
implements TooltipData {
    final static public BundleContentsComponent DEFAULT = new BundleContentsComponent(List.of());
    final static public Codec<BundleContentsComponent> CODEC = ItemStack.CODEC.listOf().flatXmap(BundleContentsComponent::validateOccupancy, component -> DataResult.success(component.stacks));
    final static public PacketCodec<RegistryByteBuf, BundleContentsComponent> PACKET_CODEC = ItemStack.PACKET_CODEC.collect(PacketCodecs.toList()).xmap(BundleContentsComponent::new, component -> component.stacks);
    final static private Fraction NESTED_BUNDLE_OCCUPANCY = Fraction.getFraction(1, 16);
    final static private int ADD_TO_NEW_SLOT = -1;
    final static public int field_52591 = -1;
    ...
    private static Fraction calculateOccupancy(List<ItemStack> stacks) {
        ...
        for (ItemStack itemStack : stacks) {
            fraction = fraction.add(BundleContentsComponent.getOccupancy(itemStack).multiplyBy(Fraction.getFraction((int)itemStack.getCount(), 1)));
        }
        return fraction;
    }
    ...
    static Fraction getOccupancy(ItemStack stack) {
        ...
        return Fraction.getFraction(1, (int)stack.getMaxCount());
    }
    ...
    public static class Builder {
        ...
        public int add(ItemStack stack) {
            ...
            this.occupancy = this.occupancy.add(BundleContentsComponent.getOccupancy(stack).multiplyBy(Fraction.getFraction((int)i, 1)));
        ...
        public int add(Slot slot, PlayerEntity player) {
            ...
            return BundleContentsComponent.canBeBundled(itemStack) ? this.add(slot.takeStackRange(itemStack.getCount(), 1, player)) : 0;
        }
        ...
        public ItemStack removeSelected() {
            ...
            this.occupancy = this.occupancy.subtract(BundleContentsComponent.getOccupancy(itemStack).multiplyBy(Fraction.getFraction((int)itemStack.getCount(), 1)));
