...
public record ToolComponent(List<Rule> rules, float defaultMiningSpeed, int damagePerBlock, boolean canDestroyBlocksInCreative) {
    ...
    extends Record {
        ...
        final static public Codec<Rule> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RegistryCodecs.entryList(RegistryKeys.BLOCK).fieldOf("blocks").forGetter(Rule::blocks), (App)Codecs.POSITIVE_FLOAT.optionalFieldOf("speed").forGetter(Rule::speed), (App)Codec.BOOL.optionalFieldOf("correct_for_drops").forGetter(Rule::correctForDrops)).apply((Applicative)instance, Rule::new));
        final static public PacketCodec<RegistryByteBuf, Rule> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.registryEntryList(RegistryKeys.BLOCK), Rule::blocks, PacketCodecs.FLOAT.collect(PacketCodecs::optional), Rule::speed, PacketCodecs.BOOLEAN.collect(PacketCodecs::optional), Rule::correctForDrops, Rule::new);

        public Rule(RegistryEntryList<Block> registryEntryList, Optional<Float> optional, Optional<Boolean> optional2) {
