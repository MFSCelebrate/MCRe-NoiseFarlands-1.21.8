...
implements TooltipAppender {
    ...
    final static public BannerPatternsComponent DEFAULT = new BannerPatternsComponent(List.of());
    final static public Codec<BannerPatternsComponent> CODEC = Layer.CODEC.listOf().xmap(BannerPatternsComponent::new, BannerPatternsComponent::layers);
    final static public PacketCodec<RegistryByteBuf, BannerPatternsComponent> PACKET_CODEC = Layer.PACKET_CODEC.collect(PacketCodecs.toList()).xmap(BannerPatternsComponent::new, BannerPatternsComponent::layers);
    ...
    public record Layer(RegistryEntry<BannerPattern> pattern, DyeColor color) {
        final static public Codec<Layer> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BannerPattern.ENTRY_CODEC.fieldOf("pattern").forGetter(Layer::pattern), (App)DyeColor.CODEC.fieldOf("color").forGetter(Layer::color)).apply((Applicative)instance, Layer::new));
        final static public PacketCodec<RegistryByteBuf, Layer> PACKET_CODEC = PacketCodec.tuple(BannerPattern.ENTRY_PACKET_CODEC, Layer::pattern, DyeColor.PACKET_CODEC, Layer::color, Layer::new);
    ...
    public static class Builder {
        final private ImmutableList.Builder<Layer> entries = ImmutableList.builder();
