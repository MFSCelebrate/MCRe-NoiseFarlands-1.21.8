...
implements TooltipAppender {
    final static private int ALL_SLOTS_EMPTY = -1;
    final static private int MAX_SLOTS = 256;
    final static public ContainerComponent DEFAULT = new ContainerComponent(DefaultedList.of());
    final static public Codec<ContainerComponent> CODEC = Slot.CODEC.sizeLimitedListOf(256).xmap(ContainerComponent::fromSlots, ContainerComponent::collectSlots);
    final static public PacketCodec<RegistryByteBuf, ContainerComponent> PACKET_CODEC = ItemStack.OPTIONAL_PACKET_CODEC.collect(PacketCodecs.toList(256)).xmap(ContainerComponent::new, component -> component.stacks);
    final private DefaultedList<ItemStack> stacks;
    ...
    record Slot(int index, ItemStack item) {
        final static public Codec<Slot> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.intRange(0, 255).fieldOf("slot").forGetter(Slot::index), (App)ItemStack.CODEC.fieldOf("item").forGetter(Slot::item)).apply((Applicative)instance, Slot::new));
    }
}
