...
public record AttributeModifiersComponent(List<Entry> modifiers) {
    final static public AttributeModifiersComponent DEFAULT = new AttributeModifiersComponent(List.of());
    final static public Codec<AttributeModifiersComponent> CODEC = Entry.CODEC.listOf().xmap(AttributeModifiersComponent::new, AttributeModifiersComponent::modifiers);
    final static public PacketCodec<RegistryByteBuf, AttributeModifiersComponent> PACKET_CODEC = PacketCodec.tuple(Entry.PACKET_CODEC.collect(PacketCodecs.toList()), AttributeModifiersComponent::modifiers, AttributeModifiersComponent::new);
    final static public DecimalFormat DECIMAL_FORMAT = Util.make(new DecimalFormat("#.##"), format -> format.setDecimalFormatSymbols(DecimalFormatSymbols.getInstance(Locale.ROOT)));
    ...
    public static class Builder {
        final private ImmutableList.Builder<Entry> entries = ImmutableList.builder();
    ...
    extends Record {
        ...
        public Entry(RegistryEntry<EntityAttribute> registryEntry, EntityAttributeModifier entityAttributeModifier, AttributeModifierSlot attributeModifierSlot, Display display) {
    ...
    public static interface Display {
        final static public Codec<Display> CODEC = Type.CODEC.dispatch("type", Display::getType, type -> type.codec);
        final static public PacketCodec<RegistryByteBuf, Display> PACKET_CODEC = Type.PACKET_CODEC.cast().dispatch(Display::getType, Type::getPacketCodec);
        ...
        {
            final static Default INSTANCE = new Default();
            final static MapCodec<Default> CODEC = MapCodec.unit((Object)INSTANCE);
            final static PacketCodec<RegistryByteBuf, Default> PACKET_CODEC = PacketCodec.unit(INSTANCE);
        ...
        public record Hidden() implements Display
        {
            ...
            final static MapCodec<Hidden> CODEC = MapCodec.unit((Object)INSTANCE);
            final static PacketCodec<RegistryByteBuf, Hidden> PACKET_CODEC = PacketCodec.unit(INSTANCE);
        ...
        {
            final static MapCodec<Override> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)TextCodecs.CODEC.fieldOf("value").forGetter(Override::value)).apply((Applicative)instance, Override::new));
            final static PacketCodec<RegistryByteBuf, Override> PACKET_CODEC = PacketCodec.tuple(TextCodecs.REGISTRY_PACKET_CODEC, Override::value, Override::new);
        ...
        implements StringIdentifiable {
            final static public Type DEFAULT = new Type("default", 0, Default.CODEC, Default.PACKET_CODEC);
            final static public Type HIDDEN = new Type("hidden", 1, Hidden.CODEC, Hidden.PACKET_CODEC);
            final static public Type OVERRIDE = new Type("override", 2, Override.CODEC, Override.PACKET_CODEC);
            final static Codec<Type> CODEC;
            final static private IntFunction<Type> INDEX_MAPPER;
            final static PacketCodec<ByteBuf, Type> PACKET_CODEC;
            final private String id;
            final private int index;
            final MapCodec<? extends Display> codec;
            final private PacketCodec<RegistryByteBuf, ? extends Display> packetCodec;
            final static private Type[] field_59749;
