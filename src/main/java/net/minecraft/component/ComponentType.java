...
public interface ComponentType<T> {
    final static public Codec<ComponentType<?>> CODEC = Codec.lazyInitialized(() -> Registries.DATA_COMPONENT_TYPE.getCodec());
    final static public PacketCodec<RegistryByteBuf, ComponentType<?>> PACKET_CODEC = PacketCodec.recursive(packetCodec -> PacketCodecs.registryValue(RegistryKeys.DATA_COMPONENT_TYPE));
    final static public Codec<ComponentType<?>> PERSISTENT_CODEC = CODEC.validate(componentType -> componentType.shouldSkipSerialization() ? DataResult.error(() -> "Encountered transient component " + String.valueOf(Registries.DATA_COMPONENT_TYPE.getId((ComponentType<?>)componentType))) : DataResult.success((Object)componentType));
    final static public Codec<Map<ComponentType<?>, Object>> TYPE_TO_VALUE_MAP_CODEC = Codec.dispatchedMap(PERSISTENT_CODEC, ComponentType::getCodecOrThrow);
    ...
    public static class Builder<T> {
        ...
        public ComponentType<T> build() {
            ...
            return new SimpleDataComponentType<T>(codec, packetCodec);
        }
        ...
        implements ComponentType<T> {
            ...
            final private PacketCodec<? super RegistryByteBuf, T> packetCodec;
