...
extends Block {
    final static public MapCodec<RedstoneWireBlock> CODEC = RedstoneWireBlock.createCodec(RedstoneWireBlock::new);
    final static public EnumProperty<WireConnection> WIRE_CONNECTION_NORTH = Properties.NORTH_WIRE_CONNECTION;
    final static public EnumProperty<WireConnection> WIRE_CONNECTION_EAST = Properties.EAST_WIRE_CONNECTION;
    final static public EnumProperty<WireConnection> WIRE_CONNECTION_SOUTH = Properties.SOUTH_WIRE_CONNECTION;
    final static public EnumProperty<WireConnection> WIRE_CONNECTION_WEST = Properties.WEST_WIRE_CONNECTION;
    final static public IntProperty POWER = Properties.POWER;
    final static public Map<Direction, EnumProperty<WireConnection>> DIRECTION_TO_WIRE_CONNECTION_PROPERTY = ImmutableMap.copyOf((Map)Maps.newEnumMap(Map.of(Direction.NORTH, WIRE_CONNECTION_NORTH, Direction.EAST, WIRE_CONNECTION_EAST, Direction.SOUTH, WIRE_CONNECTION_SOUTH, Direction.WEST, WIRE_CONNECTION_WEST)));
    final static private int[] COLORS = Util.make(new int[16], colors -> {
        for (int i = 0; i <= 15; ++i) {
            float f;
    ...
    final static private float field_31221 = 0.2f;
    final private Function<BlockState, VoxelShape> shapeFunction;
    final private BlockState dotState;
    final private RedstoneController redstoneController = new DefaultRedstoneController(this);
    ...
    private BlockState getDefaultWireState(BlockView world, BlockState state, BlockPos pos) {
        boolean bl = !world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isSolidBlock(world, pos);
    ...
    protected void prepare(BlockState state, WorldAccess world, BlockPos pos, int flags, int maxUpdateDepth) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (blockState.isOf(this)) {
                Vec3i blockPos = mutable.net_minecraft_util_math_Vec3i_offset(direction.getOpposite());
            ...
            Vec3i blockPos2 = mutable.net_minecraft_util_math_Vec3i_offset(direction.getOpposite());
    ...
    private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction) {
        return this.getRenderConnectionType(world, pos, direction, !world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isSolidBlock(world, pos));
    }
    ...
    private WireConnection getRenderConnectionType(BlockView world, BlockPos pos, Direction direction, boolean bl) {
        ...
        if (bl) {
            ...
            if (bl2 && RedstoneWireBlock.connectsTo(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_up()))) {
        ...
        if (RedstoneWireBlock.connectsTo(blockState, direction) || !blockState.isSolidBlock(world, blockPos) && RedstoneWireBlock.connectsTo(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_down()))) {
            return WireConnection.SIDE;
        }
    ...
    private void updateNeighbors(World world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            world.updateNeighbors(pos.net_minecraft_util_math_BlockPos_offset(direction), this);
        }
    }
    ...
    private void updateOffsetNeighbors(World world, BlockPos pos) {
        for (Direction direction : Direction.Type.HORIZONTAL) {
            this.updateNeighbors(world, pos.net_minecraft_util_math_BlockPos_offset(direction));
        }
        for (Direction direction : Direction.Type.HORIZONTAL) {
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction);
            if (world.getBlockState(blockPos).isSolidBlock(world, blockPos)) {
                this.updateNeighbors(world, blockPos.net_minecraft_util_math_BlockPos_up());
                continue;
            }
            this.updateNeighbors(world, blockPos.net_minecraft_util_math_BlockPos_down());
        }
    }
