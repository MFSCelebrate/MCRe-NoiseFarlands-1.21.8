...
extends AbstractCauldronBlock {
    final static public MapCodec<LeveledCauldronBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Biome.Precipitation.CODEC.fieldOf("precipitation").forGetter(block -> block.precipitation), (App)CauldronBehavior.CODEC.fieldOf("interactions").forGetter(block -> block.behaviorMap), LeveledCauldronBlock.createSettingsCodec()).apply((Applicative)instance, LeveledCauldronBlock::new));
    final static public int MIN_LEVEL = 1;
    final static public int MAX_LEVEL = 3;
    final static public IntProperty LEVEL = Properties.LEVEL_3;
    final static private int BASE_FLUID_HEIGHT = 6;
    final static private double FLUID_HEIGHT_PER_LEVEL = 3.0;
    final static private VoxelShape[] INSIDE_COLLISION_SHAPE_BY_LEVEL = Util.make(() -> Block.createShapeArray(2, level -> VoxelShapes.union(AbstractCauldronBlock.OUTLINE_SHAPE, Block.createColumnShape(12.0, 4.0, LeveledCauldronBlock.getFluidHeight(level + 1)))));
    final private Biome.Precipitation precipitation;
