...
extends HorizontalFacingBlock {
    final static public MapCodec<FenceGateBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)WoodType.CODEC.fieldOf("wood_type").forGetter(block -> block.type), FenceGateBlock.createSettingsCodec()).apply((Applicative)instance, FenceGateBlock::new));
    final static public BooleanProperty OPEN = Properties.OPEN;
    final static public BooleanProperty POWERED = Properties.POWERED;
    final static public BooleanProperty IN_WALL = Properties.IN_WALL;
    final static private Map<Direction.Axis, VoxelShape> REGULAR_OUTLINE_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createCuboidShape(16.0, 16.0, 4.0));
    final static private Map<Direction.Axis, VoxelShape> IN_WALL_OUTLINE_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_OUTLINE_SHAPES, shape -> VoxelShapes.combineAndSimplify(shape, Block.createColumnShape(16.0, 13.0, 16.0), BooleanBiFunction.ONLY_FIRST)));
    final static private Map<Direction.Axis, VoxelShape> CLOSED_COLLISION_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16.0, 4.0, 0.0, 24.0));
    final static private Map<Direction.Axis, VoxelShape> CLOSED_SIDES_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16.0, 4.0, 5.0, 24.0));
    final static private Map<Direction.Axis, VoxelShape> REGULAR_CULLING_SHAPES = VoxelShapes.createHorizontalAxisShapeMap(VoxelShapes.union(Block.createCuboidShape(0.0, 5.0, 7.0, 2.0, 16.0, 9.0), Block.createCuboidShape(14.0, 5.0, 7.0, 16.0, 16.0, 9.0)));
    final static private Map<Direction.Axis, VoxelShape> IN_WALL_CULLING_SHAPES = Maps.newEnumMap(Util.transformMapValues(REGULAR_CULLING_SHAPES, shape -> shape.offset(0.0, -0.1875, 0.0).simplify()));
    final private WoodType type;
    ...
    protected BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        ...
        if (((Direction)state.get(FACING)).rotateYClockwise().getAxis() == axis) {
            boolean bl = this.isWall(neighborState) || this.isWall(world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite())));
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        Direction.Axis axis = direction.getAxis();
        boolean bl2 = axis == Direction.Axis.Z && (this.isWall(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_west())) || this.isWall(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_east()))) || axis == Direction.Axis.X && (this.isWall(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_north())) || this.isWall(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_south())));
