...
extends Block {
    final static private int SET_ON_FIRE_SECONDS = 8;
    final static private int MIN_FIRE_TICK_INCREMENT = 1;
    final static private int MAX_FIRE_TICK_INCREMENT = 3;
    final private float damage;
    final static protected VoxelShape BASE_SHAPE = Block.createColumnShape(16.0, 0.0, 1.0);
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        block12: {
            ...
            block11: {
                ...
                if (!this.isFlammable(blockState = world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_down())) && !blockState.isSideSolidFullSquare(world, blockPos, Direction.UP)) break block11;
            ...
            if (this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_west()))) {
                for (i = 0; i < 2; ++i) {
            ...
            if (this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_east()))) {
                for (i = 0; i < 2; ++i) {
            ...
            if (this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_north()))) {
                for (i = 0; i < 2; ++i) {
            ...
            if (this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_south()))) {
                for (i = 0; i < 2; ++i) {
                    d = (double)pos.getX() + random.nextDouble();
                    e = (double)pos.getY() + random.nextDouble();
            ...
            if (!this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()))) break block12;
            for (i = 0; i < 2; ++i) {
                d = (double)pos.getX() + random.nextDouble();
    ...
    @Override
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity2, EntityCollisionHandler handler) {
        handler.addEvent(CollisionEvent.FIRE_IGNITE);
        handler.addPostCallback(CollisionEvent.FIRE_IGNITE, entity -> entity.serverDamage(entity.net_minecraft_world_World_getWorld().getDamageSources().inFire(), this.damage));
    }
    ...
    public static void igniteEntity(Entity entity) {
        if (!entity.isFireImmune()) {
            if (entity.getFireTicks() < 0) {
                ...
                int i = entity.net_minecraft_world_World_getWorld().getRandom().nextBetweenExclusive(1, 3);
