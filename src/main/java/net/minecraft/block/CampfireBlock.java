...
implements Waterloggable {
    final static public MapCodec<CampfireBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.BOOL.fieldOf("spawn_particles").forGetter(block -> block.emitsParticles), (App)Codec.intRange(0, 1000).fieldOf("fire_damage").forGetter(block -> block.fireDamage), CampfireBlock.createSettingsCodec()).apply((Applicative)instance, CampfireBlock::new));
    final static public BooleanProperty LIT = Properties.LIT;
    final static public BooleanProperty SIGNAL_FIRE = Properties.SIGNAL_FIRE;
    final static public BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
    final static public EnumProperty<Direction> FACING = Properties.HORIZONTAL_FACING;
    final static private VoxelShape SHAPE = Block.createColumnShape(16.0, 0.0, 7.0);
    final static private VoxelShape SMOKEY_SHAPE = Block.createColumnShape(4.0, 0.0, 16.0);
    final static private int field_31049 = 5;
    final private boolean emitsParticles;
    final private int fireDamage;
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (blockEntity instanceof CampfireBlockEntity) {
            ...
            if (world.net_minecraft_recipe_RecipeManager_getRecipeManager().getPropertySet(RecipePropertySet.CAMPFIRE_INPUT).canUse(itemStack)) {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        return (BlockState)((BlockState)((BlockState)((BlockState)this.getDefaultState().with(WATERLOGGED, bl)).with(SIGNAL_FIRE, this.isSignalFireBaseBlock(worldAccess.getBlockState(blockPos.net_minecraft_util_math_BlockPos_down())))).with(LIT, !bl)).with(FACING, ctx.getHorizontalPlayerFacing());
    }
    ...
    protected void onProjectileHit(World world, BlockState state, BlockHitResult hit, ProjectileEntity projectile) {
        ...
        if (world instanceof ServerWorld) {
            ...
            if (projectile.isOnFire() && projectile.canModifyAt(serverWorld, blockPos) && !state.get(LIT).booleanValue() && !state.get(WATERLOGGED).booleanValue()) {
                world.setBlockState(blockPos, (BlockState)state.with(Properties.LIT, true), Block.NOTIFY_ALL_AND_REDRAW);
            }
        }
    }
    ...
    public static boolean isLitCampfireInRange(World world, BlockPos pos) {
        for (int i = 1; i <= 5; ++i) {
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_down(i);
            ...
            if (!bl) continue;
            BlockState blockState2 = world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_down());
