...
extends Block {
    final static public MapCodec<DoorBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)BlockSetType.CODEC.fieldOf("block_set_type").forGetter(DoorBlock::getBlockSetType), DoorBlock.createSettingsCodec()).apply((Applicative)instance, DoorBlock::new));
    final static public EnumProperty<Direction> FACING = HorizontalFacingBlock.FACING;
    final static public EnumProperty<DoubleBlockHalf> HALF = Properties.DOUBLE_BLOCK_HALF;
    final static public EnumProperty<DoorHinge> HINGE = Properties.DOOR_HINGE;
    final static public BooleanProperty OPEN = Properties.OPEN;
    final static public BooleanProperty POWERED = Properties.POWERED;
    final static private Map<Direction, VoxelShape> SHAPES_BY_DIRECTION = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(16.0, 13.0, 16.0));
    final private BlockSetType blockSetType;
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (blockPos.getY() < world.getTopYInclusive() && world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_up()).canReplace(ctx)) {
            boolean bl = world.isReceivingRedstonePower(blockPos) || world.isReceivingRedstonePower(blockPos.net_minecraft_util_math_BlockPos_up());
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
        world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(), (BlockState)state.with(HALF, DoubleBlockHalf.UPPER), Block.NOTIFY_ALL);
    }
    ...
    private DoorHinge getHinge(ItemPlacementContext ctx) {
        ...
        BlockPos blockPos3 = blockPos.net_minecraft_util_math_BlockPos_offset(direction2);
        BlockState blockState = blockView.getBlockState(blockPos3);
        BlockPos blockPos4 = blockPos2.net_minecraft_util_math_BlockPos_offset(direction2);
        BlockState blockState2 = blockView.getBlockState(blockPos4);
        Direction direction3 = direction.rotateYClockwise();
        BlockPos blockPos5 = blockPos.net_minecraft_util_math_BlockPos_offset(direction3);
        BlockState blockState3 = blockView.getBlockState(blockPos5);
        BlockPos blockPos6 = blockPos2.net_minecraft_util_math_BlockPos_offset(direction3);
    ...
    protected void neighborUpdate(BlockState state, World world, BlockPos pos, Block sourceBlock, @Nullable WireOrientation wireOrientation, boolean notify) {
        boolean bl;
        boolean bl2 = world.isReceivingRedstonePower(pos) || world.isReceivingRedstonePower(pos.net_minecraft_util_math_BlockPos_offset(state.get(HALF) == DoubleBlockHalf.LOWER ? Direction.UP : Direction.DOWN)) ? true : (bl = false);
    ...
    protected long getRenderingSeed(BlockState state, BlockPos pos) {
        return MathHelper.hashCode(pos.getX(), pos.net_minecraft_util_math_BlockPos_down(state.get(HALF) == DoubleBlockHalf.LOWER ? 0 : 1).getY(), pos.getZ());
    }
