...
extends Block {
    final static public MapCodec<TripwireHookBlock> CODEC = TripwireHookBlock.createCodec(TripwireHookBlock::new);
    ...
    final static protected int field_31268 = 1;
    final static protected int field_31269 = 42;
    final static private int SCHEDULED_TICK_DELAY = 10;
    final static private Map<Direction, VoxelShape> SHAPES_BY_DIRECTION = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(6.0, 0.0, 10.0, 10.0, 16.0));
    ...
    public static void update(World world, BlockPos pos, BlockState state, boolean bl, boolean bl2, int i, @Nullable BlockState blockState) {
        ...
        for (int k = 1; k < 42; ++k) {
            blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction, k);
        ...
        if (j > 0) {
            blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction, j);
        ...
        if (bl3 != bl5) {
            for (int l = 1; l < j; ++l) {
                ...
                BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_offset(direction, l);
                ...
                world.setBlockState(blockPos2, (BlockState)blockState4.withIfExists(ATTACHED, bl5), Block.NOTIFY_ALL);
            }
        }
    }
    ...
    private static void updateNeighborsOnAxis(Block block, World world, BlockPos pos, Direction direction) {
        ...
        world.updateNeighborsAlways(pos.net_minecraft_util_math_BlockPos_offset(direction2), block, wireOrientation);
    }
