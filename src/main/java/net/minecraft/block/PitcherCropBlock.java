...
implements Fertilizable {
    final static public MapCodec<PitcherCropBlock> CODEC = PitcherCropBlock.createCodec(PitcherCropBlock::new);
    final static public int field_43240 = 4;
    final static public IntProperty AGE = Properties.AGE_4;
    final static public EnumProperty<DoubleBlockHalf> HALF = TallPlantBlock.HALF;
    final static private int field_43241 = 3;
    final static private int field_43391 = 1;
    final static private VoxelShape AGE_0_SHAPE = Block.createColumnShape(6.0, -1.0, 3.0);
    final static private VoxelShape LOWER_COLLISION_SHAPE = Block.createColumnShape(10.0, -1.0, 5.0);
    final private Function<BlockState, VoxelShape> shapeFunction = this.createShapeFunction();
    ...
    private void tryGrow(ServerWorld world, BlockState state, BlockPos pos, int amount) {
        ...
        if (PitcherCropBlock.isDoubleTallAtAge(i)) {
            world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(), (BlockState)blockState.with(HALF, DoubleBlockHalf.UPPER), Block.NOTIFY_ALL);
        }
    }
    ...
    private boolean canGrow(WorldView world, BlockPos pos, BlockState state, int age) {
        return !this.isFullyGrown(state) && PitcherCropBlock.canPlaceAt(world, pos) && (!PitcherCropBlock.isDoubleTallAtAge(age) || PitcherCropBlock.canGrowAt(world, pos.net_minecraft_util_math_BlockPos_up()));
    }
