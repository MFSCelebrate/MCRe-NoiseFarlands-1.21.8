...
implements Fertilizable {
    final static public MapCodec<PaleMossCarpetBlock> CODEC = PaleMossCarpetBlock.createCodec(PaleMossCarpetBlock::new);
    final static public BooleanProperty BOTTOM = Properties.BOTTOM;
    final static public EnumProperty<WallShape> NORTH = Properties.NORTH_WALL_SHAPE;
    final static public EnumProperty<WallShape> EAST = Properties.EAST_WALL_SHAPE;
    final static public EnumProperty<WallShape> SOUTH = Properties.SOUTH_WALL_SHAPE;
    final static public EnumProperty<WallShape> WEST = Properties.WEST_WALL_SHAPE;
    final static public Map<Direction, EnumProperty<WallShape>> WALL_SHAPE_PROPERTIES_BY_DIRECTION = ImmutableMap.copyOf((Map)Maps.newEnumMap(Map.of(Direction.NORTH, NORTH, Direction.EAST, EAST, Direction.SOUTH, SOUTH, Direction.WEST, WEST)));
    ...
    private static BlockState updateState(BlockState state, BlockView world, BlockPos pos, boolean bl) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (wallShape == WallShape.LOW) {
                if (blockState == null) {
                    blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up());
                }
                ...
                if (!state.get(BOTTOM).booleanValue()) {
                    if (blockState2 == null) {
                        blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down());
                    }
    ...
    public static void placeAt(WorldAccess world, BlockPos pos, Random random, int flags) {
        ...
        if (!blockState3.isAir()) {
            world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(), blockState3, flags);
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        if (!blockState.isAir()) {
            world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(), blockState, Block.NOTIFY_ALL);
        }
    }
    ...
    private static BlockState createUpperState(BlockView world, BlockPos pos, BooleanSupplier booleanSupplier) {
        ...
        BlockState blockState3 = PaleMossCarpetBlock.updateState(blockState2, world, pos.net_minecraft_util_math_BlockPos_up(), true);
        for (Direction direction : Direction.Type.HORIZONTAL) {
            EnumProperty<WallShape> enumProperty = PaleMossCarpetBlock.getWallShape(direction);
