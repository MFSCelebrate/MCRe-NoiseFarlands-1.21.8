...
extends BlockWithEntity {
    final static public MapCodec<BeehiveBlock> CODEC = BeehiveBlock.createCodec(BeehiveBlock::new);
    final static public EnumProperty<Direction> FACING = HorizontalFacingBlock.FACING;
    final static public IntProperty HONEY_LEVEL = Properties.HONEY_LEVEL;
    final static public int FULL_HONEY_LEVEL = 5;
    final static private int DROPPED_HONEYCOMB_COUNT = 3;
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (i >= 5) {
            ...
            if (stack.isOf(Items.SHEARS)) {
                world.playSound((Entity)player, player.getX(), player.getY(), player.getZ(), SoundEvents.BLOCK_BEEHIVE_SHEAR, SoundCategory.BLOCKS, 1.0f, 1.0f);
                BeehiveBlock.dropHoneycomb(world, pos);
                stack.damage(1, (LivingEntity)player, LivingEntity.getSlotForHand(hand));
                bl = true;
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        if (world instanceof ServerWorld) {
            ...
            if (player.shouldSkipBlockDrops() && serverWorld.getGameRules().getBoolean(GameRules.DO_TILE_DROPS) && (blockEntity = world.getBlockEntity(pos)) instanceof BeehiveBlockEntity) {
                boolean bl;
                BeehiveBlockEntity beehiveBlockEntity = (BeehiveBlockEntity)blockEntity;
                int i = state.get(HONEY_LEVEL);
