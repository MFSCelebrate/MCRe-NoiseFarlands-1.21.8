...
extends EndPortalBlockEntity {
    ...
    final static private int field_31368 = 200;
    final static private int field_31369 = 40;
    final static private int field_31370 = 2400;
    final static private int field_31371 = 1;
    final static private int field_31372 = 10;
    final static private long DEFAULT_AGE = 0L;
    final static private boolean DEFAULT_EXACT_TELEPORT = false;
    private long age = 0L;
    ...
    public Vec3d getOrCreateExitPortalPos(ServerWorld world, BlockPos pos) {
        ...
        if (this.exitPortalPos == null && world.getRegistryKey() == World.END) {
            ...
            blockPos = blockPos.net_minecraft_util_math_BlockPos_up(10);
    ...
    private static BlockPos findBestPortalExitPos(World world, BlockPos pos) {
        BlockPos blockPos = EndGatewayBlockEntity.findExitPortalPos(world, pos.net_minecraft_util_math_BlockPos_add(0, 2, 0), 5, false);
        LOGGER.debug("Best exit position for portal at {} is {}", (Object)pos, (Object)blockPos);
        return blockPos.net_minecraft_util_math_BlockPos_up();
    }
    ...
    private static BlockPos setupExitPortalLocation(ServerWorld world, BlockPos pos) {
        ...
        if (blockPos == null) {
            ...
            world.getRegistryManager().getOptional(RegistryKeys.CONFIGURED_FEATURE).flatMap(registry -> registry.getOptional(EndConfiguredFeatures.END_ISLAND)).ifPresent(reference -> ((ConfiguredFeature)reference.value()).generate(world, world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator(), Random.create(blockPos2.asLong()), blockPos2));
            blockPos = blockPos2;
        } else {
    ...
    private static WorldChunk getChunk(World world, Vec3d pos) {
        return world.net_minecraft_world_chunk_WorldChunk_getChunk(MathHelper.floor(pos.x / 16.0), MathHelper.floor(pos.z / 16.0));
    }
    ...
    private static BlockPos findPortalPosition(WorldChunk chunk) {
        ...
        for (BlockPos blockPos4 : BlockPos.iterate(blockPos, blockPos2)) {
            ...
            BlockPos blockPos5 = blockPos4.net_minecraft_util_math_BlockPos_up();
            BlockPos blockPos6 = blockPos4.net_minecraft_util_math_BlockPos_up(2);
    ...
    private static void createPortal(ServerWorld world, BlockPos pos, EndGatewayFeatureConfig config) {
        Feature.END_GATEWAY.generateIfValid(config, world, world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator(), Random.create(), pos);
    }

    @Override
    public boolean shouldDrawSide(Direction direction) {
        return Block.shouldDrawSide(this.getCachedState(), this.world.getBlockState(this.getPos().net_minecraft_util_math_BlockPos_offset(direction)), direction);
    }
