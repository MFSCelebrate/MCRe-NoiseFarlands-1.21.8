...
implements Clearable {
    ...
    final static private int field_31330 = 2;
    final static private int field_31331 = 4;
    final private DefaultedList<ItemStack> itemsBeingCooked = DefaultedList.ofSize(4, ItemStack.EMPTY);
    final private int[] cookingTimes = new int[4];
    final private int[] cookingTotalTimes = new int[4];
    ...
    public boolean addItem(ServerWorld world, @Nullable LivingEntity entity, ItemStack stack) {
        for (int i = 0; i < this.itemsBeingCooked.size(); ++i) {
            ...
            if (!itemStack.isEmpty()) continue;
            Optional<RecipeEntry<CampfireCookingRecipe>> optional = world.net_minecraft_recipe_ServerRecipeManager_getRecipeManager().getFirstMatch(RecipeType.CAMPFIRE_COOKING, new SingleStackRecipeInput(stack), world);
            if (optional.isEmpty()) {
                return false;
            }
