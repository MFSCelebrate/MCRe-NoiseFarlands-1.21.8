...
public class SculkSpreadManager {
    final static public int field_37609 = 24;
    final static public int MAX_CHARGE = 1000;
    final static public float field_37611 = 0.5f;
    final static private int MAX_CURSORS = 32;
    final static public int field_37612 = 11;
    final static public int MAX_CURSOR_DISTANCE = 1024;
    final boolean worldGen;
    final private TagKey<Block> replaceableTag;
    final private int extraBlockChance;
    final private int maxDistance;
    final private int spreadChance;
    final private int decayChance;
    ...
    public static class Cursor {
        final static private ObjectArrayList<Vec3i> OFFSETS = Util.make(new ObjectArrayList(18), list -> BlockPos.stream(new BlockPos(-1, -1, -1), new BlockPos(1, 1, 1)).filter(pos -> (pos.getX() == 0 || pos.getY() == 0 || pos.getZ() == 0) && !pos.equals(BlockPos.ORIGIN)).map(BlockPos::toImmutable).forEach(arg_0 -> ((ObjectArrayList)list).add(arg_0)));
        final static public int field_37622 = 1;
        ...
        final static private Codec<Set<Direction>> DIRECTION_SET_CODEC = Direction.CODEC.listOf().xmap(directions -> Sets.newEnumSet((Iterable)directions, Direction.class), Lists::newArrayList);
        final static public Codec<Cursor> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)BlockPos.CODEC.fieldOf("pos").forGetter(Cursor::getPos), (App)Codec.intRange(0, 1000).fieldOf("charge").orElse((Object)0).forGetter(Cursor::getCharge), (App)Codec.intRange(0, 1).fieldOf("decay_delay").orElse((Object)1).forGetter(Cursor::getDecay), (App)Codec.intRange(0, Integer.MAX_VALUE).fieldOf("update_delay").orElse((Object)0).forGetter(cursor -> cursor.update), (App)DIRECTION_SET_CODEC.lenientOptionalFieldOf("facings").forGetter(cursor -> Optional.ofNullable(cursor.getFaces()))).apply((Applicative)instance, Cursor::new));
        ...
        private static boolean canSpread(WorldAccess world, BlockPos sourcePos, BlockPos targetPos) {
            ...
            BlockPos blockPos = targetPos.net_minecraft_util_math_BlockPos_subtract(sourcePos);
