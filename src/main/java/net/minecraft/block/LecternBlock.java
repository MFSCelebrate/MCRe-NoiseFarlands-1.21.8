...
extends BlockWithEntity {
    final static public MapCodec<LecternBlock> CODEC = LecternBlock.createCodec(LecternBlock::new);
    ...
    final static public BooleanProperty HAS_BOOK = Properties.HAS_BOOK;
    final static private VoxelShape BASE_SHAPE = VoxelShapes.union(Block.createColumnShape(16.0, 0.0, 2.0), Block.createColumnShape(8.0, 2.0, 14.0));
    final static private Map<Direction, VoxelShape> OUTLINE_SHAPES_BY_DIRECTION = VoxelShapes.createHorizontalFacingShapeMap(VoxelShapes.union(Block.createCuboidZShape(16.0, 10.0, 14.0, 1.0, 5.333333), Block.createCuboidZShape(16.0, 12.0, 16.0, 5.333333, 9.666667), Block.createCuboidZShape(16.0, 14.0, 18.0, 9.666667, 14.0), BASE_SHAPE));
    final static private int SCHEDULED_TICK_DELAY = 2;
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        boolean bl = false;
        if (!world.isClient && playerEntity != null && playerEntity.isCreativeLevelTwoOp() && (nbtComponent = itemStack.getOrDefault(DataComponentTypes.BLOCK_ENTITY_DATA, NbtComponent.DEFAULT)).contains("Book")) {
            bl = true;
        }
    ...
    private static void updateNeighborAlways(World world, BlockPos pos, BlockState state) {
        ...
        world.updateNeighborsAlways(pos.net_minecraft_util_math_BlockPos_down(), state.getBlock(), wireOrientation);
    }
