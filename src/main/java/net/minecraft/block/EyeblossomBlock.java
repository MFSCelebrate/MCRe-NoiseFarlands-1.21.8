...
extends FlowerBlock {
    final static public MapCodec<EyeblossomBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.BOOL.fieldOf("open").forGetter(block -> block.state.open), EyeblossomBlock.createSettingsCodec()).apply((Applicative)instance, EyeblossomBlock::new));
    final static private int NOTIFY_RANGE_XZ = 3;
    final static private int NOTIFY_RANGE_Y = 2;
    final private EyeblossomState state;
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (this.state.isOpen() && random.nextInt(700) == 0 && (blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down())).isOf(Blocks.PALE_MOSS_BLOCK)) {
    ...
    private boolean updateStateAndNotifyOthers(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        BlockPos.iterate(pos.net_minecraft_util_math_BlockPos_add(-3, -2, -3), pos.net_minecraft_util_math_BlockPos_add(3, 2, 3)).forEach(otherPos -> {
    ...
    extends Enum<EyeblossomState> {
        final static public EyeblossomState OPEN = new EyeblossomState(true, StatusEffects.BLINDNESS, 11.0f, SoundEvents.BLOCK_EYEBLOSSOM_OPEN_LONG, SoundEvents.BLOCK_EYEBLOSSOM_OPEN, 16545810);
        final static public EyeblossomState CLOSED = new EyeblossomState(false, StatusEffects.NAUSEA, 7.0f, SoundEvents.BLOCK_EYEBLOSSOM_CLOSE_LONG, SoundEvents.BLOCK_EYEBLOSSOM_CLOSE, 0x5F5F5F);
        final boolean open;
        final RegistryEntry<StatusEffect> stewEffect;
        final float effectLengthInSeconds;
        final SoundEvent longSound;
        final SoundEvent sound;
        final private int particleColor;
        final static private EyeblossomState[] field_55078;
