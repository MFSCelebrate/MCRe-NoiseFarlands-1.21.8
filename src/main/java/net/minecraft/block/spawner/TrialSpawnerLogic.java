...
public final class TrialSpawnerLogic {
    ...
    public Optional<UUID> trySpawnMob(ServerWorld world, BlockPos pos) {
        ...
        (ErrorReporter.Context) () -> "spawner@" + pos.toString(),
        TrialSpawnerLogic.LOGGER
        );

        try {
            NbtReadView readView = NbtReadView.create(logging, world.getRegistryManager(), mobSpawnerEntry.entity());
            ...
            Entity entity = EntityType.loadEntityWithPassengers(
                    readView, world, SpawnReason.TRIAL_SPAWNER,
                    e -> {
                        e.refreshPositionAndAngles(vec3d.x, vec3d.y, vec3d.z, random.nextFloat() * 360.0f, 0.0f);
                        return e;
                    }
            );
    ...
    public void ejectLootTable(ServerWorld world, BlockPos pos, RegistryKey<LootTable> lootTable) {
        ...
        ObjectArrayList<
                ItemStack> objectArrayList = lootTable2.generateLoot(lootWorldContext = new LootWorldContext.Builder(world).build(LootContextTypes.EMPTY));
    ...
    private static boolean shouldRemoveMobFromData(ServerWorld world, BlockPos pos, UUID uuid) {
        ...
        return entity == null || !entity.isAlive() || !entity.net_minecraft_world_World_getWorld().getRegistryKey().equals(world.getRegistryKey()) || entity.getBlockPos().getSquaredDistance(pos) > (double) MAX_ENTITY_DISTANCE_SQUARED;
    }
    ...
    private static Entity method_55157(Vec3d vec3d, Random random, Entity entity) {
        entity.refreshPositionAndAngles(vec3d.x, vec3d.y, vec3d.z, random.nextFloat() * 360.0f, 0.0f);
        return entity;
    }

    private Vec3d method_67684(BlockPos blockPos, Random random) {
        TrialSpawnerConfig trialSpawnerConfig = this.getConfig();
        return new Vec3d((double) blockPos.getX() + (random.nextDouble() - random.nextDouble()) * (double) trialSpawnerConfig.spawnRange() + 0.5, blockPos.getY() + random.nextInt(3) - 1, (double) blockPos.getZ() + (random.nextDouble() - random.nextDouble()) * (double) trialSpawnerConfig.spawnRange() + 0.5);
    }

    private static String method_71406(BlockPos blockPos) {
        return "spawner@" + String.valueOf(blockPos);
    }

    public static final class FullConfig extends Record {
        ...
        public static final MapCodec<
                FullConfig> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App) TrialSpawnerConfig.ENTRY_CODEC.optionalFieldOf("normal_config", RegistryEntry.of(TrialSpawnerConfig.DEFAULT)).forGetter(FullConfig
                        ::normal), (App) TrialSpawnerConfig.ENTRY_CODEC.optionalFieldOf("ominous_config", RegistryEntry.of(TrialSpawnerConfig.DEFAULT)).forGetter(FullConfig
                        ::ominous), (App) Codecs.NON_NEGATIVE_INT.optionalFieldOf("target_cooldown_length", (Object) 36000).forGetter(FullConfig
                        ::targetCooldownLength), (App) Codec.intRange(1, 128).optionalFieldOf("required_player_range", (Object) 14).forGetter(FullConfig
                        ::requiredPlayerRange)).apply((Applicative) instance, FullConfig::new));
        ...
        public FullConfig(RegistryEntry<TrialSpawnerConfig> registryEntry, RegistryEntry<
                        TrialSpawnerConfig> registryEntry2, int i, int j) {
        ...
        public final String toString() {
            return ObjectMethods.bootstrap("toString", new MethodHandle
                    []{FullConfig.class, "normal;ominous;targetCooldownLength;requiredPlayerRange", "normal", "ominous", "targetCooldownLength", "requiredPlayerRange"}, this);
        }
        ...
        public final boolean equals(Object object) {
            return (boolean) ObjectMethods.bootstrap("equals", new MethodHandle
                            []{FullConfig.class, "normal;ominous;targetCooldownLength;requiredPlayerRange", "normal", "ominous", "targetCooldownLength", "requiredPlayerRange"}, this, object);
        }
    ...
    public static final class Type extends Enum<Type> {
        public static final Type NORMAL = new Type(ParticleTypes.FLAME);
        public static final Type OMINOUS = new Type(ParticleTypes.SOUL_FIRE_FLAME);
        public final SimpleParticleType particle;
        private static final Type[] field_50189;
