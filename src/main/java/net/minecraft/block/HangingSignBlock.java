...
extends AbstractSignBlock {
    final static public MapCodec<HangingSignBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)WoodType.CODEC.fieldOf("wood_type").forGetter(AbstractSignBlock::getWoodType), HangingSignBlock.createSettingsCodec()).apply((Applicative)instance, HangingSignBlock::new));
    ...
    final static private VoxelShape DEFAULT_SHAPE = Block.createColumnShape(10.0, 0.0, 16.0);
    final static private Map<Integer, VoxelShape> SHAPES_BY_ROTATION = VoxelShapes.createHorizontalFacingShapeMap(Block.createColumnShape(14.0, 2.0, 0.0, 10.0)).entrySet().stream().collect(Collectors.toMap(entry -> RotationPropertyHelper.fromDirection((Direction)entry.getKey()), Map.Entry::getValue));
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isSideSolid(world, pos.net_minecraft_util_math_BlockPos_up(), Direction.DOWN, SideShapeType.CENTER);
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        BlockPos blockPos = ctx.getBlockPos().net_minecraft_util_math_BlockPos_up();
