...
implements FluidDrainable {
    final static public MapCodec<BubbleColumnBlock> CODEC = BubbleColumnBlock.createCodec(BubbleColumnBlock::new);
    final static public BooleanProperty DRAG = Properties.DRAG;
    final static private int SCHEDULED_TICK_DELAY = 5;
    ...
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity, EntityCollisionHandler handler) {
        ...
        boolean bl2 = bl = blockState.getCollisionShape(world, pos).isEmpty() && blockState.getFluidState().isEmpty();
        if (bl) {
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        BubbleColumnBlock.update(world, pos, state, world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()));
    }
