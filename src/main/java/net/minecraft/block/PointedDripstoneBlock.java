...
implements Falling,
Waterloggable {
    final static public MapCodec<PointedDripstoneBlock> CODEC = PointedDripstoneBlock.createCodec(PointedDripstoneBlock::new);
    final static public EnumProperty<Direction> VERTICAL_DIRECTION = Properties.VERTICAL_DIRECTION;
    final static public EnumProperty<Thickness> THICKNESS = Properties.THICKNESS;
    final static public BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
    final static private int field_31205 = 11;
    final static private int field_31207 = 2;
    final static private float field_31208 = 0.02f;
    final static private float field_31209 = 0.12f;
    final static private int field_31210 = 11;
    final static private float WATER_DRIP_CHANCE = 0.17578125f;
    final static private float LAVA_DRIP_CHANCE = 0.05859375f;
    final static private double field_31213 = 0.6;
    final static private float field_31214 = 1.0f;
    final static private int field_31215 = 40;
    final static private int field_31200 = 6;
    final static private float field_31201 = 2.5f;
    final static private int field_31202 = 2;
    final static private float field_33566 = 5.0f;
    final static private float field_33567 = 0.011377778f;
    final static private int MAX_STALACTITE_GROWTH = 7;
    final static private int STALACTITE_FLOOR_SEARCH_RANGE = 10;
    final static private VoxelShape TIP_MERGE_SHAPE = Block.createColumnShape(6.0, 0.0, 16.0);
    final static private VoxelShape UP_TIP_SHAPE = Block.createColumnShape(6.0, 0.0, 11.0);
    final static private VoxelShape DOWN_TIP_SHAPE = Block.createColumnShape(6.0, 5.0, 16.0);
    final static private VoxelShape BASE_SHAPE = Block.createColumnShape(8.0, 0.0, 16.0);
    final static private VoxelShape FRUSTUM_SHAPE = Block.createColumnShape(10.0, 0.0, 16.0);
    final static private VoxelShape MIDDLE_SHAPE = Block.createColumnShape(12.0, 0.0, 16.0);
    final static private double DOWN_TIP_Y = DOWN_TIP_SHAPE.getMin(Direction.Axis.Y);
    final static private float MAX_HORIZONTAL_MODEL_OFFSET = (float)MIDDLE_SHAPE.getMin(Direction.Axis.X);
    final static private VoxelShape DRIP_COLLISION_SHAPE = Block.createColumnShape(4.0, 0.0, 16.0);
    ...
    protected VoxelShape getOutlineShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        VoxelShape voxelShape = switch (state.get(THICKNESS)) {
            ...
            case Thickness.FRUSTUM -> BASE_SHAPE;
            case Thickness.MIDDLE -> FRUSTUM_SHAPE;
            case Thickness.BASE -> MIDDLE_SHAPE;
        };
    ...
    public static void tryGrow(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        BlockState blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up(1));
        if (!PointedDripstoneBlock.canGrow(blockState, blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up(2)))) {
            return;
        }
    ...
    private static void tryGrowStalagmite(ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (PointedDripstoneBlock.canPlaceAtWithDirection(world, mutable, Direction.UP) && !world.isWater((BlockPos)mutable.net_minecraft_util_math_Vec3i_down())) {
                PointedDripstoneBlock.tryGrow(world, (BlockPos)mutable.net_minecraft_util_math_Vec3i_down(), Direction.UP);
                return;
            }
    ...
    private static void growMerged(BlockState state, WorldAccess world, BlockPos pos) {
        ...
        if (state.get(VERTICAL_DIRECTION) == Direction.UP) {
            blockPos = pos;
            blockPos2 = pos.net_minecraft_util_math_BlockPos_up();
        } else {
            blockPos2 = pos;
            blockPos = pos.net_minecraft_util_math_BlockPos_down();
        }
    ...
    private static void createParticle(World world, BlockPos pos, BlockState state, Fluid fluid) {
        ...
        SimpleParticleType particleEffect = fluid2.isIn(FluidTags.LAVA) ? ParticleTypes.DRIPPING_DRIPSTONE_LAVA : ParticleTypes.DRIPPING_DRIPSTONE_WATER;
    ...
    private static Thickness getThickness(WorldView world, BlockPos pos, Direction direction, boolean tryMerge) {
        ...
        BlockState blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction2));
    ...
    private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite());
        BlockState blockState = world.getBlockState(blockPos);
    ...
    private static boolean isHeldByPointedDripstone(BlockState state, WorldView world, BlockPos pos) {
        return PointedDripstoneBlock.isPointingDown(state) && !world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isOf(Blocks.POINTED_DRIPSTONE);
    }
    ...
    private static Optional<DrippingFluid> getFluid(World world, BlockPos pos, BlockState state) {
        ...
        return PointedDripstoneBlock.getSupportingPos(world, pos, state, 11).map(posx -> {
            BlockPos blockPos = posx.net_minecraft_util_math_BlockPos_up();
            BlockState blockState = world.getBlockState(blockPos);
