...
public class PistonHandler {
    final static public int MAX_MOVABLE_BLOCKS = 12;
    final private World world;
    final private BlockPos posFrom;
    final private boolean retracted;
    final private BlockPos posTo;
    final private Direction motionDirection;
    final private List<BlockPos> movedBlocks = Lists.newArrayList();
    final private List<BlockPos> brokenBlocks = Lists.newArrayList();
    final private Direction pistonDirection;
    ...
    public PistonHandler(World world, BlockPos pos, Direction dir, boolean retracted) {
        ...
        if (retracted) {
            ...
            this.posTo = pos.net_minecraft_util_math_BlockPos_offset(dir);
        } else {
            this.motionDirection = dir.getOpposite();
            this.posTo = pos.net_minecraft_util_math_BlockPos_offset(dir, 2);
        }
    }
    ...
    private boolean tryMove(BlockPos pos, Direction dir) {
        ...
        while (PistonHandler.isBlockSticky(blockState)) {
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(this.motionDirection.getOpposite(), i);
        ...
        int j = 0;
        for (k = i - 1; k >= 0; --k) {
            this.movedBlocks.add(pos.net_minecraft_util_math_BlockPos_offset(this.motionDirection.getOpposite(), k));
            ++j;
        }
        k = 1;
        while (true) {
            BlockPos blockPos2;
            int l;
            if ((l = this.movedBlocks.indexOf(blockPos2 = pos.net_minecraft_util_math_BlockPos_offset(this.motionDirection, k))) > -1) {
    ...
    private boolean tryMoveAdjacentBlock(BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            if (direction.getAxis() == this.motionDirection.getAxis() || !PistonHandler.isAdjacentBlockStuck(blockState2 = this.world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction)), blockState) || this.tryMove(blockPos, direction)) continue;
            return false;
        }
        return true;
    }
