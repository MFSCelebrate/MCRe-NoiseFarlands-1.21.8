...
implements Fertilizable {
    final static public IntProperty AGE = Properties.AGE_25;
    final static public int MAX_AGE = 25;
    final private double growthChance;
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        BlockPos blockPos;
        if (state.get(AGE) < 25 && random.nextDouble() < this.growthChance && this.chooseStemState(world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_offset(this.growthDirection)))) {
    ...
    protected BlockState getStateForNeighborUpdate(BlockState state, WorldView world, ScheduledTickView tickView, BlockPos pos, Direction direction, BlockPos neighborPos, BlockState neighborState, Random random) {
        if (direction == this.growthDirection.getOpposite()) {
            if (!state.canPlaceAt(world, pos)) {
                ...
                BlockState blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(this.growthDirection));
    ...
    public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state) {
        return this.chooseStemState(world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(this.growthDirection)));
    }
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(this.growthDirection);
        ...
        for (int k = 0; k < j && this.chooseStemState(world.getBlockState(blockPos)); ++k) {
            ...
            blockPos = blockPos.net_minecraft_util_math_BlockPos_offset(this.growthDirection);
            i = Math.min(i + 1, 25);
        }
    }
