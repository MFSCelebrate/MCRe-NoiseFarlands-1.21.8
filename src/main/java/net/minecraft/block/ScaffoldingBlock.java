...
implements Waterloggable {
    final static public MapCodec<ScaffoldingBlock> CODEC = ScaffoldingBlock.createCodec(ScaffoldingBlock::new);
    final static private int field_31238 = 1;
    final static private VoxelShape NORMAL_OUTLINE_SHAPE = VoxelShapes.union(Block.createColumnShape(16.0, 14.0, 16.0), VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidShape(0.0, 0.0, 0.0, 2.0, 16.0, 2.0)).values().stream().reduce(VoxelShapes.empty(), VoxelShapes::union));
    final static private VoxelShape COLLISION_SHAPE = Block.createColumnShape(16.0, 0.0, 2.0);
    final static private VoxelShape BOTTOM_OUTLINE_SHAPE = VoxelShapes.union(NORMAL_OUTLINE_SHAPE, COLLISION_SHAPE, VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(16.0, 0.0, 2.0, 0.0, 2.0)).values().stream().reduce(VoxelShapes.empty(), VoxelShapes::union));
    final static private VoxelShape OUTLINE_SHAPE = VoxelShapes.fullCube().offset(0.0, -1.0, 0.0).simplify();
    final static public int MAX_DISTANCE = 7;
    final static public IntProperty DISTANCE = Properties.DISTANCE_0_7;
    ...
    private boolean shouldBeBottom(BlockView world, BlockPos pos, int distance) {
        return distance > 0 && !world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isOf(this);
    }
