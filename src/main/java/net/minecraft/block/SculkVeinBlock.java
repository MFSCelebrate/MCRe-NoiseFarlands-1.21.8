...
implements SculkSpreadable {
    final static public MapCodec<SculkVeinBlock> CODEC = SculkVeinBlock.createCodec(SculkVeinBlock::new);
    final private MultifaceGrower allGrowTypeGrower = new MultifaceGrower(new SculkVeinGrowChecker(this, MultifaceGrower.GROW_TYPES));
    final private MultifaceGrower samePositionOnlyGrower = new MultifaceGrower(new SculkVeinGrowChecker(this, MultifaceGrower.GrowType.SAME_POSITION));
    ...
    public void spreadAtSamePosition(WorldAccess world, BlockState state, BlockPos pos, Random random) {
        ...
        for (Direction direction : DIRECTIONS) {
            ...
            if (!state.get(booleanProperty).booleanValue() || !world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction)).isOf(Blocks.SCULK)) continue;
    ...
    private boolean convertToBlock(SculkSpreadManager spreadManager, WorldAccess world, BlockPos pos, Random random) {
        ...
        for (Direction direction : Direction.shuffle(random)) {
            ...
            if (!SculkVeinBlock.hasDirection(blockState, direction) || !(blockState2 = world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction))).isIn(tagKey)) continue;
            ...
            for (Direction direction3 : DIRECTIONS) {
                ...
                if (direction3 == direction2 || !(blockState4 = world.getBlockState(blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(direction3))).isOf(this)) continue;
    ...
    public static boolean veinCoversSculkReplaceable(WorldAccess world, BlockState state, BlockPos pos) {
        ...
        for (Direction direction : DIRECTIONS) {
            if (!SculkVeinBlock.hasDirection(state, direction) || !world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction)).isIn(BlockTags.SCULK_REPLACEABLE)) continue;
            return true;
        }
        return false;
    }
    ...
    extends MultifaceGrower.LichenGrowChecker {
        final private MultifaceGrower.GrowType[] growTypes;
        ...
        public boolean canGrow(BlockView world, BlockPos pos, BlockPos growPos, Direction direction, BlockState state) {
            BlockPos blockPos;
            BlockState blockState = world.getBlockState(growPos.net_minecraft_util_math_BlockPos_offset(direction));
            if (blockState.isOf(Blocks.SCULK) || blockState.isOf(Blocks.SCULK_CATALYST) || blockState.isOf(Blocks.MOVING_PISTON)) {
                return false;
            }
            if (pos.getManhattanDistance(growPos) == 2 && world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite())).isSideSolidFullSquare(world, blockPos, direction)) {
                return false;
            }
