...
implements Waterloggable {
    final static public MapCodec<StairsBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)BlockState.CODEC.fieldOf("base_state").forGetter(block -> block.baseBlockState), StairsBlock.createSettingsCodec()).apply((Applicative)instance, StairsBlock::new));
    final static public EnumProperty<Direction> FACING = HorizontalFacingBlock.FACING;
    final static public EnumProperty<BlockHalf> HALF = Properties.BLOCK_HALF;
    final static public EnumProperty<StairShape> SHAPE = Properties.STAIR_SHAPE;
    final static public BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
    final static private VoxelShape OUTER_SHAPE = VoxelShapes.union(Block.createColumnShape(16.0, 0.0, 8.0), Block.createCuboidShape(0.0, 8.0, 0.0, 8.0, 16.0, 8.0));
    final static private VoxelShape STRAIGHT_SHAPE = VoxelShapes.union(OUTER_SHAPE, VoxelShapes.transform(OUTER_SHAPE, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90)));
    final static private VoxelShape INNER_SHAPE = VoxelShapes.union(STRAIGHT_SHAPE, VoxelShapes.transform(STRAIGHT_SHAPE, DirectionTransformation.fromRotations(AxisRotation.R0, AxisRotation.R90)));
    final static private Map<Direction, VoxelShape> OUTER_BOTTOM_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(OUTER_SHAPE);
    final static private Map<Direction, VoxelShape> STRAIGHT_BOTTOM_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(STRAIGHT_SHAPE);
    final static private Map<Direction, VoxelShape> INNER_BOTTOM_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(INNER_SHAPE);
    final static private Map<Direction, VoxelShape> OUTER_TOP_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(VoxelShapes.transform(OUTER_SHAPE, DirectionTransformation.INVERT_Y));
    final static private Map<Direction, VoxelShape> STRAIGHT_TOP_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(VoxelShapes.transform(STRAIGHT_SHAPE, DirectionTransformation.INVERT_Y));
    final static private Map<Direction, VoxelShape> INNER_TOP_SHAPES = VoxelShapes.createHorizontalFacingShapeMap(VoxelShapes.transform(INNER_SHAPE, DirectionTransformation.INVERT_Y));
    final private Block baseBlock;
    final protected BlockState baseBlockState;
    ...
    private static StairShape getStairShape(BlockState state, BlockView world, BlockPos pos) {
        ...
        BlockState blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite()));
    ...
    private static boolean isDifferentOrientation(BlockState state, BlockView world, BlockPos pos, Direction dir) {
        BlockState blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(dir));
