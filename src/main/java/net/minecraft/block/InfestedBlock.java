...
extends Block {
    final static public MapCodec<InfestedBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Registries.BLOCK.getCodec().fieldOf("host").forGetter(InfestedBlock::getRegularBlock), InfestedBlock.createSettingsCodec()).apply((Applicative)instance, InfestedBlock::new));
    final private Block regularBlock;
    final static private Map<Block, Block> REGULAR_TO_INFESTED_BLOCK = Maps.newIdentityHashMap();
    final static private Map<BlockState, BlockState> REGULAR_TO_INFESTED_STATE = Maps.newIdentityHashMap();
    final static private Map<BlockState, BlockState> INFESTED_TO_REGULAR_STATE = Maps.newIdentityHashMap();
