...
implements Waterloggable {
    final static public MapCodec<LadderBlock> CODEC = LadderBlock.createCodec(LadderBlock::new);
    ...
    final static public Map<Direction, VoxelShape> SHAPES_BY_DIRECTION = VoxelShapes.createHorizontalFacingShapeMap(Block.createCuboidZShape(16.0, 13.0, 16.0));
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        return this.canPlaceOn(world, pos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite()), direction);
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        if (!ctx.canReplaceExisting() && (blockState = ctx.getWorld().getBlockState(ctx.getBlockPos().net_minecraft_util_math_BlockPos_offset(ctx.getSide().getOpposite()))).isOf(this) && blockState.get(FACING) == ctx.getSide()) {
            return null;
        }
