...
extends AbstractFireBlock {
    final static public MapCodec<FireBlock> CODEC = FireBlock.createCodec(FireBlock::new);
    final static public int field_31093 = 15;
    ...
    final static public Map<Direction, BooleanProperty> DIRECTION_PROPERTIES = ConnectingBlock.FACING_PROPERTIES.entrySet().stream().filter(entry -> entry.getKey() != Direction.DOWN).collect(Util.toMap());
    final private Function<BlockState, VoxelShape> shapeFunction;
    final static private int field_31085 = 60;
    final static private int field_31086 = 30;
    final static private int field_31087 = 15;
    final static private int field_31088 = 5;
    final static private int field_31089 = 100;
    final static private int field_31090 = 60;
    final static private int field_31091 = 20;
    final static private int field_31092 = 5;
    final private Object2IntMap<Block> burnChances = new Object2IntOpenHashMap();
    final private Object2IntMap<Block> spreadChances = new Object2IntOpenHashMap();
    ...
    protected BlockState getStateForPosition(BlockView world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            blockState2 = (BlockState)blockState2.with(booleanProperty, this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction))));
        }
        return blockState2;
    }
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (!bl) {
            ...
            if (i == 15 && random.nextInt(4) == 0 && !this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()))) {
                world.removeBlock(pos, false);
                return;
            }
        }
        int k = (bl2 = world.getBiome(pos).isIn(BiomeTags.INCREASED_FIRE_BURNOUT)) ? -50 : 0;
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_east(), 300 + k, random, i);
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_west(), 300 + k, random, i);
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_down(), 250 + k, random, i);
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_up(), 250 + k, random, i);
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_north(), 300 + k, random, i);
        this.trySpreadingFire(world, pos.net_minecraft_util_math_BlockPos_south(), 300 + k, random, i);
    ...
    protected boolean isRainingAround(World world, BlockPos pos) {
        return world.hasRain(pos) || world.hasRain(pos.net_minecraft_util_math_BlockPos_west()) || world.hasRain(pos.net_minecraft_util_math_BlockPos_east()) || world.hasRain(pos.net_minecraft_util_math_BlockPos_north()) || world.hasRain(pos.net_minecraft_util_math_BlockPos_south());
    }
    ...
    private boolean areBlocksAroundFlammable(BlockView world, BlockPos pos) {
        for (Direction direction : Direction.values()) {
            if (!this.isFlammable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction)))) continue;
            return true;
        }
        return false;
    }
