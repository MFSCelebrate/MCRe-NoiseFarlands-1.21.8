...
extends BlockWithEntity {
    final static public MapCodec<EnchantingTableBlock> CODEC = EnchantingTableBlock.createCodec(EnchantingTableBlock::new);
    final static public List<BlockPos> POWER_PROVIDER_OFFSETS = BlockPos.stream(-2, 0, -2, 2, 1, 2).filter(pos -> Math.abs(pos.getX()) == 2 || Math.abs(pos.getZ()) == 2).map(BlockPos::toImmutable).toList();
    final static private VoxelShape SHAPE = Block.createColumnShape(16.0, 0.0, 12.0);
    ...
    public static boolean canAccessPowerProvider(World world, BlockPos tablePos, BlockPos providerOffset) {
        return world.getBlockState(tablePos.net_minecraft_util_math_BlockPos_add(providerOffset)).isIn(BlockTags.ENCHANTMENT_POWER_PROVIDER) && world.getBlockState(tablePos.net_minecraft_util_math_BlockPos_add(providerOffset.getX() / 2, providerOffset.getY(), providerOffset.getZ() / 2)).isIn(BlockTags.ENCHANTMENT_POWER_TRANSMITTER);
    }
