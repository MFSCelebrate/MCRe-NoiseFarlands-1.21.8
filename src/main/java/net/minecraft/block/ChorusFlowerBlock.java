...
extends Block {
    final static public MapCodec<ChorusFlowerBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Registries.BLOCK.getCodec().fieldOf("plant").forGetter(block -> block.plantBlock), ChorusFlowerBlock.createSettingsCodec()).apply((Applicative)instance, ChorusFlowerBlock::new));
    final static public int MAX_AGE = 5;
    final static public IntProperty AGE = Properties.AGE_5;
    final static private VoxelShape SHAPE = Block.createColumnShape(14.0, 0.0, 15.0);
    final private Block plantBlock;
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (blockState.isOf(Blocks.END_STONE)) {
            ...
            j = 1;
            for (int k = 0; k < 4; ++k) {
                BlockState blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down(j + 1));
        ...
        if (bl && ChorusFlowerBlock.isSurroundedByAir(world, blockPos, null) && world.isAir(pos.net_minecraft_util_math_BlockPos_up(2))) {
            ...
            for (int l = 0; l < j; ++l) {
                ...
                BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_offset(direction);
                if (!world.isAir(blockPos2) || !world.isAir(blockPos2.net_minecraft_util_math_BlockPos_down()) || !ChorusFlowerBlock.isSurroundedByAir(world, blockPos2, direction.getOpposite())) continue;
    ...
    private static boolean isSurroundedByAir(WorldView world, BlockPos pos, @Nullable Direction exceptDirection) {
        for (Direction direction : Direction.Type.HORIZONTAL) {
            if (direction == exceptDirection || world.isAir(pos.net_minecraft_util_math_BlockPos_offset(direction))) continue;
            return false;
        }
        return true;
    }
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        boolean bl = false;
        for (Direction direction : Direction.Type.HORIZONTAL) {
            BlockState blockState2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset(direction));
            if (blockState2.isOf(this.plantBlock)) {
                if (bl) {
                    return false;
                }
    ...
    private static void generate(WorldAccess world, BlockPos pos, Random random, BlockPos rootPos, int size, int layer) {
        ...
        for (int j = 0; j < i; ++j) {
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_up(j + 1);
            ...
            world.setBlockState(blockPos.net_minecraft_util_math_BlockPos_down(), ChorusPlantBlock.withConnectionProperties(world, blockPos.net_minecraft_util_math_BlockPos_down(), block.getDefaultState()), Block.NOTIFY_LISTENERS);
        }
        boolean bl = false;
        if (layer < 4) {
            ...
            for (int l = 0; l < k; ++l) {
                Direction direction = Direction.Type.HORIZONTAL.random(random);
                BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_up(i).net_minecraft_util_math_BlockPos_offset(direction);
                if (Math.abs(blockPos2.getX() - rootPos.getX()) >= size || Math.abs(blockPos2.getZ() - rootPos.getZ()) >= size || !world.isAir(blockPos2) || !world.isAir(blockPos2.net_minecraft_util_math_BlockPos_down()) || !ChorusFlowerBlock.isSurroundedByAir(world, blockPos2, direction.getOpposite())) continue;
                bl = true;
                ...
                world.setBlockState(blockPos2.net_minecraft_util_math_BlockPos_offset(direction.getOpposite()), ChorusPlantBlock.withConnectionProperties(world, blockPos2.net_minecraft_util_math_BlockPos_offset(direction.getOpposite()), block.getDefaultState()), Block.NOTIFY_LISTENERS);
                ChorusFlowerBlock.generate(world, blockPos2, random, rootPos, size, layer + 1);
            }
        }
        if (!bl) {
            world.setBlockState(pos.net_minecraft_util_math_BlockPos_up(i), (BlockState)Blocks.CHORUS_FLOWER.getDefaultState().with(AGE, 5), Block.NOTIFY_LISTENERS);
        }
    }
