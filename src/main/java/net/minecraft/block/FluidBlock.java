...
implements FluidDrainable {
    final static private Codec<FlowableFluid> FLUID_CODEC = Registries.FLUID.getCodec().comapFlatMap(fluid -> {
    ...
    final static public MapCodec<FluidBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)FLUID_CODEC.fieldOf("fluid").forGetter(block -> block.fluid), FluidBlock.createSettingsCodec()).apply((Applicative)instance, FluidBlock::new));
    final static public IntProperty LEVEL = Properties.LEVEL_15;
    final protected FlowableFluid fluid;
    final private List<FluidState> statesByLevel;
    final static public VoxelShape COLLISION_SHAPE = Block.createColumnShape(16.0, 0.0, 8.0);
    final static public ImmutableList<Direction> FLOW_DIRECTIONS = ImmutableList.of((Object)Direction.DOWN, (Object)Direction.SOUTH, (Object)Direction.NORTH, (Object)Direction.EAST, (Object)Direction.WEST);
    ...
    protected VoxelShape getCollisionShape(BlockState state, BlockView world, BlockPos pos, ShapeContext context) {
        if (context.isAbove(COLLISION_SHAPE, pos, true) && state.get(LEVEL) == 0 && context.canWalkOnFluid(world.getFluidState(pos.net_minecraft_util_math_BlockPos_up()), state.getFluidState())) {
    ...
    private boolean receiveNeighborFluids(World world, BlockPos pos, BlockState state) {
        if (this.fluid.isIn(FluidTags.LAVA)) {
            boolean bl = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isOf(Blocks.SOUL_SOIL);
