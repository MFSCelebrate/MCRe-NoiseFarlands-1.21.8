...
implements Waterloggable {
    final static public MapCodec<PropaguleBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)SaplingGenerator.CODEC.fieldOf("tree").forGetter(block -> block.generator), PropaguleBlock.createSettingsCodec()).apply((Applicative)instance, PropaguleBlock::new));
    final static public IntProperty AGE = Properties.AGE_4;
    final static public int field_37589 = 4;
    final static private int[] MIN_Y_BY_AGE = new int[]{13, 10, 7, 3, 0};
    final static private VoxelShape[] SHAPES_BY_AGE = Block.createShapeArray(4, age -> Block.createColumnShape(2.0, MIN_Y_BY_AGE[age], 16.0));
    final static private BooleanProperty WATERLOGGED = Properties.WATERLOGGED;
    final static public BooleanProperty HANGING = Properties.HANGING;
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        if (PropaguleBlock.isHanging(state)) {
            return world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isOf(Blocks.MANGROVE_LEAVES);
        }
