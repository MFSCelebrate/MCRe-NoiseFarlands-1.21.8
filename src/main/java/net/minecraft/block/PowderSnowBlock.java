...
implements FluidDrainable {
    final static public MapCodec<PowderSnowBlock> CODEC = PowderSnowBlock.createCodec(PowderSnowBlock::new);
    final static private float field_31216 = 0.083333336f;
    final static private float HORIZONTAL_MOVEMENT_MULTIPLIER = 0.9f;
    final static private float VERTICAL_MOVEMENT_MULTIPLIER = 1.5f;
    final static private float field_31219 = 2.5f;
    final static private VoxelShape FALLING_SHAPE = VoxelShapes.cuboid(0.0, 0.0, 0.0, 1.0, 0.9f, 1.0);
    final static private double field_36189 = 4.0;
    final static private double SMALL_FALL_SOUND_MAX_DISTANCE = 7.0;
    ...
    protected void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity2, EntityCollisionHandler handler) {
        if (!(entity2 instanceof LivingEntity) || entity2.getBlockStateAtPos().isOf(this)) {
            ...
            if (world.isClient) {
                ...
                boolean bl2 = bl = entity2.lastRenderX != entity2.getX() || entity2.lastRenderZ != entity2.getZ();
        ...
        handler.addPreCallback(CollisionEvent.EXTINGUISH, entity -> {
            if (world instanceof ServerWorld) {
                ...
                if (entity.isOnFire() && (serverWorld.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING) || entity instanceof PlayerEntity) && entity.canModifyAt(serverWorld, blockPos)) {
