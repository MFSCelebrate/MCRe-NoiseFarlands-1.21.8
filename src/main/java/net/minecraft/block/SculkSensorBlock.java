...
implements Waterloggable {
    final static public MapCodec<SculkSensorBlock> CODEC = SculkSensorBlock.createCodec(SculkSensorBlock::new);
    final static public int field_31239 = 30;
    final static public int field_44607 = 10;
    final static public EnumProperty<SculkSensorPhase> SCULK_SENSOR_PHASE = Properties.SCULK_SENSOR_PHASE;
    ...
    final static private VoxelShape OUTLINE_SHAPE = Block.createColumnShape(16.0, 0.0, 8.0);
    final static private float[] RESONATION_NOTE_PITCHES = Util.make(new float[16], frequency -> {
        ...
        if (!world.isClient() && SculkSensorBlock.isInactive(state) && entity.getType() != EntityType.WARDEN && (blockEntity = world.getBlockEntity(pos)) instanceof SculkSensorBlockEntity) {
            ...
            if (world instanceof ServerWorld) {
                ...
                if (sculkSensorBlockEntity.getVibrationCallback().accepts(serverWorld, pos, GameEvent.STEP, GameEvent.Emitter.of(state))) {
                    sculkSensorBlockEntity.net_minecraft_world_event_Vibrations$VibrationListener_getEventListener().forceListen(serverWorld, GameEvent.STEP, GameEvent.Emitter.of(entity), entity.getPos());
                }
            }
        }
    ...
    private static void updateNeighbors(World world, BlockPos pos, BlockState state) {
        ...
        world.updateNeighbors(pos.net_minecraft_util_math_BlockPos_down(), block);
    }
