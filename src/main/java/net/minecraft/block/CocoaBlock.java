...
implements Fertilizable {
    final static public MapCodec<CocoaBlock> CODEC = CocoaBlock.createCodec(CocoaBlock::new);
    final static public int MAX_AGE = 2;
    final static public IntProperty AGE = Properties.AGE_2;
    final static private List<Map<Direction, VoxelShape>> SHAPES = IntStream.rangeClosed(0, 2).mapToObj(age -> VoxelShapes.createHorizontalFacingShapeMap(Block.createColumnShape(4 + age * 2, 7 - age * 2, 12.0).offset(0.0, 0.0, (double)(age - 5) / 16.0).simplify())).toList();
    ...
    protected boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        BlockState blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_offset((Direction)state.get(FACING)));
