...
extends SnowyBlock {
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (world.getLightLevel(pos.net_minecraft_util_math_BlockPos_up()) >= 9) {
            BlockState blockState = this.getDefaultState();
            for (int i = 0; i < 4; ++i) {
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_add(random.nextInt(3) - 1, random.nextInt(5) - 3, random.nextInt(3) - 1);
                if (!world.getBlockState(blockPos).isOf(Blocks.DIRT) || !SpreadableBlock.canSpread(blockState, world, blockPos)) continue;
                world.setBlockState(blockPos, (BlockState)blockState.with(SNOWY, SpreadableBlock.isSnow(world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_up()))));
            }
        }
    }
}
