...
implements BlockEntityProvider {
    final static public MapCodec<BedBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)DyeColor.CODEC.fieldOf("color").forGetter(BedBlock::getColor), BedBlock.createSettingsCodec()).apply((Applicative)instance, BedBlock::new));
    final static public EnumProperty<BedPart> PART = Properties.BED_PART;
    final static public BooleanProperty OCCUPIED = Properties.OCCUPIED;
    final static private Map<Direction, VoxelShape> SHAPES_BY_DIRECTION = Util.make(() -> {
    ...
    protected ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, BlockHitResult hit) {
        ...
        if (state.get(PART) != BedPart.HEAD && !(state = world.getBlockState(pos = pos.net_minecraft_util_math_BlockPos_offset((Direction)state.get(FACING)))).isOf(this)) {
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        BedPart bedPart;
        if (!world.isClient && player.shouldSkipBlockDrops() && (bedPart = state.get(PART)) == BedPart.FOOT && (blockState = world.getBlockState(blockPos = pos.net_minecraft_util_math_BlockPos_offset(BedBlock.getDirectionTowardsOtherPart(bedPart, (Direction)state.get(FACING))))).isOf(this) && blockState.get(PART) == BedPart.HEAD) {
    ...
    private static boolean isBedBelow(BlockView world, BlockPos pos) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).getBlock() instanceof BedBlock;
    }
    ...
    public void onPlaced(World world, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack itemStack) {
        ...
        if (!world.isClient) {
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset((Direction)state.get(FACING));
    ...
    protected long getRenderingSeed(BlockState state, BlockPos pos) {
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset((Direction)state.get(FACING), state.get(PART) == BedPart.HEAD ? 0 : 1);
