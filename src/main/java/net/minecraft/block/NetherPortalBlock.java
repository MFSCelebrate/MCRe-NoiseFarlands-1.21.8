...
implements Portal {
    ...
    final static public MapCodec<NetherPortalBlock> CODEC = NetherPortalBlock.createCodec(NetherPortalBlock::new);
    final static public EnumProperty<Direction.Axis> AXIS = Properties.HORIZONTAL_AXIS;
    final static private Map<Direction.Axis, VoxelShape> SHAPES_BY_AXIS = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(4.0, 16.0, 0.0, 16.0));
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (world.getDimension().natural() && world.getGameRules().getBoolean(GameRules.DO_MOB_SPAWNING) && random.nextInt(2000) < world.getDifficulty().getId() && world.shouldTickBlockAt(pos)) {
            ...
            if (world.getBlockState(pos).allowsSpawning(world, pos, EntityType.ZOMBIFIED_PIGLIN) && (entity = EntityType.ZOMBIFIED_PIGLIN.spawn(world, pos.net_minecraft_util_math_BlockPos_up(), SpawnReason.STRUCTURE)) != null) {
    ...
    private TeleportTarget getOrCreateExitPortalTarget(ServerWorld world, Entity entity2, BlockPos pos, BlockPos scaledPos, boolean inNether, WorldBorder worldBorder) {
        ...
        if (optional.isPresent()) {
            ...
            Direction.Axis axis = entity2.net_minecraft_world_World_getWorld().getBlockState(pos).getOrEmpty(AXIS).orElse(Direction.Axis.X);
    ...
    private static TeleportTarget getExitPortalTarget(Entity entity, BlockPos pos, BlockLocating.Rectangle exitPortalRectangle, ServerWorld world, TeleportTarget.PostDimensionTransition postDimensionTransition) {
        Vec3d vec3d;
        Direction.Axis axis;
        BlockState blockState = entity.net_minecraft_world_World_getWorld().getBlockState(pos);
        if (blockState.contains(Properties.HORIZONTAL_AXIS)) {
            axis = blockState.get(Properties.HORIZONTAL_AXIS);
            BlockLocating.Rectangle rectangle = BlockLocating.getLargestRectangle(pos, axis, 21, Direction.Axis.Y, 21, posx -> entity.net_minecraft_world_World_getWorld().getBlockState((BlockPos)posx) == blockState);
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (world.getBlockState(pos.net_minecraft_util_math_BlockPos_west()).isOf(this) || world.getBlockState(pos.net_minecraft_util_math_BlockPos_east()).isOf(this)) {
