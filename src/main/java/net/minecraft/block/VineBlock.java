...
extends Block {
    final static public MapCodec<VineBlock> CODEC = VineBlock.createCodec(VineBlock::new);
    ...
    final static public Map<Direction, BooleanProperty> FACING_PROPERTIES = ConnectingBlock.FACING_PROPERTIES.entrySet().stream().filter(entry -> entry.getKey() != Direction.DOWN).collect(Util.toMap());
    ...
    private boolean shouldHaveSide(BlockView world, BlockPos pos, Direction side) {
        ...
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(side);
        if (VineBlock.shouldConnectTo(world, blockPos, side)) {
            return true;
        }
        if (side.getAxis() != Direction.Axis.Y) {
            BooleanProperty booleanProperty = FACING_PROPERTIES.get(side);
            BlockState blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up());
    ...
    protected void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (direction.getAxis().isHorizontal() && !state.get(VineBlock.getFacingProperty(direction)).booleanValue()) {
            ...
            BlockPos blockPos22 = pos.net_minecraft_util_math_BlockPos_offset(direction);
            ...
            if (blockState2.isAir()) {
                ...
                BlockPos blockPos3 = blockPos22.net_minecraft_util_math_BlockPos_offset(direction2);
                BlockPos blockPos4 = blockPos22.net_minecraft_util_math_BlockPos_offset(direction3);
                if (bl && VineBlock.shouldConnectTo(world, blockPos3, direction2)) {
                    ...
                    if (bl && world.isAir(blockPos3) && VineBlock.shouldConnectTo(world, pos.net_minecraft_util_math_BlockPos_offset(direction2), direction4)) {
                    ...
                    } else if (bl2 && world.isAir(blockPos4) && VineBlock.shouldConnectTo(world, pos.net_minecraft_util_math_BlockPos_offset(direction3), direction4)) {
                    ...
                    } else if ((double)random.nextFloat() < 0.05 && VineBlock.shouldConnectTo(world, blockPos22.net_minecraft_util_math_BlockPos_up(), Direction.UP)) {
        ...
        if (direction == Direction.UP && pos.getY() < world.getTopYInclusive()) {
            ...
            if (world.isAir(blockPos)) {
                ...
                for (Direction direction2 : Direction.Type.HORIZONTAL) {
                    if (!random.nextBoolean() && VineBlock.shouldConnectTo(world, blockPos.net_minecraft_util_math_BlockPos_offset(direction2), direction2)) continue;
        ...
        if (pos.getY() > world.getBottomY() && ((blockState = world.getBlockState(blockPos2 = pos.net_minecraft_util_math_BlockPos_down())).isAir() || blockState.isOf(this)) && (blockState3 = blockState.isAir() ? this.getDefaultState() : blockState) != (blockState4 = this.getGrownState(state, blockState3, random)) && this.hasHorizontalSide(blockState4)) {
