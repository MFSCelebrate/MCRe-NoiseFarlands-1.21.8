...
extends Block {
    final static public MapCodec<NoteBlock> CODEC = NoteBlock.createCodec(NoteBlock::new);
    final static public EnumProperty<NoteBlockInstrument> INSTRUMENT = Properties.INSTRUMENT;
    final static public BooleanProperty POWERED = Properties.POWERED;
    final static public IntProperty NOTE = Properties.NOTE;
    final static public int field_41678 = 3;
    ...
    private BlockState getStateWithInstrument(WorldView world, BlockPos pos, BlockState state) {
        NoteBlockInstrument noteBlockInstrument = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).getInstrument();
        if (noteBlockInstrument.isNotBaseBlock()) {
            return (BlockState)state.with(INSTRUMENT, noteBlockInstrument);
        }
        NoteBlockInstrument noteBlockInstrument2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).getInstrument();
    ...
    private void playNote(@Nullable Entity entity, BlockState state, World world, BlockPos pos) {
        if (state.get(INSTRUMENT).isNotBaseBlock() || world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isAir()) {
    ...
    private Identifier getCustomSound(World world, BlockPos pos) {
        BlockEntity blockEntity = world.getBlockEntity(pos.net_minecraft_util_math_BlockPos_up());
