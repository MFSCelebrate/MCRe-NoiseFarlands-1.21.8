...
extends Block {
    final static public MapCodec<Block> DEAD_FIELD = Registries.BLOCK.getCodec().fieldOf("dead");
    final static public MapCodec<CoralBlockBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)DEAD_FIELD.forGetter(block -> block.deadCoralBlock), CoralBlockBlock.createSettingsCodec()).apply((Applicative)instance, CoralBlockBlock::new));
    final private Block deadCoralBlock;
    ...
    protected boolean isInWater(BlockView world, BlockPos pos) {
        for (Direction direction : Direction.values()) {
            FluidState fluidState = world.getFluidState(pos.net_minecraft_util_math_BlockPos_offset(direction));
