...
implements Fertilizable {
    final static public MapCodec<GrassBlock> CODEC = GrassBlock.createCodec(GrassBlock::new);
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        Optional optional = world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.PLACED_FEATURE).getOptional(VegetationPlacedFeatures.GRASS_BONEMEAL);
        block0: for (int i = 0; i < 128; ++i) {
            RegistryEntry<PlacedFeature> registryEntry;
            Fertilizable fertilizable;
            BlockPos blockPos2 = blockPos;
            for (int j = 0; j < i / 16; ++j) {
                if (!world.getBlockState((blockPos2 = blockPos2.net_minecraft_util_math_BlockPos_add(random.nextInt(3) - 1, (random.nextInt(3) - 1) * random.nextInt(3) / 2, random.nextInt(3) - 1)).net_minecraft_util_math_BlockPos_down()).isOf(this) || world.getBlockState(blockPos2).isFullCube(world, blockPos2)) continue block0;
            }
            ...
            ((PlacedFeature)registryEntry.value()).generateUnregistered(world, world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator(), random, blockPos2);
        }
    }
