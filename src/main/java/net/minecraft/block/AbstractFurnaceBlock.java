...
extends BlockWithEntity {
    ...
    @Nullable
    protected static <T extends BlockEntity> BlockEntityTicker<T> validateTicker(World world2, BlockEntityType<T> givenType, BlockEntityType<? extends AbstractFurnaceBlockEntity> expectedType) {
        BlockEntityTicker<T> blockEntityTicker;
        if (world2 instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld)world2;
            blockEntityTicker = AbstractFurnaceBlock.validateTicker(givenType, expectedType, (World world, BlockPos pos, BlockState state, BlockEntity blockEntity) -> AbstractFurnaceBlockEntity.tick(serverWorld, pos, state, blockEntity));
        } else {
