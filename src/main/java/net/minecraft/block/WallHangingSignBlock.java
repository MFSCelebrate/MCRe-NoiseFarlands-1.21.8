...
extends AbstractSignBlock {
    final static public MapCodec<WallHangingSignBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)WoodType.CODEC.fieldOf("wood_type").forGetter(AbstractSignBlock::getWoodType), WallHangingSignBlock.createSettingsCodec()).apply((Applicative)instance, WallHangingSignBlock::new));
    final static public EnumProperty<Direction> FACING = HorizontalFacingBlock.FACING;
    final static private Map<Direction.Axis, VoxelShape> COLLISION_SHAPES_BY_AXIS = VoxelShapes.createHorizontalAxisShapeMap(Block.createColumnShape(16.0, 4.0, 14.0, 16.0));
    final static private Map<Direction.Axis, VoxelShape> OUTLINE_SHAPES_BY_AXIS = VoxelShapes.createHorizontalAxisShapeMap(VoxelShapes.union(COLLISION_SHAPES_BY_AXIS.get(Direction.Axis.Z), Block.createColumnShape(14.0, 2.0, 0.0, 10.0)));
    ...
    public boolean canAttachAt(BlockState state, WorldView world, BlockPos pos) {
        ...
        return this.canAttachTo(world, state, pos.net_minecraft_util_math_BlockPos_offset(direction), direction2) || this.canAttachTo(world, state, pos.net_minecraft_util_math_BlockPos_offset(direction2), direction);
    }
