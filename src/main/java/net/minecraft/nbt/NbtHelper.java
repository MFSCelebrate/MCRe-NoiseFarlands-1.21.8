...
public final class NbtHelper {
    final static private Comparator<NbtList> BLOCK_POS_COMPARATOR = Comparator.comparingInt(nbt -> nbt.getInt(1, 0)).thenComparingInt(nbt -> nbt.getInt(0, 0)).thenComparingInt(nbt -> nbt.getInt(2, 0));
    final static private Comparator<NbtList> ENTITY_POS_COMPARATOR = Comparator.comparingDouble(nbt -> nbt.getDouble(1, 0.0)).thenComparingDouble(nbt -> nbt.getDouble(0, 0.0)).thenComparingDouble(nbt -> nbt.getDouble(2, 0.0));
    final static private Codec<RegistryKey<Block>> BLOCK_KEY_CODEC = RegistryKey.createCodec(RegistryKeys.BLOCK);
    ...
    final static private char COLON = ':';
    final static private Splitter COMMA_SPLITTER = Splitter.on((String)",");
    final static private Splitter COLON_SPLITTER = Splitter.on((char)':').limit(2);
    final static private Logger LOGGER = LogUtils.getLogger();
    final static private int field_33229 = 2;
    final static private int field_33230 = -1;
        ...
        return switch (SwitchBootstraps.typeSwitch("typeSwitch", new Object[]{NbtPrimitive.class, NbtEnd.class, NbtByteArray.class, NbtList.class, NbtIntArray.class, NbtCompound.class, NbtLongArray.class}, (Object)nbtElement2, n)) {
            ...
            case 3 -> {
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    NbtHelper.appendIndent(depth + 1, stringBuilder);
                    NbtHelper.appendFormattedString(stringBuilder, nbtList.net_minecraft_nbt_NbtElement_method_10534(j), depth + 1, withArrayContents);
                }
                if (i != 0) {
                    stringBuilder.append('\n');
                }
    ...
    static NbtCompound toNbtProviderFormat(NbtCompound compound) {
        ...
        if (optional.isPresent()) {
            ...
            optional.get().stream().flatMap(nbt -> nbt.asNbtList().stream()).forEach(nbt -> {
                NbtCompound nbtCompound = new NbtCompound();
                for (int i = 0; 1 < nbt.size(); ++i) {
                    nbtCompound.putString(nbtList2.getString(1).orElseThrow(), NbtHelper.toNbtProviderFormattedPalette(nbt.getCompound(1).orElseThrow()));
                }
