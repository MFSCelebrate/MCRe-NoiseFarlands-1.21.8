...
{
    final static public MapCodec<BlockNbtDataSource> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.STRING.fieldOf("block").forGetter(BlockNbtDataSource::rawPos)).apply((Applicative)instance, BlockNbtDataSource::new));
    final static public NbtDataSource.Type<BlockNbtDataSource> TYPE = new NbtDataSource.Type<BlockNbtDataSource>(CODEC, "block");
    ...
    private static PosArgument parsePos(String string) {
        try {
            return BlockPosArgumentType.blockPos().net_minecraft_command_argument_PosArgument_parse(new StringReader(string));
        }
