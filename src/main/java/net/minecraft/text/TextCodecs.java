...
public class TextCodecs {
    final static public Codec<Text> CODEC = Codec.recursive((String)"Component", TextCodecs::createCodec);
    final static public PacketCodec<RegistryByteBuf, Text> REGISTRY_PACKET_CODEC = PacketCodecs.registryCodec(CODEC);
    final static public PacketCodec<RegistryByteBuf, Optional<Text>> OPTIONAL_PACKET_CODEC = REGISTRY_PACKET_CODEC.collect(PacketCodecs::optional);
    final static public PacketCodec<RegistryByteBuf, Text> UNLIMITED_REGISTRY_PACKET_CODEC = PacketCodecs.unlimitedRegistryCodec(CODEC);
    final static public PacketCodec<RegistryByteBuf, Optional<Text>> OPTIONAL_UNLIMITED_REGISTRY_PACKET_CODEC = UNLIMITED_REGISTRY_PACKET_CODEC.collect(PacketCodecs::optional);
    final static public PacketCodec<ByteBuf, Text> PACKET_CODEC = PacketCodecs.unlimitedCodec(CODEC);
    ...
    public static Codec<Text> withJsonLengthLimit(final int maxLength) {
        return new Codec<Text>(){
            ...
            private <T> boolean isTooLarge(DynamicOps<T> ops, Text text) {
                DataResult dataResult = CODEC.encodeStart(_1.toJsonOps(ops), (Object)text);
    ...
    private static Codec<Text> createCodec(Codec<Text> selfCodec) {
        StringIdentifiable[] types = new TextContent.Type[]{PlainTextContent.TYPE, TranslatableTextContent.TYPE, KeybindTextContent.TYPE, ScoreTextContent.TYPE, SelectorTextContent.TYPE, NbtTextContent.TYPE};
        MapCodec mapCodec = TextCodecs.dispatchingCodec((StringIdentifiable[])types, TextContent.Type::codec, TextContent::getType, (String)"type");
    ...
    extends MapCodec<T> {
        final private List<MapCodec<? extends T>> codecs;
        final private Function<T, MapEncoder<? extends T>> codecGetter;
    ...
    extends MapCodec<T> {
        final private String dispatchingKey;
        final private MapCodec<T> withKeyCodec;
        final private MapCodec<T> withoutKeyCodec;
