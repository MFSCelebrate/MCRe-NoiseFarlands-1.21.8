...
implements TextContent {
    final static public Object[] EMPTY_ARGUMENTS = new Object[0];
    final static private Codec<Object> OBJECT_ARGUMENT_CODEC = Codecs.BASIC_OBJECT.validate(TranslatableTextContent::validate);
    final static private Codec<Object> ARGUMENT_CODEC = Codec.either(OBJECT_ARGUMENT_CODEC, TextCodecs.CODEC).xmap(either -> either.map(object -> object, text -> Objects.requireNonNullElse(text.getLiteralString(), text)), argument -> {
        Either either;
    ...
    final static public MapCodec<TranslatableTextContent> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.STRING.fieldOf("translate").forGetter(content -> content.key), (App)Codec.STRING.lenientOptionalFieldOf("fallback").forGetter(content -> Optional.ofNullable(content.fallback)), (App)ARGUMENT_CODEC.listOf().optionalFieldOf("with").forGetter(content -> TranslatableTextContent.toOptionalList(content.args))).apply((Applicative)instance, TranslatableTextContent::of));
    final static public TextContent.Type<TranslatableTextContent> TYPE = new TextContent.Type<TranslatableTextContent>(CODEC, "translatable");
    final static private StringVisitable LITERAL_PERCENT_SIGN = StringVisitable.plain("%");
    final static private StringVisitable NULL_ARGUMENT = StringVisitable.plain("null");
    final private String key;
    @Nullable
    final private String fallback;
    final private Object[] args;
    @Nullable
    private Language languageCache;
    private List<StringVisitable> translations = ImmutableList.of();
    final static private Pattern ARG_FORMAT = Pattern.compile("%(?:(\\d+)\\$)?([A-Za-z%]|$)");
