...
{
    final static public MapCodec<ScoreTextContent> INNER_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.either(ParsedSelector.CODEC, (Codec)Codec.STRING).fieldOf("name").forGetter(ScoreTextContent::name), (App)Codec.STRING.fieldOf("objective").forGetter(ScoreTextContent::objective)).apply((Applicative)instance, ScoreTextContent::new));
    final static public MapCodec<ScoreTextContent> CODEC = INNER_CODEC.fieldOf("score");
    final static public TextContent.Type<ScoreTextContent> TYPE = new TextContent.Type<ScoreTextContent>(CODEC, "score");
