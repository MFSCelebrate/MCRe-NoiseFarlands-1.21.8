...
implements TextContent {
    ...
    final static public MapCodec<NbtTextContent> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codec.STRING.fieldOf("nbt").forGetter(NbtTextContent::getPath), (App)Codec.BOOL.lenientOptionalFieldOf("interpret", (Object)false).forGetter(NbtTextContent::shouldInterpret), (App)TextCodecs.CODEC.lenientOptionalFieldOf("separator").forGetter(NbtTextContent::getSeparator), (App)NbtDataSource.CODEC.forGetter(NbtTextContent::getDataSource)).apply((Applicative)instance, NbtTextContent::new));
    final static public TextContent.Type<NbtTextContent> TYPE = new TextContent.Type<NbtTextContent>(CODEC, "nbt");
    final private boolean interpret;
    final private Optional<Text> separator;
    final private String rawPath;
    final private NbtDataSource dataSource;
    @Nullable
    final protected NbtPathArgumentType.NbtPath path;
    ...
    private static NbtPathArgumentType.NbtPath parsePath(String rawPath) {
        try {
            return new NbtPathArgumentType().net_minecraft_command_argument_NbtPathArgumentType$NbtPath_parse(new StringReader(rawPath));
        }
        ...
        if (this.interpret) {
            ...
            return stream.flatMap(nbt -> {
                try {
                    ...
                    return Stream.of(Texts.parse(source, text, sender, 1));
                }
