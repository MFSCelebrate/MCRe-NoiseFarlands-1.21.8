...
{
    final static public MapCodec<TrailParticleEffect> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Vec3d.CODEC.fieldOf("target").forGetter(TrailParticleEffect::target), (App)Codecs.RGB.fieldOf("color").forGetter(TrailParticleEffect::color), (App)Codecs.POSITIVE_INT.fieldOf("duration").forGetter(TrailParticleEffect::duration)).apply((Applicative)instance, TrailParticleEffect::new));
    final static public PacketCodec<RegistryByteBuf, TrailParticleEffect> PACKET_CODEC = PacketCodec.tuple(Vec3d.PACKET_CODEC, TrailParticleEffect::target, PacketCodecs.INTEGER, TrailParticleEffect::color, PacketCodecs.VAR_INT, TrailParticleEffect::duration, TrailParticleEffect::new);
