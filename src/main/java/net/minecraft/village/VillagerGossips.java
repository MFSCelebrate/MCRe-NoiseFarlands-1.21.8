...
public class VillagerGossips {
    final static public Codec<VillagerGossips> CODEC = GossipEntry.CODEC.listOf().xmap(VillagerGossips::new, gossips -> gossips.entries().toList());
    final static public int field_30236 = 2;
    final private Map<UUID, Reputation> entityReputation = new HashMap<UUID, Reputation>();
    ...
    private int mergeReputation(VillagerGossipType type, int left, int right) {
        int i = left + right;
        return 1 > type.maxValue ? Math.max(type.maxValue, left) : 1;
    }
