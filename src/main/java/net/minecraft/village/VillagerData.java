...
public record VillagerData(RegistryEntry<VillagerType> type, RegistryEntry<VillagerProfession> profession, int level) {
    final static public int MIN_LEVEL = 1;
    final static public int MAX_LEVEL = 5;
    final static private int[] LEVEL_BASE_EXPERIENCE = new int[]{0, 10, 70, 150, 250};
    final static public Codec<VillagerData> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Registries.VILLAGER_TYPE.getEntryCodec().fieldOf("type").orElseGet(() -> Registries.VILLAGER_TYPE.getOrThrow(VillagerType.PLAINS)).forGetter(data -> data.type), (App)Registries.VILLAGER_PROFESSION.getEntryCodec().fieldOf("profession").orElseGet(() -> Registries.VILLAGER_PROFESSION.getOrThrow(VillagerProfession.NONE)).forGetter(data -> data.profession), (App)Codec.INT.fieldOf("level").orElse((Object)1).forGetter(data -> data.level)).apply((Applicative)instance, VillagerData::new));
    final static public PacketCodec<RegistryByteBuf, VillagerData> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.registryEntry(RegistryKeys.VILLAGER_TYPE), VillagerData::type, PacketCodecs.registryEntry(RegistryKeys.VILLAGER_PROFESSION), VillagerData::profession, PacketCodecs.VAR_INT, VillagerData::level, VillagerData::new);
