...
extends PersistentState {
    final static private String RAIDS = "raids";
    final static public Codec<RaidManager> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)RaidWithId.CODEC.listOf().optionalFieldOf(RAIDS, List.of()).forGetter(raidManager -> raidManager.raids.int2ObjectEntrySet().stream().map(RaidWithId::fromMapEntry).toList()), (App)Codec.INT.fieldOf("next_id").forGetter(raidManager -> raidManager.nextAvailableId), (App)Codec.INT.fieldOf("tick").forGetter(raidManager -> raidManager.currentTime)).apply((Applicative)instance, RaidManager::new));
    final static public PersistentStateType<RaidManager> STATE_TYPE = new PersistentStateType<RaidManager>("raids", RaidManager::new, CODEC, DataFixTypes.SAVED_DATA_RAIDS);
    final static public PersistentStateType<RaidManager> END_STATE_TYPE = new PersistentStateType<RaidManager>("raids_end", RaidManager::new, CODEC, DataFixTypes.SAVED_DATA_RAIDS);
    final private Int2ObjectMap<Raid> raids = new Int2ObjectOpenHashMap();
    ...
    public void tick(ServerWorld world) {
        ...
        DebugInfoSender.sendRaids(world, (Collection<Raid>)this.raids.values());
    }
    ...
    extends Record {
        final int id;
        final Raid raid;
        final static public Codec<RaidWithId> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.INT.fieldOf("id").forGetter(RaidWithId::id), (App)Raid.CODEC.forGetter(RaidWithId::raid)).apply((Applicative)instance, RaidWithId::new));
