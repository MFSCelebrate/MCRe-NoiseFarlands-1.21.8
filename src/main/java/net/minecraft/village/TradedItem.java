...
public record TradedItem(RegistryEntry<Item> item, int count, ComponentMapPredicate components, ItemStack itemStack) {
    final static public Codec<TradedItem> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Item.ENTRY_CODEC.fieldOf("id").forGetter(TradedItem::item), (App)Codecs.POSITIVE_INT.fieldOf("count").orElse((Object)1).forGetter(TradedItem::count), (App)ComponentMapPredicate.CODEC.optionalFieldOf("components", (Object)ComponentMapPredicate.EMPTY).forGetter(TradedItem::components)).apply((Applicative)instance, TradedItem::new));
    final static public PacketCodec<RegistryByteBuf, TradedItem> PACKET_CODEC = PacketCodec.tuple(Item.ENTRY_PACKET_CODEC, TradedItem::item, PacketCodecs.VAR_INT, TradedItem::count, ComponentMapPredicate.PACKET_CODEC, TradedItem::components, TradedItem::new);
    final static public PacketCodec<RegistryByteBuf, Optional<TradedItem>> OPTIONAL_PACKET_CODEC = PACKET_CODEC.collect(PacketCodecs::optional);
