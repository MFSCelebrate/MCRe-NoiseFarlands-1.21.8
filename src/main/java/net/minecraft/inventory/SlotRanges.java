...
public class SlotRanges {
    final static private List<SlotRange> SLOT_RANGES = Util.make(new ArrayList(), list -> {
    ...
    final static public Codec<SlotRange> CODEC = StringIdentifiable.createBasicCodec(() -> SLOT_RANGES.toArray(new SlotRange[0]));
    final static private Function<String, SlotRange> FROM_NAME = StringIdentifiable.createMapper((StringIdentifiable[])SLOT_RANGES.toArray(new SlotRange[0]), name -> name);
