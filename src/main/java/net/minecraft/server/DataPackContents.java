...
public class DataPackContents {
    ...
    final static private CompletableFuture<Unit> COMPLETED_UNIT = CompletableFuture.completedFuture(Unit.INSTANCE);
    final private ReloadableRegistries.Lookup reloadableRegistries;
    final private CommandManager commandManager;
    final private ServerRecipeManager recipeManager;
    final private ServerAdvancementLoader serverAdvancementLoader;
    final private FunctionLoader functionLoader;
    final private List<Registry.PendingTagLoad<?>> pendingTagLoads;
    ...
    public static CompletableFuture<DataPackContents> reload(ResourceManager resourceManager, CombinedDynamicRegistries<ServerDynamicRegistryType> dynamicRegistries, List<Registry.PendingTagLoad<?>> pendingTagLoads, FeatureSet enabledFeatures, CommandManager.RegistrationEnvironment environment, int functionPermissionLevel, Executor prepareExecutor, Executor applyExecutor) {
        return ReloadableRegistries.reload(dynamicRegistries, pendingTagLoads, resourceManager, prepareExecutor).thenCompose(reloadResult -> {
            DataPackContents dataPackContents = new DataPackContents(reloadResult.layers(), reloadResult.lookupWithUpdatedTags(), enabledFeatures, environment, pendingTagLoads, 1);
