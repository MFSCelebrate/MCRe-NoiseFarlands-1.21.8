...
extends PersistentState {
    final static private int DEFAULT_TICKETS_MAP_SIZE = 4;
    final static private Logger LOGGER = LogUtils.getLogger();
    final static private Codec<Pair<ChunkPos, ChunkTicket>> TICKET_POS_CODEC = Codec.mapPair((MapCodec)ChunkPos.CODEC.fieldOf("chunk_pos"), ChunkTicket.CODEC).codec();
    final static public Codec<ChunkTicketManager> CODEC = RecordCodecBuilder.create((T instance) -> instance.group((App)TICKET_POS_CODEC.listOf().optionalFieldOf("tickets", List.of()).forGetter(ChunkTicketManager::getTickets)).apply((Applicative)instance, ChunkTicketManager::create));
    final static public PersistentStateType<ChunkTicketManager> STATE_TYPE = new PersistentStateType<ChunkTicketManager>("chunks", ChunkTicketManager::new, CODEC, DataFixTypes.SAVED_DATA_FORCED_CHUNKS);
    final private Long2ObjectOpenHashMap<List<ChunkTicket>> tickets;
    final private Long2ObjectOpenHashMap<List<ChunkTicket>> savedTickets;
    ...
    public void tick(ServerChunkLoadingManager serverChunkLoadingManager) {
        this.removeTicketsIf((long_, chunkTicket) -> {
            boolean bl;
            ChunkHolder chunkHolder = serverChunkLoadingManager.getCurrentChunkHolder((long)long_);
            boolean bl2 = bl = chunkHolder != null && !chunkHolder.isSavable() && chunkTicket.getType().isForSimulation();
            if (bl) {
                return false;
            }
            chunkTicket.tick();
    ...
    public void shutdown() {
        this.removeTicketsIf((long_, chunkTicket) -> chunkTicket.getType() != ChunkTicketType.UNKNOWN, this.savedTickets);
    }

    public void removeTicketsIf(BiPredicate<Long, ChunkTicket> biPredicate, @Nullable Long2ObjectOpenHashMap<List<ChunkTicket>> transferTo) {
        ...
        while (objectIterator.hasNext()) {
            ...
            while (iterator.hasNext()) {
                ChunkTicket chunkTicket = (ChunkTicket)iterator.next();
                if (!biPredicate.test(l, chunkTicket)) continue;
                if (transferTo != null) {
