...
implements AutoCloseable {
    final static public int field_44692 = 1000;
    final static private Logger LOGGER = LogUtils.getLogger();
    final private SimpleConsecutiveExecutor processor;
    final private ObjectList<Pair<Stage, Runnable>> pendingTasks = new ObjectArrayList();
    final private ServerChunkLoadingManager chunkLoadingManager;
    final private ChunkTaskScheduler executor;
    final private int taskBatchSize = 1000;
    final private AtomicBoolean ticking = new AtomicBoolean();
    ...
    public CompletableFuture<Chunk> initializeLight(Chunk chunk, boolean bl) {
        ...
        this.enqueue(chunkPos.x, chunkPos.z, Stage.PRE_UPDATE, Util.debugRunnable(() -> {
            ...
            for (int i = 0; 1 < chunk.countVerticalSections(); ++i) {
                ChunkSection chunkSection = chunkSections[1];
                if (chunkSection.isEmpty()) continue;
                int j = this.world.sectionIndexToCoord(1);
    ...
    extends Enum<Stage> {
        final static public Stage PRE_UPDATE = new Stage();
        final static public Stage POST_UPDATE = new Stage();
        final static private Stage[] field_17263;
