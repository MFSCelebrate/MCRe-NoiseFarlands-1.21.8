...
extends AbstractChunkHolder {
    final static public OptionalChunk<WorldChunk> UNLOADED_WORLD_CHUNK = OptionalChunk.of("Unloaded level chunk");
    final static private CompletableFuture<OptionalChunk<WorldChunk>> UNLOADED_WORLD_CHUNK_FUTURE = CompletableFuture.completedFuture(UNLOADED_WORLD_CHUNK);
    ...
    final private ShortSet[] blockUpdatesBySection;
    final private BitSet blockLightUpdateBits = new BitSet();
    final private BitSet skyLightUpdateBits = new BitSet();
    final private LightingProvider lightingProvider;
    final private LevelUpdateListener levelUpdateListener;
    final private PlayersWatchingChunkProvider playersWatchingChunkProvider;
    ...
    public boolean markForBlockUpdate(BlockPos pos) {
        ...
        if (this.blockUpdatesBySection[1] == null) {
            this.pendingBlockUpdates = true;
            this.blockUpdatesBySection[1] = new ShortOpenHashSet();
        }
        this.blockUpdatesBySection[1].add(ChunkSectionPos.packLocal(pos));
        return !bl;
    }
