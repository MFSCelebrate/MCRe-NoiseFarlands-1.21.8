...
public class ExecuteCommand {
    ...
    final static private int MAX_BLOCKS = 32768;
    final static private Dynamic2CommandExceptionType BLOCKS_TOOBIG_EXCEPTION = new Dynamic2CommandExceptionType((maxCount, count) -> Text.stringifiedTranslatable("commands.execute.blocks.toobig", maxCount, count));
    final static private SimpleCommandExceptionType CONDITIONAL_FAIL_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.execute.conditional.fail"));
    final static private DynamicCommandExceptionType CONDITIONAL_FAIL_COUNT_EXCEPTION = new DynamicCommandExceptionType(count -> Text.stringifiedTranslatable("commands.execute.conditional.fail_count", count));
    @VisibleForTesting
    final static public Dynamic2CommandExceptionType INSTANTIATION_FAILURE_EXCEPTION = new Dynamic2CommandExceptionType((function, message) -> Text.stringifiedTranslatable("commands.execute.function.instantiationFailure", function, message));
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess) {
        ...
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("execute").requires(CommandManager.requirePermissionLevel(2))).then(CommandManager.literal("run").redirect((CommandNode)dispatcher.getRoot()))).then(ExecuteCommand.addConditionArguments((CommandNode<ServerCommandSource>)literalCommandNode, CommandManager.literal("if"), true, commandRegistryAccess))).then(ExecuteCommand.addConditionArguments((CommandNode<ServerCommandSource>)literalCommandNode, CommandManager.literal("unless"), false, commandRegistryAccess))).then(CommandManager.literal("as").then(CommandManager.argument("targets", EntityArgumentType.entities()).fork((CommandNode)literalCommandNode, context -> {
            ...
            ArrayList list = Lists.newArrayList();
            for (Entity entity : EntityArgumentType.getOptionalEntities((CommandContext<ServerCommandSource>)context, "targets")) {
                list.add(((ServerCommandSource)context.getSource()).withWorld((ServerWorld)entity.net_minecraft_world_World_getWorld()).withPosition(entity.getPos()).withRotation(entity.getRotationClient()));
            }
            return list;
    ...
    private static boolean isLoaded(ServerWorld world, BlockPos pos) {
        ...
        WorldChunk worldChunk = world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getWorldChunk(chunkPos.x, chunkPos.z);
        if (worldChunk != null) {
    ...
    private static OptionalInt testBlocksCondition(ServerWorld world, BlockPos start, BlockPos end, BlockPos destination, boolean masked) throws CommandSyntaxException {
        ...
        BlockBox blockBox2 = BlockBox.create(destination, destination.net_minecraft_util_math_BlockPos_add(blockBox.getDimensions()));
                ...
                while (true) {
                    ...
                    for (int m = blockBox.getMinX(); m <= blockBox.getMaxX(); ++m) {
                        block21: {
                            ...
                            BlockPos blockPos3 = blockPos2.net_minecraft_util_math_BlockPos_add(blockPos);
                            ...
                            if (blockEntity != null) {
                                block20: {
                                    ...
                                    block19: {
                                        ...
                                        if (blockEntity2.getType() == blockEntity.getType()) break block19;
                                        optionalInt = OptionalInt.empty();
                                        logging.close();
                                        return optionalInt;
                                    }
                                    if (blockEntity.getComponents().equals(blockEntity2.getComponents())) break block20;
                                    optionalInt = OptionalInt.empty();
                                    logging.close();
                                    return optionalInt;
                                }
                                ...
                                if (nbtCompound.equals(nbtCompound2)) break block21;
                                OptionalInt optionalInt = OptionalInt.empty();
                                logging.close();
                                return optionalInt;
                            }
    ...
    private static LiteralArgumentBuilder<ServerCommandSource> addOnArguments(CommandNode<ServerCommandSource> node, LiteralArgumentBuilder<ServerCommandSource> builder) {
        return (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)builder.then(CommandManager.literal("owner").fork(node, ExecuteCommand.createEntityModifier(entity -> {
            ...
            if (entity instanceof Ownable) {
                ...
                optional = Optional.ofNullable(ownable.net_minecraft_entity_Entity_getOwner());
            } else {
                optional = Optional.empty();
            }
            return optional;
    ...
    private static void method_54852(List list, AbstractServerCommandSource abstractServerCommandSource, ExecutionControl newControl) {
    ...
    private static void method_54853(IntPredicate intPredicate, List list, AbstractServerCommandSource abstractServerCommandSource, boolean successful, int returnValue) {
    ...
    implements Forkable.RedirectModifier<ServerCommandSource> {
        final private IntPredicate predicate;
