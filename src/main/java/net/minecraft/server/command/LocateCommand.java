...
public class LocateCommand {
    ...
    final static private DynamicCommandExceptionType STRUCTURE_NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.locate.structure.not_found", id));
    final static private DynamicCommandExceptionType STRUCTURE_INVALID_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.locate.structure.invalid", id));
    final static private DynamicCommandExceptionType BIOME_NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.locate.biome.not_found", id));
    final static private DynamicCommandExceptionType POI_NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.locate.poi.not_found", id));
    final static private int LOCATE_STRUCTURE_RADIUS = 100;
    final static private int LOCATE_BIOME_RADIUS = 6400;
    final static private int LOCATE_BIOME_HORIZONTAL_BLOCK_CHECK_INTERVAL = 32;
    final static private int LOCATE_BIOME_VERTICAL_BLOCK_CHECK_INTERVAL = 64;
    final static private int LOCATE_POI_RADIUS = 256;
    ...
    private static int executeLocateStructure(ServerCommandSource source, RegistryPredicateArgumentType.RegistryPredicate<Structure> predicate) throws CommandSyntaxException {
        RegistryWrapper.Impl registry = source.getWorld().getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.STRUCTURE);
        ...
        Pair<BlockPos, RegistryEntry<Structure>> pair = serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator().locateStructure(serverWorld, registryEntryList, blockPos, 100, false);
        stopwatch.stop();
        if (pair == null) {
    ...
    private static int sendCoordinates(ServerCommandSource source, BlockPos currentPos, Pair<BlockPos, ? extends RegistryEntry<?>> result, String successMessage, boolean includeY, String entryString, Duration timeTaken) {
        ...
        source.sendFeedback(() -> Text.translatable(successMessage, entryString, text, 1), false);
