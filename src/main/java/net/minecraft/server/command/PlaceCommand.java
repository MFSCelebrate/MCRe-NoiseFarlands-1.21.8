...
public class PlaceCommand {
    final static private SimpleCommandExceptionType FEATURE_FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.place.feature.failed"));
    final static private SimpleCommandExceptionType JIGSAW_FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.place.jigsaw.failed"));
    final static private SimpleCommandExceptionType STRUCTURE_FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.place.structure.failed"));
    final static private DynamicCommandExceptionType TEMPLATE_INVALID_EXCEPTION = new DynamicCommandExceptionType(id -> Text.stringifiedTranslatable("commands.place.template.invalid", id));
    final static private SimpleCommandExceptionType TEMPLATE_FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.place.template.failed"));
    final static private SuggestionProvider<ServerCommandSource> SUGGESTION_PROVIDER = (context, builder) -> {
    ...
    public static int executePlaceFeature(ServerCommandSource source, RegistryEntry.Reference<ConfiguredFeature<?, ?>> feature, BlockPos pos) throws CommandSyntaxException {
        ...
        if (!configuredFeature.generate(serverWorld, serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator(), serverWorld.getRandom(), pos)) {
    ...
    public static int executePlaceStructure(ServerCommandSource source, RegistryEntry.Reference<Structure> structure, BlockPos pos) throws CommandSyntaxException {
        ...
        ChunkGenerator chunkGenerator = serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator();
        StructureStart structureStart = structure2.createStructureStart(structure, serverWorld.getRegistryKey(), source.getRegistryManager(), chunkGenerator, chunkGenerator.getBiomeSource(), serverWorld.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig(), serverWorld.getStructureTemplateManager(), serverWorld.getSeed(), new ChunkPos(pos), 0, serverWorld, biome -> true);
    ...
    public static int executePlaceTemplate(ServerCommandSource source, Identifier id, BlockPos pos, BlockRotation rotation, BlockMirror mirror, float integrity, int seed, boolean strict) throws CommandSyntaxException {
        ...
        PlaceCommand.throwOnUnloadedPos(serverWorld, new ChunkPos(pos), new ChunkPos(pos.net_minecraft_util_math_BlockPos_add(structureTemplate.getSize())));
        ...
        if (!(bl = structureTemplate.place(serverWorld, pos, pos, structurePlacementData, StructureBlockBlockEntity.createRandom(seed), Block.NOTIFY_LISTENERS | (strict ? Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS : 0)))) {
