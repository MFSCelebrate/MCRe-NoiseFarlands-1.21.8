...
public class RaidCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("raid").requires(CommandManager.requirePermissionLevel(3))).then(CommandManager.literal("start").then(CommandManager.argument("omenlvl", IntegerArgumentType.integer(0)).executes(context -> RaidCommand.executeStart((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger((CommandContext)context, (String)"omenlvl")))))).then(CommandManager.literal("stop").executes(context -> RaidCommand.executeStop((ServerCommandSource)context.getSource())))).then(CommandManager.literal("check").executes(context -> RaidCommand.executeCheck((ServerCommandSource)context.getSource())))).then(CommandManager.literal("sound").then(CommandManager.argument("type", TextArgumentType.text(registryAccess)).executes(context -> RaidCommand.executeSound((ServerCommandSource)context.getSource(), TextArgumentType.parseTextArgument((CommandContext<ServerCommandSource>)context, "type")))))).then(CommandManager.literal("spawnleader").executes(context -> RaidCommand.executeSpawnLeader((ServerCommandSource)context.getSource())))).then(CommandManager.literal("setomen").then(CommandManager.argument("level", IntegerArgumentType.integer(0)).executes(context -> RaidCommand.executeSetOmen((ServerCommandSource)context.getSource(), IntegerArgumentType.getInteger((CommandContext)context, (String)"level")))))).then(CommandManager.literal("glow").executes(context -> RaidCommand.executeGlow((ServerCommandSource)context.getSource()))));
    }
    ...
    private static int executeSetOmen(ServerCommandSource source, int level) throws CommandSyntaxException {
        ...
        if (raid != null) {
            ...
            if (level > 1) {
                source.sendError(Text.literal("Sorry, the max raid omen level you can set is " + 1));
            } else {
    ...
    private static int executeSpawnLeader(ServerCommandSource source) {
        ...
        raiderEntity.equipStack(EquipmentSlot.HEAD, Raid.createOminousBanner(source.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)));
    ...
    private static int executeStart(ServerCommandSource source, int level) throws CommandSyntaxException {
        ...
        if (serverPlayerEntity.net_minecraft_server_world_ServerWorld_getWorld().hasRaidAt(blockPos)) {
            source.sendError(Text.literal("Raid already started close by"));
            return -1;
        }
        RaidManager raidManager = serverPlayerEntity.net_minecraft_server_world_ServerWorld_getWorld().getRaidManager();
    ...
    private static int executeStop(ServerCommandSource source) throws CommandSyntaxException {
        ServerPlayerEntity serverPlayerEntity = source.getPlayerOrThrow();
        BlockPos blockPos = serverPlayerEntity.getBlockPos();
        Raid raid = serverPlayerEntity.net_minecraft_server_world_ServerWorld_getWorld().getRaidAt(blockPos);
        if (raid != null) {
            raid.invalidate();
    ...
    private static Raid getRaid(ServerPlayerEntity player) {
        return player.net_minecraft_server_world_ServerWorld_getWorld().getRaidAt(player.getBlockPos());
    }
}
