...
TestInstanceBlockFinder {
    final static TestInstanceFinder NOOP_TEST_FUNCTION_FINDER = Stream::empty;
    final static TestInstanceBlockFinder NOOP_TEST_INSTANCE_BLOCK_FINDER = Stream::empty;
    final private TestInstanceFinder instanceFinder;
    final private TestInstanceBlockFinder blockFinder;
    final private ServerCommandSource commandSource;
    ...
    public static class Builder {
        final private UnaryOperator<Supplier<Stream<RegistryEntry.Reference<TestInstance>>>> testInstanceFinderMapper;
        final private UnaryOperator<Supplier<Stream<BlockPos>>> testInstanceBlockFinderMapper;
        ...
        public TestFinder allStructures(CommandContext<ServerCommandSource> context) {
            ServerCommandSource serverCommandSource = (ServerCommandSource)context.getSource();
            BlockPos blockPos = BlockPos.ofFloored(serverCommandSource.getPosition());
            return this.build(serverCommandSource, NOOP_TEST_FUNCTION_FINDER, () -> TestInstanceUtil.findTestInstanceBlocks(blockPos, 200, serverCommandSource.getWorld()));
        }
