...
public class FillBiomeCommand {
    ...
    public static Either<Integer, CommandSyntaxException> fillBiome(ServerWorld world, BlockPos from, BlockPos to, RegistryEntry<Biome> biome, Predicate<RegistryEntry<Biome>> filter, Consumer<Supplier<Text>> feedbackConsumer) {
        ...
        if (i > (j = world.getGameRules().getInt(GameRules.COMMAND_MODIFICATION_BLOCK_LIMIT))) {
            return Either.right((Object)((Object)TOO_BIG_EXCEPTION.create((Object)j, (Object)i)));
        }
        ...
        for (int k = ChunkSectionPos.getSectionCoord(blockBox.getMinZ()); k <= ChunkSectionPos.getSectionCoord(blockBox.getMaxZ()); ++k) {
            for (int l = ChunkSectionPos.getSectionCoord(blockBox.getMinX()); l <= ChunkSectionPos.getSectionCoord(blockBox.getMaxX()); ++l) {
                ...
                if (chunk == null) {
                    return Either.right((Object)((Object)UNLOADED_EXCEPTION.create()));
                }
                list.add(chunk);
            }
        }
        ...
        for (Chunk chunk : list) {
            chunk.populateBiomes(FillBiomeCommand.createBiomeSupplier(mutableInt, chunk, blockBox, biome, filter), world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getNoiseConfig().getMultiNoiseSampler());
            chunk.markNeedsSaving();
        }
        world.net_minecraft_server_world_ServerChunkManager_getChunkManager().chunkLoadingManager.sendChunkBiomePackets(list);
        feedbackConsumer.accept(() -> Text.translatable("commands.fillbiome.success.count", mutableInt.getValue(), blockBox.getMinX(), blockBox.getMinY(), blockBox.getMinZ(), blockBox.getMaxX(), blockBox.getMaxY(), blockBox.getMaxZ()));
