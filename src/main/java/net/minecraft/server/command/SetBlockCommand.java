...
public class SetBlockCommand {
    final static private SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.setblock.failed"));
    ...
    private static int execute(ServerCommandSource source, BlockPos pos, BlockStateArgument block, Mode mode, @Nullable Predicate<CachedBlockPosition> condition, boolean strict) throws CommandSyntaxException {
        ...
        if (bl && !block.setBlockState(serverWorld, pos, Block.NOTIFY_LISTENERS | (strict ? Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS : Block.SKIP_BLOCK_ENTITY_REPLACED_CALLBACK))) {
            throw FAILED_EXCEPTION.create();
        }
        if (!strict) {
    ...
    extends Enum<Mode> {
        final static public Mode REPLACE = new Mode();
        final static public Mode DESTROY = new Mode();
        final static private Mode[] field_13720;
