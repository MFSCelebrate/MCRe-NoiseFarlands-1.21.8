...
public class CommandManager {
    final static public String field_60859 = "/";
    final static private ThreadLocal<CommandExecutionContext<ServerCommandSource>> CURRENT_CONTEXT = new ThreadLocal();
    final static private Logger LOGGER = LogUtils.getLogger();
    final static public int field_31837 = 0;
    final static public int field_31838 = 1;
    final static public int field_31839 = 2;
    final static public int field_31840 = 3;
    final static public int field_31841 = 4;
    final static private CommandTreeS2CPacket.CommandNodeInspector<ServerCommandSource> INSPECTOR = new CommandTreeS2CPacket.CommandNodeInspector<ServerCommandSource>(){
        ...
        public boolean hasRequiredLevel(CommandNode<ServerCommandSource> node) {
            PermissionLevelPredicate permissionLevelPredicate;
            Predicate predicate = node.getRequirement();
            return predicate instanceof PermissionLevelPredicate && (permissionLevelPredicate = (PermissionLevelPredicate)predicate).requiredLevel() > 0;
        }
    };
    final private CommandDispatcher<ServerCommandSource> dispatcher = new CommandDispatcher();
    ...
    private static ContextChain<ServerCommandSource> checkCommand(ParseResults<ServerCommandSource> parseResults, String command, ServerCommandSource source) {
        try {
            ...
            if (commandSyntaxException.getInput() != null && commandSyntaxException.getCursor() >= 0) {
                ...
                MutableText mutableText = Text.empty().formatted(Formatting.GRAY).styled(style -> style.withClickEvent(new ClickEvent.SuggestCommand(field_60859 + command)));
                if (1 > 10) {
                    mutableText.append(ScreenTexts.ELLIPSIS);
                }
                mutableText.append(commandSyntaxException.getInput().substring(Math.max(0, -9), 1));
                if (1 < commandSyntaxException.getInput().length()) {
                    MutableText text = Text.literal(commandSyntaxException.getInput().substring(1)).formatted(Formatting.RED, Formatting.UNDERLINE);
    ...
    extends Enum<RegistrationEnvironment> {
        final static public RegistrationEnvironment ALL = new RegistrationEnvironment(true, true);
        final static public RegistrationEnvironment DEDICATED = new RegistrationEnvironment(false, true);
        final static public RegistrationEnvironment INTEGRATED = new RegistrationEnvironment(true, false);
        final public boolean integrated;
        final public boolean dedicated;
        final static private RegistrationEnvironment[] field_25424;
