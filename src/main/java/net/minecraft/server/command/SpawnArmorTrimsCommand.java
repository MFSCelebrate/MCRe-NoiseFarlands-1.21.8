...
public class SpawnArmorTrimsCommand {
    final static private List<RegistryKey<ArmorTrimPattern>> PATTERNS = List.of(ArmorTrimPatterns.SENTRY, ArmorTrimPatterns.DUNE, ArmorTrimPatterns.COAST, ArmorTrimPatterns.WILD, ArmorTrimPatterns.WARD, ArmorTrimPatterns.EYE, ArmorTrimPatterns.VEX, ArmorTrimPatterns.TIDE, ArmorTrimPatterns.SNOUT, ArmorTrimPatterns.RIB, ArmorTrimPatterns.SPIRE, ArmorTrimPatterns.WAYFINDER, ArmorTrimPatterns.SHAPER, ArmorTrimPatterns.SILENCE, ArmorTrimPatterns.RAISER, ArmorTrimPatterns.HOST, ArmorTrimPatterns.FLOW, ArmorTrimPatterns.BOLT);
    final static private List<RegistryKey<ArmorTrimMaterial>> MATERIALS = List.of(ArmorTrimMaterials.QUARTZ, ArmorTrimMaterials.IRON, ArmorTrimMaterials.NETHERITE, ArmorTrimMaterials.REDSTONE, ArmorTrimMaterials.COPPER, ArmorTrimMaterials.GOLD, ArmorTrimMaterials.EMERALD, ArmorTrimMaterials.DIAMOND, ArmorTrimMaterials.LAPIS, ArmorTrimMaterials.AMETHYST, ArmorTrimMaterials.RESIN);
    final static private ToIntFunction<RegistryKey<ArmorTrimPattern>> PATTERN_INDEX_GETTER = Util.lastIndexGetter(PATTERNS);
    final static private ToIntFunction<RegistryKey<ArmorTrimMaterial>> MATERIAL_INDEX_GETTER = Util.lastIndexGetter(MATERIALS);
    final static private DynamicCommandExceptionType INVALID_PATTERN_EXCEPTION = new DynamicCommandExceptionType(pattern -> Text.stringifiedTranslatable("Invalid pattern", pattern));
    ...
    private static int execute(ServerCommandSource source, PlayerEntity player) {
        return SpawnArmorTrimsCommand.execute(source, player, source.getServer().getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.TRIM_PATTERN).streamEntries());
    }

    private static int execute(ServerCommandSource source, PlayerEntity player, RegistryKey<ArmorTrimPattern> pattern) {
        return SpawnArmorTrimsCommand.execute(source, player, Stream.of((RegistryEntry.Reference)source.getServer().getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.TRIM_PATTERN).getOptional(pattern).orElseThrow()));
    }
    ...
    private static int execute(ServerCommandSource source, PlayerEntity player, Stream<RegistryEntry.Reference<ArmorTrimPattern>> patterns) {
        ...
        List<RegistryEntry.Reference> list2 = serverWorld.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.TRIM_MATERIAL).streamEntries().sorted(Comparator.comparing(material -> MATERIAL_INDEX_GETTER.applyAsInt(material.registryKey()))).toList();
        List<RegistryEntry.Reference<Item>> list3 = SpawnArmorTrimsCommand.getArmorItems(serverWorld.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.ITEM));
        BlockPos blockPos = player.getBlockPos().net_minecraft_util_math_BlockPos_offset(player.getHorizontalFacing(), 5);
        double d = 3.0;
