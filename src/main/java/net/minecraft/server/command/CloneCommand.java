...
public class CloneCommand {
    ...
    final static private SimpleCommandExceptionType OVERLAP_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.clone.overlap"));
    final static private Dynamic2CommandExceptionType TOO_BIG_EXCEPTION = new Dynamic2CommandExceptionType((maxCount, count) -> Text.stringifiedTranslatable("commands.clone.toobig", maxCount, count));
    final static private SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.clone.failed"));
    final static public Predicate<CachedBlockPosition> IS_AIR_PREDICATE = pos -> !pos.getBlockState().isAir();

    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess) {
        dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("clone").requires(CommandManager.requirePermissionLevel(2))).then(CloneCommand.createSourceArgs(commandRegistryAccess, context -> ((ServerCommandSource)context.getSource()).getWorld()))).then(CommandManager.literal("from").then(CommandManager.argument("sourceDimension", DimensionArgumentType.dimension()).then(CloneCommand.createSourceArgs(commandRegistryAccess, context -> DimensionArgumentType.getDimensionArgument((CommandContext<ServerCommandSource>)context, "sourceDimension"))))));
    }
    ...
    private static int execute(ServerCommandSource source, DimensionalPos begin, DimensionalPos end, DimensionalPos destination, Predicate<CachedBlockPosition> filter, Mode mode, boolean strict) throws CommandSyntaxException {
        ...
        BlockPos blockPos4 = blockPos3.net_minecraft_util_math_BlockPos_add(blockBox.getDimensions());
            ...
            for (l = blockBox.getMinZ(); l <= blockBox.getMaxZ(); ++l) {
                for (m = blockBox.getMinY(); m <= blockBox.getMaxY(); ++m) {
                    for (int n = blockBox.getMinX(); n <= blockBox.getMaxX(); ++n) {
                        ...
                        BlockPos blockPos7 = blockPos6.net_minecraft_util_math_BlockPos_add(blockPos5);
    ...
    extends Enum<Mode> {
        final static public Mode FORCE = new Mode(true);
        final static public Mode MOVE = new Mode(true);
        final static public Mode NORMAL = new Mode(false);
        final private boolean allowsOverlap;
        final static private Mode[] field_13501;
