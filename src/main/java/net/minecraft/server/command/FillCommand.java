...
public class FillCommand {
    final static private Dynamic2CommandExceptionType TOO_BIG_EXCEPTION = new Dynamic2CommandExceptionType((maxCount, count) -> Text.stringifiedTranslatable("commands.fill.toobig", maxCount, count));
    final static BlockStateArgument AIR_BLOCK_ARGUMENT = new BlockStateArgument(Blocks.AIR.getDefaultState(), Collections.emptySet(), null);
    final static private SimpleCommandExceptionType FAILED_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.fill.failed"));
    ...
    private static int execute(ServerCommandSource source, BlockBox range, BlockStateArgument block, Mode mode, @Nullable Predicate<CachedBlockPosition> filter, boolean strict) throws CommandSyntaxException {
        ...
        for (BlockPos blockPos : BlockPos.iterate(range.getMinX(), range.getMinY(), range.getMinZ(), range.getMaxX(), range.getMaxY(), range.getMaxZ())) {
            ...
            if (!blockStateArgument.setBlockState(serverWorld, blockPos, Block.NOTIFY_LISTENERS | (strict ? Block.FORCE_STATE_AND_SKIP_CALLBACKS_AND_DROPS : Block.SKIP_BLOCK_ENTITY_REPLACED_CALLBACK))) {
                if (!bl) continue;
                ++k;
                continue;
            }
    ...
    extends Enum<Mode> {
        final static public Mode REPLACE = new Mode(PostProcessor.EMPTY, Filter.IDENTITY);
        final static public Mode OUTLINE = new Mode(PostProcessor.EMPTY, (range, pos, block, world) -> {
        ...
        final static public Mode HOLLOW = new Mode(PostProcessor.EMPTY, (range, pos, block, world) -> {
            if (pos.getX() == range.getMinX() || pos.getX() == range.getMaxX() || pos.getY() == range.getMinY() || pos.getY() == range.getMaxY() || pos.getZ() == range.getMinZ() || pos.getZ() == range.getMaxZ()) {
                return block;
            }
            return AIR_BLOCK_ARGUMENT;
        });
        final static public Mode DESTROY = new Mode((world, pos) -> world.breakBlock(pos, true), Filter.IDENTITY);
        final public Filter filter;
        final public PostProcessor postProcessor;
        final static private Mode[] field_13653;
    ...
    public static interface PostProcessor {
        final static public PostProcessor EMPTY = (world, pos) -> false;
    ...
    public static interface Filter {
        final static public Filter IDENTITY = (box, pos, block, world) -> block;
