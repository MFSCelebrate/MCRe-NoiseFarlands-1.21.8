...
public class TeamMsgCommand {
    final static private Style STYLE = Style.EMPTY.withHoverEvent(new HoverEvent.ShowText(Text.translatable("chat.type.team.hover"))).withClickEvent(new ClickEvent.SuggestCommand("/teammsg "));
    final static private SimpleCommandExceptionType NO_TEAM_EXCEPTION = new SimpleCommandExceptionType((Message)Text.translatable("commands.teammsg.failed.noteam"));
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        LiteralCommandNode literalCommandNode = dispatcher.register((LiteralArgumentBuilder)CommandManager.literal("teammsg").then(CommandManager.argument("message", MessageArgumentType.message()).executes(context -> {
            ...
            List<ServerPlayerEntity> list = serverCommandSource.getServer().net_minecraft_server_PlayerManager_getPlayerManager().getPlayerList().stream().filter(player -> player == entity || player.getScoreboardTeam() == team).toList();
            if (!list.isEmpty()) {
