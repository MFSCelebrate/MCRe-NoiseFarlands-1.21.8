...
public class TeleportCommand {
    ...
    private static int execute(ServerCommandSource source, Collection<? extends Entity> targets, Entity destination) throws CommandSyntaxException {
        for (Entity entity : targets) {
            TeleportCommand.teleport(source, entity, (ServerWorld)destination.net_minecraft_world_World_getWorld(), destination.getX(), destination.getY(), destination.getZ(), EnumSet.noneOf(PositionFlag.class), destination.getYaw(), destination.getPitch(), null);
        }
        if (targets.size() == 1) {
    ...
    private static int execute(ServerCommandSource source, Collection<? extends Entity> targets, ServerWorld world, PosArgument location, @Nullable PosArgument rotation, @Nullable LookTarget facingLocation) throws CommandSyntaxException {
        ...
        for (Entity entity : targets) {
            Set<PositionFlag> set = TeleportCommand.getFlags(location, rotation, entity.net_minecraft_world_World_getWorld().getRegistryKey() == world.getRegistryKey());
            if (vec2f == null) {
