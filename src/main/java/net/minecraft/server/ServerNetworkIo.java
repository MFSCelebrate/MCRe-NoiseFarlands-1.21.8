...
public class ServerNetworkIo {
    ...
    final static public Supplier<NioEventLoopGroup> DEFAULT_CHANNEL = Suppliers.memoize(() -> new NioEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Server IO #%d").setDaemon(true).build()));
    final static public Supplier<EpollEventLoopGroup> EPOLL_CHANNEL = Suppliers.memoize(() -> new EpollEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Epoll Server IO #%d").setDaemon(true).build()));
    final MinecraftServer server;
    public volatile boolean active;
    final private List<ChannelFuture> channels = Collections.synchronizedList(Lists.newArrayList());
    ...
    extends ChannelInboundHandlerAdapter {
        final static private Timer TIMER = new HashedWheelTimer();
        final private int baseDelay;
        final private int extraDelay;
        final private List<Packet> packets = Lists.newArrayList();
        ...
        private void delay(ChannelHandlerContext ctx, Object msg) {
            ...
            TIMER.newTimeout(this::forward, 1L, TimeUnit.MILLISECONDS);
        }
        ...
        static class Packet {
            final public ChannelHandlerContext context;
