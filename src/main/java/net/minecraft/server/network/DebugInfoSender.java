...
public class DebugInfoSender {
    ...
    public static void sendRedstoneUpdateOrder(World world, DebugRedstoneUpdateOrderCustomPayload payload) {
        if (world instanceof ServerWorld) {
            ...
            DebugInfoSender.sendToAll(serverWorld, payload);
        }
    }
        ...
        for (Map.Entry<MemoryModuleType<?>, Optional<Memory<?>>> entry : map.entrySet()) {
            ...
            if (optional.isPresent()) {
                ...
                if (memoryModuleType == MemoryModuleType.HEARD_BELL_TIME) {
                    ...
                    string = memory.isTimed() ? DebugInfoSender.format((ServerWorld)entity.net_minecraft_world_World_getWorld(), object) + " (ttl: " + memory.getExpiry() + ")" : DebugInfoSender.format((ServerWorld)entity.net_minecraft_world_World_getWorld(), object);
                }
            } else {
                string = "-";
            }
    ...
    private static String format(ServerWorld world, @Nullable Object object) {
        ...
        if (object instanceof UUID) {
            return DebugInfoSender.format(world, world.net_minecraft_entity_Entity_getEntity((UUID)object));
        }
    ...
    private static void method_55630(ServerWorld serverWorld, Vec3d vec3d, RegistryKey registryKey) {
        DebugInfoSender.sendToAll(serverWorld, new DebugGameEventCustomPayload(registryKey, vec3d));
    }

    private static void method_52277(List list, UUID uUID, Object2IntMap object2IntMap) {
        String string = NameGenerator.name(uUID);
        object2IntMap.forEach((villagerGossipType, integer) -> list.add(string + ": " + String.valueOf(villagerGossipType) + ": " + integer));
    }
