...
public class SpawnLocating {
    ...
    protected static BlockPos findOverworldSpawn(ServerWorld world, int x, int z) {
        ...
        WorldChunk worldChunk = world.net_minecraft_world_chunk_WorldChunk_getChunk(ChunkSectionPos.getSectionCoord(x), ChunkSectionPos.getSectionCoord(z));
        int n = i = bl ? world.net_minecraft_server_world_ServerChunkManager_getChunkManager().getChunkGenerator().getSpawnHeight(world) : worldChunk.sampleHeightmap(Heightmap.Type.MOTION_BLOCKING, x & 0xF, z & 0xF);
        ...
        for (int k = i + 1; k >= world.getBottomY(); --k) {
            ...
            return ((BlockPos)mutable.net_minecraft_util_math_Vec3i_up()).toImmutable();
        }
        return null;
    }
