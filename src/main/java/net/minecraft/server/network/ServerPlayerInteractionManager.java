...
public class ServerPlayerInteractionManager {
    ...
    final protected ServerPlayerEntity player;
    ...
    public ServerPlayerInteractionManager(ServerPlayerEntity player) {
        this.player = player;
        this.world = player.net_minecraft_server_world_ServerWorld_getWorld();
    }
    ...
    public boolean changeGameMode(GameMode gameMode) {
        ...
        this.world.getServer().net_minecraft_server_PlayerManager_getPlayerManager().sendToAll(new PlayerListS2CPacket(PlayerListS2CPacket.Action.UPDATE_GAME_MODE, this.player));
    ...
    private float continueMining(BlockState state, BlockPos pos, int failedStartMiningTime) {
        ...
        float f = state.calcBlockBreakingDelta(this.player, this.player.net_minecraft_server_world_ServerWorld_getWorld(), pos) * 2.0f;
        int j = (int)(f * 10.0f);
    ...
    public void processBlockBreakingAction(BlockPos pos, PlayerActionC2SPacket.Action action, Direction direction, int worldHeight, int sequence) {
        ...
        if (action == PlayerActionC2SPacket.Action.START_DESTROY_BLOCK) {
            ...
            if (!blockState.isAir()) {
                ...
                f = blockState.calcBlockBreakingDelta(this.player, this.player.net_minecraft_server_world_ServerWorld_getWorld(), pos);
            }
            ...
            if (pos.equals(this.miningPos)) {
                ...
                BlockState blockState = this.world.getBlockState(pos);
                if (!blockState.isAir()) {
                    float g = blockState.calcBlockBreakingDelta(this.player, this.player.net_minecraft_server_world_ServerWorld_getWorld(), pos) * (float)(j + 1);
                    if (g >= 0.7f) {
                        this.mining = false;
