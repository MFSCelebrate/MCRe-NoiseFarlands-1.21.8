...
public class EntityTrackerEntry {
    ...
    final static private int field_29767 = 1;
    final static private double field_44988 = 7.62939453125E-6;
    final static public int field_44987 = 60;
    final static private int field_44989 = 400;
    final private ServerWorld world;
    final private Entity entity;
    final private int tickInterval;
    final private boolean alwaysUpdateVelocity;
    final private Consumer<Packet<?>> watchingSender;
    final private BiConsumer<Packet<?>, List<UUID>> filteredWatchingSender;
    final private TrackedPosition trackedPos = new TrackedPosition();
    ...
    public EntityTrackerEntry(ServerWorld world, Entity entity, int tickInterval, boolean alwaysUpdateVelocity, Consumer<Packet<?>> watchingSender, BiConsumer<Packet<?>, List<UUID>> filteredWatchingSender) {
        this.world = world;
                ...
                if (entity2 instanceof AbstractMinecartEntity && (minecartController = (abstractMinecartEntity = (AbstractMinecartEntity)entity2).getController()) instanceof ExperimentalMinecartController) {
                    ...
                    if ((this.entity.velocityDirty || this.alwaysUpdateVelocity || this.entity instanceof LivingEntity && ((LivingEntity)this.entity).isGliding()) && ((d = (vec3d2 = this.entity.getVelocity()).squaredDistanceTo(this.velocity)) > 1.0E-7 || d > 0.0 && vec3d2.lengthSquared() == 0.0)) {
                        ...
                        if (entity3 instanceof ExplosiveProjectileEntity) {
                            ...
                            this.watchingSender.accept(new BundleS2CPacket((Iterable<Packet<? super ClientPlayPacketListener>>)List.of(new EntityVelocityUpdateS2CPacket(this.entity.getId(), this.velocity), new ProjectilePowerS2CPacket(explosiveProjectileEntity.getId(), explosiveProjectileEntity.accelerationPower))));
                        } else {
    ...
    private Stream<Entity> streamChangedPassengers(List<Entity> newPassengers) {
        return Streams.concat((Stream[])new Stream[]{this.lastPassengers.stream().filter(passenger -> !newPassengers.contains(passenger)), newPassengers.stream().filter(passenger -> !this.lastPassengers.contains(passenger))});
    }
