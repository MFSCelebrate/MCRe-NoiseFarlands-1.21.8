...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                ...
                saveLoader = (SaveLoader)Util.waitAndApply(arg_0 -> Main.method_43612(serverConfig, dynamic2, optionSet, (OptionSpec)optionSpec3, serverPropertiesLoader, (OptionSpec)optionSpec4, arg_0)).get();
            }
            ...
            LOGGER.error(LogUtils.FATAL_MARKER, "Failed to start the minecraft server", (Throwable)exception4);
        }
    }
    ...
    private static MinecraftDedicatedServer method_29734(LevelStorage.Session session, ResourcePackManager resourcePackManager, SaveLoader saveLoader, ServerPropertiesLoader serverPropertiesLoader, ApiServices apiServices, OptionSet optionSet, OptionSpec optionSpec, OptionSpec optionSpec2, OptionSpec optionSpec3, OptionSpec optionSpec4, OptionSpec optionSpec5, Thread thread) {
        boolean bl;
        MinecraftDedicatedServer minecraftDedicatedServer = new MinecraftDedicatedServer(thread, session, resourcePackManager, saveLoader, serverPropertiesLoader, Schemas.getFixer(), apiServices, WorldGenerationProgressLogger::create);
    ...
    private static CompletableFuture method_43612(SaveLoading.ServerConfig serverConfig, Dynamic dynamic, OptionSet optionSet, OptionSpec optionSpec, ServerPropertiesLoader serverPropertiesLoader, OptionSpec optionSpec2, Executor applyExecutor) {
        return SaveLoading.load(serverConfig, context -> {
            DimensionOptionsRegistryHolder dimensionOptionsRegistryHolder;
            GeneratorOptions generatorOptions;
            LevelInfo levelInfo;
            RegistryWrapper.Impl registry = context.dimensionsRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.DIMENSION);
            if (dynamic != null) {
                ParsedSaveProperties parsedSaveProperties = LevelStorage.parseSaveProperties(dynamic, context.dataConfiguration(), (Registry<DimensionOptions>)registry, context.worldGenRegistryManager());
            ...
            DimensionOptionsRegistryHolder.DimensionsConfig dimensionsConfig = dimensionOptionsRegistryHolder.toConfig((Registry<DimensionOptions>)registry);
