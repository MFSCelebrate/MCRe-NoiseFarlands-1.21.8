...
public record ServerMetadata(Text description, Optional<Players> players, Optional<Version> version, Optional<Favicon> favicon, boolean secureChatEnforced) {
    ...
    public record Players(int max, int online, List<GameProfile> sample) {
        final static private Codec<GameProfile> GAME_PROFILE_CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Uuids.STRING_CODEC.fieldOf("id").forGetter(GameProfile::getId), (App)Codec.STRING.fieldOf("name").forGetter(GameProfile::getName)).apply((Applicative)instance, GameProfile::new));
        final static public Codec<Players> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.INT.fieldOf("max").forGetter(Players::max), (App)Codec.INT.fieldOf("online").forGetter(Players::online), (App)GAME_PROFILE_CODEC.listOf().lenientOptionalFieldOf("sample", List.of()).forGetter(Players::sample)).apply((Applicative)instance, Players::new));
    }

    public record Version(String gameVersion, int protocolVersion) {
        final static public Codec<Version> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.STRING.fieldOf("name").forGetter(Version::gameVersion), (App)Codec.INT.fieldOf("protocol").forGetter(Version::protocolVersion)).apply((Applicative)instance, Version::new));
    ...
    public record Favicon(byte[] iconBytes) {
        final static private String DATA_URI_PREFIX = "data:image/png;base64,";
        final static public Codec<Favicon> CODEC = Codec.STRING.comapFlatMap(uri -> {
