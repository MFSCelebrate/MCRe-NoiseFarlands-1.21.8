...
public class SaveLoading {
    ...
    public static <D, R> CompletableFuture<R> load(ServerConfig serverConfig, LoadContextSupplier<D> loadContextSupplier, SaveApplierFactory<D, R> saveApplierFactory, Executor prepareExecutor, Executor applyExecutor) {
        try {
            ...
            return ((CompletableFuture)DataPackContents.reload(lifecycledResourceManager, combinedDynamicRegistries2, list, dataConfiguration.enabledFeatures(), serverConfig.commandEnvironment(), serverConfig.functionPermissionLevel(), prepareExecutor, applyExecutor).whenComplete((dataPackContents, throwable) -> {
                if (throwable != null) {
    ...
    extends Record {
        ...
        final private CommandManager.RegistrationEnvironment commandEnvironment;
        final private int functionPermissionLevel;
        ...
        public ServerConfig(DataPacks dataPacks, CommandManager.RegistrationEnvironment registrationEnvironment, int i) {
            ...
            this.functionPermissionLevel = 1;
        }
