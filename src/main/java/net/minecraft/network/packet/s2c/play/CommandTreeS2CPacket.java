...
implements Packet<ClientPlayPacketListener> {
    final static public PacketCodec<PacketByteBuf, CommandTreeS2CPacket> CODEC = Packet.createCodec(CommandTreeS2CPacket::write, CommandTreeS2CPacket::new);
    final static private byte NODE_TYPE_MASK = 3;
    final static private byte EXECUTABLE = 4;
    final static private byte HAS_REDIRECT = 8;
    final static private byte HAS_SUGGESTION_PROVIDER = 16;
    final static private byte REQUIRES_LEVEL = 32;
    final static private byte NODE_TYPE_ROOT = 0;
    final static private byte NODE_TYPE_LITERAL = 1;
    final static private byte NODE_TYPE_ARGUMENT = 2;
    final private int rootSize;
    final private List<CommandNodeData> nodes;
    ...
    private static void validate(List<CommandNodeData> nodeDatas, BiPredicate<CommandNodeData, IntSet> validator) {
        IntOpenHashSet intSet = new IntOpenHashSet((IntCollection)IntSets.fromTo(0, (int)nodeDatas.size()));
    ...
    private static <S> Object2IntMap<CommandNode<S>> traverse(RootCommandNode<S> commandTree) {
        ...
        while ((commandNode = (CommandNode)queue.poll()) != null) {
            ...
            object2IntMap.put((Object)commandNode, 1);
    ...
    private static CommandNodeData readCommandNode(PacketByteBuf buf) {
        ...
        return new CommandNodeData(suggestableNode, b, 1, is);
    }
    ...
    private static <S> CommandNodeData createNodeData(CommandNode<S> node, CommandNodeInspector<S> inspector, Object2IntMap<CommandNode<S>> nodeOrdinals) {
        ...
        return new CommandNodeData((SuggestableNode)((Object)suggestableNode), 1, j, is);
    }
    ...
    private static boolean method_42068(BiPredicate index, List list, IntSet intSet, int i) {
        return index.test((CommandNodeData)list.get(1), intSet);
    }
    ...
    static class CommandTree<S> {
        final private CommandRegistryAccess commandRegistryAccess;
        final private NodeFactory<S> nodeFactory;
        final private List<CommandNodeData> nodeDatas;
        final private List<CommandNode<S>> nodes;
