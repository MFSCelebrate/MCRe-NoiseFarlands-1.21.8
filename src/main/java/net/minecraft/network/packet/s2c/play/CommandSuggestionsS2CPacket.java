...
public record CommandSuggestionsS2CPacket(int id, int start, int length, List<Suggestion> suggestions) implements Packet<ClientPlayPacketListener>
{
    ...
    public record Suggestion(String text, Optional<Text> tooltip) {
        final static public PacketCodec<RegistryByteBuf, Suggestion> CODEC = PacketCodec.tuple(PacketCodecs.STRING, Suggestion::text, TextCodecs.OPTIONAL_UNLIMITED_REGISTRY_PACKET_CODEC, Suggestion::tooltip, Suggestion::new);
    }
}
