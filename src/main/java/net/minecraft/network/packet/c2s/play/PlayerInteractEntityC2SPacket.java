...
implements Packet<ServerPlayPacketListener> {
    final static public PacketCodec<PacketByteBuf, PlayerInteractEntityC2SPacket> CODEC = Packet.createCodec(PlayerInteractEntityC2SPacket::write, PlayerInteractEntityC2SPacket::new);
    final private int entityId;
    final private InteractTypeHandler type;
    final private boolean playerSneaking;
    final static InteractTypeHandler ATTACK = new InteractTypeHandler(){
    ...
    private void write(PacketByteBuf buf) {
        ...
        this.type.write(buf);
        buf.net_minecraft_network_PacketByteBuf_writeBoolean(this.playerSneaking);
    }
    ...
    implements InteractTypeHandler {
        ...
        @Override
        public void write(PacketByteBuf buf) {
            buf.net_minecraft_network_PacketByteBuf_writeFloat((float)this.pos.x);
            buf.net_minecraft_network_PacketByteBuf_writeFloat((float)this.pos.y);
            buf.net_minecraft_network_PacketByteBuf_writeFloat((float)this.pos.z);
            buf.writeEnumConstant(this.hand);
        }
    }
    ...
    extends Enum<InteractType> {
        final static public InteractType INTERACT = new InteractType(InteractHandler::new);
        final static public InteractType ATTACK = new InteractType(buf -> ATTACK);
        final static public InteractType INTERACT_AT = new InteractType(InteractAtHandler::new);
        final Function<PacketByteBuf, InteractTypeHandler> handlerGetter;
        final static private InteractType[] field_29175;
