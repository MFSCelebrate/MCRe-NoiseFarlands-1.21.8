...
public record SyncedClientOptions(String language, int viewDistance, ChatVisibility chatVisibility, boolean chatColorsEnabled, int playerModelParts, Arm mainArm, boolean filtersText, boolean allowsServerListing, ParticlesMode particleStatus) {
    final static public int MAX_LANGUAGE_CODE_LENGTH = 16;
    ...
    public void write(PacketByteBuf buf) {
        ...
        buf.net_minecraft_network_PacketByteBuf_writeByte(this.viewDistance);
        buf.writeEnumConstant(this.chatVisibility);
        buf.net_minecraft_network_PacketByteBuf_writeBoolean(this.chatColorsEnabled);
        buf.net_minecraft_network_PacketByteBuf_writeByte(this.playerModelParts);
        buf.writeEnumConstant(this.mainArm);
        buf.net_minecraft_network_PacketByteBuf_writeBoolean(this.filtersText);
        buf.net_minecraft_network_PacketByteBuf_writeBoolean(this.allowsServerListing);
