...
extends SimpleChannelInboundHandler<Packet<?>> {
    final static private float CURRENT_PACKET_COUNTER_WEIGHT = 0.75f;
    final static private Logger LOGGER = LogUtils.getLogger();
    final static public Marker NETWORK_MARKER = MarkerFactory.getMarker((String)"NETWORK");
    final static public Marker NETWORK_PACKETS_MARKER = Util.make(MarkerFactory.getMarker((String)"NETWORK_PACKETS"), marker -> marker.add(NETWORK_MARKER));
    final static public Marker PACKET_RECEIVED_MARKER = Util.make(MarkerFactory.getMarker((String)"PACKET_RECEIVED"), marker -> marker.add(NETWORK_PACKETS_MARKER));
    final static public Marker PACKET_SENT_MARKER = Util.make(MarkerFactory.getMarker((String)"PACKET_SENT"), marker -> marker.add(NETWORK_PACKETS_MARKER));
    final static public Supplier<NioEventLoopGroup> CLIENT_IO_GROUP = Suppliers.memoize(() -> new NioEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Client IO #%d").setDaemon(true).build()));
    final static public Supplier<EpollEventLoopGroup> EPOLL_CLIENT_IO_GROUP = Suppliers.memoize(() -> new EpollEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Epoll Client IO #%d").setDaemon(true).build()));
    final static public Supplier<DefaultEventLoopGroup> LOCAL_CLIENT_IO_GROUP = Suppliers.memoize(() -> new DefaultEventLoopGroup(0, new ThreadFactoryBuilder().setNameFormat("Netty Local Client IO #%d").setDaemon(true).build()));
    final static private NetworkState<ServerHandshakePacketListener> C2S_HANDSHAKE_STATE = HandshakeStates.C2S;
    final private NetworkSide side;
    private volatile boolean duringLogin = true;
    final private Queue<Consumer<ClientConnection>> queuedTasks = Queues.newConcurrentLinkedQueue();
    ...
    private <S extends ServerPacketListener, C extends ClientPacketListener> void connect(String address, int port, NetworkState<S> outboundState, NetworkState<C> inboundState, C prePlayStateListener, ConnectionIntent intent) {
        ...
        this.submit(connection -> {
            ...
            connection.sendImmediately(new HandshakeC2SPacket(SharedConstants.getGameVersion().protocolVersion(), address, 1, intent), null, true);
    ...
    public void send(Packet<?> packet, @Nullable ChannelFutureListener channelFutureListener, boolean flush) {
        if (this.isOpen()) {
            ...
            this.queuedTasks.add(connection -> connection.sendImmediately(packet, channelFutureListener, flush));
        }
    }
    ...
    private void sendImmediately(Packet<?> packet, @Nullable ChannelFutureListener channelFutureListener, boolean flush) {
        ...
        if (this.channel.eventLoop().inEventLoop()) {
            ...
            this.channel.eventLoop().execute(() -> this.sendInternal(packet, channelFutureListener, flush));
        }
    }
