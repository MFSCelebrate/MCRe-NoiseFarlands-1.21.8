...
implements NetworkStateTransitionHandler {
    ...
    final private NetworkState<T> state;
    ...
    protected void decode(ChannelHandlerContext context, ByteBuf buf, List<Object> objects) throws Exception {
        ...
        FlightProfiler.INSTANCE.onPacketReceived(this.state.id(), packetType, context.channel().remoteAddress(), 1);
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(ClientConnection.PACKET_RECEIVED_MARKER, " IN: [{}:{}] {} -> {} bytes", new Object[]{this.state.id().getId(), packetType, packet.getClass().getName(), 1});
        }
