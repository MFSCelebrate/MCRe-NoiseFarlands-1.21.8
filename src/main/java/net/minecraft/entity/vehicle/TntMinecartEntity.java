...
extends AbstractMinecartEntity {
    final static private byte PRIME_TNT_STATUS = 10;
    final static private String EXPLOSION_POWER_NBT_KEY = "explosion_power";
    final static private String EXPLOSION_SPEED_FACTOR_NBT_KEY = "explosion_speed_factor";
    final static private String FUSE_NBT_KEY = "fuse";
    final static private float DEFAULT_EXPLOSION_POWER = 4.0f;
    final static private float DEFAULT_EXPLOSION_SPEED_FACTOR = 1.0f;
    final static private int DEFAULT_FUSE_TICKS = -1;
    @Nullable
    ...
    public void prime(@Nullable DamageSource source) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            ...
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.SET_SHEEP_EAT_GRASS_TIMER_OR_PRIME_TNT_MINECART);
            if (!this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_TNT_PRIMED, SoundCategory.BLOCKS, 1.0f, 1.0f);
            }
        }
    }
    ...
    public float getEffectiveExplosionResistance(Explosion explosion, BlockView world, BlockPos pos, BlockState blockState, FluidState fluidState, float max) {
        if (this.isPrimed() && (blockState.isIn(BlockTags.RAILS) || world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isIn(BlockTags.RAILS))) {
            return 0.0f;
        }
    ...
    public boolean canExplosionDestroyBlock(Explosion explosion, BlockView world, BlockPos pos, BlockState state, float explosionPower) {
        if (this.isPrimed() && (state.isIn(BlockTags.RAILS) || world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isIn(BlockTags.RAILS))) {
            return false;
        }
