...
implements Leashable {
    final static private TrackedData<Boolean> LEFT_PADDLE_MOVING = DataTracker.registerData(AbstractBoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Boolean> RIGHT_PADDLE_MOVING = DataTracker.registerData(AbstractBoatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Integer> BUBBLE_WOBBLE_TICKS = DataTracker.registerData(AbstractBoatEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static public int field_54427 = 0;
    final static public int field_54445 = 1;
    final static private int field_54451 = 60;
    final static private float NEXT_PADDLE_PHASE = 0.3926991f;
    final static public double EMIT_SOUND_EVENT_PADDLE_ROTATION = 0.7853981852531433;
    final static public int field_54447 = 60;
    final private float[] paddlePhases = new float[2];
    private float ticksUnderwater;
    private float yawVelocity;
    final private PositionInterpolator interpolator = new PositionInterpolator((Entity)this, 3);
    ...
    final private Supplier<Item> itemSupplier;
    ...
    public void onBubbleColumnSurfaceCollision(boolean drag, BlockPos pos) {
        ...
        if (!this.isSubmergedInWater() && this.random.nextInt(100) == 0) {
            this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), this.getSplashSound(), this.getSoundCategory(), 1.0f, 0.8f + 0.4f * this.random.nextFloat(), false);
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.SPLASH, this.getX() + (double)this.random.nextFloat(), this.getY() + 0.7, this.getZ() + (double)this.random.nextFloat(), 0.0, 0.0, 0.0);
    ...
    public void tick() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && this.ticksUnderwater >= 60.0f) {
        ...
        if (this.isLogicalSideForUpdatingMovement()) {
            ...
            if (this.net_minecraft_world_World_getWorld().isClient) {
                this.updatePaddles();
                this.net_minecraft_world_World_getWorld().sendPacket(new BoatPaddleStateC2SPacket(this.isPaddleMoving(0), this.isPaddleMoving(1)));
            }
        ...
        for (int i = 0; i <= 1; ++i) {
            if (this.isPaddleMoving(i)) {
                ...
                if (!this.isSilent() && (double)(this.paddlePhases[i] % ((float)Math.PI * 2)) <= 0.7853981852531433 && (double)((this.paddlePhases[i] + 0.3926991f) % ((float)Math.PI * 2)) >= 0.7853981852531433 && (soundEvent = this.getPaddleSound()) != null) {
                    ...
                    this.net_minecraft_world_World_getWorld().playSound(null, this.getX() + d, this.getY(), this.getZ() + e, soundEvent, this.getSoundCategory(), 1.0f, 0.8f + 0.4f * this.random.nextFloat());
                }
                int n = i;
        ...
        List<Entity> list = this.net_minecraft_world_World_getWorld().getOtherEntities(this, this.getBoundingBox().expand(0.2f, -0.01f, 0.2f), EntityPredicates.canBePushedBy(this));
        if (!list.isEmpty()) {
            boolean bl = !this.net_minecraft_world_World_getWorld().isClient && !(this.getControllingPassenger() instanceof PlayerEntity);
            ...
            for (int p = i; p < j; ++p) {
                for (int q = m; q < n; ++q) {
                    ...
                    if (fluidState.isIn(FluidTags.WATER)) {
                        f = Math.max(f, fluidState.getHeight(this.net_minecraft_world_World_getWorld(), mutable));
                    }
    ...
    public float getNearbySlipperiness() {
        ...
        for (int p = 1; p < j; ++p) {
            for (int q = m; q < n; ++q) {
                int r = (p == 1 || p == j - 1 ? 1 : 0) + (q == m || q == n - 1 ? 1 : 0);
                if (r == 2) continue;
                for (int s = k; s < l; ++s) {
                    if (r > 0 && (s == k || s == l - 1)) continue;
                    mutable.set(p, s, q);
                    BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(mutable);
                    if (blockState.getBlock() instanceof LilyPadBlock || !VoxelShapes.matchesAnywhere(blockState.getCollisionShape(this.net_minecraft_world_World_getWorld(), mutable).offset(mutable), voxelShape, BooleanBiFunction.AND)) continue;
        ...
        for (int o = i; o < j; ++o) {
            for (int p = k; p < l; ++p) {
                for (int q = m; q < n; ++q) {
                    ...
                    float f = (float)p + fluidState.getHeight(this.net_minecraft_world_World_getWorld(), mutable);
        ...
        for (int o = i; o < j; ++o) {
            for (int p = k; p < l; ++p) {
                for (int q = m; q < n; ++q) {
                    mutable.set(o, p, q);
                    FluidState fluidState = this.net_minecraft_world_World_getWorld().getFluidState(mutable);
                    if (!fluidState.isIn(FluidTags.WATER) || !(d < (double)((float)mutable.getY() + fluidState.getHeight(this.net_minecraft_world_World_getWorld(), mutable)))) continue;
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == Location.IN_AIR && this.location != Location.IN_AIR && this.location != Location.ON_LAND) {
            ...
            if (this.net_minecraft_world_World_getWorld().isSpaceEmpty(this, this.getBoundingBox().offset(0.0, g - this.getY(), 0.0))) {
    ...
    protected void updatePassengerPosition(Entity passenger, Entity.PositionUpdater positionUpdater) {
        ...
        if (passenger instanceof AnimalEntity && this.getPassengerList().size() == this.getMaxPassengers()) {
            ...
            passenger.setBodyYaw(((AnimalEntity)passenger).bodyYaw + 1.0f);
            passenger.setHeadYaw(passenger.getHeadYaw() + 1.0f);
        }
    }
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isWater(blockPos2)) {
            double g;
            ArrayList list = Lists.newArrayList();
            double f = this.net_minecraft_world_World_getWorld().getDismountHeight(blockPos);
            if (Dismounting.canDismountInBlock(f)) {
                list.add(new Vec3d(d, (double)blockPos.getY() + f, e));
            }
            if (Dismounting.canDismountInBlock(g = this.net_minecraft_world_World_getWorld().getDismountHeight(blockPos2))) {
                list.add(new Vec3d(d, (double)blockPos2.getY() + g, e));
            }
            for (EntityPose entityPose : passenger.getPoses()) {
                for (Vec3d vec3d2 : list) {
                    if (!Dismounting.canPlaceEntityAt(this.net_minecraft_world_World_getWorld(), vec3d2, passenger, entityPose)) continue;
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        if (!player.shouldCancelInteraction() && this.ticksUnderwater < 60.0f && (this.net_minecraft_world_World_getWorld().isClient || player.startRiding(this))) {
    ...
    public void remove(Entity.RemovalReason reason) {
        if (!this.net_minecraft_world_World_getWorld().isClient && reason.shouldDestroy() && this.isLeashed()) {
            this.detachLeash();
        }
        super.remove(reason);
    }
    ...
    protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
        ...
        if (onGround) {
            this.onLanding();
        } else if (!this.net_minecraft_world_World_getWorld().getFluidState(this.getBlockPos().net_minecraft_util_math_BlockPos_down()).isIn(FluidTags.WATER) && heightDifference < 0.0) {
    ...
    extends Enum<Location> {
        final static public Location IN_WATER = new Location();
        final static public Location UNDER_WATER = new Location();
        final static public Location UNDER_FLOWING_WATER = new Location();
        final static public Location ON_LAND = new Location();
        final static public Location IN_AIR = new Location();
        final static private Location[] field_7715;
