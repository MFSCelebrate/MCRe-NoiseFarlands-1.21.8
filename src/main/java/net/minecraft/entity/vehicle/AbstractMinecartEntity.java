...
extends VehicleEntity {
    final static private Vec3d VILLAGER_PASSENGER_ATTACHMENT_POS = new Vec3d(0.0, 0.0, 0.0);
    final static private TrackedData<Optional<BlockState>> CUSTOM_BLOCK_STATE = DataTracker.registerData(AbstractMinecartEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_STATE);
    final static private TrackedData<Integer> BLOCK_OFFSET = DataTracker.registerData(AbstractMinecartEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private ImmutableMap<EntityPose, ImmutableList<Integer>> DISMOUNT_FREE_Y_SPACES_NEEDED = ImmutableMap.of((Object)((Object)EntityPose.STANDING), (Object)ImmutableList.of((Object)0, (Object)1, (Object)-1), (Object)((Object)EntityPose.CROUCHING), (Object)ImmutableList.of((Object)0, (Object)1, (Object)-1), (Object)((Object)EntityPose.SWIMMING), (Object)ImmutableList.of((Object)0, (Object)1));
    final static protected float VELOCITY_SLOWDOWN_MULTIPLIER = 0.95f;
    final static private boolean DEFAULT_YAW_FLIPPED = false;
    private boolean onRail;
    private boolean yawFlipped = false;
    final private MinecartController controller;
    final static private Map<RailShape, Pair<Vec3i, Vec3i>> ADJACENT_RAIL_POSITIONS_BY_SHAPE = Maps.newEnumMap((Map)((Map)Util.make(() -> {
        ...
        Vec3i vec3i5 = vec3i.net_minecraft_util_math_Vec3i_down();
        Vec3i vec3i6 = vec3i2.net_minecraft_util_math_Vec3i_down();
        Vec3i vec3i7 = vec3i3.net_minecraft_util_math_Vec3i_down();
        Vec3i vec3i8 = vec3i4.net_minecraft_util_math_Vec3i_down();
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        for (EntityPose entityPose : immutableList) {
            ...
            while (unmodifiableIterator.hasNext()) {
                ...
                for (int[] js : is) {
                    ...
                    double d = this.net_minecraft_world_World_getWorld().getDismountHeight(Dismounting.getCollisionShape(this.net_minecraft_world_World_getWorld(), mutable), () -> Dismounting.getCollisionShape(this.net_minecraft_world_World_getWorld(), (BlockPos)mutable.net_minecraft_util_math_Vec3i_down()));
                    if (!Dismounting.canDismountInBlock(d)) continue;
                    Box box = new Box(-f, 0.0, -f, f, entityDimensions.height(), f);
                    Vec3d vec3d = Vec3d.ofCenter(mutable, d);
                    if (!Dismounting.canPlaceEntityAt(this.net_minecraft_world_World_getWorld(), passenger, box.offset(vec3d))) continue;
        ...
        for (EntityPose entityPose2 : immutableList) {
            ...
            if (!(e + g <= (h = Dismounting.getCeilingHeight(mutable, j = MathHelper.ceil(e - (double)mutable.getY() + g), pos -> this.net_minecraft_world_World_getWorld().getBlockState((BlockPos)pos).getCollisionShape(this.net_minecraft_world_World_getWorld(), (BlockPos)pos))))) continue;
    ...
    public BlockPos getRailOrMinecartPos() {
        ...
        if (AbstractMinecartEntity.areMinecartImprovementsEnabled(this.net_minecraft_world_World_getWorld())) {
            double d = this.getY() - 0.1 - (double)1.0E-5f;
            if (this.net_minecraft_world_World_getWorld().getBlockState(BlockPos.ofFloored(i, d, k)).isIn(BlockTags.RAILS)) {
                j = MathHelper.floor(d);
            }
        } else if (this.net_minecraft_world_World_getWorld().getBlockState(new BlockPos(i, j - 1, k)).isIn(BlockTags.RAILS)) {
            --j;
        }
    ...
    public Vec3d getLaunchDirection(BlockPos railPos) {
        BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(railPos);
        if (!blockState.isOf(Blocks.POWERED_RAIL) || !blockState.get(PoweredRailBlock.POWERED).booleanValue()) {
            return Vec3d.ZERO;
        }
        RailShape railShape = blockState.get(((AbstractRailBlock)blockState.getBlock()).getShapeProperty());
        if (railShape == RailShape.EAST_WEST) {
            if (this.willHitBlockAt(railPos.net_minecraft_util_math_BlockPos_west())) {
                return new Vec3d(1.0, 0.0, 0.0);
            }
            if (this.willHitBlockAt(railPos.net_minecraft_util_math_BlockPos_east())) {
                return new Vec3d(-1.0, 0.0, 0.0);
            }
        } else if (railShape == RailShape.NORTH_SOUTH) {
            if (this.willHitBlockAt(railPos.net_minecraft_util_math_BlockPos_north())) {
                return new Vec3d(0.0, 0.0, 1.0);
            }
            if (this.willHitBlockAt(railPos.net_minecraft_util_math_BlockPos_south())) {
                return new Vec3d(0.0, 0.0, -1.0);
            }
        }
        return Vec3d.ZERO;
    ...
    public boolean willHitBlockAt(BlockPos pos) {
        return this.net_minecraft_world_World_getWorld().getBlockState(pos).isSolidBlock(this.net_minecraft_world_World_getWorld(), pos);
    }
    ...
    private void pushAwayFromMinecart(AbstractMinecartEntity entity, double xDiff, double zDiff) {
        ...
        if (f < (double)0.8f && !AbstractMinecartEntity.areMinecartImprovementsEnabled(this.net_minecraft_world_World_getWorld())) {
            return;
        }
