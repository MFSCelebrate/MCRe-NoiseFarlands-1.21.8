...
public abstract class LivingEntity extends Entity implements Attackable, ServerWaypoint {
    ...
    protected static final TrackedData<
            Byte> LIVING_FLAGS = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final TrackedData<
            Float> HEALTH = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<
            List<
                    ParticleEffect>> POTION_SWIRLS = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.PARTICLE_LIST);
    private static final TrackedData<
            Boolean> POTION_SWIRLS_AMBIENT = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private static final TrackedData<
            Integer> STUCK_ARROW_COUNT = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<
            Integer> STINGER_COUNT = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<
            Optional<
                    BlockPos>> SLEEPING_POSITION = DataTracker.registerData(LivingEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_POS);
    ...
    private static final Dynamic<
            ?> BRAIN = new Dynamic((DynamicOps) JavaOps.INSTANCE, Map.of("memories", Map.of()));
    private final AttributeContainer attributes;
    private final DamageTracker damageTracker = new DamageTracker(this);
    private final Map<
            RegistryEntry<StatusEffect>,
            StatusEffectInstance> activeStatusEffects = Maps.newHashMap();
    private final Map<
            EquipmentSlot,
            ItemStack> lastEquipmentStacks = Util.mapEnum(EquipmentSlot.class, slot -> ItemStack.EMPTY);
    ...
    @Nullable protected LazyEntityReference<PlayerEntity> attackingPlayer;
    ...
    @Nullable private LazyEntityReference<LivingEntity> attackerReference;
    private int lastAttackedTime;
    @Nullable private LivingEntity attacking;
    ...
    @Nullable protected ItemStack riptideStack;
    ...
    private final EnumMap<
            ...
            Reference2ObjectMap<
                    Enchantment,
                    Set<
                            EnchantmentLocationBasedEffect>>> locationBasedEnchantmentEffects = new EnumMap(EquipmentSlot.class);
    ...
    protected void fall(double heightDifference, boolean onGround, BlockState state, BlockPos landedPosition) {
        ...
        if ((world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
    ...
    public void baseTick() {
        ...
        if ((world2 = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
        ...
        if (this.isFireImmune() || this.net_minecraft_world_World_getWorld().isClient) {
            this.extinguish();
        }
        if (this.isAlive() && (world = this.net_minecraft_world_World_getWorld())
        ...
        if (this.isDead() && this.net_minecraft_world_World_getWorld().shouldUpdatePostDeath(this)) {
    ...
    protected void updatePostDeath() {
        ++this.deathTime;
        if (this.deathTime >= 20 && !this.net_minecraft_world_World_getWorld().isClient() && !this.isRemoved()) {
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.ADD_DEATH_PARTICLES);
    ...
    public LivingEntity getAttacker() {
        return LazyEntityReference.resolve(this.attackerReference, this.net_minecraft_world_World_getWorld(), LivingEntity.class);
    }

    @Nullable
    public PlayerEntity getAttackingPlayer() {
        return LazyEntityReference.resolve(this.attackingPlayer, this.net_minecraft_world_World_getWorld(), PlayerEntity.class);
    }
    ...
    public void setAttacking(PlayerEntity attackingPlayer, int playerHitTimer) {
        this.setAttacking(new LazyEntityReference<PlayerEntity>(attackingPlayer), playerHitTimer);
    }
    ...
    private void setAttacking(LazyEntityReference<
                    PlayerEntity> attackingPlayer, int playerHitTimer) {
        this.attackingPlayer = attackingPlayer;
        this.playerHitTimer = playerHitTimer;
    }

    public void setAttacker(@Nullable LivingEntity attacker) {
        this.attackerReference = attacker != null ? new LazyEntityReference<
    ...
    public void onEquipStack(EquipmentSlot slot, ItemStack oldStack, ItemStack newStack) {
        if (this.net_minecraft_world_World_getWorld().isClient() || this.isSpectator()) {
            return;
        }
        ...
        if (!this.isSilent() && equippableComponent != null && slot == equippableComponent.slot()) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), this.getEquipSound(slot, newStack, equippableComponent), this.getSoundCategory(), 1.0f, 1.0f, this.random.nextLong());
        }
    ...
    protected RegistryEntry<
                    SoundEvent> getEquipSound(EquipmentSlot slot, ItemStack stack, EquippableComponent equippableComponent) {
    ...
    public void remove(Entity.RemovalReason reason) {
        World world;
        if ((reason == Entity.RemovalReason.KILLED || reason == Entity.RemovalReason.DISCARDED) && (world = this.net_minecraft_world_World_getWorld())
                        instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld) world;
    ...
    protected void readCustomData(ReadView view) {
        ...
        if (this.net_minecraft_world_World_getWorld() != null && !this.net_minecraft_world_World_getWorld().isClient) {
            view.read(ATTRIBUTES_KEY, EntityAttributeInstance.Packed.LIST_CODEC).ifPresent(this.getAttributes()
                    ::unpack);
        }
        ...
        this.hurtTime = view.getShort(HURT_TIME_KEY, 0);
        this.deathTime = view.getShort(DEATH_TIME_KEY, 0);
        this.lastAttackedTime = view.getInt(HURT_BY_TIMESTAMP_KEY, 0);
        view.getOptionalString("Team").ifPresent(team -> {
            boolean bl;
            Scoreboard scoreboard = this.net_minecraft_world_World_getWorld().net_minecraft_scoreboard_Scoreboard_getScoreboard();
        ...
        this.equipment.copyFrom(view.read(EQUIPMENT_KEY, EntityEquipment.CODEC).orElseGet(EntityEquipment
                ::new));
        this.waypointConfig = view.read("locator_bar_icon", Waypoint.Config.CODEC).orElseGet(Waypoint.Config
                ::new);
    }
        ...
        if (world instanceof ServerWorld) {
            ...
            if (!list.isEmpty()) {
                ...
                if (this.random.nextInt(i * j) == 0) {
                    this.net_minecraft_world_World_getWorld().addParticleClient(Util.getRandom(list, this.random), this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 1.0, 1.0, 1.0);
                }
            }
        }
    }
    ...
    private void updatePotionSwirls() {
        List<
                ParticleEffect> list = this.activeStatusEffects.values().stream().filter(StatusEffectInstance
                ::shouldShowParticles).map(StatusEffectInstance::createParticle).toList();
    ...
    public boolean canTarget(LivingEntity target) {
        if (target
                        instanceof
                        PlayerEntity && this.net_minecraft_world_World_getWorld().getDifficulty() == Difficulty.PEACEFUL) {
            return false;
        }
    ...
    @Nullable
    public final StatusEffectInstance removeStatusEffectInternal(RegistryEntry<
                    StatusEffect> effect) {
    ...
    protected void onStatusEffectUpgraded(StatusEffectInstance effect, boolean reapplyEffect, @Nullable
                    Entity source) {
        if (this.net_minecraft_world_World_getWorld().isClient) {
            return;
        }
    ...
    protected void updateAttribute(RegistryEntry<EntityAttribute> attribute) {
        ...
        } else if (attribute.matches(EntityAttributes.WAYPOINT_TRANSMIT_RANGE) && (world = this.net_minecraft_world_World_getWorld())
                        instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld) world;
    ...
    public boolean damage(ServerWorld world, DamageSource source, float amount) {
        ...
        if (bl3) {
            ...
            this.lastDamageTime = this.net_minecraft_world_World_getWorld().getTime();
            for (StatusEffectInstance statusEffectInstance : this.getStatusEffects()) {
    ...
    public float getDamageBlockedAmount(ServerWorld world, DamageSource source, float amount) {
        ...
        block10:
        {
            block9:
            {
                ...
                if (!blocksAttacksComponent.bypassedBy().map(source
                        ::isIn).orElse(false).booleanValue()) break block10;
            }
            return 0.0f;
        }
        Entity entity2 = source.getSource();
        if (entity2
                        instanceof
                        PersistentProjectileEntity && (persistentProjectileEntity = (PersistentProjectileEntity) entity2).getPierceLevel() > 0) {
            return 0.0f;
        }
        ...
        blocksAttacksComponent.onShieldHit(this.net_minecraft_world_World_getWorld(), itemStack, this, this.getActiveHand(), f);
        if (!source.isIn(DamageTypeTags.IS_PROJECTILE) && (entity = source.getSource())
    ...
    private void playThornsSound(DamageSource damageSource) {
        if (damageSource.isOf(DamageTypes.THORNS)) {
            SoundCategory soundCategory = this
                            instanceof PlayerEntity ? SoundCategory.PLAYERS : SoundCategory.HOSTILE;
            this.net_minecraft_world_World_getWorld().playSound(null, this.getPos().x, this.getPos().y, this.getPos().z, SoundEvents.ENCHANT_THORNS_HIT, soundCategory);
        }
    }
    ...
    private boolean tryUseDeathProtector(DamageSource source) {
        ...
        if (itemStack != null) {
            ...
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.USE_TOTEM_OF_UNDYING);
        }
    ...
    public DamageSource getRecentDamageSource() {
        if (this.net_minecraft_world_World_getWorld().getTime() - this.lastDamageTime > 40L) {
    ...
    private void playEquipmentBreakEffects(ItemStack stack) {
        if (!stack.isEmpty()) {
            ...
            if (registryEntry != null && !this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), registryEntry.value(), this.getSoundCategory(), 0.8f, 0.8f + this.net_minecraft_world_World_getWorld().random.nextFloat() * 0.4f, false);
            }
    ...
    public void onDeath(DamageSource damageSource) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && this.hasCustomName()) {
        ...
        if (adversary instanceof WitherEntity) {
            if (serverWorld.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                ...
                if (this.net_minecraft_world_World_getWorld().getBlockState(blockPos).isAir() && blockState.canPlaceAt(this.net_minecraft_world_World_getWorld(), blockPos)) {
                    this.net_minecraft_world_World_getWorld().setBlockState(blockPos, blockState, Block.NOTIFY_ALL);
                    bl = true;
                }
            }
            if (!bl) {
                ItemEntity itemEntity = new ItemEntity(this.net_minecraft_world_World_getWorld(), this.getX(), this.getY(), this.getZ(), new ItemStack(Items.WITHER_ROSE));
                this.net_minecraft_world_World_getWorld().spawnEntity(itemEntity);
            }
        }
    }
    ...
    protected void dropLoot(ServerWorld world, DamageSource damageSource, boolean causedByPlayer) {
        ...
        lootTable.generateLoot(lootWorldContext, this.getLootTableSeed(), stack -> this.dropStack(world, (ItemStack) stack));
    }

    public boolean forEachGiftedItem(ServerWorld world, RegistryKey<
                    LootTable> lootTableKey, BiConsumer<ServerWorld, ItemStack> lootConsumer) {
        return this.forEachGeneratedItem(world, lootTableKey, parameterSetBuilder -> parameterSetBuilder.add(LootContextParameters.ORIGIN, this.getPos()).add(LootContextParameters.THIS_ENTITY, this).build(LootContextTypes.GIFT), lootConsumer);
    }

    protected void forEachShearedItem(ServerWorld world, RegistryKey<
                    LootTable> lootTableKey, ItemStack tool, BiConsumer<
                    ServerWorld, ItemStack> lootConsumer) {
        this.forEachGeneratedItem(world, lootTableKey, parameterSetBuilder -> parameterSetBuilder.add(LootContextParameters.ORIGIN, this.getPos()).add(LootContextParameters.THIS_ENTITY, this).add(LootContextParameters.TOOL, tool).build(LootContextTypes.SHEARING), lootConsumer);
    }

    protected boolean forEachGeneratedItem(ServerWorld world, RegistryKey<
                    LootTable> lootTableKey, Function<
                    LootWorldContext.Builder,
                    LootWorldContext> lootContextParametersFactory, BiConsumer<
                    ServerWorld, ItemStack> lootConsumer) {
        LootWorldContext lootWorldContext;
        LootTable lootTable = world.getServer().getReloadableRegistries().getLootTable(lootTableKey);
        ObjectArrayList<
                ItemStack> list = lootTable.generateLoot(lootWorldContext = lootContextParametersFactory.apply(new LootWorldContext.Builder(world)));
        if (!list.isEmpty()) {
    ...
    public void takeKnockback(double strength, double x, double z) {
        if ((strength *=
                        1.0 - this.getAttributeValue(EntityAttributes.KNOCKBACK_RESISTANCE)) <= 0.0) {
            return;
        }
    ...
    public Map<
                    Enchantment,
                    Set<
                            EnchantmentLocationBasedEffect>> getLocationBasedEnchantmentEffects(EquipmentSlot slot) {
        return (Map) this.locationBasedEnchantmentEffects.computeIfAbsent(slot, equipmentSlot -> new Reference2ObjectArrayMap());
    }
    ...
    public boolean isClimbing() {
        ...
        if (blockState.getBlock()
                        instanceof TrapdoorBlock && this.canEnterTrapdoor(blockPos, blockState)) {
            this.climbingPos = Optional.of(blockPos);
            return true;
        }
        return false;
    ...
    private boolean canEnterTrapdoor(BlockPos pos, BlockState state) {
        if (state.get(TrapdoorBlock.OPEN).booleanValue()) {
            BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(pos.net_minecraft_util_math_BlockPos_down());
    ...
    public boolean isEntityLookingAtMe(LivingEntity entity, double d, boolean bl, boolean visualShape, double
                    ... checkedYs) {
        ...
        for (double e : checkedYs) {
            ...
            double d2 = bl ? f : 1.0;
            if (!(g > 1.0 - d / d2) || !entity.canSee(this, visualShape ? RaycastContext.ShapeType.VISUAL : RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, e))
                continue;
            return true;
        }
        return false;
    ...
    protected void playBlockFallSound() {
        ...
        BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(new BlockPos(i, j, k));
        if (!blockState.isAir()) {
    ...
    protected void damageEquipment(DamageSource source, float amount, EquipmentSlot... slots) {
        ...
        for (EquipmentSlot equipmentSlot : slots) {
            ...
            if (equippableComponent == null || !equippableComponent.damageOnHurt() || !itemStack.isDamageable() || !itemStack.takesDamageFrom(source))
                continue;
            itemStack.damage(1, this, equipmentSlot);
        }
    }
    ...
    protected void applyDamage(ServerWorld world, DamageSource source, float amount) {
        ...
        float g = f - amount;
        if (g > 0.0f && g < 3.4028235E37f && (entity = source.getAttacker())
    ...
    public LivingEntity getPrimeAdversary() {
        if (this.attackingPlayer != null) {
            return this.attackingPlayer.resolve(this.net_minecraft_world_World_getWorld(), PlayerEntity.class);
        }
        if (this.attackerReference != null) {
            return this.attackerReference.resolve(this.net_minecraft_world_World_getWorld(), LivingEntity.class);
        }
        return null;
    }
    ...
    public void swingHand(Hand hand, boolean fromServerPlayer) {
        if (!this.handSwinging || this.handSwingTicks >= this.getHandSwingDuration() / 2 || this.handSwingTicks < 0) {
            ...
            if (this.net_minecraft_world_World_getWorld() instanceof ServerWorld) {
                ...
                ServerChunkManager serverChunkManager = ((ServerWorld) this.net_minecraft_world_World_getWorld()).net_minecraft_server_world_ServerChunkManager_getChunkManager();
                if (fromServerPlayer) {
            ...
            case 46:
                {
                    ...
                    for (int j = 0; j < 128; ++j) {
                        ...
                        this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.PORTAL, e, k, l, f, g, h);
                    }
                    break;
                }
            case 47:
    ...
    public void addDeathParticles() {
        for (int i = 0; i < 20; ++i) {
            ...
            double g = 10.0;
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.POOF, this.getParticleX(1.0) - d * 10.0, this.getRandomBodyY() - e * 10.0, this.getParticleZ(1.0) - f * 10.0, d, e, f);
        }
    }
    ...
    private void addBubbleParticles() {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, this.getX() + d, this.getY() + e, this.getZ() + f, vec3d.x, vec3d.y, vec3d.z);
        }
    }
    ...
    private void onDismounted(Entity vehicle) {
        ...
        if (this.isRemoved()) {
            vec3d = this.getPos();
        } else if (vehicle.isRemoved() || this.net_minecraft_world_World_getWorld().getBlockState(vehicle.getBlockPos()).isIn(BlockTags.PORTALS)) {
            boolean bl;
            ...
            if (bl) {
                ...
                vec3d = this.net_minecraft_world_World_getWorld().findClosestCollision(this, voxelShape, vec3d2, this.getWidth(), this.getHeight(), this.getWidth()).map(pos -> pos.add(0.0, -e, 0.0)).orElse(vec3d);
            }
        } else {
    ...
    public void travel(Vec3d movementInput) {
        FluidState fluidState = this.net_minecraft_world_World_getWorld().getFluidState(this.getBlockPos());
    ...
    private void travelMidAir(Vec3d movementInput) {
        ...
        float f = this.isOnGround() ? this.net_minecraft_world_World_getWorld().getBlockState(blockPos).getBlock().getSlipperiness() : 1.0f;
        float g = f * 0.91f;
        ...
        d = statusEffectInstance != null ? (d +=
                        (0.05 * (double) (statusEffectInstance.getAmplifier() + 1) - vec3d.y) * 0.2) : (!this.net_minecraft_world_World_getWorld().isClient || this.net_minecraft_world_World_getWorld().isChunkLoaded(blockPos) ? (d -=
                                this.getEffectiveGravity()) : (this.getY() > (double) this.net_minecraft_world_World_getWorld().getBottomY() ? -0.1 : 0.0));
        if (this.hasNoDrag()) {
    ...
    private void travelInFluid(Vec3d movementInput) {
        ...
        if (this.horizontalCollision && this.doesNotCollide(vec3d2.x, vec3d2.y + (double) 0.6f - this.getY() + d, vec3d2.z)) {
    ...
    private Vec3d applyClimbingSpeed(Vec3d motion) {
        if (this.isClimbing()) {
            ...
            if (g < 0.0 && !this.getBlockStateAtPos().isOf(Blocks.SCAFFOLDING) && this.isHoldingOntoLadder() && this
    ...
    protected float getOffGroundSpeed() {
        return this.getControllingPassenger()
                        instanceof PlayerEntity ? this.getMovementSpeed() * 0.1f : 0.02f;
    }
    ...
    private Map<EquipmentSlot, ItemStack> getEquipmentChanges() {
        ...
        for (EquipmentSlot equipmentSlot : EquipmentSlot.VALUES) {
            ...
            if (!this.areItemsDifferent(itemStack, itemStack2 = this.getEquippedStack(equipmentSlot)))
                continue;
            if (map == null) {
    ...
    private void checkHandStackSwap(Map<EquipmentSlot, ItemStack> equipmentChanges) {
        ...
        if (itemStack != null && itemStack2 != null && ItemStack.areEqual(itemStack, this.lastEquipmentStacks.get(EquipmentSlot.OFFHAND)) && ItemStack.areEqual(itemStack2, this.lastEquipmentStacks.get(EquipmentSlot.MAINHAND))) {
            ((ServerWorld) this.net_minecraft_world_World_getWorld()).net_minecraft_server_world_ServerChunkManager_getChunkManager().sendToOtherNearbyPlayers(this, new EntityStatusS2CPacket(this, EntityStatuses.SWAP_HANDS));
    ...
    private void sendEquipmentChanges(Map<EquipmentSlot, ItemStack> equipmentChanges) {
        ...
        ((ServerWorld) this.net_minecraft_world_World_getWorld()).net_minecraft_server_world_ServerChunkManager_getChunkManager().sendToOtherNearbyPlayers(this, new EntityEquipmentUpdateS2CPacket(this.getId(), list));
    }
    ...
    public void tickMovement() {
        ...
        } else if (this.canActVoluntarily() && !this.net_minecraft_world_World_getWorld().isClient) {
            profiler.push("newAi");
            this.tickNewAi();
            profiler.pop();
        }
        ...
        if (this.jumping && this.shouldSwimInFluids()) {
            float g = this.isInLava() ? this.getFluidHeight(FluidTags.LAVA) : this.getFluidHeight(FluidTags.WATER);
            boolean bl = this.isTouchingWater() && g > 0.0;
            float h = this.getSwimHeight();
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient() || this.isLogicalSideForUpdatingMovement()) {
    ...
    protected void tickGliding() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            ...
            if (i % 10 == 0) {
                ...
                if (j % 2 == 0) {
                    List<
                            EquipmentSlot> list = EquipmentSlot.VALUES.stream().filter(slot -> LivingEntity.canGlideWith(this.getEquippedStack((EquipmentSlot) slot), slot)).toList();
    ...
    protected boolean canGlide() {
        ...
        for (EquipmentSlot equipmentSlot : EquipmentSlot.VALUES) {
            if (!LivingEntity.canGlideWith(this.getEquippedStack(equipmentSlot), equipmentSlot))
                continue;
            return true;
        }
        return false;
    ...
    protected void tickCramming() {
        ...
        int i;
        List<
                Entity> list = this.net_minecraft_world_World_getWorld().getCrammedEntities(this, this.getBoundingBox());
        if (list.isEmpty()) {
            return;
        }
        World world = this.net_minecraft_world_World_getWorld();
        if (world
                        instanceof
                        ServerWorld && (i = (serverWorld = (ServerWorld) world).getGameRules().getInt(GameRules.MAX_ENTITY_CRAMMING)) > 0 && list.size() > 0 && this.random.nextInt(4) == 0) {
            int j = 0;
    ...
    protected void tickRiptide(Box a, Box b) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && this.riptideTicks <= 0) {
    ...
    public void stopRiding() {
        Entity entity = this.getVehicle();
        super.stopRiding();
        if (entity != null && entity != this.getVehicle() && !this.net_minecraft_world_World_getWorld().isClient) {
    ...
    public void triggerItemPickedUpByEntityCriteria(ItemEntity item) {
        Entity entity = item.net_minecraft_entity_Entity_getOwner();
    ...
    public void sendPickup(Entity item, int count) {
        if (!item.isRemoved() && !this.net_minecraft_world_World_getWorld().isClient && (item
                                instanceof ItemEntity || item
                                instanceof PersistentProjectileEntity || item
                                instanceof ExperienceOrbEntity)) {
            ((ServerWorld) this.net_minecraft_world_World_getWorld()).net_minecraft_server_world_ServerChunkManager_getChunkManager().sendToOtherNearbyPlayers(item, new ItemPickupAnimationS2CPacket(item.getId(), this.getId(), count));
        }
    }
    ...
    public boolean canSee(Entity entity, RaycastContext.ShapeType shapeType, RaycastContext.FluidHandling fluidHandling, double entityY) {
        if (entity.net_minecraft_world_World_getWorld() != this.net_minecraft_world_World_getWorld()) {
            return false;
        }
        ...
        return this.net_minecraft_world_World_getWorld().raycast(new RaycastContext(vec3d, vec3d2, shapeType, fluidHandling, this)).getType() == HitResult.Type.MISS;
    }
    ...
    private ItemEntity createItemEntity(ItemStack stack, boolean atSelf, boolean retainOwnership) {
        ...
        ItemEntity itemEntity = new ItemEntity(this.net_minecraft_world_World_getWorld(), this.getX(), d, this.getZ(), stack);
    ...
    protected void tickItemStackUsage(ItemStack stack) {
        stack.usageTick(this.net_minecraft_world_World_getWorld(), this, this.getItemUseTimeLeft());
        if (--this.itemUseTimeLeft == 0 && !this.net_minecraft_world_World_getWorld().isClient && !stack.isUsedOnRelease()) {
            this.consumeItem();
        }
    }
    ...
    protected void setLivingFlag(int mask, boolean value) {
        ...
        this.dataTracker.set(LIVING_FLAGS, 1);
    }
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        ...
        } else if (LIVING_FLAGS.equals(data) && this.net_minecraft_world_World_getWorld().isClient) {
    ...
    public void spawnItemParticles(ItemStack stack, int count) {
        for (int i = 0; 1 < count; ++i) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(new ItemStackParticleEffect(ParticleTypes.ITEM, stack), vec3d2.x, vec3d2.y, vec3d2.z, vec3d.x, vec3d.y + 0.05, vec3d.z);
        }
    }

    protected void consumeItem() {
        if (this.net_minecraft_world_World_getWorld().isClient && !this.isUsingItem()) {
            return;
        }
        ...
        if (!this.activeItemStack.isEmpty() && this.isUsingItem()) {
            ItemStack itemStack = this.activeItemStack.finishUsing(this.net_minecraft_world_World_getWorld(), this);
    ...
    public void stopUsingItem() {
        ...
        if (!this.activeItemStack.isEmpty() && ItemStack.areItemsEqual(itemStack, this.activeItemStack)) {
            this.activeItemStack = itemStack;
            this.activeItemStack.onStoppedUsing(this.net_minecraft_world_World_getWorld(), this, this.getItemUseTimeLeft());
    ...
    protected boolean wouldNotSuffocateInPose(EntityPose pose) {
        ...
        return this.net_minecraft_world_World_getWorld().isBlockSpaceEmpty(this, box);
    }
    ...
    public void sleep(BlockPos pos) {
        ...
        if ((blockState = this.net_minecraft_world_World_getWorld().getBlockState(pos)).getBlock()
                instanceof BedBlock) {
            this.net_minecraft_world_World_getWorld().setBlockState(pos, (BlockState) blockState.with(BedBlock.OCCUPIED, true), Block.NOTIFY_ALL);
        }
    ...
    private boolean isSleepingInBed() {
        return this.getSleepingPosition().map(pos -> this.net_minecraft_world_World_getWorld().getBlockState((BlockPos) pos).getBlock()
                instanceof BedBlock).orElse(false);
    }

    public void wakeUp() {
        this.getSleepingPosition().filter(this.net_minecraft_world_World_getWorld()
                ::isChunkLoaded).ifPresent(pos -> {
            ...
            if (blockState.getBlock() instanceof BedBlock) {
                ...
                this.net_minecraft_world_World_getWorld().setBlockState((BlockPos) pos, (BlockState) blockState.with(BedBlock.OCCUPIED, false), Block.NOTIFY_ALL);
                Vec3d vec3d = BedBlock.findWakeUpPosition(this.getType(), (CollisionView) this.net_minecraft_world_World_getWorld(), pos, direction, this.getYaw()).orElseGet(() -> {
    ...
    public Direction getSleepingDirection() {
        ...
        return blockPos != null ? BedBlock.getDirection(this.net_minecraft_world_World_getWorld(), blockPos) : null;
    }
    ...
    public void sendEquipmentBreakStatus(Item item, EquipmentSlot slot) {
        this.net_minecraft_world_World_getWorld().sendEntityStatus(this, LivingEntity.getEquipmentBreakStatus(slot));
    ...
    private void onEquipmentRemoved(ItemStack removedEquipment, EquipmentSlot slot, AttributeContainer container) {
        removedEquipment.applyAttributeModifiers(slot, (attribute, modifier) -> {
            EntityAttributeInstance entityAttributeInstance = container.getCustomInstance((RegistryEntry<
                            EntityAttribute>) attribute);
            if (entityAttributeInstance != null) {
    ...
    public boolean isGlowing() {
        return !this.net_minecraft_world_World_getWorld().isClient() && this.hasStatusEffect(StatusEffects.GLOWING) || super.isGlowing();
    }
