...
implements ItemSteerable {
    final static private Identifier SUFFOCATING_MODIFIER_ID = Identifier.ofVanilla("suffocating");
    final static private EntityAttributeModifier SUFFOCATING_MODIFIER = new EntityAttributeModifier(SUFFOCATING_MODIFIER_ID, -0.34f, EntityAttributeModifier.Operation.ADD_MULTIPLIED_BASE);
    final static private float COLD_SADDLED_SPEED = 0.35f;
    final static private float DEFAULT_SADDLED_SPEED = 0.55f;
    final static private TrackedData<Integer> BOOST_TIME = DataTracker.registerData(StriderEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Boolean> COLD = DataTracker.registerData(StriderEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final private SaddledComponent saddledComponent;
    @Nullable
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        if (BOOST_TIME.equals(data) && this.net_minecraft_world_World_getWorld().isClient) {
    ...
    public Vec3d updatePassengerForDismount(LivingEntity passenger) {
        ...
        for (BlockPos blockPos : set) {
            double g;
            if (this.net_minecraft_world_World_getWorld().getFluidState(blockPos).isIn(FluidTags.LAVA) || !Dismounting.canDismountInBlock(g = this.net_minecraft_world_World_getWorld().getDismountHeight(blockPos))) continue;
            Vec3d vec3d2 = Vec3d.ofCenter(blockPos, g);
            for (EntityPose entityPose : passenger.getPoses()) {
                Box box = passenger.getBoundingBox(entityPose);
                if (!Dismounting.canPlaceEntityAt(this.net_minecraft_world_World_getWorld(), passenger, box.offset(vec3d2))) continue;
    ...
    private void updateFloating() {
        if (this.isInLava()) {
            ...
            if (!shapeContext.isAbove(FluidBlock.COLLISION_SHAPE, this.getBlockPos(), true) || this.net_minecraft_world_World_getWorld().getFluidState(this.getBlockPos().net_minecraft_util_math_BlockPos_up()).isIn(FluidTags.LAVA)) {
    ...
    @Override
    @Nullable
    public StriderEntity net_minecraft_entity_passive_StriderEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (bl && !this.isSilent()) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_STRIDER_EAT, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
        }
        return actionResult;
    }
    ...
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_StriderEntity_createChild(world, entity);
    }

    static class GoBackToLavaGoal
    extends MoveToTargetPosGoal {
        final private StriderEntity strider;
        ...
        public boolean shouldContinue() {
            return !this.strider.isInLava() && this.isTargetPos(this.strider.net_minecraft_world_World_getWorld(), this.targetPos);
        }
        ...
        protected boolean isTargetPos(WorldView world, BlockPos pos) {
            return world.getBlockState(pos).isOf(Blocks.LAVA) && world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).canPathfindThrough(NavigationType.LAND);
        }
    }
