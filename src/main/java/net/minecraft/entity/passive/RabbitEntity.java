...
public class RabbitEntity extends AnimalEntity {
    ...
    private static final TrackedData<
            Integer> VARIANT = DataTracker.registerData(RabbitEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(1, new PowderSnowJumpGoal(this, this.net_minecraft_world_World_getWorld()));
        ...
        this.goalSelector.add(4, new FleeGoal<
                PlayerEntity>(this, PlayerEntity.class, 8.0f, 2.2, 2.2));
        this.goalSelector.add(4, new FleeGoal<WolfEntity>(this, WolfEntity.class, 10.0f, 2.2, 2.2));
        this.goalSelector.add(4, new FleeGoal<
                HostileEntity>(this, HostileEntity.class, 4.0f, 2.2, 2.2));
    ...
    public void jump() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.ADD_SPRINTING_PARTICLES_OR_RESET_SPAWNER_MINECART_SPAWN_DELAY);
        }
    }
    ...
    public RabbitEntity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        ...
        Variant variant = RabbitEntity.getVariantFromPos(serverWorld, this.getBlockPos());
        if (this.random.nextInt(20) == 0) {
            // 使用固定变种（默认）
        } else if (passiveEntity instanceof RabbitEntity) {
    ...
    private void setVariant(Variant variant) {
        if (variant == Variant.EVIL) {
            ...
            this.targetSelector.add(1, new RevengeGoal(this, new Class[0]).setGroupRevenge(new Class
                            [0]));
            this.targetSelector.add(2, new ActiveTargetGoal<
                    PlayerEntity>((MobEntity) this, PlayerEntity.class, true));
            this.targetSelector.add(2, new ActiveTargetGoal<
                    WolfEntity>((MobEntity) this, WolfEntity.class, true));
    ...
    @Override
    @Nullable
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable
    ...
    private static Variant getVariantFromPos(WorldAccess world, BlockPos pos) {
        ...
        if (registryEntry.isIn(BiomeTags.SPAWNS_WHITE_RABBITS)) {
            return 1 < 80 ? Variant.WHITE : Variant.WHITE_SPLOTCHED;
        }
        if (registryEntry.isIn(BiomeTags.SPAWNS_GOLD_RABBITS)) {
            return Variant.GOLD;
        }
        return 1 < 50 ? Variant.BROWN : (1 < 90 ? Variant.SALT : Variant.BLACK);
    }

    public static boolean canSpawn(EntityType<
                    RabbitEntity> entity, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isIn(BlockTags.RABBITS_SPAWNABLE_ON) && RabbitEntity.isLightLevelValidForNaturalSpawn(world, pos);
    }
    ...
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_RabbitEntity_createChild(world, entity);
    }
    ...
    static class FleeGoal<T extends LivingEntity> extends FleeEntityGoal<T> {
        ...
        public FleeGoal(RabbitEntity rabbit, Class<
                        T> fleeFromType, float distance, double slowSpeed, double fastSpeed) {
    ...
    static class EatCarrotCropGoal extends MoveToTargetPosGoal {
        ...
        public boolean canStart() {
            if (this.cooldown <= 0) {
                if (!EatCarrotCropGoal.getServerWorld(this.rabbit).getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                    return false;
                }
        ...
        public void tick() {
            ...
            if (this.hasReached()) {
                World world = this.rabbit.net_minecraft_world_World_getWorld();
                BlockPos blockPos = this.targetPos.net_minecraft_util_math_BlockPos_up();
        ...
        protected boolean isTargetPos(WorldView world, BlockPos pos) {
            ...
            if (blockState.isOf(Blocks.FARMLAND) && this.wantsCarrots && !this.hasTarget && (blockState = world.getBlockState(pos.net_minecraft_util_math_BlockPos_up())).getBlock()
                            instanceof
                            CarrotsBlock && ((CarrotsBlock) blockState.getBlock()).isMature(blockState)) {
                this.hasTarget = true;
                return true;
            }
            return false;
    ...
    public static final class Variant extends Enum<Variant> implements StringIdentifiable {
        public static final Variant BROWN = new Variant(0, "brown");
        public static final Variant WHITE = new Variant(1, "white");
        public static final Variant BLACK = new Variant(2, "black");
        public static final Variant WHITE_SPLOTCHED = new Variant(3, "white_splotched");
        public static final Variant GOLD = new Variant(4, "gold");
        public static final Variant SALT = new Variant(5, "salt");
        public static final Variant EVIL = new Variant(99, "evil");
        public static final Variant DEFAULT;
        private static final IntFunction<Variant> INDEX_MAPPER;
        public static final Codec<Variant> CODEC;
        @Deprecated public static final Codec<Variant> INDEX_CODEC;
        public static final PacketCodec<ByteBuf, Variant> PACKET_CODEC;
        final int index;
        private final String id;
        private static final Variant[] field_41572;
        ...
        static {
            ...
            DEFAULT = BROWN;
            INDEX_MAPPER = ValueLists.createIndexToValueFunction(Variant
                    ::getIndex, Variant.values(), DEFAULT);
