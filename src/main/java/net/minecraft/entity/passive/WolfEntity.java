...
implements Angerable {
    final static private TrackedData<Boolean> BEGGING = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Integer> COLLAR_COLOR = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> ANGER_TIME = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<RegistryEntry<WolfVariant>> VARIANT = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.WOLF_VARIANT);
    final static private TrackedData<RegistryEntry<WolfSoundVariant>> SOUND_VARIANT = DataTracker.registerData(WolfEntity.class, TrackedDataHandlerRegistry.WOLF_SOUND_VARIANT);
    final static public TargetPredicate.EntityPredicate FOLLOW_TAMED_PREDICATE = (entity, world) -> {
        EntityType<?> entityType = entity.getType();
        return entityType == EntityType.SHEEP || entityType == EntityType.RABBIT || entityType == EntityType.FOX;
    };
    final static private float WILD_MAX_HEALTH = 8.0f;
    final static private float TAMED_MAX_HEALTH = 40.0f;
    final static private float field_49237 = 0.125f;
    final static public float field_52477 = 0.62831855f;
    final static private DyeColor DEFAULT_COLLAR_COLOR = DyeColor.RED;
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        RegistryWrapper.Impl registry = this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.WOLF_SOUND_VARIANT);
    ...
    protected void readCustomData(ReadView view) {
        ...
        view.read("sound_variant", RegistryKey.createCodec(RegistryKeys.WOLF_SOUND_VARIANT)).flatMap(soundVariantKey -> this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.WOLF_SOUND_VARIANT).getOptional((RegistryKey)soundVariantKey)).ifPresent(this::setSoundVariant);
    }
    ...
    public void tickMovement() {
        super.tickMovement();
        if (!this.net_minecraft_world_World_getWorld().isClient && this.furWet && !this.canShakeWaterOff && !this.isNavigating() && this.isOnGround()) {
            this.canShakeWaterOff = true;
            this.shakeProgress = 0.0f;
            this.lastShakeProgress = 0.0f;
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.SHAKE_OFF_WATER);
        }
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            this.tickAngerLogic((ServerWorld)this.net_minecraft_world_World_getWorld(), true);
        }
    }
    ...
    public void tick() {
        ...
        if (this.isTouchingWaterOrRain()) {
            this.furWet = true;
            if (this.canShakeWaterOff && !this.net_minecraft_world_World_getWorld().isClient) {
                this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.RESET_WOLF_SHAKE);
                this.resetShake();
            }
            ...
            if (this.shakeProgress > 0.4f) {
                ...
                for (int j = 0; j < i; ++j) {
                    ...
                    this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.SPLASH, this.getX() + (double)g, f + 0.8f, this.getZ() + (double)h, vec3d.x, vec3d.y, vec3d.z);
                }
            }
        }
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient || !itemStack.isOf(Items.BONE) || this.hasAngerTime()) return super.interactMob(player, hand);
        itemStack.decrementUnlessCreative(1, player);
        this.tryTame(player);
    ...
    private void tryTame(PlayerEntity player) {
        if (this.random.nextInt(3) == 0) {
            ...
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.ADD_NEGATIVE_PLAYER_REACTION_PARTICLES);
        }
    }
    ...
    @Override
    @Nullable
    public WolfEntity net_minecraft_entity_passive_WolfEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    public static boolean canSpawn(EntityType<WolfEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isIn(BlockTags.WOLVES_SPAWNABLE_ON) && WolfEntity.isLightLevelValidForNaturalSpawn(world, pos);
    }

    @Override
    @Nullable
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_WolfEntity_createChild(world, entity);
    }
    ...
    extends PassiveEntity.PassiveData {
        final public RegistryEntry<WolfVariant> variant;
