...
extends FishEntity {
    ...
    public void tick() {
        List<?> list;
        super.tick();
        if (this.hasOtherFishInGroup() && this.net_minecraft_world_World_getWorld().random.nextInt(200) == 1 && (list = this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(this.getClass(), this.getBoundingBox().expand(8.0, 8.0, 8.0))).size() <= 1) {
            this.groupSize = 1;
        }
    }
    ...
    implements EntityData {
        final public SchoolingFishEntity leader;
