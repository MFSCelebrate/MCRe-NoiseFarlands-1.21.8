...
extends FishEntity {
    final static private int DEFAULT_TADPOLE_AGE = 0;
    @VisibleForTesting
    static public int MAX_TADPOLE_AGE = Math.abs(-24000);
    final static public float WIDTH = 0.4f;
    final static public float HEIGHT = 0.3f;
    private int tadpoleAge = 0;
    final static protected ImmutableList<SensorType<? extends Sensor<? super TadpoleEntity>>> SENSORS = ImmutableList.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.NEAREST_PLAYERS, SensorType.HURT_BY, SensorType.FROG_TEMPTATIONS);
    final static protected ImmutableList<MemoryModuleType<?>> MEMORY_MODULES = ImmutableList.of(MemoryModuleType.LOOK_TARGET, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.WALK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.NEAREST_VISIBLE_ADULT, MemoryModuleType.TEMPTATION_COOLDOWN_TICKS, MemoryModuleType.IS_TEMPTED, MemoryModuleType.TEMPTING_PLAYER, MemoryModuleType.BREED_TARGET, MemoryModuleType.IS_PANICKING);
    ...
    private void growUp() {
        ...
        if (world instanceof ServerWorld) {
            ...
            this.convertTo(EntityType.FROG, EntityConversionContext.create(this, false, false), frog -> {
                frog.initialize(serverWorld, this.net_minecraft_world_World_getWorld().getLocalDifficulty(frog.getBlockPos()), SpawnReason.CONVERSION, null);
                frog.setPersistent();
