...
implements Angerable {
    final static private TrackedData<Boolean> WARNING = DataTracker.registerData(PolarBearEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private float field_30352 = 6.0f;
    ...
    public static boolean canSpawn(EntityType<PolarBearEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        ...
        if (registryEntry.isIn(BiomeTags.POLAR_BEARS_SPAWN_ON_ALTERNATE_BLOCKS)) {
            return PolarBearEntity.isLightLevelValidForNaturalSpawn(world, pos) && world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isIn(BlockTags.POLAR_BEARS_SPAWNABLE_ON_ALTERNATE);
        }
    ...
    extends ActiveTargetGoal<PlayerEntity> {
        ...
        public boolean canStart() {
            ...
            if (super.canStart()) {
                List<PolarBearEntity> list = PolarBearEntity.this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(PolarBearEntity.class, PolarBearEntity.this.getBoundingBox().expand(8.0, 4.0, 8.0));
