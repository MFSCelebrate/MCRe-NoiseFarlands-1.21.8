...
extends AnimalEntity {
    final static private TrackedData<Integer> ASK_FOR_BAMBOO_TICKS = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> SNEEZE_PROGRESS = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> EATING_TICKS = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Byte> MAIN_GENE = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static private TrackedData<Byte> HIDDEN_GENE = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static private TrackedData<Byte> PANDA_FLAGS = DataTracker.registerData(PandaEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static TargetPredicate ASK_FOR_BAMBOO_TARGET = TargetPredicate.createNonAttackable().setBaseMaxDistance(8.0);
    final static private EntityDimensions BABY_BASE_DIMENSIONS = EntityType.PANDA.getDimensions().scaled(0.5f).withAttachments(EntityAttachments.builder().add(EntityAttachmentType.PASSENGER, 0.0f, 0.40625f, 0.0f));
    final static private int SNEEZING_FLAG = 2;
    final static private int PLAYING_FLAG = 4;
    final static private int SITTING_FLAG = 8;
    final static private int LYING_ON_BACK_FLAG = 16;
    final static private int EATING_ANIMATION_INTERVAL = 5;
    final static public int MAIN_GENE_MUTATION_CHANCE = 32;
    final static private int HIDDEN_GENE_MUTATION_CHANCE = 32;
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(MAIN_GENE, 0);
        builder.add(HIDDEN_GENE, 0);
        builder.add(PANDA_FLAGS, 0);
    ...
    public void tick() {
        ...
        super.tick();
        if (this.isWorried()) {
            if (this.net_minecraft_world_World_getWorld().isThundering() && !this.isTouchingWater()) {
    ...
    public boolean isScaredByThunderstorm() {
        return this.isWorried() && this.net_minecraft_world_World_getWorld().isThundering();
    }
    ...
    private void updateEatingAnimation() {
        ...
        if (this.isEating()) {
            ...
            if (!this.net_minecraft_world_World_getWorld().isClient && this.getEatingTicks() > 80 && this.random.nextInt(20) == 1) {
    ...
    private void playEatingAnimation() {
        if (this.getEatingTicks() % 5 == 0) {
            ...
            for (int i = 0; i < 6; ++i) {
                ...
                this.net_minecraft_world_World_getWorld().addParticleClient(new ItemStackParticleEffect(ParticleTypes.ITEM, this.getEquippedStack(EquipmentSlot.MAINHAND)), vec3d2.x, vec3d2.y, vec3d2.z, vec3d.x, vec3d.y + 0.05, vec3d.z);
            }
        }
    }
    ...
    private void sneeze() {
        ...
        if (world2 instanceof ServerWorld && (serverWorld = (ServerWorld)world2).getGameRules().getBoolean(GameRules.DO_MOB_LOOT)) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        } else if (!this.net_minecraft_world_World_getWorld().isClient && this.getBreedingAge() == 0 && this.canEat()) {
            this.eat(player, hand, itemStack);
    ...
    extends MoveControl {
        final private PandaEntity panda;
    ...
    implements StringIdentifiable {
        final static public Gene NORMAL = new Gene(0, "normal", false);
        final static public Gene LAZY = new Gene(1, "lazy", false);
        final static public Gene WORRIED = new Gene(2, "worried", false);
        final static public Gene PLAYFUL = new Gene(3, "playful", false);
        final static public Gene BROWN = new Gene(4, "brown", true);
        final static public Gene WEAK = new Gene(5, "weak", true);
        final static public Gene AGGRESSIVE = new Gene(6, "aggressive", false);
        final static public Codec<Gene> CODEC;
        final static private IntFunction<Gene> BY_ID;
        final static private int field_30350 = 6;
        final private int id;
        final private String name;
        final private boolean recessive;
        final static private Gene[] field_6796;
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            return !PandaEntity.this.net_minecraft_world_World_getWorld().getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(6.0, 6.0, 6.0), PandaEntity::canEatFromGround).isEmpty();
        }
        ...
        public void start() {
            if (PandaEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
                List<ItemEntity> list = PandaEntity.this.net_minecraft_world_World_getWorld().getEntitiesByClass(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), PandaEntity::canEatFromGround);
                if (!list.isEmpty()) {
        ...
        public void stop() {
            ...
            if (!itemStack.isEmpty()) {
                PandaEntity.this.dropStack(PickUpFoodGoal.castToServerWorld(PandaEntity.this.net_minecraft_world_World_getWorld()), itemStack);
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (this.panda.net_minecraft_world_World_getWorld().getBlockState(this.panda.getBlockPos().net_minecraft_util_math_BlockPos_add(1, -1, j)).isAir()) {
                return true;
            }
