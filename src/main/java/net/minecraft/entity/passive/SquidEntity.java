...
extends WaterAnimalEntity {
    ...
    public void tickMovement() {
        ...
        if ((double)this.thrustTimer > Math.PI * 2) {
            if (this.net_minecraft_world_World_getWorld().isClient) {
                ...
                this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.RESET_SQUID_THRUST_TIMER);
            }
        }
    ...
    private void squirt() {
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            ((ServerWorld)this.net_minecraft_world_World_getWorld()).spawnParticles(this.getInkParticle(), vec3d.x, vec3d.y + 0.5, vec3d.z, 0, vec3d3.x, vec3d3.y, vec3d3.z, 0.1f);
        }
    }
    ...
    extends Goal {
        final private SquidEntity squid;
    ...
    extends Goal {
        final static private float field_30375 = 3.0f;
        final static private float field_30376 = 5.0f;
        final static private float field_30377 = 10.0f;
        private int timer;
        ...
        public void tick() {
            ...
            BlockState blockState = SquidEntity.this.net_minecraft_world_World_getWorld().getBlockState(BlockPos.ofFloored(SquidEntity.this.getX() + vec3d.x, SquidEntity.this.getY() + vec3d.y, SquidEntity.this.getZ() + vec3d.z));
            FluidState fluidState = SquidEntity.this.net_minecraft_world_World_getWorld().getFluidState(BlockPos.ofFloored(SquidEntity.this.getX() + vec3d.x, SquidEntity.this.getY() + vec3d.y, SquidEntity.this.getZ() + vec3d.z));
            ...
            if (this.timer % 10 == 5) {
                SquidEntity.this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, SquidEntity.this.getX(), SquidEntity.this.getY(), SquidEntity.this.getZ(), 0.0, 0.0, 0.0);
            }
        }
    }
}
