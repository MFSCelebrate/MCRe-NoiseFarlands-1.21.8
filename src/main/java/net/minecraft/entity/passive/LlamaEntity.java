...
implements RangedAttackMob {
    final static private int MAX_STRENGTH = 5;
    final static private TrackedData<Integer> STRENGTH = DataTracker.registerData(LlamaEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> VARIANT = DataTracker.registerData(LlamaEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private EntityDimensions BABY_BASE_DIMENSIONS = EntityType.LLAMA.getDimensions().withAttachments(EntityAttachments.builder().add(EntityAttachmentType.PASSENGER, 0.0f, EntityType.LLAMA.getHeight() - 0.8125f, -0.3f)).scaled(0.5f);
    boolean spit;
    @Nullable
    ...
    protected boolean receiveFood(PlayerEntity player, ItemStack item) {
        ...
        if (bl && !this.isSilent() && (soundEvent = this.getEatSound()) != null) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), this.getEatSound(), this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
        }
        return bl;
    }
    ...
    @Override
    @Nullable
    public LlamaEntity net_minecraft_entity_passive_LlamaEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    protected LlamaEntity createChild() {
        return EntityType.LLAMA.create(this.net_minecraft_world_World_getWorld(), SpawnReason.BREEDING);
    }

    private void spitAt(LivingEntity target) {
        LlamaSpitEntity llamaSpitEntity = new LlamaSpitEntity(this.net_minecraft_world_World_getWorld(), this);
        ...
        if (!this.isSilent()) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_LLAMA_SPIT, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
        }
        this.spit = true;
    }
    ...
    public PassiveEntity net_minecraft_entity_passive_LlamaEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_LlamaEntity_createChild(world, entity);
    }

    public static final class Variant
    extends Enum<Variant>
    implements StringIdentifiable {
        final static public Variant CREAMY = new Variant(0, "creamy");
        final static public Variant WHITE = new Variant(1, "white");
        final static public Variant BROWN = new Variant(2, "brown");
        final static public Variant GRAY = new Variant(3, "gray");
        ...
        final static private Variant[] field_41594;
