...
public class FrogBrain {
    ...
    final static private float field_37471 = 1.0f;
    final static private float field_37472 = 1.0f;
    final static private float field_37473 = 0.75f;
    final static private UniformIntProvider LONG_JUMP_COOLDOWN_RANGE = UniformIntProvider.create(100, 140);
    final static private int field_37475 = 2;
    final static private int field_37476 = 4;
    final static private float field_49092 = 3.5714288f;
    ...
    private static <E extends MobEntity> boolean shouldJumpTo(E frog, BlockPos pos) {
        World world = frog.net_minecraft_world_World_getWorld();
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_down();
        if (!(world.getFluidState(pos).isEmpty() && world.getFluidState(blockPos).isEmpty() && world.getFluidState(pos.net_minecraft_util_math_BlockPos_up()).isEmpty())) {
            return false;
        }
        ...
        PathContext pathContext = new PathContext(frog.net_minecraft_world_World_getWorld(), frog);
