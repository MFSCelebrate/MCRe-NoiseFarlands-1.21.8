...
extends WaterAnimalEntity {
    final static private TrackedData<Boolean> HAS_FISH = DataTracker.registerData(DolphinEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Integer> MOISTNESS = DataTracker.registerData(DolphinEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static TargetPredicate CLOSE_PLAYER_PREDICATE = TargetPredicate.createNonAttackable().setBaseMaxDistance(10.0).ignoreVisibility();
    final static public int MAX_AIR = 4800;
    final static private int MAX_MOISTNESS = 2400;
    final static public Predicate<ItemEntity> CAN_TAKE = item -> !item.cannotPickup() && item.isAlive() && item.isTouchingWater();
    final static public float BABY_SCALE_FACTOR = 0.65f;
    final static private boolean DEFAULT_HAS_FISH = false;
    @Nullable
    ...
    @Override
    @Nullable
    public DolphinEntity net_minecraft_entity_passive_DolphinEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    public void tick() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient && this.isTouchingWater() && this.getVelocity().lengthSquared() > 0.03) {
            ...
            for (int i = 0; i < 2; ++i) {
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * (double)h + (double)f, this.getY() - vec3d.y, this.getZ() - vec3d.z * (double)h + (double)g, 0.0, 0.0, 0.0);
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.DOLPHIN, this.getX() - vec3d.x * (double)h - (double)f, this.getY() - vec3d.y, this.getZ() - vec3d.z * (double)h - (double)g, 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    private void spawnParticlesAround(ParticleEffect parameters) {
        for (int i = 0; i < 7; ++i) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(parameters, this.getParticleX(1.0), this.getRandomBodyY() + 0.2, this.getParticleZ(1.0), d, e, f);
        }
    }
    ...
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_DolphinEntity_createChild(world, entity);
    }

    static class LeadToNearbyTreasureGoal
    extends Goal {
        final private DolphinEntity dolphin;
        ...
        @Override
        public void start() {
            if (!(this.dolphin.net_minecraft_world_World_getWorld() instanceof ServerWorld)) {
                return;
            }
            ServerWorld serverWorld = (ServerWorld)this.dolphin.net_minecraft_world_World_getWorld();
        ...
        public void stop() {
            BlockPos blockPos = this.dolphin.treasurePos;
            if (blockPos == null || BlockPos.ofFloored(blockPos.getX(), this.dolphin.getY(), blockPos.getZ()).isWithinDistance(this.dolphin.getPos(), 4.0) || this.noPathToStructure) {
        ...
        public void tick() {
            ...
            World world = this.dolphin.net_minecraft_world_World_getWorld();
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            if (this.closestPlayer.isSwimming() && this.closestPlayer.net_minecraft_world_World_getWorld().random.nextInt(6) == 0) {
                this.closestPlayer.addStatusEffect(new StatusEffectInstance(StatusEffects.DOLPHINS_GRACE, 100), this.dolphin);
            }
        }
    }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            List<ItemEntity> list = DolphinEntity.this.net_minecraft_world_World_getWorld().getEntitiesByClass(ItemEntity.class, DolphinEntity.this.getBoundingBox().expand(8.0, 8.0, 8.0), CAN_TAKE);
        ...
        private void spitOutItem(ItemStack stack) {
            ...
            ItemEntity itemEntity = new ItemEntity(DolphinEntity.this.net_minecraft_world_World_getWorld(), DolphinEntity.this.getX(), d, DolphinEntity.this.getZ(), stack);
            ...
            DolphinEntity.this.net_minecraft_world_World_getWorld().spawnEntity(itemEntity);
        }
    }
}
