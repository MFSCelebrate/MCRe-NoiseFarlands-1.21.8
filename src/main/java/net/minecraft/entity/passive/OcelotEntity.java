...
extends AnimalEntity {
    ...
    final static private TrackedData<Boolean> TRUSTING = DataTracker.registerData(OcelotEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private boolean DEFAULT_TRUSTING = false;
    @Nullable
        ...
        if ((this.temptGoal == null || this.temptGoal.isActive()) && !this.isTrusting() && this.isBreedingItem(itemStack) && player.squaredDistanceTo(this) < 9.0) {
            ...
            if (!this.net_minecraft_world_World_getWorld().isClient) {
                if (this.random.nextInt(3) == 0) {
                    ...
                    this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.TAME_OCELOT_SUCCESS);
                } else {
                    this.showEmoteParticle(false);
                    this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.TAME_OCELOT_FAILED);
                }
            }
    ...
    @Override
    @Nullable
    public OcelotEntity net_minecraft_entity_passive_OcelotEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_OcelotEntity_createChild(world, entity);
    }

    static class OcelotTemptGoal
    extends TemptGoal {
        final private OcelotEntity ocelot;
