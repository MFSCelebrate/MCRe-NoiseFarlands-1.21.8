...
implements Flutterer {
    final static private TrackedData<Integer> VARIANT = DataTracker.registerData(ParrotEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private Predicate<MobEntity> CAN_IMITATE = new Predicate<MobEntity>(){
    ...
    final static Map<EntityType<?>, SoundEvent> MOB_SOUNDS = Util.make(Maps.newHashMap(), map -> {
    ...
    public void tickMovement() {
        if (this.songSource == null || !this.songSource.isWithinDistance(this.getPos(), 3.46) || !this.net_minecraft_world_World_getWorld().getBlockState(this.songSource).isOf(Blocks.JUKEBOX)) {
            this.songPlaying = false;
            this.songSource = null;
        }
        if (this.net_minecraft_world_World_getWorld().random.nextInt(400) == 0) {
            ParrotEntity.imitateNearbyMob(this.net_minecraft_world_World_getWorld(), this);
        }
        super.tickMovement();
        this.flapWings();
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (!this.isTamed() && itemStack.isIn(ItemTags.PARROT_FOOD)) {
            ...
            if (!this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PARROT_EAT, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
            }
    ...
    public static boolean canSpawn(EntityType<ParrotEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isIn(BlockTags.PARROTS_SPAWNABLE_ON) && ParrotEntity.isLightLevelValidForNaturalSpawn(world, pos);
    }
    ...
    public SoundEvent getAmbientSound() {
        return ParrotEntity.getRandomSound(this.net_minecraft_world_World_getWorld(), this.net_minecraft_world_World_getWorld().random);
    }
    ...
    implements StringIdentifiable {
        final static public Variant RED_BLUE = new Variant(0, "red_blue");
        final static public Variant BLUE = new Variant(1, "blue");
        final static public Variant GREEN = new Variant(2, "green");
        final static public Variant YELLOW_BLUE = new Variant(3, "yellow_blue");
        final static public Variant GRAY = new Variant(4, "gray");
        ...
        final static private Variant[] field_41559;
    ...
    extends FlyGoal {
        ...
        private Vec3d locateTree() {
            ...
            for (BlockPos blockPos2 : iterable) {
                ...
                boolean bl;
                if (blockPos.equals(blockPos2) || !(bl = (blockState = this.mob.net_minecraft_world_World_getWorld().getBlockState(mutable2.set((Vec3i)blockPos2, Direction.DOWN))).getBlock() instanceof LeavesBlock || blockState.isIn(BlockTags.LOGS)) || !this.mob.net_minecraft_world_World_getWorld().isAir(blockPos2) || !this.mob.net_minecraft_world_World_getWorld().isAir(mutable.set((Vec3i)blockPos2, Direction.UP))) continue;
