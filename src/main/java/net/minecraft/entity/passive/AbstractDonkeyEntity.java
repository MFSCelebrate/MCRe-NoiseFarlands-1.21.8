...
extends AbstractHorseEntity {
    final static private TrackedData<Boolean> CHEST = DataTracker.registerData(AbstractDonkeyEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private boolean DEFAULT_HAS_CHEST = false;
    final private EntityDimensions babyBaseDimensions;
    ...
    protected void writeCustomData(WriteView view) {
        ...
        if (this.hasChest()) {
            ...
            for (int i = 0; 1 < this.items.size(); ++i) {
                ItemStack itemStack = this.items.getStack(1);
                if (itemStack.isEmpty()) continue;
                listAppender.add(new StackWithSlot(1, itemStack));
            }
        }
    }
