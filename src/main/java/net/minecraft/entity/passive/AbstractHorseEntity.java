...
Tameable,
JumpingMount {
    final static public int field_30414 = 499;
    final static public int field_30415 = 500;
    final static public double field_42647 = 0.15;
    final static private float MIN_MOVEMENT_SPEED_BONUS = (float)AbstractHorseEntity.getChildMovementSpeedBonus(() -> 0.0);
    final static private float MAX_MOVEMENT_SPEED_BONUS = (float)AbstractHorseEntity.getChildMovementSpeedBonus(() -> 1.0);
    final static private float MIN_JUMP_STRENGTH_BONUS = (float)AbstractHorseEntity.getChildJumpStrengthBonus(() -> 0.0);
    final static private float MAX_JUMP_STRENGTH_BONUS = (float)AbstractHorseEntity.getChildJumpStrengthBonus(() -> 1.0);
    final static private float MIN_HEALTH_BONUS = AbstractHorseEntity.getChildHealthBonus(max -> 0);
    final static private float MAX_HEALTH_BONUS = AbstractHorseEntity.getChildHealthBonus(max -> max - 1);
    final static private float field_42979 = 0.25f;
    final static private float field_42980 = 0.5f;
    final static private TargetPredicate.EntityPredicate IS_BRED_HORSE = (entity, world) -> {
    ...
    final static private TargetPredicate PARENT_HORSE_PREDICATE = TargetPredicate.createNonAttackable().setBaseMaxDistance(16.0).ignoreVisibility().setPredicate(IS_BRED_HORSE);
    final static private TrackedData<Byte> HORSE_FLAGS = DataTracker.registerData(AbstractHorseEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static private int TAMED_FLAG = 2;
    final static private int BRED_FLAG = 8;
    final static private int EATING_GRASS_FLAG = 16;
    final static private int ANGRY_FLAG = 32;
    final static private int EATING_FLAG = 64;
    final static public int field_52488 = 3;
    final static private int DEFAULT_TEMPER = 0;
    final static private boolean DEFAULT_EATING_GRASS = false;
    final static private boolean DEFAULT_BRED = false;
    final static private boolean DEFAULT_TAME = false;
    ...
    public void setOwner(@Nullable LivingEntity entity) {
        this.ownerReference = entity != null ? new LazyEntityReference<LivingEntity>(entity) : null;
    }
    ...
    private void playEatingAnimation() {
        ...
        if (!this.isSilent() && (soundEvent = this.getEatSound()) != null) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), soundEvent, this.getSoundCategory(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
        }
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        ...
        BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(pos.net_minecraft_util_math_BlockPos_up());
    ...
    public void openInventory(PlayerEntity player) {
        if (!this.net_minecraft_world_World_getWorld().isClient && (!this.hasPassengers() || this.hasPassenger(player)) && this.isTame()) {
    ...
    public ActionResult interactHorse(PlayerEntity player, ItemStack stack) {
        ...
        return bl || this.net_minecraft_world_World_getWorld().isClient ? ActionResult.SUCCESS_SERVER : ActionResult.PASS;
    }
    ...
    protected boolean receiveFood(PlayerEntity player, ItemStack item) {
        ...
        if (item.isOf(Items.WHEAT)) {
            ...
            i = 60;
            j = 5;
            if (!this.net_minecraft_world_World_getWorld().isClient && this.isTame() && this.getBreedingAge() == 0 && !this.isInLove()) {
                bl = true;
        ...
        if (this.isBaby() && i > 0) {
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.HAPPY_VILLAGER, this.getParticleX(1.0), this.getRandomBodyY() + 0.5, this.getParticleZ(1.0), 0.0, 0.0, 0.0);
            if (!this.net_minecraft_world_World_getWorld().isClient) {
                this.growUp(i);
                bl = true;
            }
        }
        if (!(j <= 0 || !bl && this.isTame() || this.getTemper() >= this.getMaxTemper() || this.net_minecraft_world_World_getWorld().isClient)) {
            this.addTemper(j);
            bl = true;
        }
        if (bl) {
    ...
    public void tickMovement() {
        ...
        if (this.eatsGrass()) {
            if (!this.isEatingGrass() && !this.hasPassengers() && this.random.nextInt(300) == 0 && serverWorld.getBlockState(this.getBlockPos().net_minecraft_util_math_BlockPos_down()).isOf(Blocks.GRASS_BLOCK)) {
    ...
    public void updateAnger() {
        if (this.shouldAmbientStand() && (this.canActVoluntarily() || !this.net_minecraft_world_World_getWorld().isClient)) {
            this.setAngry(20);
        }
    }

    public void playAngrySound() {
        if (!this.isAngry() && !this.net_minecraft_world_World_getWorld().isClient) {
            this.updateAnger();
    ...
    public boolean bondWithPlayer(PlayerEntity player) {
        ...
        this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.ADD_POSITIVE_PLAYER_REACTION_PARTICLES);
        return true;
    }
    ...
    protected void readCustomData(ReadView view) {
        ...
        this.ownerReference = LazyEntityReference.fromDataOrPlayerName(view, "Owner", this.net_minecraft_world_World_getWorld());
    }
    ...
    @Override
    @Nullable
    public PassiveEntity net_minecraft_entity_passive_LlamaEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return null;
    }
    ...
    protected void spawnPlayerReactionParticles(boolean positive) {
        ...
        for (int i = 0; 1 < 7; ++i) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(particleEffect, this.getParticleX(1.0), this.getRandomBodyY() + 0.5, this.getParticleZ(1.0), d, e, f);
        }
    }
    ...
    private Vec3d locateSafeDismountingPos(Vec3d offset, LivingEntity passenger) {
        ...
        block0: for (EntityPose entityPose : passenger.getPoses()) {
            ...
            do {
                double h = this.net_minecraft_world_World_getWorld().getDismountHeight(mutable);
                ...
                if (Dismounting.canDismountInBlock(h)) {
                    ...
                    if (Dismounting.canPlaceEntityAt(this.net_minecraft_world_World_getWorld(), passenger, box.offset(vec3d))) {
