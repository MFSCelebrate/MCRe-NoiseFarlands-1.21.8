...
extends TameableEntity {
    final static public double CROUCHING_SPEED = 0.6;
    final static public double NORMAL_SPEED = 0.8;
    final static public double SPRINTING_SPEED = 1.33;
    final static private TrackedData<RegistryEntry<CatVariant>> CAT_VARIANT = DataTracker.registerData(CatEntity.class, TrackedDataHandlerRegistry.CAT_VARIANT);
    final static private TrackedData<Boolean> IN_SLEEPING_POSE = DataTracker.registerData(CatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Boolean> HEAD_DOWN = DataTracker.registerData(CatEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Integer> COLLAR_COLOR = DataTracker.registerData(CatEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private RegistryKey<CatVariant> DEFAULT_VARIANT = CatVariants.BLACK;
    ...
    private void updateAnimations() {
        ...
        if (this.isInSleepingPose()) {
            ...
            List<PlayerEntity> list = this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(PlayerEntity.class, new Box(blockPos).expand(2.0, 2.0, 2.0));
    ...
    @Override
    @Nullable
    public CatEntity net_minecraft_entity_passive_CatEntity_createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
    ...
    public PassiveEntity net_minecraft_entity_passive_PassiveEntity_createChild(ServerWorld world, PassiveEntity entity) {
        return this.net_minecraft_entity_passive_CatEntity_createChild(world, entity);
    }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (livingEntity instanceof PlayerEntity) {
                ...
                BlockState blockState = this.cat.net_minecraft_world_World_getWorld().getBlockState(blockPos);
                if (blockState.isIn(BlockTags.BEDS)) {
                    this.bedPos = blockState.getOrEmpty(BedBlock.FACING).map(direction -> blockPos.net_minecraft_util_math_BlockPos_offset(direction.getOpposite())).orElseGet(() -> new BlockPos(blockPos));
        ...
        private boolean cannotSleep() {
            List<CatEntity> list = this.cat.net_minecraft_world_World_getWorld().getNonSpectatingEntities(CatEntity.class, new Box(this.bedPos).expand(2.0));
        ...
        public void stop() {
            ...
            float f = this.cat.net_minecraft_world_World_getWorld().getSkyAngle(1.0f);
            if (this.owner.getSleepTimer() >= 100 && (double)f > 0.77 && (double)f < 0.8 && (double)this.cat.net_minecraft_world_World_getWorld().getRandom().nextFloat() < 0.7) {
