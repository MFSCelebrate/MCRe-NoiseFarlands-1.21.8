...
extends AnimalEntity {
    final static public float field_47778 = 0.6f;
    final static public float field_48332 = 32.5f;
    final static public int field_47779 = 80;
    final static private double field_48333 = 7.0;
    final static private double field_48334 = 2.0;
    final static private TrackedData<State> STATE = DataTracker.registerData(ArmadilloEntity.class, TrackedDataHandlerRegistry.ARMADILLO_STATE);
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.PEEKING && this.net_minecraft_world_World_getWorld().isClient) {
            this.peeking = true;
            this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ARMADILLO_PEEK, this.getSoundCategory(), 1.0f, 1.0f, false);
        } else {
    ...
    public static boolean canSpawn(EntityType<ArmadilloEntity> entityType, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isIn(BlockTags.ARMADILLO_SPAWNABLE_ON) && ArmadilloEntity.isLightLevelValidForNaturalSpawn(world, pos);
    }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (itemStack.isOf(Items.BRUSH) && this.brushScute()) {
            itemStack.damage(16, (LivingEntity)player, ArmadilloEntity.getSlotForHand(hand));
    ...
    implements StringIdentifiable {
        final static public State IDLE = new State("idle", false, 0, 0){
        ...
        final static public State ROLLING = new State("rolling", true, 10, 1){
        ...
        final static public State SCARED = new State("scared", true, 50, 2){
        ...
        final static public State UNROLLING = new State("unrolling", true, 30, 3){
        ...
        final static Codec<State> CODEC;
        final static private IntFunction<State> INDEX_TO_VALUE;
        final static public PacketCodec<ByteBuf, State> PACKET_CODEC;
        final private String name;
        final private boolean runRollUpTask;
        final private int lengthInTicks;
        final private int index;
        final static private State[] field_47795;
