...
implements Comparable<StatusEffectInstance> {
    ...
    final static public int INFINITE = -1;
    final static public int MIN_AMPLIFIER = 0;
    final static public int MAX_AMPLIFIER = 255;
    final static public Codec<StatusEffectInstance> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)StatusEffect.ENTRY_CODEC.fieldOf("id").forGetter(StatusEffectInstance::getEffectType), (App)Parameters.CODEC.forGetter(StatusEffectInstance::asParameters)).apply((Applicative)instance, StatusEffectInstance::new));
    final static public PacketCodec<RegistryByteBuf, StatusEffectInstance> PACKET_CODEC = PacketCodec.tuple(StatusEffect.ENTRY_PACKET_CODEC, StatusEffectInstance::getEffectType, Parameters.PACKET_CODEC, StatusEffectInstance::asParameters, StatusEffectInstance::new);
    final private RegistryEntry<StatusEffect> type;
    ...
    final private Fading fading = new Fading();
    ...
    public boolean update(ServerWorld world, LivingEntity entity, Runnable hiddenEffectCallback) {
        ...
        if (this.type.value().canApplyUpdateEffect(1, this.amplifier) && !this.type.value().applyUpdateEffect(world, entity, this.amplifier)) {
            return false;
        }
        this.updateDuration();
