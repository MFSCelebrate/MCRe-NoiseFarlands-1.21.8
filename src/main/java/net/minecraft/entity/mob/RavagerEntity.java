...
extends RaiderEntity {
    final static private Predicate<Entity> CAN_KNOCK_BACK_WITH_ROAR = entity -> !(entity instanceof RavagerEntity) && entity.isAlive();
    final static private Predicate<Entity> CAN_KNOCK_BACK_WITH_ROAR_NO_MOB_GRIEFING = entity -> CAN_KNOCK_BACK_WITH_ROAR.test((Entity)entity) && !entity.getType().equals(EntityType.ARMOR_STAND);
    final static private Predicate<LivingEntity> CAN_KNOCK_BACK_WITH_ROAR_ON_CLIENT = entity -> !(entity instanceof RavagerEntity) && entity.isAlive() && entity.isLogicalSideForUpdatingMovement();
    final static private double field_30480 = 0.3;
    final static private double field_30481 = 0.35;
    final static private int field_30482 = 8356754;
    final static private float STUNNED_PARTICLE_BLUE = 0.57254905f;
    final static private float STUNNED_PARTICLE_GREEN = 0.5137255f;
    final static private float STUNNED_PARTICLE_RED = 0.49803922f;
    final static public int field_30486 = 10;
    final static public int field_30479 = 40;
    final static private int DEFAULT_ATTACK_TICK = 0;
    final static private int DEFAULT_STUN_TICK = 0;
    final static private int DEFAULT_ROAR_TICK = 0;
    ...
    private void spawnStunnedParticles() {
        if (this.random.nextInt(6) == 0) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.49803922f, 0.5137255f, 0.57254905f), d, e, f, 0.0, 0.0, 0.0);
        }
    }
    ...
    protected void knockback(LivingEntity target) {
        if (this.roarTick == 0) {
            if (this.random.nextDouble() < 0.5) {
                ...
                this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.STUN_RAVAGER);
    ...
    private void roar() {
        World world;
        if (this.isAlive() && (world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld)world;
            Predicate<Entity> predicate = serverWorld.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING) ? CAN_KNOCK_BACK_WITH_ROAR : CAN_KNOCK_BACK_WITH_ROAR_NO_MOB_GRIEFING;
            List<Entity> list = this.net_minecraft_world_World_getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), predicate);
            ...
            serverWorld.sendEntityStatus(this, EntityStatuses.RAVAGER_ROAR);
        }
    }

    private void roarKnockBackOnClient() {
        List<LivingEntity> list = this.net_minecraft_world_World_getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(4.0), CAN_KNOCK_BACK_WITH_ROAR_ON_CLIENT);
        for (LivingEntity livingEntity : list) {
            this.knockBack(livingEntity);
        }
    }
    ...
    private void addRoarParticlesOnClient() {
        ...
        for (int i = 0; i < 40; ++i) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.POOF, vec3d.x, vec3d.y, vec3d.z, d, e, f);
        }
    }
