...
extends HostileEntity {
    final static private boolean DEFAULT_PATROL_LEADER = false;
    final static private boolean DEFAULT_PATROLLING = false;
    @Nullable
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        ...
        if (this.isPatrolLeader()) {
            this.equipStack(EquipmentSlot.HEAD, Raid.createOminousBanner(this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)));
    ...
    public void setRandomPatrolTarget() {
        this.patrolTarget = this.getBlockPos().net_minecraft_util_math_BlockPos_add(-500 + this.random.nextInt(1000), 0, -500 + this.random.nextInt(1000));
        this.patrolling = true;
    }
    ...
    extends Goal {
        final static private int field_30474 = 200;
        final private T entity;
        final private double leaderSpeed;
        final private double followSpeed;
        ...
        public boolean canStart() {
            boolean bl = ((Entity)this.entity).net_minecraft_world_World_getWorld().getTime() < this.nextPatrolSearchTime;
        ...
        public void tick() {
            ...
            if (entityNavigation.isIdle()) {
                ...
                if (((PatrolEntity)this.entity).isRaidCenterSet() && list.isEmpty()) {
                    ...
                    blockPos = ((Entity)this.entity).net_minecraft_world_World_getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, blockPos);
                    if (!entityNavigation.startMovingTo(blockPos.getX(), blockPos.getY(), blockPos.getZ(), bl ? this.followSpeed : this.leaderSpeed)) {
                        this.wander();
                        this.nextPatrolSearchTime = ((Entity)this.entity).net_minecraft_world_World_getWorld().getTime() + 200L;
                    } else if (bl) {
        ...
        private List<PatrolEntity> findPatrolTargets() {
            return ((Entity)this.entity).net_minecraft_world_World_getWorld().getEntitiesByClass(PatrolEntity.class, ((Entity)this.entity).getBoundingBox().expand(16.0), patrolEntity -> patrolEntity.hasNoRaid() && !patrolEntity.isPartOf((Entity)this.entity));
        }

        private boolean wander() {
            Random random = ((Entity)this.entity).getRandom();
            BlockPos blockPos = ((Entity)this.entity).net_minecraft_world_World_getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, ((Entity)this.entity).getBlockPos().net_minecraft_util_math_BlockPos_add(-8 + random.nextInt(16), 0, -8 + random.nextInt(16)));
