...
extends MobEntity
implements Monster {
    final static public float field_30475 = 7.448451f;
    final static public int WING_FLAP_TICKS = MathHelper.ceil(24.166098f);
    final static private TrackedData<Integer> SIZE = DataTracker.registerData(PhantomEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public void tick() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            ...
            if (f > 0.0f && g <= 0.0f) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_PHANTOM_FLAP, this.getSoundCategory(), 0.95f + this.random.nextFloat() * 0.05f, 0.95f + this.random.nextFloat() * 0.05f, false);
            }
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.MYCELIUM, this.getX() + 1.0, this.getY() + (double)k, this.getZ() + (double)j, 0.0, 0.0, 0.0);
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.MYCELIUM, this.getX() - 1.0, this.getY() + (double)k, this.getZ() - (double)j, 0.0, 0.0, 0.0);
        }
    }
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData) {
        this.circlingCenter = this.getBlockPos().net_minecraft_util_math_BlockPos_up(5);
    ...
    extends Enum<PhantomMovementType> {
        final static public PhantomMovementType CIRCLE = new PhantomMovementType();
        final static public PhantomMovementType SWOOP = new PhantomMovementType();
        final static private PhantomMovementType[] field_7316;
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            if (livingEntity != null) {
                return PhantomEntity.this.testTargetPredicate(StartAttackGoal.castToServerWorld(PhantomEntity.this.net_minecraft_world_World_getWorld()), livingEntity, TargetPredicate.DEFAULT);
            }
            return false;
        }
        ...
        public void stop() {
            if (PhantomEntity.this.circlingCenter != null) {
                PhantomEntity.this.circlingCenter = PhantomEntity.this.net_minecraft_world_World_getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING, PhantomEntity.this.circlingCenter).net_minecraft_util_math_BlockPos_up(10 + PhantomEntity.this.random.nextInt(20));
            }
        }
        ...
        private void startSwoop() {
            ...
            PhantomEntity.this.circlingCenter = PhantomEntity.this.getTarget().getBlockPos().net_minecraft_util_math_BlockPos_up(20 + PhantomEntity.this.random.nextInt(20));
            if (PhantomEntity.this.circlingCenter.getY() < PhantomEntity.this.net_minecraft_world_World_getWorld().getSeaLevel()) {
                PhantomEntity.this.circlingCenter = new BlockPos(PhantomEntity.this.circlingCenter.getX(), PhantomEntity.this.net_minecraft_world_World_getWorld().getSeaLevel() + 1, PhantomEntity.this.circlingCenter.getZ());
            }
        }
    }
    ...
    extends MovementGoal {
        final static private int CAT_CHECK_INTERVAL = 20;
        ...
        public boolean shouldContinue() {
            ...
            if (PhantomEntity.this.age > this.nextCatCheckAge) {
                ...
                List<Entity> list = PhantomEntity.this.net_minecraft_world_World_getWorld().getEntitiesByClass(CatEntity.class, PhantomEntity.this.getBoundingBox().expand(16.0), EntityPredicates.VALID_ENTITY);
        ...
        public void tick() {
            ...
            if (PhantomEntity.this.getBoundingBox().expand(0.2f).intersects(livingEntity.getBoundingBox())) {
                PhantomEntity.this.tryAttack(SwoopMovementGoal.castToServerWorld(PhantomEntity.this.net_minecraft_world_World_getWorld()), livingEntity);
                PhantomEntity.this.movementType = PhantomMovementType.CIRCLE;
                if (!PhantomEntity.this.isSilent()) {
                    PhantomEntity.this.net_minecraft_world_World_getWorld().syncWorldEvent(WorldEvents.PHANTOM_BITES, PhantomEntity.this.getBlockPos(), 0);
                }
    ...
    extends MovementGoal {
        ...
        public void tick() {
            ...
            if (PhantomEntity.this.targetPosition.y < PhantomEntity.this.getY() && !PhantomEntity.this.net_minecraft_world_World_getWorld().isAir(PhantomEntity.this.getBlockPos().net_minecraft_util_math_BlockPos_down(1))) {
                this.yOffset = Math.max(1.0f, this.yOffset);
                this.adjustDirection();
            }
            if (PhantomEntity.this.targetPosition.y > PhantomEntity.this.getY() && !PhantomEntity.this.net_minecraft_world_World_getWorld().isAir(PhantomEntity.this.getBlockPos().net_minecraft_util_math_BlockPos_up(1))) {
    ...
    extends Goal {
        final private TargetPredicate PLAYERS_IN_RANGE_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(64.0);
        ...
        public boolean canStart() {
            ...
            ServerWorld serverWorld = FindTargetGoal.castToServerWorld(PhantomEntity.this.net_minecraft_world_World_getWorld());
        ...
        public boolean shouldContinue() {
            LivingEntity livingEntity = PhantomEntity.this.getTarget();
            if (livingEntity != null) {
                return PhantomEntity.this.testTargetPredicate(FindTargetGoal.castToServerWorld(PhantomEntity.this.net_minecraft_world_World_getWorld()), livingEntity, TargetPredicate.DEFAULT);
            }
            return false;
        }
    }
