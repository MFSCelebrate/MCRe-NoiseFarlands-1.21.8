...
public interface Hoglin {
    final static public int field_30546 = 10;
    final static public float field_52501 = 0.2f;
    ...
    public static void knockback(LivingEntity attacker, LivingEntity target) {
        ...
        float i = attacker.net_minecraft_world_World_getWorld().random.nextInt(21) - 10;
        double j = f * (double)(attacker.net_minecraft_world_World_getWorld().random.nextFloat() * 0.5f + 0.2f);
        Vec3d vec3d = new Vec3d(g, 0.0, h).normalize().multiply(j).rotateY(i);
        double k = f * (double)attacker.net_minecraft_world_World_getWorld().random.nextFloat() * 0.5;
