...
extends HostileEntity {
    final static private TrackedData<Integer> FUSE_SPEED = DataTracker.registerData(CreeperEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Boolean> CHARGED = DataTracker.registerData(CreeperEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Boolean> IGNITED = DataTracker.registerData(CreeperEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private boolean DEFAULT_CHARGED = false;
    final static private boolean DEFAULT_IGNITED = false;
    final static private short DEFAULT_FUSE = 30;
    final static private byte DEFAULT_EXPLOSION_RADIUS = 3;
    ...
    protected void readCustomData(ReadView view) {
        ...
        this.fuseTime = view.getShort("Fuse", 30);
        this.explosionRadius = view.getByte("ExplosionRadius", 3);
    ...
    protected ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (itemStack.isIn(ItemTags.CREEPER_IGNITERS)) {
            ...
            this.net_minecraft_world_World_getWorld().playSound((Entity)player, this.getX(), this.getY(), this.getZ(), soundEvent, this.getSoundCategory(), 1.0f, this.random.nextFloat() * 0.4f + 0.8f);
            if (!this.net_minecraft_world_World_getWorld().isClient) {
                this.ignite();
                if (!itemStack.isDamageable()) {
                    itemStack.decrement(1);
                } else {
                    itemStack.damage(1, (LivingEntity)player, CreeperEntity.getSlotForHand(hand));
                }
            }
    ...
    private void spawnEffectsCloud() {
        ...
        if (!collection.isEmpty()) {
            AreaEffectCloudEntity areaEffectCloudEntity = new AreaEffectCloudEntity(this.net_minecraft_world_World_getWorld(), this.getX(), this.getY(), this.getZ());
            ...
            this.net_minecraft_world_World_getWorld().spawnEntity(areaEffectCloudEntity);
        }
    }
