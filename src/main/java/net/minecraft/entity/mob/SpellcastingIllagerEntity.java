...
extends IllagerEntity {
    final static private TrackedData<Byte> SPELL = DataTracker.registerData(SpellcastingIllagerEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static private int DEFAULT_SPELL_TICKS = 0;
    ...
    public void tick() {
        super.tick();
        if (this.net_minecraft_world_World_getWorld().isClient && this.isSpellcasting()) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h), this.getX() + (double)j * d, this.getY() + e, this.getZ() + (double)k * d, 0.0, 0.0, 0.0);
            this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h), this.getX() - (double)j * d, this.getY() + e, this.getZ() - (double)k * d, 0.0, 0.0, 0.0);
        }
    }
    ...
    extends Enum<Spell> {
        final static public Spell NONE = new Spell(0, 0.0, 0.0, 0.0);
        final static public Spell SUMMON_VEX = new Spell(1, 0.7, 0.7, 0.8);
        final static public Spell FANGS = new Spell(2, 0.4, 0.3, 0.35);
        final static public Spell WOLOLO = new Spell(3, 0.7, 0.5, 0.2);
        final static public Spell DISAPPEAR = new Spell(4, 0.3, 0.3, 0.8);
        final static public Spell BLINDNESS = new Spell(5, 0.1, 0.1, 0.2);
        final static private IntFunction<Spell> BY_ID;
        final int id;
        final double[] particleVelocity;
        final static private Spell[] field_7376;
