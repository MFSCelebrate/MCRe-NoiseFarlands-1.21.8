...
extends AbstractHorseEntity {
    final static private EntityDimensions BABY_BASE_DIMENSIONS = EntityType.ZOMBIE_HORSE.getDimensions().withAttachments(EntityAttachments.builder().add(EntityAttachmentType.PASSENGER, 0.0f, EntityType.ZOMBIE_HORSE.getHeight() - 0.03125f, 0.0f)).scaled(0.5f);
    ...
    public static DefaultAttributeContainer.Builder createZombieHorseAttributes() {
        return ZombieHorseEntity.createBaseHorseAttributes().add(EntityAttributes.MAX_HEALTH, 15.0).add(EntityAttributes.MOVEMENT_SPEED, 0.2f);
    }
    ...
    protected void initAttributes(Random random) {
        this.getAttributeInstance(EntityAttributes.JUMP_STRENGTH).setBaseValue(ZombieHorseEntity.getChildJumpStrengthBonus(random::nextDouble));
    }
