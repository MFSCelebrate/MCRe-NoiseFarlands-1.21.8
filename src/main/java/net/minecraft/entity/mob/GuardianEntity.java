...
extends HostileEntity {
    final static protected int WARMUP_TIME = 80;
    final static private TrackedData<Boolean> SPIKES_RETRACTED = DataTracker.registerData(GuardianEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Integer> BEAM_TARGET_ID = DataTracker.registerData(GuardianEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public LivingEntity getBeamTarget() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            ...
            Entity entity = this.net_minecraft_world_World_getWorld().getEntityById(this.dataTracker.get(BEAM_TARGET_ID));
            ...
            if (this.net_minecraft_world_World_getWorld().isClient) {
                ...
                if (!this.isTouchingWater()) {
                    ...
                    if (vec3d.y > 0.0 && this.flopping && !this.isSilent()) {
                        this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), this.getFlopSound(), this.getSoundCategory(), 1.0f, 1.0f, false);
                    }
                    this.flopping = vec3d.y < 0.0 && this.net_minecraft_world_World_getWorld().isTopSolid(this.getBlockPos().net_minecraft_util_math_BlockPos_down(), this);
                } else {
                ...
                if (this.areSpikesRetracted() && this.isTouchingWater()) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, this.getParticleX(0.5) - vec3d.x * 1.5, this.getRandomBodyY() - vec3d.y * 1.5, this.getParticleZ(0.5) - vec3d.z * 1.5, 0.0, 0.0, 0.0);
                    }
                }
                if (this.hasBeamTarget()) {
                    ...
                    if ((livingEntity = this.getBeamTarget()) != null) {
                        ...
                        while (j < h) {
                            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, this.getX() + e * (j += 1.8 - d + this.random.nextDouble() * (1.7 - d)), this.getEyeY() + f * j, this.getZ() + g * j, 0.0, 0.0, 0.0);
                        }
                    }
                }
            }
    ...
    public static boolean canSpawn(EntityType<? extends GuardianEntity> type, WorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return !(random.nextInt(20) != 0 && world.isSkyVisibleAllowingSea(pos) || world.getDifficulty() == Difficulty.PEACEFUL || !SpawnReason.isAnySpawner(spawnReason) && !world.getFluidState(pos).isIn(FluidTags.WATER) || !world.getFluidState(pos.net_minecraft_util_math_BlockPos_down()).isIn(FluidTags.WATER));
    }
    ...
    extends MoveControl {
        final private GuardianEntity guardian;
        ...
        public void tick() {
            ...
            if (this.beamTicks == 0) {
                ...
                if (!this.guardian.isSilent()) {
                    this.guardian.net_minecraft_world_World_getWorld().sendEntityStatus(this.guardian, EntityStatuses.PLAY_GUARDIAN_ATTACK_SOUND);
                }
            } else if (this.beamTicks >= this.guardian.getWarmupTime()) {
                float f = 1.0f;
                if (this.guardian.net_minecraft_world_World_getWorld().getDifficulty() == Difficulty.HARD) {
                    f += 2.0f;
                }
                if (this.elder) {
                    f += 2.0f;
    ...
    implements TargetPredicate.EntityPredicate {
        final private GuardianEntity owner;
