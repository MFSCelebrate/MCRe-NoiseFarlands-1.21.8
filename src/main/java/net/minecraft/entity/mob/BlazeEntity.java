...
extends HostileEntity {
    ...
    final static private TrackedData<Byte> BLAZE_FLAGS = DataTracker.registerData(BlazeEntity.class, TrackedDataHandlerRegistry.BYTE);
    ...
    public void tickMovement() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            if (this.random.nextInt(24) == 0 && !this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX() + 0.5, this.getY() + 0.5, this.getZ() + 0.5, SoundEvents.ENTITY_BLAZE_BURN, this.getSoundCategory(), 1.0f + this.random.nextFloat(), this.random.nextFloat() * 0.7f + 0.3f, false);
            }
            for (int i = 0; i < 2; ++i) {
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.LARGE_SMOKE, this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0, 0.0, 0.0);
            }
        }
        super.tickMovement();
    }
    ...
    extends Goal {
        final private BlazeEntity blaze;
                ...
                if (this.fireballCooldown <= 0) {
                    ...
                    if (this.fireballsFired > 1) {
                        ...
                        if (!this.blaze.isSilent()) {
                            this.blaze.net_minecraft_world_World_getWorld().syncWorldEvent(null, WorldEvents.BLAZE_SHOOTS, this.blaze.getBlockPos(), 0);
                        }
                        for (int i = 0; i < 1; ++i) {
                            Vec3d vec3d = new Vec3d(this.blaze.getRandom().nextTriangular(e, 2.297 * h), f, this.blaze.getRandom().nextTriangular(g, 2.297 * h));
                            SmallFireballEntity smallFireballEntity = new SmallFireballEntity(this.blaze.net_minecraft_world_World_getWorld(), this.blaze, vec3d.normalize());
                            smallFireballEntity.setPosition(smallFireballEntity.getX(), this.blaze.getBodyY(0.5) + 0.5, smallFireballEntity.getZ());
                            this.blaze.net_minecraft_world_World_getWorld().spawnEntity(smallFireballEntity);
                        }
                    }
                }
