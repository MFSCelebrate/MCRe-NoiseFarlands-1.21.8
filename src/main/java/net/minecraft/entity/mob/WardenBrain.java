...
public class WardenBrain {
    final static private float STROLL_SPEED = 0.5f;
    final static private float CELEBRATE_TIME = 0.7f;
    final static private float RANGED_APPROACH_SPEED = 1.2f;
    final static private int MELEE_ATTACK_INTERVAL = 18;
    final static private int DIG_DURATION = MathHelper.ceil(100.0f);
    final static public int EMERGE_DURATION = MathHelper.ceil(133.59999f);
    final static public int ROAR_DURATION = MathHelper.ceil(84.0f);
    final static private int SNIFF_DURATION = MathHelper.ceil(83.2f);
    final static public int DIG_COOLDOWN = 1200;
    final static private int field_38181 = 100;
    final static private List<SensorType<? extends Sensor<? super WardenEntity>>> SENSORS = List.of(SensorType.NEAREST_PLAYERS, SensorType.WARDEN_ENTITY_SENSOR);
    final static private List<MemoryModuleType<?>> MEMORY_MODULES = List.of(MemoryModuleType.MOBS, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_NEMESIS, MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.ATTACK_TARGET, MemoryModuleType.ATTACK_COOLING_DOWN, MemoryModuleType.NEAREST_ATTACKABLE, MemoryModuleType.ROAR_TARGET, MemoryModuleType.DISTURBANCE_LOCATION, MemoryModuleType.RECENT_PROJECTILE, MemoryModuleType.IS_SNIFFING, MemoryModuleType.IS_EMERGING, MemoryModuleType.ROAR_SOUND_DELAY, MemoryModuleType.DIG_COOLDOWN, MemoryModuleType.ROAR_SOUND_COOLDOWN, MemoryModuleType.SNIFF_COOLDOWN, MemoryModuleType.TOUCH_COOLDOWN, MemoryModuleType.VIBRATION_COOLDOWN, MemoryModuleType.SONIC_BOOM_COOLDOWN, MemoryModuleType.SONIC_BOOM_SOUND_COOLDOWN, MemoryModuleType.SONIC_BOOM_SOUND_DELAY);
    final static private Task<WardenEntity> RESET_DIG_COOLDOWN_TASK = TaskTriggerer.task(context -> context.group(context.queryMemoryOptional(MemoryModuleType.DIG_COOLDOWN)).apply((Applicative)context, digCooldown -> (world, entity, time) -> {
    ...
    public static void lookAtDisturbance(WardenEntity warden, BlockPos pos) {
        if (!warden.net_minecraft_world_World_getWorld().getWorldBorder().contains(pos) || warden.getPrimeSuspect().isPresent() || warden.getBrain().getOptionalRegisteredMemory(MemoryModuleType.ATTACK_TARGET).isPresent()) {
            return;
        }
        WardenBrain.resetDigCooldown(warden);
