...
extends MobEntity
implements Monster {
    final static private TrackedData<Integer> SLIME_SIZE = DataTracker.registerData(SlimeEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static public int MIN_SIZE = 1;
    final static public int MAX_SIZE = 127;
    final static public int field_50136 = 4;
    final static private boolean DEFAULT_ON_GROUND_LAST_TICK = false;
    ...
    public void setSize(int size, boolean heal) {
        ...
        this.dataTracker.set(SLIME_SIZE, 1);
        this.refreshPosition();
        this.calculateDimensions();
        this.getAttributeInstance(EntityAttributes.MAX_HEALTH).setBaseValue(1);
        this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(0.3f);
        this.getAttributeInstance(EntityAttributes.ATTACK_DAMAGE).setBaseValue(1);
        if (heal) {
    ...
    public void tick() {
        ...
        if (this.isOnGround() && !this.onGroundLastTick) {
            ...
            while (1.0f < f * 16.0f) {
                ...
                this.net_minecraft_world_World_getWorld().addParticleClient(this.getParticles(), this.getX() + (double)k, this.getY(), this.getZ() + (double)l, 0.0, 0.0, 0.0);
                ++i;
            }
    ...
    public void remove(Entity.RemovalReason reason) {
        int i = this.getSize();
        if (!this.net_minecraft_world_World_getWorld().isClient && i > 1 && this.isDead()) {
    ...
    extends MoveControl {
        ...
        final private SlimeEntity slime;
