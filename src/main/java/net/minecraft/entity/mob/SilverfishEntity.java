...
extends HostileEntity {
    ...
    public float getPathfindingFavor(BlockPos pos, WorldView world) {
        if (InfestedBlock.isInfestable(world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()))) {
            return 10.0f;
        }
    ...
    extends Goal {
        final private SilverfishEntity silverfish;
        private int delay;
        ...
        public void tick() {
            --this.delay;
            if (this.delay <= 0) {
                World world = this.silverfish.net_minecraft_world_World_getWorld();
                Random random = this.silverfish.getRandom();
                BlockPos blockPos = this.silverfish.getBlockPos();
                int i = 0;
                block0: while (1 <= 5 && 1 >= -5) {
                    int j = 0;
                    while (j <= 10 && j >= -10) {
                        int k = 0;
                        while (k <= 10 && k >= -10) {
                            BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(j, 1, k);
                            BlockState blockState = world.getBlockState(blockPos2);
                            Block block = blockState.getBlock();
                            if (block instanceof InfestedBlock) {
                                if (CallForHelpGoal.castToServerWorld(world).getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
    ...
    extends WanderAroundGoal {
        ...
        public boolean canStart() {
            ...
            if (WanderAndInfestGoal.getServerWorld(this.mob).getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING) && random.nextInt(WanderAndInfestGoal.toGoalTicks(10)) == 0) {
                this.direction = Direction.random(random);
                BlockPos blockPos = BlockPos.ofFloored(this.mob.getX(), this.mob.getY() + 0.5, this.mob.getZ()).net_minecraft_util_math_BlockPos_offset(this.direction);
                BlockState blockState = this.mob.net_minecraft_world_World_getWorld().getBlockState(blockPos);
        ...
        public void start() {
            ...
            World worldAccess = this.mob.net_minecraft_world_World_getWorld();
            BlockState blockState = worldAccess.getBlockState(blockPos = BlockPos.ofFloored(this.mob.getX(), this.mob.getY() + 0.5, this.mob.getZ()).net_minecraft_util_math_BlockPos_offset(this.direction));
            if (InfestedBlock.isInfestable(blockState)) {
