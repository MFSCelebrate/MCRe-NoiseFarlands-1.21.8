...
implements Monster,
Hoglin {
    final static private TrackedData<Boolean> BABY = DataTracker.registerData(HoglinEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private int MAX_HEALTH = 40;
    final static private float MOVEMENT_SPEED = 0.3f;
    final static private int ATTACK_KNOCKBACK = 1;
    final static private float KNOCKBACK_RESISTANCE = 0.6f;
    final static private int ATTACK_DAMAGE = 6;
    final static private float BABY_ATTACK_DAMAGE = 0.5f;
    final static private boolean DEFAULT_IS_IMMUNE_TO_ZOMBIFICATION = false;
    final static private int DEFAULT_TIME_IN_OVERWORLD = 0;
    final static private boolean DEFAULT_CANNOT_BE_HUNTED = false;
    final static public int CONVERSION_TIME = 300;
    private int movementCooldownTicks;
    private int timeInOverworld = 0;
    private boolean cannotBeHunted = false;
    final static protected ImmutableList<? extends SensorType<? extends Sensor<? super HoglinEntity>>> SENSOR_TYPES = ImmutableList.of(SensorType.NEAREST_LIVING_ENTITIES, SensorType.NEAREST_PLAYERS, SensorType.NEAREST_ADULT, SensorType.HOGLIN_SPECIFIC_SENSOR);
    final static protected ImmutableList<? extends MemoryModuleType<?>> MEMORY_MODULE_TYPES = ImmutableList.of(MemoryModuleType.BREED_TARGET, MemoryModuleType.MOBS, MemoryModuleType.VISIBLE_MOBS, MemoryModuleType.NEAREST_VISIBLE_PLAYER, MemoryModuleType.NEAREST_VISIBLE_TARGETABLE_PLAYER, MemoryModuleType.LOOK_TARGET, MemoryModuleType.WALK_TARGET, MemoryModuleType.CANT_REACH_WALK_TARGET_SINCE, MemoryModuleType.PATH, MemoryModuleType.ATTACK_TARGET, MemoryModuleType.ATTACK_COOLING_DOWN, MemoryModuleType.NEAREST_VISIBLE_ADULT_PIGLIN, (Object[])new MemoryModuleType[]{MemoryModuleType.AVOID_TARGET, MemoryModuleType.VISIBLE_ADULT_PIGLIN_COUNT, MemoryModuleType.VISIBLE_ADULT_HOGLIN_COUNT, MemoryModuleType.NEAREST_VISIBLE_ADULT_HOGLINS, MemoryModuleType.NEAREST_VISIBLE_ADULT, MemoryModuleType.NEAREST_REPELLENT, MemoryModuleType.PACIFIED, MemoryModuleType.IS_PANICKING});
    ...
    public float getPathfindingFavor(BlockPos pos, WorldView world) {
        ...
        if (world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).isOf(Blocks.CRIMSON_NYLIUM)) {
            return 10.0f;
        }
        return 0.0f;
    }
    ...
    public boolean canConvert() {
        return !this.net_minecraft_world_World_getWorld().getDimension().piglinSafe() && !this.isImmuneToZombification() && !this.isAiDisabled();
    }
