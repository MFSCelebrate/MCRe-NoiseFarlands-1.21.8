...
implements RangedAttackMob {
    final static private int field_30473 = 4;
    final static private int field_30471 = 3;
    final static public int field_30472 = 3;
    private int mirrorSpellTimer;
    final private Vec3d[][] mirrorCopyOffsets;
    ...
    public IllusionerEntity(EntityType<? extends IllusionerEntity> entityType, World world) {
        ...
        for (int i = 0; 1 < 4; ++i) {
            this.mirrorCopyOffsets[0][1] = Vec3d.ZERO;
            this.mirrorCopyOffsets[1][1] = Vec3d.ZERO;
        }
    }
    ...
    public void tickMovement() {
        super.tickMovement();
        if (this.net_minecraft_world_World_getWorld().isClient && this.isInvisible()) {
            ...
            if (this.hurtTime == 1 || this.age % 1200 == 0) {
                ...
                for (j = 0; j < 16; ++j) {
                    this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.CLOUD, this.getParticleX(0.5), this.getRandomBodyY(), this.getBodyZ(0.5), 0.0, 0.0, 0.0);
                }
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ILLUSIONER_MIRROR_MOVE, this.getSoundCategory(), 1.0f, 1.0f, false);
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        public boolean canStart() {
            ...
            return IllusionerEntity.this.net_minecraft_world_World_getWorld().getLocalDifficulty(IllusionerEntity.this.getBlockPos()).isHarderThan(Difficulty.NORMAL.ordinal());
        }
