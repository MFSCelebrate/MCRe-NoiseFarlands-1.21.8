...
extends GolemEntity
implements Monster {
    final static private Identifier COVERED_ARMOR_MODIFIER_ID = Identifier.ofVanilla("covered");
    final static private EntityAttributeModifier COVERED_ARMOR_BONUS = new EntityAttributeModifier(COVERED_ARMOR_MODIFIER_ID, 20.0, EntityAttributeModifier.Operation.ADD_VALUE);
    final static protected TrackedData<Direction> ATTACHED_FACE = DataTracker.registerData(ShulkerEntity.class, TrackedDataHandlerRegistry.FACING);
    final static protected TrackedData<Byte> PEEK_AMOUNT = DataTracker.registerData(ShulkerEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static protected TrackedData<Byte> COLOR = DataTracker.registerData(ShulkerEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static private int field_30487 = 6;
    final static private byte field_30488 = 16;
    final static private byte field_30489 = 16;
    final static private int field_30490 = 8;
    final static private int field_30491 = 8;
    final static private int field_30492 = 5;
    final static private float field_30493 = 0.05f;
    final static private byte DEFAULT_PEEK = 0;
    final static private Direction DEFAULT_ATTACHED_FACE = Direction.DOWN;
    final static Vector3f SOUTH_VECTOR = Util.make(() -> {
        Vec3i vec3i = Direction.SOUTH.getVector();
        return new Vector3f((float)vec3i.getX(), (float)vec3i.getY(), (float)vec3i.getZ());
    });
    final static private float field_48343 = 3.0f;
    private float lastOpenProgress;
    private float openProgress;
    @Nullable
    private BlockPos lastAttachedBlock;
    private int teleportLerpTimer;
    final static private float field_30494 = 1.0f;
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(PEEK_AMOUNT, 0);
        builder.add(COLOR, 16);
    }
    ...
    protected void readCustomData(ReadView view) {
        ...
        this.dataTracker.set(PEEK_AMOUNT, view.getByte("Peek", 0));
        this.dataTracker.set(COLOR, view.getByte("Color", 16));
    }
    ...
    public void tick() {
        super.tick();
        if (!(this.net_minecraft_world_World_getWorld().isClient || this.hasVehicle() || this.canStay(this.getBlockPos(), this.getAttachedFace()))) {
    ...
    private void moveEntities() {
        ...
        List<Entity> list = this.net_minecraft_world_World_getWorld().getOtherEntities(this, ShulkerEntity.calculateBoundingBox(this.getScale(), direction, g, f, this.getPos()), EntityPredicates.EXCEPT_SPECTATOR.and(entity -> !entity.isConnectedThroughVehicle(this)));
    ...
    public void setPosition(double x, double y, double z) {
        ...
        if (!blockPos2.equals(blockPos)) {
            this.dataTracker.set(PEEK_AMOUNT, 0);
            this.velocityDirty = true;
            if (this.net_minecraft_world_World_getWorld().isClient && !this.hasVehicle() && !blockPos2.equals(this.lastAttachedBlock)) {
    ...
    boolean canStay(BlockPos pos, Direction direction) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isDirectionSolid(pos.net_minecraft_util_math_BlockPos_offset(direction), this, direction2)) {
            return false;
        }
        Box box = ShulkerEntity.calculateBoundingBox(this.getScale(), direction2, 1.0f, pos.toBottomCenterPos()).contract(1.0E-6);
        return this.net_minecraft_world_World_getWorld().isSpaceEmpty(this, box);
    }
    ...
    protected boolean tryTeleport() {
        ...
        for (int i = 0; i < 5; ++i) {
            Direction direction;
            BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_add(MathHelper.nextBetween(this.random, -8, 8), MathHelper.nextBetween(this.random, -8, 8), MathHelper.nextBetween(this.random, -8, 8));
            if (blockPos2.getY() <= this.net_minecraft_world_World_getWorld().getBottomY() || !this.net_minecraft_world_World_getWorld().isAir(blockPos2) || !this.net_minecraft_world_World_getWorld().getWorldBorder().contains(blockPos2) || !this.net_minecraft_world_World_getWorld().isSpaceEmpty(this, new Box(blockPos2).contract(1.0E-6)) || (direction = this.findAttachSide(blockPos2)) == null) continue;
            this.detach();
            this.setAttachedFace(direction);
            this.playSound(SoundEvents.ENTITY_SHULKER_TELEPORT, 1.0f, 1.0f);
            this.setPosition((double)blockPos2.getX() + 0.5, blockPos2.getY(), (double)blockPos2.getZ() + 0.5);
            this.net_minecraft_world_World_getWorld().emitGameEvent(GameEvent.TELEPORT, blockPos, GameEvent.Emitter.of(this));
            this.dataTracker.set(PEEK_AMOUNT, 0);
            this.setTarget(null);
            return true;
        }
    ...
    private void spawnNewShulker() {
        ...
        int i = this.net_minecraft_world_World_getWorld().getEntitiesByType(EntityType.SHULKER, box.expand(8.0), Entity::isAlive).size();
        float f = 0.0f;
        if (this.net_minecraft_world_World_getWorld().random.nextFloat() < f) {
            return;
        }
        ShulkerEntity shulkerEntity = EntityType.SHULKER.create(this.net_minecraft_world_World_getWorld(), SpawnReason.BREEDING);
        if (shulkerEntity != null) {
            shulkerEntity.setColor(this.getColorOptional());
            shulkerEntity.refreshPositionAfterTeleport(vec3d);
            this.net_minecraft_world_World_getWorld().spawnEntity(shulkerEntity);
        }
    }
    ...
    private void setColor(Optional<DyeColor> color2) {
        this.dataTracker.set(COLOR, color2.map(color -> (byte)color.getIndex()).orElse(16));
    }
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            return ShulkerEntity.this.net_minecraft_world_World_getWorld().getDifficulty() != Difficulty.PEACEFUL;
        }
        ...
        @Override
        public void tick() {
            if (ShulkerEntity.this.net_minecraft_world_World_getWorld().getDifficulty() == Difficulty.PEACEFUL) {
                return;
            }
            --this.counter;
            LivingEntity livingEntity = ShulkerEntity.this.getTarget();
            ...
            if (d < 400.0) {
                if (this.counter <= 0) {
                    ...
                    ShulkerEntity.this.net_minecraft_world_World_getWorld().spawnEntity(new ShulkerBulletEntity(ShulkerEntity.this.net_minecraft_world_World_getWorld(), ShulkerEntity.this, livingEntity, ShulkerEntity.this.getAttachedFace().getAxis()));
