...
implements Angerable {
    final static private Identifier ATTACKING_SPEED_MODIFIER_ID = Identifier.ofVanilla("attacking");
    final static private EntityAttributeModifier ATTACKING_SPEED_BOOST = new EntityAttributeModifier(ATTACKING_SPEED_MODIFIER_ID, 0.15f, EntityAttributeModifier.Operation.ADD_VALUE);
    final static private int field_30462 = 400;
    final static private int field_30461 = 600;
    final static private TrackedData<Optional<BlockState>> CARRIED_BLOCK = DataTracker.registerData(EndermanEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_STATE);
    final static private TrackedData<Boolean> ANGRY = DataTracker.registerData(EndermanEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private TrackedData<Boolean> PROVOKED = DataTracker.registerData(EndermanEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    public void playAngrySound() {
        if (this.age >= this.lastAngrySoundAge + 400) {
            ...
            if (!this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getEyeY(), this.getZ(), SoundEvents.ENTITY_ENDERMAN_STARE, this.getSoundCategory(), 2.5f, 1.0f, false);
            }
        }
    }
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        if (ANGRY.equals(data) && this.isProvoked() && this.net_minecraft_world_World_getWorld().isClient) {
            this.playAngrySound();
        }
    ...
    public void tickMovement() {
        if (this.net_minecraft_world_World_getWorld().isClient) {
            for (int i = 0; i < 2; ++i) {
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.PORTAL, this.getParticleX(0.5), this.getRandomBodyY() - 0.25, this.getParticleZ(0.5), (this.random.nextDouble() - 0.5) * 2.0, -this.random.nextDouble(), (this.random.nextDouble() - 0.5) * 2.0);
            }
        }
        this.jumping = false;
        if (!this.net_minecraft_world_World_getWorld().isClient) {
    ...
    protected boolean teleportRandomly() {
        if (this.net_minecraft_world_World_getWorld().isClient() || !this.isAlive()) {
            return false;
        }
    ...
    private boolean teleportTo(double x, double y, double z) {
        ...
        while (mutable.getY() > this.net_minecraft_world_World_getWorld().getBottomY() && !this.net_minecraft_world_World_getWorld().getBlockState(mutable).blocksMovement()) {
        ...
        if (bl3) {
            this.net_minecraft_world_World_getWorld().emitGameEvent(GameEvent.TELEPORT, vec3d, GameEvent.Emitter.of(this));
            if (!this.isSilent()) {
                this.net_minecraft_world_World_getWorld().playSound(null, this.lastX, this.lastY, this.lastZ, SoundEvents.ENTITY_ENDERMAN_TELEPORT, this.getSoundCategory(), 1.0f, 1.0f);
    ...
    protected void dropEquipment(ServerWorld world, DamageSource source, boolean causedByPlayer) {
        ...
        if (blockState != null) {
            ...
            LootWorldContext.Builder builder = new LootWorldContext.Builder((ServerWorld)this.net_minecraft_world_World_getWorld()).add(LootContextParameters.ORIGIN, this.getPos()).add(LootContextParameters.TOOL, itemStack).addOptional(LootContextParameters.THIS_ENTITY, this);
    ...
    extends Goal {
        final private EndermanEntity enderman;
        @Nullable
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            World world = this.enderman.net_minecraft_world_World_getWorld();
            ...
            BlockPos blockPos = new BlockPos(1, j, k);
            ...
            if (this.canPlaceOn(world, blockPos, blockState3 = Block.postProcessState(blockState3, this.enderman.net_minecraft_world_World_getWorld(), blockPos), blockState, blockState2, blockPos2)) {
    ...
    extends Goal {
        ...
        public void tick() {
            ...
            Vec3d vec3d2 = new Vec3d(1.5, (double)j + 0.5, (double)k + 0.5);
    ...
    extends ActiveTargetGoal<PlayerEntity> {
        ...
        final private TargetPredicate staringPlayerPredicate;
        final private TargetPredicate validTargetPredicate = TargetPredicate.createAttackable().ignoreVisibility();
        final private TargetPredicate.EntityPredicate angerPredicate;
