...
extends SpellcastingIllagerEntity {
    ...
    protected boolean isInSameTeam(Entity other) {
        ...
        if (other instanceof VexEntity && (vexEntity = (VexEntity)other).net_minecraft_entity_mob_MobEntity_getOwner() != null) {
            return this.isInSameTeam(vexEntity.net_minecraft_entity_mob_MobEntity_getOwner());
        }
        return false;
    }
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        final private TargetPredicate closeVexPredicate;
        ...
        public boolean canStart() {
            ...
            int i = SummonVexGoal.castToServerWorld(EvokerEntity.this.net_minecraft_world_World_getWorld()).getTargets(VexEntity.class, this.closeVexPredicate, EvokerEntity.this, EvokerEntity.this.getBoundingBox().expand(16.0)).size();
        ...
        protected void castSpell() {
            ServerWorld serverWorld = (ServerWorld)EvokerEntity.this.net_minecraft_world_World_getWorld();
            Team team = EvokerEntity.this.getScoreboardTeam();
            for (int i = 0; 1 < 3; ++i) {
                BlockPos blockPos = EvokerEntity.this.getBlockPos().net_minecraft_util_math_BlockPos_add(-2 + EvokerEntity.this.random.nextInt(5), 1, -2 + EvokerEntity.this.random.nextInt(5));
                VexEntity vexEntity = EntityType.VEX.create(EvokerEntity.this.net_minecraft_world_World_getWorld(), SpawnReason.MOB_SUMMONED);
                if (vexEntity == null) continue;
                vexEntity.refreshPositionAndAngles(blockPos, 0.0f, 0.0f);
                vexEntity.initialize(serverWorld, EvokerEntity.this.net_minecraft_world_World_getWorld().getLocalDifficulty(blockPos), SpawnReason.MOB_SUMMONED, null);
                ...
                if (team != null) {
                    serverWorld.net_minecraft_scoreboard_ServerScoreboard_getScoreboard().addScoreHolderToTeam(vexEntity.getNameForScoreboard(), team);
                }
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        ...
        private void conjureFangs(double x, double z, double maxY, double y, float yaw, int warmup) {
            ...
            do {
                ...
                BlockState blockState = EvokerEntity.this.net_minecraft_world_World_getWorld().getBlockState(blockPos2);
                if (!blockState.isSideSolidFullSquare(EvokerEntity.this.net_minecraft_world_World_getWorld(), blockPos2, Direction.UP)) continue;
                if (!EvokerEntity.this.net_minecraft_world_World_getWorld().isAir(blockPos) && !(voxelShape = (blockState2 = EvokerEntity.this.net_minecraft_world_World_getWorld().getBlockState(blockPos)).getCollisionShape(EvokerEntity.this.net_minecraft_world_World_getWorld(), blockPos)).isEmpty()) {
                    d = voxelShape.getMax(Direction.Axis.Y);
                }
                bl = true;
                break;
            } while ((blockPos = blockPos.net_minecraft_util_math_BlockPos_down()).getY() >= MathHelper.floor(maxY) - 1);
            if (bl) {
                EvokerEntity.this.net_minecraft_world_World_getWorld().spawnEntity(new EvokerFangsEntity(EvokerEntity.this.net_minecraft_world_World_getWorld(), x, (double)blockPos.getY() + d, z, yaw, warmup, EvokerEntity.this));
                EvokerEntity.this.net_minecraft_world_World_getWorld().emitGameEvent(GameEvent.ENTITY_PLACE, new Vec3d(x, (double)blockPos.getY() + d, z), GameEvent.Emitter.of(EvokerEntity.this));
            }
        }
    ...
    extends SpellcastingIllagerEntity.CastSpellGoal {
        final private TargetPredicate convertibleSheepPredicate;
        ...
        public boolean canStart() {
            ...
            ServerWorld serverWorld = WololoGoal.castToServerWorld(EvokerEntity.this.net_minecraft_world_World_getWorld());
