...
extends LivingEntity {
    final static public int field_30443 = 5;
    final static private boolean field_30445 = true;
    final static public EulerAngle DEFAULT_HEAD_ROTATION = new EulerAngle(0.0f, 0.0f, 0.0f);
    final static public EulerAngle DEFAULT_BODY_ROTATION = new EulerAngle(0.0f, 0.0f, 0.0f);
    final static public EulerAngle DEFAULT_LEFT_ARM_ROTATION = new EulerAngle(-10.0f, 0.0f, -10.0f);
    final static public EulerAngle DEFAULT_RIGHT_ARM_ROTATION = new EulerAngle(-15.0f, 0.0f, 10.0f);
    final static public EulerAngle DEFAULT_LEFT_LEG_ROTATION = new EulerAngle(-1.0f, 0.0f, -1.0f);
    final static public EulerAngle DEFAULT_RIGHT_LEG_ROTATION = new EulerAngle(1.0f, 0.0f, 1.0f);
    final static private EntityDimensions MARKER_DIMENSIONS = EntityDimensions.fixed(0.0f, 0.0f);
    final static private EntityDimensions SMALL_DIMENSIONS = EntityType.ARMOR_STAND.getDimensions().scaled(0.5f).withEyeHeight(0.9875f);
    final static private double field_30447 = 0.1;
    final static private double field_30448 = 0.9;
    final static private double field_30449 = 0.4;
    final static private double field_30450 = 1.6;
    final static public int field_30446 = 8;
    final static public int field_30451 = 16;
    final static public int SMALL_FLAG = 1;
    final static public int SHOW_ARMS_FLAG = 4;
    final static public int HIDE_BASE_PLATE_FLAG = 8;
    final static public int MARKER_FLAG = 16;
    final static public TrackedData<Byte> ARMOR_STAND_FLAGS = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.BYTE);
    final static public TrackedData<EulerAngle> TRACKER_HEAD_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static public TrackedData<EulerAngle> TRACKER_BODY_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static public TrackedData<EulerAngle> TRACKER_LEFT_ARM_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static public TrackedData<EulerAngle> TRACKER_RIGHT_ARM_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static public TrackedData<EulerAngle> TRACKER_LEFT_LEG_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static public TrackedData<EulerAngle> TRACKER_RIGHT_LEG_ROTATION = DataTracker.registerData(ArmorStandEntity.class, TrackedDataHandlerRegistry.ROTATION);
    final static private Predicate<Entity> RIDEABLE_MINECART_PREDICATE = entity -> {
    ...
    final static private boolean field_57644 = false;
    final static private int DEFAULT_DISABLED_SLOTS = 0;
    final static private boolean field_57646 = false;
    final static private boolean field_57647 = false;
    final static private boolean field_57648 = false;
    final static private boolean field_57649 = false;
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(ARMOR_STAND_FLAGS, 0);
    ...
    protected void tickCramming() {
        List<Entity> list = this.net_minecraft_world_World_getWorld().getOtherEntities(this, this.getBoundingBox(), RIDEABLE_MINECART_PREDICATE);
    ...
    public ActionResult interactAt(PlayerEntity player, Vec3d hitPos, Hand hand) {
        ...
        if (player.net_minecraft_world_World_getWorld().isClient) {
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.HIT_ARMOR_STAND) {
            if (this.net_minecraft_world_World_getWorld().isClient) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ARMOR_STAND_HIT, this.getSoundCategory(), 0.3f, 1.0f, false);
                this.lastHitTime = this.net_minecraft_world_World_getWorld().getTime();
            }
        } else {
    ...
    private void spawnBreakParticles() {
        if (this.net_minecraft_world_World_getWorld() instanceof ServerWorld) {
            ((ServerWorld)this.net_minecraft_world_World_getWorld()).spawnParticles(new BlockStateParticleEffect(ParticleTypes.BLOCK, Blocks.OAK_PLANKS.getDefaultState()), this.getX(), this.getBodyY(0.6666666666666666), this.getZ(), 10, this.getWidth() / 4.0f, this.getHeight() / 4.0f, this.getWidth() / 4.0f, 0.05);
        }
    }
    ...
    private void breakAndDropItem(ServerWorld world, DamageSource damageSource) {
        ...
        Block.dropStack(this.net_minecraft_world_World_getWorld(), this.getBlockPos(), itemStack);
        this.onBreak(world, damageSource);
    }
    ...
    private void onBreak(ServerWorld world, DamageSource damageSource) {
        ...
        for (EquipmentSlot equipmentSlot : EquipmentSlot.VALUES) {
            ...
            Block.dropStack(this.net_minecraft_world_World_getWorld(), this.getBlockPos().net_minecraft_util_math_BlockPos_up(), itemStack);
        }
    }

    private void playBreakSound() {
        this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ARMOR_STAND_BREAK, this.getSoundCategory(), 1.0f, 1.0f);
    }
    ...
    public boolean handleAttack(Entity attacker) {
        ...
        if (this.net_minecraft_world_World_getWorld().canEntityModifyAt(playerEntity, this.getBlockPos())) return false;
        return true;
    }
    ...
    public Vec3d getClientCameraPosVec(float tickProgress) {
        if (this.isMarker()) {
            ...
            for (BlockPos blockPos2 : BlockPos.iterate(BlockPos.ofFloored(box.minX, box.minY, box.minZ), BlockPos.ofFloored(box.maxX, box.maxY, box.maxZ))) {
                int j = Math.max(this.net_minecraft_world_World_getWorld().getLightLevel(LightType.BLOCK, blockPos2), this.net_minecraft_world_World_getWorld().getLightLevel(LightType.SKY, blockPos2));
                if (j == 15) {
