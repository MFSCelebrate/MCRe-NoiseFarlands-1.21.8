...
extends Entity {
    final static private TrackedData<Optional<BlockPos>> BEAM_TARGET = DataTracker.registerData(EndCrystalEntity.class, TrackedDataHandlerRegistry.OPTIONAL_BLOCK_POS);
    final static private TrackedData<Boolean> SHOW_BOTTOM = DataTracker.registerData(EndCrystalEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private boolean DEFAULT_SHOW_BOTTOM = true;
    ...
    public void tick() {
        ...
        if (this.net_minecraft_world_World_getWorld() instanceof ServerWorld) {
            ...
            if (((ServerWorld)this.net_minecraft_world_World_getWorld()).getEnderDragonFight() != null && this.net_minecraft_world_World_getWorld().getBlockState(blockPos).isAir()) {
