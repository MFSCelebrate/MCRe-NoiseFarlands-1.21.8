...
extends AbstractDecorationEntity {
    final static private TrackedData<ItemStack> ITEM_STACK = DataTracker.registerData(ItemFrameEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    final static private TrackedData<Integer> ROTATION = DataTracker.registerData(ItemFrameEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static public int field_30454 = 8;
    final static private float field_51592 = 0.0625f;
    final static private float field_51593 = 0.75f;
    final static private float field_51594 = 0.75f;
    final static private byte DEFAULT_ITEM_ROTATION = 0;
    final static private float DEFAULT_ITEM_DROP_CHANCE = 1.0f;
    final static private boolean DEFAULT_INVISIBLE = false;
    final static private boolean DEFAULT_FIXED = false;
    ...
    public boolean canStayAttached() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isSpaceEmpty(this)) {
            return false;
        }
        BlockState blockState = this.net_minecraft_world_World_getWorld().getBlockState(this.attachedBlockPos.net_minecraft_util_math_BlockPos_offset(this.getHorizontalFacing().getOpposite()));
        if (!(blockState.isSolid() || this.getHorizontalFacing().getAxis().isHorizontal() && AbstractRedstoneGateBlock.isRedstoneGate(blockState))) {
            return false;
        }
        return this.net_minecraft_world_World_getWorld().getOtherEntities(this, this.getBoundingBox(), PREDICATE).isEmpty();
    }
    ...
    private void removeFromFrame(ItemStack stack) {
        ...
        if (mapIdComponent != null && (mapState = FilledMapItem.getMapState(mapIdComponent, this.net_minecraft_world_World_getWorld())) != null) {
    ...
    public void setHeldItemStack(ItemStack value, boolean update) {
        ...
        if (update && this.attachedBlockPos != null) {
            this.net_minecraft_world_World_getWorld().updateComparators(this.attachedBlockPos, Blocks.AIR);
        }
    }
    ...
    protected void readCustomData(ReadView view) {
        ...
        this.setRotation(view.getByte("ItemRotation", 0), false);
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        if (!bl) {
            if (bl2 && !this.isRemoved()) {
                MapState mapState = FilledMapItem.getMapState(itemStack, this.net_minecraft_world_World_getWorld());
