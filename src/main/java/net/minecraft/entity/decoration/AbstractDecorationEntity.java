...
extends BlockAttachedEntity {
    final static protected Predicate<Entity> PREDICATE = entity -> entity instanceof AbstractDecorationEntity;
    final static private TrackedData<Direction> FACING = DataTracker.registerData(AbstractDecorationEntity.class, TrackedDataHandlerRegistry.FACING);
    final static private Direction DEFAULT_FACING = Direction.SOUTH;
    ...
    public ItemEntity dropStack(ServerWorld world, ItemStack stack, float yOffset) {
        ItemEntity itemEntity = new ItemEntity(this.net_minecraft_world_World_getWorld(), this.getX() + (double)((float)this.getHorizontalFacing().getOffsetX() * 0.15f), this.getY() + (double)yOffset, this.getZ() + (double)((float)this.getHorizontalFacing().getOffsetZ() * 0.15f), stack);
