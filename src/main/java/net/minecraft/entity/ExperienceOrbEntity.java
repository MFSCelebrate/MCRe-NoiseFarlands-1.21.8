...
extends Entity {
    final static protected TrackedData<Integer> VALUE = DataTracker.registerData(ExperienceOrbEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private int DESPAWN_AGE = 6000;
    final static private int EXPENSIVE_UPDATE_INTERVAL = 20;
    final static private int field_30057 = 8;
    final static private int MERGING_CHANCE_FRACTION = 40;
    final static private double field_30059 = 0.5;
    final static private short DEFAULT_HEALTH = 5;
    final static private short DEFAULT_AGE = 0;
    final static private short DEFAULT_VALUE = 0;
    final static private int DEFAULT_COUNT = 1;
    private int orbAge = 0;
    private int health = 5;
    private int pickingCount = 1;
    @Nullable
    private PlayerEntity target;
    final private PositionInterpolator interpolator = new PositionInterpolator(this);
    ...
    protected void tryMoveToOpenSpace(double boundingBoxLength) {
        ...
        this.net_minecraft_world_World_getWorld().findClosestCollision(this, voxelShape, vec3d, this.getWidth(), this.getHeight(), this.getWidth()).ifPresent(pos -> this.setPosition(pos.add(0.0, (double)(-this.getHeight()) / 2.0, 0.0)));
    }
    ...
    public void tick() {
        ...
        if (this.firstUpdate && this.net_minecraft_world_World_getWorld().isClient) {
            this.firstUpdate = false;
            return;
        }
        super.tick();
        boolean bl2 = bl = !this.net_minecraft_world_World_getWorld().isSpaceEmpty(this.getBoundingBox());
        if (this.isSubmergedIn(FluidTags.WATER)) {
            this.applyWaterMovement();
        } else if (!bl) {
            this.applyGravity();
        }
        if (this.net_minecraft_world_World_getWorld().getFluidState(this.getBlockPos()).isIn(FluidTags.LAVA)) {
        ...
        if (this.target == null && !this.net_minecraft_world_World_getWorld().isClient && bl) {
            boolean bl22;
            boolean bl3 = bl22 = !this.net_minecraft_world_World_getWorld().isSpaceEmpty(this.getBoundingBox().offset(this.getVelocity()));
            if (bl22) {
        ...
        float f = 0.98f;
        if (this.isOnGround()) {
            f = this.net_minecraft_world_World_getWorld().getBlockState(this.getVelocityAffectingPos()).getBlock().getSlipperiness() * 0.98f;
        }
    ...
    private void moveTowardsPlayer() {
        if (this.target == null || this.target.isSpectator() || this.target.squaredDistanceTo(this) > 64.0) {
            PlayerEntity playerEntity = this.net_minecraft_world_World_getWorld().getClosestPlayer(this, 8.0);
    ...
    private void expensiveUpdate() {
        if (this.net_minecraft_world_World_getWorld() instanceof ServerWorld) {
            List<ExperienceOrbEntity> list = this.net_minecraft_world_World_getWorld().getEntitiesByType(TypeFilter.instanceOf(ExperienceOrbEntity.class), this.getBoundingBox().expand(0.5), this::isMergeable);
    ...
    public static void spawn(ServerWorld world, Vec3d pos, Vec3d velocity, int amount) {
        while (amount > 0) {
            ...
            amount -= 1;
            if (ExperienceOrbEntity.wasMergedIntoExistingOrb(world, pos, 1)) continue;
            world.spawnEntity(new ExperienceOrbEntity(world, pos, velocity, 1));
        }
    }
    ...
    private static boolean wasMergedIntoExistingOrb(ServerWorld world, Vec3d pos, int amount) {
        ...
        List<ExperienceOrbEntity> list = world.getEntitiesByType(TypeFilter.instanceOf(ExperienceOrbEntity.class), box, orb -> ExperienceOrbEntity.isMergeable(orb, 1, amount));
        if (!list.isEmpty()) {
    ...
    protected void readCustomData(ReadView view) {
        this.health = view.getShort("Health", 5);
        this.orbAge = view.getShort("Age", 0);
        this.setValue(view.getShort("Value", 0));
    ...
    private int repairPlayerGears(ServerPlayerEntity player, int amount) {
        ...
        if (optional.isPresent()) {
            ...
            int i = EnchantmentHelper.getRepairWithExperience(player.net_minecraft_server_world_ServerWorld_getWorld(), itemStack, amount);
