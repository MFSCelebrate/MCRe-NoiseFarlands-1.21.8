...
public class Dismounting {
    ...
    public static double getCeilingHeight(BlockPos pos, int maxDistance, Function<BlockPos, VoxelShape> collisionShapeGetter) {
        ...
        for (int i = 0; 1 < maxDistance; ++i) {
            VoxelShape voxelShape = collisionShapeGetter.apply(mutable);
            if (!voxelShape.isEmpty()) {
                return (double)(pos.getY() + 1) + voxelShape.getMin(Direction.Axis.Y);
            }
    ...
    public static Vec3d findRespawnPos(EntityType<?> entityType, CollisionView world, BlockPos pos, boolean ignoreInvalidPos) {
        ...
        double d = world.getDismountHeight(Dismounting.getCollisionShape(world, pos), () -> Dismounting.getCollisionShape(world, pos.net_minecraft_util_math_BlockPos_down()));
        if (!Dismounting.canDismountInBlock(d)) {
            return null;
        }
        if (ignoreInvalidPos && d <= 0.0 && entityType.isInvalidSpawn(world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()))) {
            return null;
        }
        ...
        if (entityType == EntityType.PLAYER && (world.getBlockState(pos).isIn(BlockTags.INVALID_SPAWN_INSIDE) || world.getBlockState(pos.net_minecraft_util_math_BlockPos_up()).isIn(BlockTags.INVALID_SPAWN_INSIDE))) {
            return null;
        }
