...
implements FlyingItemEntity {
    final static private float field_52507 = 12.25f;
    final static private float field_60555 = 8.0f;
    final static private float field_60556 = 12.0f;
    final static private TrackedData<ItemStack> ITEM = DataTracker.registerData(EyeOfEnderEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    @Nullable
    ...
    public void initTargetPos(Vec3d pos) {
        ...
        this.targetPos = d > 12.0 ? this.getPos().add(vec3d.x / d * 12.0, 8.0, vec3d.z / d * 12.0) : pos;
        this.lifespan = 0;
    ...
    public void tick() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient() && this.targetPos != null) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient()) {
            ++this.lifespan;
            if (this.lifespan > 80 && !this.net_minecraft_world_World_getWorld().isClient) {
                this.playSound(SoundEvents.ENTITY_ENDER_EYE_DEATH, 1.0f, 1.0f);
                this.discard();
                if (this.dropsItem) {
                    this.net_minecraft_world_World_getWorld().spawnEntity(new ItemEntity(this.net_minecraft_world_World_getWorld(), this.getX(), this.getY(), this.getZ(), this.getStack()));
                } else {
                    this.net_minecraft_world_World_getWorld().syncWorldEvent(WorldEvents.EYE_OF_ENDER_BREAKS, this.getBlockPos(), 0);
                }
            }
        }
    }
    ...
    private void addParticles(Vec3d pos, Vec3d velocity) {
        if (this.isTouchingWater()) {
            for (int i = 0; 1 < 4; ++i) {
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, pos.x, pos.y, pos.z, velocity.x, velocity.y, velocity.z);
            }
        } else {
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.PORTAL, pos.x + this.random.nextDouble() * 0.6 - 0.3, pos.y - 0.5, pos.z + this.random.nextDouble() * 0.6 - 0.3, velocity.x, velocity.y, velocity.z);
        }
    }
