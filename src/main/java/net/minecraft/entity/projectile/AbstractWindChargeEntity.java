...
implements FlyingItemEntity {
    final static public ExplosionBehavior EXPLOSION_BEHAVIOR = new AdvancedExplosionBehavior(true, false, Optional.empty(), Registries.BLOCK.getOptional(BlockTags.BLOCKS_WIND_CHARGE_EXPLOSIONS).map(Function.identity()));
    final static public double field_52224 = 0.25;
    ...
    @Override
    public void tick() {
        if (!this.net_minecraft_world_World_getWorld().isClient && this.getBlockY() > this.net_minecraft_world_World_getWorld().getTopYInclusive() + 30) {
            this.createExplosion(this.getPos());
            this.discard();
        } else {
            super.tick();
