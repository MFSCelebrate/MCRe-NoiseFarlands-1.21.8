...
extends ExplosiveProjectileEntity {
    final static public float DAMAGE_RANGE = 4.0f;
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            List<LivingEntity> list = this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(4.0, 2.0, 4.0));
            ...
            this.net_minecraft_world_World_getWorld().syncWorldEvent(WorldEvents.DRAGON_BREATH_CLOUD_SPAWNS, this.getBlockPos(), this.isSilent() ? -1 : 1);
