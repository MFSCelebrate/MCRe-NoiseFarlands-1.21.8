...
extends ProjectileEntity {
    final static public double DEFAULT_ACCELERATION_POWER = 0.1;
    final static public double field_51892 = 0.5;
    ...
    public void tick() {
        ...
        this.applyDrag();
        if (!this.net_minecraft_world_World_getWorld().isClient && (entity != null && entity.isRemoved() || !this.net_minecraft_world_World_getWorld().isChunkLoaded(this.getBlockPos()))) {
            this.discard();
            return;
        }
        HitResult hitResult = ProjectileUtil.getCollision((Entity)this, this::canHit, this.getRaycastShapeType());
        Vec3d vec3d = hitResult.getType() != HitResult.Type.MISS ? hitResult.getPos() : this.getPos().add(this.getVelocity());
    ...
    private void applyDrag() {
        ...
        if (this.isTouchingWater()) {
            for (int i = 0; i < 4; ++i) {
                float f = 0.25f;
                this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.BUBBLE, vec3d2.x - vec3d.x * 0.25, vec3d2.y - vec3d.y * 0.25, vec3d2.z - vec3d.z * 0.25, vec3d.x, vec3d.y, vec3d.z);
            }
    ...
    private void addParticles() {
        ...
        if (particleEffect != null) {
            this.net_minecraft_world_World_getWorld().addParticleClient(particleEffect, vec3d.x, vec3d.y + 0.5, vec3d.z, 0.0, 0.0, 0.0);
        }
    }
