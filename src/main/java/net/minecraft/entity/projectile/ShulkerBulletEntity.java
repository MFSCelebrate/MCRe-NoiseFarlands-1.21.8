...
extends ProjectileEntity {
    final static private double field_30666 = 0.15;
    @Nullable
    ...
    public ShulkerBulletEntity(World world, LivingEntity owner, Entity target, Direction.Axis axis) {
        ...
        this.target = new LazyEntityReference<Entity>(target);
    ...
    private void changeTargetDirection(@Nullable Direction.Axis axis, @Nullable Entity target) {
        ...
        double d = 0.5;
        if (target == null) {
            blockPos = this.getBlockPos().net_minecraft_util_math_BlockPos_down();
        } else {
        ...
        if (!blockPos.isWithinDistance(this.getPos(), 2.0)) {
            ...
            if (axis != Direction.Axis.X) {
                if (blockPos2.getX() < blockPos.getX() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_east())) {
                    list.add(Direction.EAST);
                } else if (blockPos2.getX() > blockPos.getX() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_west())) {
                    list.add(Direction.WEST);
                }
            }
            if (axis != Direction.Axis.Y) {
                if (blockPos2.getY() < blockPos.getY() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_up())) {
                    list.add(Direction.UP);
                } else if (blockPos2.getY() > blockPos.getY() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_down())) {
                    list.add(Direction.DOWN);
                }
            }
            if (axis != Direction.Axis.Z) {
                if (blockPos2.getZ() < blockPos.getZ() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_south())) {
                    list.add(Direction.SOUTH);
                } else if (blockPos2.getZ() > blockPos.getZ() && this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_north())) {
                    list.add(Direction.NORTH);
                }
            }
            direction = Direction.random(this.random);
            if (list.isEmpty()) {
                for (int i = 5; !this.net_minecraft_world_World_getWorld().isAir(blockPos2.net_minecraft_util_math_BlockPos_offset(direction)) && i > 0; --i) {
                    direction = Direction.random(this.random);
                }
            } else {
    ...
    public void checkDespawn() {
        if (this.net_minecraft_world_World_getWorld().getDifficulty() == Difficulty.PEACEFUL) {
            this.discard();
        }
    }
    ...
    public void tick() {
        Vec3d vec3d;
        super.tick();
        Entity entity = !this.net_minecraft_world_World_getWorld().isClient() ? LazyEntityReference.resolve(this.target, this.net_minecraft_world_World_getWorld(), Entity.class) : null;
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.END_ROD, this.getX() - vec3d.x, this.getY() - vec3d.y + 0.15, this.getZ() - vec3d.z, 0.0, 0.0, 0.0);
        } else if (entity != null) {
            if (this.stepCount > 0) {
                --this.stepCount;
                if (this.stepCount == 0) {
            ...
            if (this.direction != null) {
                ...
                if (this.net_minecraft_world_World_getWorld().isTopSolid(blockPos.net_minecraft_util_math_BlockPos_offset(this.direction), this)) {
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
        ...
        ((ServerWorld)this.net_minecraft_world_World_getWorld()).spawnParticles(ParticleTypes.EXPLOSION, this.getX(), this.getY(), this.getZ(), 2, 0.2, 0.2, 0.2, 0.0);
        this.playSound(SoundEvents.ENTITY_SHULKER_BULLET_HIT, 1.0f, 1.0f);
    }

    private void destroy() {
        this.discard();
        this.net_minecraft_world_World_getWorld().emitGameEvent(GameEvent.ENTITY_DAMAGE, this.getPos(), GameEvent.Emitter.of(this));
    }
