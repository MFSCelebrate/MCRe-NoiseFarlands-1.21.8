...
implements FlyingItemEntity {
    final static private TrackedData<ItemStack> ITEM = DataTracker.registerData(FireworkRocketEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    final static private TrackedData<OptionalInt> SHOOTER_ENTITY_ID = DataTracker.registerData(FireworkRocketEntity.class, TrackedDataHandlerRegistry.OPTIONAL_INT);
    final static private TrackedData<Boolean> SHOT_AT_ANGLE = DataTracker.registerData(FireworkRocketEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private int DEFAULT_LIFE = 0;
    final static private int DEFAULT_LIFE_TIME = 0;
    final static private boolean DEFAULT_SHOT_AT_ANGLE = false;
    private int life = 0;
    ...
    public void tick() {
        ...
        if (this.wasShotByEntity()) {
            if (this.shooter == null) {
                this.dataTracker.get(SHOOTER_ENTITY_ID).ifPresent(id -> {
                    Entity entity = this.net_minecraft_world_World_getWorld().getEntityById(id);
        ...
        if (this.life == 0 && !this.isSilent()) {
            this.net_minecraft_world_World_getWorld().playSound(null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_FIREWORK_ROCKET_LAUNCH, SoundCategory.AMBIENT, 3.0f, 1.0f);
        }
        ++this.life;
        if (this.net_minecraft_world_World_getWorld().isClient && this.life % 2 < 2) {
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.FIREWORK, this.getX(), this.getY(), this.getZ(), this.random.nextGaussian() * 0.05, -this.getVelocity().y * 0.5, this.random.nextGaussian() * 0.05);
        }
        if (this.life > this.lifeTime && (world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
    ...
    private void explodeAndRemove(ServerWorld world) {
        ...
        this.emitGameEvent(GameEvent.EXPLODE, this.net_minecraft_entity_Entity_getOwner());
        this.explode(world);
        this.discard();
    }
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
        ...
        this.net_minecraft_world_World_getWorld().getBlockState(blockPos).onEntityCollision(this.net_minecraft_world_World_getWorld(), blockPos, this, EntityCollisionHandler.DUMMY);
        World world = this.net_minecraft_world_World_getWorld();
        if (world instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld)world;
    ...
    private void explode(ServerWorld world) {
        ...
        if (f > 0.0f) {
            if (this.shooter != null) {
                this.shooter.damage(world, this.getDamageSources().fireworks(this, this.net_minecraft_entity_Entity_getOwner()), 5.0f + (float)(list.size() * 2));
            }
            double d = 5.0;
            Vec3d vec3d = this.getPos();
            List<LivingEntity> list2 = this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(LivingEntity.class, this.getBoundingBox().expand(5.0));
            for (LivingEntity livingEntity : list2) {
                if (livingEntity == this.shooter || this.squaredDistanceTo(livingEntity) > 25.0) continue;
                boolean bl = false;
                for (int i = 0; i < 2; ++i) {
                    Vec3d vec3d2 = new Vec3d(livingEntity.getX(), livingEntity.getBodyY(0.5 * (double)i), livingEntity.getZ());
                    BlockHitResult hitResult = this.net_minecraft_world_World_getWorld().raycast(new RaycastContext(vec3d, vec3d2, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this));
                ...
                livingEntity.damage(world, this.getDamageSources().fireworks(this, this.net_minecraft_entity_Entity_getOwner()), g);
            }
        }
    }
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.EXPLODE_FIREWORK_CLIENT && this.net_minecraft_world_World_getWorld().isClient) {
            Vec3d vec3d = this.getVelocity();
            this.net_minecraft_world_World_getWorld().addFireworkParticle(this.getX(), this.getY(), this.getZ(), vec3d.x, vec3d.y, vec3d.z, this.getExplosions());
        }
