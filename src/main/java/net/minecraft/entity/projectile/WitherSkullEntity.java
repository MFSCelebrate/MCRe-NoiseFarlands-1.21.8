...
extends ExplosiveProjectileEntity {
    final static private TrackedData<Boolean> CHARGED = DataTracker.registerData(WitherSkullEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static private boolean DEFAULT_DANGEROUS = false;
    ...
    protected void onCollision(HitResult hitResult) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient) {
            this.net_minecraft_world_World_getWorld().createExplosion((Entity)this, this.getX(), this.getY(), this.getZ(), 1.0f, false, World.ExplosionSourceType.MOB);
            this.discard();
        }
    }
