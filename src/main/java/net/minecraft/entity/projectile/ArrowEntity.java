...
extends PersistentProjectileEntity {
    final static private int MAX_POTION_DURATION_TICKS = 600;
    final static private int NO_POTION_COLOR = -1;
    final static private TrackedData<Integer> COLOR = DataTracker.registerData(ArrowEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private byte PARTICLE_EFFECT_STATUS = 0;
    ...
    public void tick() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            ...
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, 0);
    ...
    private void spawnParticles(int amount) {
        ...
        for (int j = 0; j < amount; ++j) {
            this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, i), this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0, 0.0, 0.0);
        }
    }
    ...
    public void handleStatus(byte status) {
        if (status == 0) {
            ...
            if (i != -1) {
                ...
                for (int j = 0; j < 20; ++j) {
                    this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, f, g, h), this.getParticleX(0.5), this.getRandomBodyY(), this.getParticleZ(0.5), 0.0, 0.0, 0.0);
                }
            }
        } else {
