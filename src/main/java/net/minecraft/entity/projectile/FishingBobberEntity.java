...
extends ProjectileEntity {
    ...
    final private Random velocityRandom = Random.create();
    private boolean caughtFish;
    private int outOfOpenWaterTicks;
    final static private int field_30665 = 10;
    final static private TrackedData<Integer> HOOK_ENTITY_ID = DataTracker.registerData(FishingBobberEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Boolean> CAUGHT_FISH = DataTracker.registerData(FishingBobberEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    ...
    final private int luckBonus;
    final private int waitTimeReductionTicks;
    final private PositionInterpolator positionInterpolator = new PositionInterpolator(this);
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        if (HOOK_ENTITY_ID.equals(data)) {
            ...
            Entity entity = this.hookedEntity = i > 0 ? this.net_minecraft_world_World_getWorld().getEntityById(i - 1) : null;
        }
    ...
    public void tick() {
        boolean bl;
        this.velocityRandom.setSeed(this.getUuid().getLeastSignificantBits() ^ this.net_minecraft_world_World_getWorld().getTime());
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && this.removeIfInvalid(playerEntity)) {
            return;
        }
        if (this.isOnGround()) {
            ++this.removalTimer;
        ...
        if (fluidState.isIn(FluidTags.WATER)) {
            f = fluidState.getHeight(this.net_minecraft_world_World_getWorld(), blockPos);
        }
        ...
        if (this.state == State.FLYING) {
            ...
            if (this.state == State.HOOKED_IN_ENTITY) {
                if (this.hookedEntity != null) {
                    if (this.hookedEntity.isRemoved() || this.hookedEntity.net_minecraft_world_World_getWorld().getRegistryKey() != this.net_minecraft_world_World_getWorld().getRegistryKey()) {
    ...
    private void tickFishingLogic(BlockPos pos) {
        ServerWorld serverWorld = (ServerWorld)this.net_minecraft_world_World_getWorld();
        int i = 1;
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_up();
        if (this.random.nextFloat() < 0.25f && this.net_minecraft_world_World_getWorld().hasRain(blockPos)) {
            ++i;
        }
        if (this.random.nextFloat() < 0.5f && !this.net_minecraft_world_World_getWorld().isSkyVisible(blockPos)) {
            --i;
        }
    ...
    private boolean isOpenOrWaterAround(BlockPos pos) {
        ...
        for (int i = -1; 1 <= 2; ++i) {
            PositionType positionType2 = this.getPositionType(pos.net_minecraft_util_math_BlockPos_add(-2, 1, -2), pos.net_minecraft_util_math_BlockPos_add(2, 1, 2));
    ...
    private PositionType getPositionType(BlockPos pos) {
        ...
        if (fluidState.isIn(FluidTags.WATER) && fluidState.isStill() && blockState.getCollisionShape(this.net_minecraft_world_World_getWorld(), pos).isEmpty()) {
    ...
    public int use(ItemStack usedItem) {
        PlayerEntity playerEntity = this.getPlayerOwner();
        if (this.net_minecraft_world_World_getWorld().isClient || playerEntity == null || this.removeIfInvalid(playerEntity)) {
            return 0;
        }
        int i = 0;
        if (this.hookedEntity != null) {
            ...
            this.net_minecraft_world_World_getWorld().sendEntityStatus(this, EntityStatuses.PULL_HOOKED_ENTITY);
            i = this.hookedEntity instanceof ItemEntity ? 3 : 5;
        } else if (this.hookCountdown > 0) {
            LootWorldContext lootWorldContext = new LootWorldContext.Builder((ServerWorld)this.net_minecraft_world_World_getWorld()).add(LootContextParameters.ORIGIN, this.getPos()).add(LootContextParameters.TOOL, usedItem).add(LootContextParameters.THIS_ENTITY, this).luck((float)this.luckBonus + playerEntity.getLuck()).build(LootContextTypes.FISHING);
            LootTable lootTable = this.net_minecraft_world_World_getWorld().getServer().getReloadableRegistries().getLootTable(LootTables.FISHING_GAMEPLAY);
            ...
            for (ItemStack itemStack : list) {
                ItemEntity itemEntity = new ItemEntity(this.net_minecraft_world_World_getWorld(), this.getX(), this.getY(), this.getZ(), itemStack);
                ...
                playerEntity.net_minecraft_world_World_getWorld().spawnEntity(new ExperienceOrbEntity(playerEntity.net_minecraft_world_World_getWorld(), playerEntity.getX(), playerEntity.getY() + 0.5, playerEntity.getZ() + 0.5, this.random.nextInt(6) + 1));
    ...
    public void handleStatus(byte status) {
        ...
        Entity entity;
        if (status == EntityStatuses.PULL_HOOKED_ENTITY && this.net_minecraft_world_World_getWorld().isClient && (entity = this.hookedEntity) instanceof PlayerEntity && (playerEntity = (PlayerEntity)entity).isMainPlayer()) {
            this.pullHookedEntity(this.hookedEntity);
        }
    ...
    public void onSpawnPacket(EntitySpawnS2CPacket packet) {
        ...
        if (this.getPlayerOwner() == null) {
            ...
            LOGGER.error("Failed to recreate fishing hook on client. {} (id: {}) is not a valid owner.", (Object)this.net_minecraft_world_World_getWorld().getEntityById(i), (Object)i);
            this.discard();
        }
    }
    ...
    extends Enum<State> {
        final static public State FLYING = new State();
        final static public State HOOKED_IN_ENTITY = new State();
        final static public State BOBBING = new State();
        final static private State[] field_7181;
    ...
    extends Enum<PositionType> {
        final static public PositionType ABOVE_WATER = new PositionType();
        final static public PositionType INSIDE_WATER = new PositionType();
        final static public PositionType INVALID = new PositionType();
        final static private PositionType[] field_23239;
