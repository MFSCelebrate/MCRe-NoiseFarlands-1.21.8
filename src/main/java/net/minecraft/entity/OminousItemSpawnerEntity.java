...
extends Entity {
    final static private int MIN_SPAWN_ITEM_AFTER_TICKS = 60;
    final static private int MAX_SPAWN_ITEM_AFTER_TICKS = 120;
    final static private String SPAWN_ITEM_AFTER_TICKS_NBT_KEY = "spawn_item_after_ticks";
    final static private String ITEM_NBT_KEY = "item";
    final static private TrackedData<ItemStack> ITEM = DataTracker.registerData(OminousItemSpawnerEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    final static public int field_50128 = 36;
    ...
    private void tickClient() {
        if (this.net_minecraft_world_World_getWorld().getTime() % 5L == 0L) {
            this.addParticles();
        }
    }
    ...
    private Entity spawnProjectile(ServerWorld world, ProjectileItem item, ItemStack stack) {
        ...
        ProjectileEntity projectileEntity = ProjectileEntity.spawnWithVelocity(item.createEntity(world, this.getPos(), stack, direction), world, stack, direction.getOffsetX(), direction.getOffsetY(), direction.getOffsetZ(), settings.power(), settings.uncertainty());
    ...
    public void addParticles() {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.OMINOUS_SPAWNING, vec3d.getX(), vec3d.getY(), vec3d.getZ(), vec3d3.getX(), vec3d3.getY(), vec3d3.getZ());
        }
    }
