...
public interface EntityCollisionHandler {
    final static public EntityCollisionHandler DUMMY = new EntityCollisionHandler(){
    ...
    implements EntityCollisionHandler {
        final static private CollisionEvent[] ALL_EVENTS = CollisionEvent.values();
        final static private int INVALID_VERSION = -1;
        final private Set<CollisionEvent> activeEvents = EnumSet.noneOf(CollisionEvent.class);
        final private Map<CollisionEvent, List<Consumer<Entity>>> preCallbacks = Util.mapEnum(CollisionEvent.class, value -> new ArrayList());
        final private Map<CollisionEvent, List<Consumer<Entity>>> postCallbacks = Util.mapEnum(CollisionEvent.class, value -> new ArrayList());
        final private List<Consumer<Entity>> callbacks = new ArrayList<Consumer<Entity>>();
