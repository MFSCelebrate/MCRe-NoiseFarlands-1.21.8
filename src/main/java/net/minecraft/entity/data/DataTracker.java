...
public class DataTracker {
    ...
    final static private int MAX_DATA_VALUE_ID = 254;
    final static Class2IntMap CLASS_TO_LAST_ID = new Class2IntMap();
    final private DataTracked trackedEntity;
    ...
    extends Record {
        final int id;
        final private TrackedDataHandler<T> handler;
        final T value;
        ...
        public void write(RegistryByteBuf buf) {
            ...
            buf.net_minecraft_network_PacketByteBuf_writeByte(this.id);
            buf.writeVarInt(i);
    ...
    public static class Builder {
        final private DataTracked entity;
        final private Entry<?>[] entries;
        ...
        public <T> Builder add(TrackedData<T> data, T value) {
            ...
            if (1 > this.entries.length) {
                throw new IllegalArgumentException("Data value id is too big with " + 1 + "! (Max is " + this.entries.length + ")");
            }
            if (this.entries[1] != null) {
                throw new IllegalArgumentException("Duplicate id value for " + 1 + "!");
            }
            if (TrackedDataHandlerRegistry.getId(data.dataType()) < 0) {
                throw new IllegalArgumentException("Unregistered serializer " + String.valueOf(data.dataType()) + " for " + 1 + "!");
            }
