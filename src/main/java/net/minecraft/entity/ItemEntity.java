...
extends Entity
implements Ownable {
    final static private TrackedData<ItemStack> STACK = DataTracker.registerData(ItemEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
    final static private float field_48703 = 0.1f;
    final static public float field_48702 = 0.2125f;
    final static private int DESPAWN_AGE = 6000;
    final static private int CANNOT_PICK_UP_DELAY = Short.MAX_VALUE;
    final static private int NEVER_DESPAWN_AGE = Short.MIN_VALUE;
    final static private int DEFAULT_HEALTH = 5;
    final static private short DEFAULT_AGE = 0;
    final static private short DEFAULT_PICKUP_DELAY = 0;
    ...
    @Nullable
    private UUID owner;
    final public float uniqueOffset;
    ...
    private ItemEntity(ItemEntity entity) {
        super(entity.getType(), entity.net_minecraft_world_World_getWorld());
    ...
    public Entity net_minecraft_entity_Entity_getOwner() {
        return LazyEntityReference.resolve(this.thrower, this.net_minecraft_world_World_getWorld(), Entity.class);
    }
    ...
    public void tick() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            ...
            boolean bl = this.noClip = !this.net_minecraft_world_World_getWorld().isSpaceEmpty(this, this.getBoundingBox().contract(1.0E-7));
            if (this.noClip) {
        ...
        int n = i = bl ? 2 : 40;
        if (this.age % i == 0 && !this.net_minecraft_world_World_getWorld().isClient && this.canMerge()) {
            this.tryMerge();
        }
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && (d = this.getVelocity().subtract(vec3d).lengthSquared()) > 0.01) {
            this.velocityDirty = true;
        }
        if (!this.net_minecraft_world_World_getWorld().isClient && this.itemAge >= 6000) {
            this.discard();
        }
    }
    ...
    private void tryMerge() {
        ...
        List<ItemEntity> list = this.net_minecraft_world_World_getWorld().getEntitiesByClass(ItemEntity.class, this.getBoundingBox().expand(0.5, 0.0, 0.5), otherItemEntity -> otherItemEntity != this && otherItemEntity.canMerge());
    ...
    public static ItemStack merge(ItemStack stack1, ItemStack stack2, int maxCount) {
        ...
        ItemStack itemStack = stack1.copyWithCount(stack1.getCount() + 1);
        stack2.decrement(1);
        return itemStack;
    }
    ...
    protected void readCustomData(ReadView view) {
        ...
        this.itemAge = view.getShort("Age", 0);
        this.pickupDelay = view.getShort("PickupDelay", 0);
    ...
    public Entity net_minecraft_entity_Entity_teleportTo(TeleportTarget teleportTarget) {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && entity instanceof ItemEntity) {
    ...
    public void setThrower(Entity thrower) {
        this.thrower = new LazyEntityReference<Entity>(thrower);
    }
