...
extends PatrolEntity {
    final static protected TrackedData<Boolean> CELEBRATING = DataTracker.registerData(RaiderEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    final static Predicate<ItemEntity> OBTAINABLE_OMINOUS_BANNER_PREDICATE = itemEntity -> !itemEntity.cannotPickup() && itemEntity.isAlive() && ItemStack.areEqual(itemEntity.getStack(), Raid.createOminousBanner(itemEntity.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)));
    final static private int DEFAULT_WAVE = 0;
    final static private boolean DEFAULT_ABLE_TO_JOIN_RAID = false;
    @Nullable
    protected Raid raid;
    private int wave = 0;
        ...
        if (world instanceof ServerWorld) {
            ...
            if (this.isAlive()) {
                ...
                if (this.canJoinRaid()) {
                    if (raid == null) {
                        Raid raid2;
                        if (this.net_minecraft_world_World_getWorld().getTime() % 20L == 0L && (raid2 = serverWorld.getRaidAt(this.getBlockPos())) != null && RaidManager.isValidRaiderFor(this)) {
    ...
    public boolean isCaptain() {
        ...
        boolean bl = !itemStack.isEmpty() && ItemStack.areEqual(itemStack, Raid.createOminousBanner(this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)));
    ...
    protected void writeCustomData(WriteView view) {
        ...
        if (this.raid != null && (world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
            ServerWorld serverWorld = (ServerWorld)world;
    ...
    protected void loot(ServerWorld world, ItemEntity itemEntity) {
        ...
        if (this.hasActiveRaid() && !bl && ItemStack.areEqual(itemStack, Raid.createOminousBanner(this.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)))) {
    ...
    extends Goal {
        ...
        public boolean canStart() {
            ...
            List<ItemEntity> list = ((Entity)this.actor).net_minecraft_world_World_getWorld().getEntitiesByClass(ItemEntity.class, ((Entity)this.actor).getBoundingBox().expand(d, 8.0, d), OBTAINABLE_OMINOUS_BANNER_PREDICATE);
            for (ItemEntity itemEntity : list) {
                long l = this.bannerItemCache.getOrDefault(itemEntity.getId(), Long.MIN_VALUE);
                if (this.field_52512.net_minecraft_world_World_getWorld().getTime() < l) {
                ...
                int2LongOpenHashMap.put(itemEntity.getId(), this.field_52512.net_minecraft_world_World_getWorld().getTime() + 600L);
            }
        ...
        private boolean shouldStop() {
            ...
            if (ItemStack.areEqual(((LivingEntity)this.actor).getEquippedStack(EquipmentSlot.HEAD), Raid.createOminousBanner(((Entity)this.actor).getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.BANNER_PATTERN)))) {
                return true;
            }
        ...
        public void tick() {
            if (this.bannerItemEntity != null && this.bannerItemEntity.isInRange((Entity)this.actor, 1.414)) {
                ((RaiderEntity)this.actor).loot(PickUpBannerAsLeaderGoal.castToServerWorld(this.field_52512.net_minecraft_world_World_getWorld()), this.bannerItemEntity);
            }
        }
    }
    ...
    extends Goal {
        final private RaiderEntity raider;
        final private double speed;
        private BlockPos home;
        final private List<BlockPos> lastHomes = Lists.newArrayList();
        ...
        private boolean tryFindHome() {
            ServerWorld serverWorld = (ServerWorld)this.raider.net_minecraft_world_World_getWorld();
    ...
    extends Goal {
        final private RaiderEntity raider;
        final private float squaredDistance;
        final public TargetPredicate closeRaiderPredicate = TargetPredicate.createNonAttackable().setBaseMaxDistance(8.0).ignoreVisibility().ignoreDistanceScalingFactor();
