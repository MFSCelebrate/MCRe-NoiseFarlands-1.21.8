...
implements RangedAttackMob {
    final static private TrackedData<Integer> TRACKED_ENTITY_ID_1 = DataTracker.registerData(WitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> TRACKED_ENTITY_ID_2 = DataTracker.registerData(WitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TrackedData<Integer> TRACKED_ENTITY_ID_3 = DataTracker.registerData(WitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private List<TrackedData<Integer>> TRACKED_ENTITY_IDS = ImmutableList.of(TRACKED_ENTITY_ID_1, TRACKED_ENTITY_ID_2, TRACKED_ENTITY_ID_3);
    final static private TrackedData<Integer> INVUL_TIMER = DataTracker.registerData(WitherEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private int ON_SUMMONED_INVUL_TIMER = 220;
    final static private int DEFAULT_INVUL_TIMER = 0;
    final private float[] sideHeadPitches = new float[2];
    final private float[] sideHeadYaws = new float[2];
    final private float[] lastSideHeadPitches = new float[2];
    final private float[] lastSideHeadYaws = new float[2];
    final private int[] skullCooldowns = new int[2];
    final private int[] chargedSkullCooldowns = new int[2];
    private int blockBreakingCooldown;
    final private ServerBossBar bossBar = (ServerBossBar)new ServerBossBar(this.getDisplayName(), BossBar.Color.PURPLE, BossBar.Style.PROGRESS).setDarkenSky(true);
    final static private TargetPredicate.EntityPredicate CAN_ATTACK_PREDICATE = (entity, world) -> !entity.getType().isIn(EntityTypeTags.WITHER_FRIENDS) && entity.isMobOrPlayer();
    final static private TargetPredicate HEAD_TARGET_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(20.0).setPredicate(CAN_ATTACK_PREDICATE);
    ...
    public void tickMovement() {
        ...
        if (!this.net_minecraft_world_World_getWorld().isClient && this.getTrackedEntityId(0) > 0 && (entity = this.net_minecraft_world_World_getWorld().getEntityById(this.getTrackedEntityId(0))) != null) {
            double d = vec3d.y;
        ...
        for (i = 0; i < 2; ++i) {
            ...
            Entity entity2 = null;
            if (j > 0) {
                entity2 = this.net_minecraft_world_World_getWorld().getEntityById(j);
            }
            if (entity2 != null) {
        ...
        for (int j = 0; j < 3; ++j) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.SMOKE, p + this.random.nextGaussian() * (double)s, q + this.random.nextGaussian() * (double)s, r + this.random.nextGaussian() * (double)s, 0.0, 0.0, 0.0);
            if (!bl || this.net_minecraft_world_World_getWorld().random.nextInt(4) != 0) continue;
            this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.7f, 0.7f, 0.5f), p + this.random.nextGaussian() * (double)s, q + this.random.nextGaussian() * (double)s, r + this.random.nextGaussian() * (double)s, 0.0, 0.0, 0.0);
        }
        if (this.getInvulnerableTimer() > 0) {
            float t = 3.3f * this.getScale();
            for (int u = 0; u < 3; ++u) {
                this.net_minecraft_world_World_getWorld().addParticleClient(TintedParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.7f, 0.7f, 0.9f), this.getX() + this.random.nextGaussian(), this.getY() + (double)(this.random.nextFloat() * t), this.getZ() + this.random.nextGaussian(), 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    private void shootSkullAt(int headIndex, double targetX, double targetY, double targetZ, boolean charged) {
        if (!this.isSilent()) {
            this.net_minecraft_world_World_getWorld().syncWorldEvent(null, WorldEvents.WITHER_SHOOTS, this.getBlockPos(), 0);
        }
        ...
        WitherSkullEntity witherSkullEntity = new WitherSkullEntity(this.net_minecraft_world_World_getWorld(), this, vec3d.normalize());
        witherSkullEntity.setOwner(this);
        if (charged) {
            witherSkullEntity.setCharged(true);
        }
        witherSkullEntity.setPosition(d, e, f);
        this.net_minecraft_world_World_getWorld().spawnEntity(witherSkullEntity);
    }
