...
extends AbstractSittingPhase {
    final static private int DURATION = 200;
    final static private int MAX_TIMES_RUN = 4;
    final static private int DRAGON_BREATH_MAX_TICK = 10;
    private int ticks;
    ...
    public void clientTick() {
        ...
        if (this.ticks % 2 == 0 && this.ticks < 10) {
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                for (int k = 0; k < 6; ++k) {
                    this.dragon.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.DRAGON_BREATH, g, h, j, -vec3d.x * (double)0.08f * (double)k, -vec3d.y * (double)0.6f, -vec3d.z * (double)0.08f * (double)k);
                }
