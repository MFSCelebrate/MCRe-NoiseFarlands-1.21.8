...
extends AbstractSittingPhase {
    final static private int DURATION = 100;
    final static private int MAX_HEIGHT_CLOSE_PLAYER_RANGE = 10;
    final static private int MAX_HORIZONTAL_CLOSE_PLAYER_RANGE = 20;
    final static private int MAX_PLAYER_RANGE = 150;
    final static private TargetPredicate PLAYER_WITHIN_RANGE_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(150.0);
    final private TargetPredicate CLOSE_PLAYER_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(20.0).setPredicate((player, world) -> Math.abs(player.getY() - enderDragonEntity.getY()) <= 10.0);
    private int ticks;
