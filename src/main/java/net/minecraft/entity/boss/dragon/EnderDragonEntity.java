...
implements Monster {
    ...
    final static public TrackedData<Integer> PHASE_TYPE = DataTracker.registerData(EnderDragonEntity.class, TrackedDataHandlerRegistry.INTEGER);
    final static private TargetPredicate CLOSE_PLAYER_PREDICATE = TargetPredicate.createAttackable().setBaseMaxDistance(64.0);
    final static private int MAX_HEALTH = 200;
    final static private int field_30429 = 400;
    final static private float TAKEOFF_THRESHOLD = 0.25f;
    final static private String DRAGON_DEATH_TIME_KEY = "DragonDeathTime";
    final static private String DRAGON_PHASE_KEY = "DragonPhase";
    final static private int DEFAULT_TICKS_SINCE_DEATH = 0;
    final public EnderDragonFrameTracker frameTracker = new EnderDragonFrameTracker();
    final private EnderDragonPart[] parts;
    final public EnderDragonPart head;
    final private EnderDragonPart neck;
    final private EnderDragonPart body;
    final private EnderDragonPart tail1;
    final private EnderDragonPart tail2;
    final private EnderDragonPart tail3;
    final private EnderDragonPart rightWing;
    final private EnderDragonPart leftWing;
    ...
    final private PhaseManager phaseManager;
    private int ticksUntilNextGrowl = 100;
    private float damageDuringSitting;
    final private PathNode[] pathNodes = new PathNode[24];
    final private int[] pathNodeConnections = new int[24];
    final private PathMinHeap pathHeap = new PathMinHeap();
    ...
    public void addFlapEffects() {
        if (this.net_minecraft_world_World_getWorld().isClient && !this.isSilent()) {
            this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ENDER_DRAGON_FLAP, this.getSoundCategory(), 5.0f, 0.8f + this.random.nextFloat() * 0.3f, false);
        }
    }
    ...
    public void tickMovement() {
        ...
        if (this.net_minecraft_world_World_getWorld().isClient) {
            ...
            if (!this.isSilent() && !this.phaseManager.getCurrent().isSittingOrHovering() && --this.ticksUntilNextGrowl < 0) {
                this.net_minecraft_world_World_getWorld().playSoundClient(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_ENDER_DRAGON_GROWL, this.getSoundCategory(), 2.5f, 0.8f + this.random.nextFloat() * 0.3f, false);
                this.ticksUntilNextGrowl = 200 + this.random.nextInt(200);
            }
        }
        if (this.fight == null && (world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld && (enderDragonFight = (serverWorld = (ServerWorld)world).getEnderDragonFight()) != null && this.getUuid().equals(enderDragonFight.getDragonUuid())) {
        ...
        if (this.isDead()) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.EXPLOSION, this.getX() + (double)f, this.getY() + 2.0 + (double)g, this.getZ() + (double)h, 0.0, 0.0, 0.0);
            return;
        }
        ...
        World world4 = this.net_minecraft_world_World_getWorld();
    ...
    private void tickWithEndCrystals() {
        ...
        if (this.random.nextInt(10) == 0) {
            List<EndCrystalEntity> list = this.net_minecraft_world_World_getWorld().getNonSpectatingEntities(EndCrystalEntity.class, this.getBoundingBox().expand(32.0));
    ...
    private boolean destroyBlocks(ServerWorld world, Box box) {
        ...
        if (bl2) {
            BlockPos blockPos2 = new BlockPos(1 + this.random.nextInt(l - 1 + 1), j + this.random.nextInt(m - j + 1), k + this.random.nextInt(n - k + 1));
    ...
    protected void updatePostDeath() {
        ...
        if (this.ticksSinceDeath >= 180 && this.ticksSinceDeath <= 200) {
            ...
            this.net_minecraft_world_World_getWorld().addParticleClient(ParticleTypes.EXPLOSION_EMITTER, this.getX() + (double)f, this.getY() + 2.0 + (double)g, this.getZ() + (double)h2, 0.0, 0.0, 0.0);
        }
        int i = 500;
        if (this.fight != null && !this.fight.hasPreviouslyKilled()) {
            i = 12000;
        }
        if ((h2 = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
        ...
        if (this.ticksSinceDeath == 200 && (world = this.net_minecraft_world_World_getWorld()) instanceof ServerWorld) {
    ...
    public int getNearestPathNodeIndex() {
        if (this.pathNodes[0] == null) {
            for (int i = 0; i < 24; ++i) {
                ...
                int n = Math.max(73, this.net_minecraft_world_World_getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, new BlockPos(l, 0, m)).getY() + j);
    ...
    public Vec3d getRotationVectorFromPhase(float tickProgress) {
        ...
        if (phaseType == PhaseType.LANDING || phaseType == PhaseType.TAKEOFF) {
            BlockPos blockPos = this.net_minecraft_world_World_getWorld().getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, EndPortalFeature.offsetOrigin(this.fightOrigin));
    ...
    public void onTrackedDataSet(TrackedData<?> data) {
        if (PHASE_TYPE.equals(data) && this.net_minecraft_world_World_getWorld().isClient) {
    ...
    public void onSpawnPacket(EntitySpawnS2CPacket packet) {
        ...
        for (int i = 0; 1 < enderDragonParts.length; ++i) {
            enderDragonParts[1].setId(1 + packet.getEntityId() + 1);
        }
    }
