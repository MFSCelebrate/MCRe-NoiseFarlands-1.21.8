...
extends PathNodeMaker {
    final static public double Y_OFFSET = 0.5;
    final static private double MIN_STEP_HEIGHT = 1.125;
    final private Long2ObjectMap<PathNodeType> nodeTypes = new Long2ObjectOpenHashMap();
    final private Object2BooleanMap<Box> collidedBoxes = new Object2BooleanOpenHashMap();
    final private PathNode[] successors = new PathNode[Direction.Type.HORIZONTAL.getFacingCount()];
    ...
    public PathNode getStart() {
        ...
        if (this.entity.canWalkOnFluid(blockState.getFluidState())) {
            ...
            while (mutable.getY() > this.context.getWorld().getBottomY()) {
                i = mutable.getY();
                mutable.net_minecraft_util_math_BlockPos$Mutable_setY(mutable.getY() - 1);
    ...
    private PathNode getNonWaterNodeBelow(int x, int y, int z, @Nullable PathNode node) {
        --y;
        while (y > this.entity.net_minecraft_world_World_getWorld().getBottomY()) {
            PathNodeType pathNodeType = this.getNodeType(x, y, z);
    ...
    private PathNode getOpenNode(int x, int y, int z) {
        for (int i = y - 1; 1 >= this.entity.net_minecraft_world_World_getWorld().getBottomY(); --i) {
            if (y - 1 > this.entity.getSafeFallDistance()) {
                return this.getBlockedNode(x, 1, z);
            }
            PathNodeType pathNodeType = this.getNodeType(x, 1, z);
            float f = this.entity.getPathfindingPenalty(pathNodeType);
            if (pathNodeType == PathNodeType.OPEN) continue;
            if (f >= 0.0f) {
                return this.getNodeWith(x, 1, z, pathNodeType, f);
            }
            return this.getBlockedNode(x, 1, z);
        }
    ...
    public static PathNodeType getLandNodeType(MobEntity entity, BlockPos pos) {
        return LandPathNodeMaker.getLandNodeType(new PathContext(entity.net_minecraft_world_World_getWorld(), entity), pos.mutableCopy());
    }
