...
extends EntityNavigation {
    ...
    public Path findPathTo(BlockPos target, int distance) {
        ...
        WorldChunk worldChunk = this.world.net_minecraft_world_chunk_ChunkManager_getChunkManager().getWorldChunk(ChunkSectionPos.getSectionCoord(target.getX()), ChunkSectionPos.getSectionCoord(target.getZ()));
        if (worldChunk == null) {
            return null;
        }
        if (worldChunk.getBlockState(target).isAir()) {
            mutable = target.mutableCopy().move(Direction.DOWN);
            ...
            if (mutable.getY() >= this.world.getBottomY()) {
                return super.findPathTo((BlockPos)mutable.net_minecraft_util_math_Vec3i_up(), distance);
            }
            mutable.net_minecraft_util_math_BlockPos$Mutable_setY(target.getY() + 1);
        ...
        if (worldChunk.getBlockState(target).isSolid()) {
            mutable = target.mutableCopy().move(Direction.UP);
