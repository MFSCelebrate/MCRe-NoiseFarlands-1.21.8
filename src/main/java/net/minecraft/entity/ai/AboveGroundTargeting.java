...
public class AboveGroundTargeting {
    ...
    public static Vec3d find(PathAwareEntity entity, int horizontalRange, int verticalRange, double x, double z, float angle, int maxAboveSolid, int minAboveSolid) {
        ...
        return FuzzyPositions.guessBestPathTarget(entity, () -> {
            ...
            if (NavigationConditions.isWaterAt(entity, blockPos2 = FuzzyPositions.upWhile(blockPos2, entity.getRandom().nextInt(maxAboveSolid - minAboveSolid + 1) + minAboveSolid, entity.net_minecraft_world_World_getWorld().getTopYInclusive(), pos -> NavigationConditions.isSolidAt(entity, pos))) || NavigationConditions.hasPathfindingPenalty(entity, blockPos2)) {
                return null;
            }
            return blockPos2;
        });
