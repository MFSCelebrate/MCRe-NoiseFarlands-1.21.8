...
public class WardenAngerManager {
    @VisibleForTesting
    final static protected int field_38733 = 2;
    @VisibleForTesting
    final static protected int MAX_ANGER = 150;
    final static private int ANGER_DECREASE_PER_TICK = 1;
    private int updateTimer = MathHelper.nextBetween(Random.create(), 0, 2);
    int primeAnger;
    final static private Codec<Pair<UUID, Integer>> SUSPECT_CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Uuids.INT_STREAM_CODEC.fieldOf("uuid").forGetter(Pair::getFirst), (App)Codecs.NON_NEGATIVE_INT.fieldOf("anger").forGetter(Pair::getSecond)).apply((Applicative)instance, Pair::of));
    final private Predicate<Entity> suspectPredicate;
    @VisibleForTesting
    final protected ArrayList<Entity> suspects;
    final private SuspectComparator suspectComparator;
    @VisibleForTesting
    final protected Object2IntMap<Entity> suspectsToAngerLevel;
    @VisibleForTesting
    final protected Object2IntMap<UUID> suspectUuidsToAngerLevel;
    ...
    private void updateSuspectsMap(ServerWorld world) {
        ...
        while (objectIterator.hasNext()) {
            Object2IntMap.Entry entry = (Object2IntMap.Entry)objectIterator.next();
            int i = entry.getIntValue();
            Entity entity = world.net_minecraft_entity_Entity_getEntity((UUID)entry.getKey());
            if (entity == null) continue;
            this.suspectsToAngerLevel.put((Object)entity, 1);
    ...
    protected record SuspectComparator(WardenAngerManager angerManagement) implements Comparator<Entity>
    {
        ...
        public int compare(Entity entity, Entity entity2) {
            ...
            this.angerManagement.primeAnger = Math.max(this.angerManagement.primeAnger, Math.max(1, j));
            boolean bl = Angriness.getForAnger(1).isAngry();
