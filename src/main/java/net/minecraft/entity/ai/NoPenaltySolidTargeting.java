...
public class NoPenaltySolidTargeting {
    ...
    public static BlockPos tryMake(PathAwareEntity entity, int horizontalRange, int verticalRange, int startHeight, double directionX, double directionZ, double rangeAngle, boolean posTargetInRange) {
        ...
        if (NavigationConditions.hasPathfindingPenalty(entity, blockPos2 = FuzzyPositions.upWhile(blockPos2, entity.net_minecraft_world_World_getWorld().getTopYInclusive(), pos -> NavigationConditions.isSolidAt(entity, pos)))) {
            return null;
        }
        return blockPos2;
    }
