...
extends Goal {
    ...
    public boolean canStart() {
        List<AbstractBoatEntity> list = this.mob.net_minecraft_world_World_getWorld().getNonSpectatingEntities(AbstractBoatEntity.class, this.mob.getBoundingBox().expand(5.0));
        boolean bl = false;
        for (AbstractBoatEntity abstractBoatEntity : list) {
            LivingEntity entity = abstractBoatEntity.getControllingPassenger();
            if (!(entity instanceof PlayerEntity)) continue;
            PlayerEntity playerEntity = (PlayerEntity)entity;
            if (!(MathHelper.abs(playerEntity.sidewaysSpeed) > 0.0f) && !(MathHelper.abs(playerEntity.forwardSpeed) > 0.0f)) continue;
            bl = true;
            break;
        }
    ...
    public void tick() {
        ...
        if (this.state == ChaseBoatState.GO_TO_BOAT) {
            BlockPos blockPos = this.passenger.getBlockPos().net_minecraft_util_math_BlockPos_offset(this.passenger.getHorizontalFacing().getOpposite());
            blockPos = blockPos.net_minecraft_util_math_BlockPos_add(0, -1, 0);
            ...
            BlockPos blockPos2 = this.passenger.getBlockPos().net_minecraft_util_math_BlockPos_offset(direction, 10);
