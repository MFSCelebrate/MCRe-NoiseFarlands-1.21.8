...
extends DiveJumpingGoal {
    final static private int[] OFFSET_MULTIPLIERS = new int[]{0, 1, 4, 5, 6, 7};
    ...
    private boolean isWater(BlockPos pos, int offsetX, int offsetZ, int multiplier) {
        BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_add(offsetX * multiplier, 0, offsetZ * multiplier);
        return this.dolphin.net_minecraft_world_World_getWorld().getFluidState(blockPos).isIn(FluidTags.WATER) && !this.dolphin.net_minecraft_world_World_getWorld().getBlockState(blockPos).blocksMovement();
    }

    private boolean isAirAbove(BlockPos pos, int offsetX, int offsetZ, int multiplier) {
        return this.dolphin.net_minecraft_world_World_getWorld().getBlockState(pos.net_minecraft_util_math_BlockPos_add(offsetX * multiplier, 1, offsetZ * multiplier)).isAir() && this.dolphin.net_minecraft_world_World_getWorld().getBlockState(pos.net_minecraft_util_math_BlockPos_add(offsetX * multiplier, 2, offsetZ * multiplier)).isAir();
    }
    ...
    public void tick() {
        ...
        if (!bl) {
            FluidState fluidState = this.dolphin.net_minecraft_world_World_getWorld().getFluidState(this.dolphin.getBlockPos());
