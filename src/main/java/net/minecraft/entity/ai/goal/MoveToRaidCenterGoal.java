...
extends Goal {
    final static private int FREE_RAIDER_CHECK_INTERVAL = 20;
    final static private float WALK_SPEED = 1.0f;
    ...
    public boolean canStart() {
        return ((MobEntity)this.actor).getTarget() == null && !((Entity)this.actor).hasControllingPassenger() && ((RaiderEntity)this.actor).hasActiveRaid() && !((RaiderEntity)this.actor).getRaid().isFinished() && !MoveToRaidCenterGoal.castToServerWorld(((Entity)this.actor).net_minecraft_world_World_getWorld()).isNearOccupiedPointOfInterest(((Entity)this.actor).getBlockPos());
    }

    @Override
    public boolean shouldContinue() {
        return ((RaiderEntity)this.actor).hasActiveRaid() && !((RaiderEntity)this.actor).getRaid().isFinished() && !MoveToRaidCenterGoal.castToServerWorld(((Entity)this.actor).net_minecraft_world_World_getWorld()).isNearOccupiedPointOfInterest(((Entity)this.actor).getBlockPos());
    }
    ...
    private void includeFreeRaiders(Raid raid) {
        if (raid.isActive()) {
            ServerWorld serverWorld = MoveToRaidCenterGoal.castToServerWorld(((Entity)this.actor).net_minecraft_world_World_getWorld());
