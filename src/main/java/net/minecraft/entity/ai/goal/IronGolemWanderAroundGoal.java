...
extends WanderAroundGoal {
    final static private int CHUNK_RANGE = 2;
    final static private int ENTITY_COLLISION_RANGE = 32;
    final static private int HORIZONTAL_RANGE = 10;
    final static private int VERTICAL_RANGE = 7;
    ...
    protected Vec3d getWanderTarget() {
        Vec3d vec3d;
        float f = this.mob.net_minecraft_world_World_getWorld().random.nextFloat();
        if (this.mob.net_minecraft_world_World_getWorld().random.nextFloat() < 0.3f) {
    ...
    private Vec3d findVillagerPos() {
        ServerWorld serverWorld = (ServerWorld)this.mob.net_minecraft_world_World_getWorld();
        ...
        VillagerEntity villagerEntity = list.get(this.mob.net_minecraft_world_World_getWorld().random.nextInt(list.size()));
    ...
    private boolean canVillagerSummonGolem(VillagerEntity villager) {
        return villager.canSummonGolem(this.mob.net_minecraft_world_World_getWorld().getTime());
    }
}
