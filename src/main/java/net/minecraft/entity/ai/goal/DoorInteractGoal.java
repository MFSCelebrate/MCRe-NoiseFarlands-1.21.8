...
extends Goal {
    ...
    protected boolean isDoorOpen() {
        ...
        BlockState blockState = this.mob.net_minecraft_world_World_getWorld().getBlockState(this.doorPos);
    ...
    protected void setDoorOpen(boolean open) {
        ...
        if (this.doorValid && (blockState = this.mob.net_minecraft_world_World_getWorld().getBlockState(this.doorPos)).getBlock() instanceof DoorBlock) {
            ((DoorBlock)blockState.getBlock()).setOpen(this.mob, this.mob.net_minecraft_world_World_getWorld(), blockState, this.doorPos, open);
        }
    }
    ...
    public boolean canStart() {
        ...
        for (int i = 0; i < Math.min(path.getCurrentNodeIndex() + 2, path.getLength()); ++i) {
            ...
            this.doorValid = DoorBlock.canOpenByHand(this.mob.net_minecraft_world_World_getWorld(), this.doorPos);
            if (!this.doorValid) continue;
            return true;
        }
        this.doorPos = this.mob.getBlockPos().net_minecraft_util_math_BlockPos_up();
        this.doorValid = DoorBlock.canOpenByHand(this.mob.net_minecraft_world_World_getWorld(), this.doorPos);
