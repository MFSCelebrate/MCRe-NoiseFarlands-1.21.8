...
extends Goal {
    final private SkeletonHorseEntity skeletonHorse;
    ...
    public boolean canStart() {
        return this.skeletonHorse.net_minecraft_world_World_getWorld().isPlayerInRange(this.skeletonHorse.getX(), this.skeletonHorse.getY(), this.skeletonHorse.getZ(), 10.0);
    }

    @Override
    public void tick() {
        ServerWorld serverWorld = (ServerWorld)this.skeletonHorse.net_minecraft_world_World_getWorld();
    ...
    private AbstractHorseEntity getHorse(LocalDifficulty localDifficulty) {
        SkeletonHorseEntity skeletonHorseEntity = EntityType.SKELETON_HORSE.create(this.skeletonHorse.net_minecraft_world_World_getWorld(), SpawnReason.TRIGGERED);
        if (skeletonHorseEntity != null) {
            skeletonHorseEntity.initialize((ServerWorld)this.skeletonHorse.net_minecraft_world_World_getWorld(), localDifficulty, SpawnReason.TRIGGERED, null);
    ...
    private SkeletonEntity getSkeleton(LocalDifficulty localDifficulty, AbstractHorseEntity vehicle) {
        SkeletonEntity skeletonEntity = EntityType.SKELETON.create(vehicle.net_minecraft_world_World_getWorld(), SpawnReason.TRIGGERED);
        if (skeletonEntity != null) {
            skeletonEntity.initialize((ServerWorld)vehicle.net_minecraft_world_World_getWorld(), localDifficulty, SpawnReason.TRIGGERED, null);
    ...
    private void enchantEquipment(SkeletonEntity rider, EquipmentSlot slot, LocalDifficulty localDifficulty) {
        ...
        EnchantmentHelper.applyEnchantmentProvider(itemStack, rider.net_minecraft_world_World_getWorld().getRegistryManager(), EnchantmentProviders.MOB_SPAWN_EQUIPMENT, localDifficulty, rider.getRandom());
