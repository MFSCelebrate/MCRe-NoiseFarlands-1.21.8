...
extends Goal {
    ...
    public boolean canStart() {
        return this.mob.isOnGround() && !this.mob.net_minecraft_world_World_getWorld().getFluidState(this.mob.getBlockPos()).isIn(FluidTags.WATER);
    }
    ...
    public void start() {
        ...
        for (BlockPos blockPos2 : iterable) {
            if (!this.mob.net_minecraft_world_World_getWorld().getFluidState(blockPos2).isIn(FluidTags.WATER)) continue;
            blockPos = blockPos2;
            break;
        }
        if (blockPos != null) {
