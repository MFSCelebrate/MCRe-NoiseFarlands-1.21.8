...
extends MultiTickTask<BreezeEntity> {
    final static private int REQUIRED_SPACE_ABOVE = 4;
    final static private int JUMP_COOLDOWN_EXPIRY = 10;
    final static private int JUMP_COOLDOWN_EXPIRY_WHEN_HURT = 2;
    final static private int JUMP_INHALING_EXPIRY = Math.round(10.0f);
    final static private float field_52499 = 24.0f;
    final static private float MAX_JUMP_VELOCITY = 1.4f;
    final static private float FOLLOW_RANGE_MULTIPLIER_FOR_VELOCITY = 0.058333334f;
    final static private ObjectArrayList<Integer> POSSIBLE_JUMP_ANGLES = new ObjectArrayList((Collection)Lists.newArrayList((Object[])new Integer[]{40, 55, 60, 75, 80}));
    ...
    public static boolean shouldJump(ServerWorld world, BreezeEntity breeze) {
        ...
        BlockState blockState = world.getBlockState(blockPos.net_minecraft_util_math_BlockPos_down());
        if (breeze.getType().isInvalidSpawn(blockState)) {
            return false;
        }
        if (!BreezeMovementUtil.canMoveTo(breeze, blockPos.toCenterPos()) && !BreezeMovementUtil.canMoveTo(breeze, blockPos.net_minecraft_util_math_BlockPos_up(4).toCenterPos())) {
            return false;
        }
    ...
    private static BlockPos getPosToJumpTo(LivingEntity breeze, Vec3d pos) {
        ...
        BlockHitResult hitResult = breeze.net_minecraft_world_World_getWorld().raycast(raycastContext);
        if (((HitResult)hitResult).getType() == HitResult.Type.BLOCK) {
            return BlockPos.ofFloored(hitResult.getPos()).net_minecraft_util_math_BlockPos_up();
        }
        ...
        BlockHitResult hitResult2 = breeze.net_minecraft_world_World_getWorld().raycast(raycastContext2);
        if (((HitResult)hitResult2).getType() == HitResult.Type.BLOCK) {
            return BlockPos.ofFloored(hitResult2.getPos()).net_minecraft_util_math_BlockPos_up();
        }
        return null;
    }
    ...
    private static boolean hasRoomToJump(ServerWorld world, BreezeEntity breeze) {
        ...
        for (int i = 1; 1 <= 4; ++i) {
            BlockPos blockPos2 = blockPos.net_minecraft_util_math_BlockPos_offset(Direction.UP, 1);
