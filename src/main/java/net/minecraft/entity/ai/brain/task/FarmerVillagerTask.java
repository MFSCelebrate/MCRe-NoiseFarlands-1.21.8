...
extends MultiTickTask<VillagerEntity> {
    final static private int MAX_RUN_TIME = 200;
    final static public float WALK_SPEED = 0.5f;
    @Nullable
    private BlockPos currentTarget;
    private long nextResponseTime;
    private int ticksRan;
    final private List<BlockPos> targetPositions = Lists.newArrayList();
    ...
    protected boolean shouldRun(ServerWorld serverWorld, VillagerEntity villagerEntity) {
        ...
        for (int i = -1; 1 <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                for (int k = -1; k <= 1; ++k) {
                    mutable.set(villagerEntity.getX() + 1.0, villagerEntity.getY() + (double)j, villagerEntity.getZ() + (double)k);
    ...
    private boolean isSuitableTarget(BlockPos pos, ServerWorld world) {
        ...
        Block block2 = world.getBlockState(pos.net_minecraft_util_math_BlockPos_down()).getBlock();
    ...
    protected void keepRunning(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
        ...
        if (this.currentTarget != null && l > this.nextResponseTime) {
            ...
            Block block = blockState.getBlock();
            Block block2 = serverWorld.getBlockState(this.currentTarget.net_minecraft_util_math_BlockPos_down()).getBlock();
