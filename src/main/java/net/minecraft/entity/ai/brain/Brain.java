...
public class Brain<E extends LivingEntity> {
    ...
    final private Supplier<Codec<Brain<E>>> codecSupplier;
    final static private int ACTIVITY_REFRESH_COOLDOWN = 20;
    final private Map<MemoryModuleType<?>, Optional<? extends Memory<?>>> memories = Maps.newHashMap();
    final private Map<SensorType<? extends Sensor<? super E>>, Sensor<? super E>> sensors = Maps.newLinkedHashMap();
    final private Map<Integer, Map<Activity, Set<Task<? super E>>>> tasks = Maps.newTreeMap();
    private Schedule schedule = Schedule.EMPTY;
    final private Map<Activity, Set<Pair<MemoryModuleType<?>, MemoryModuleState>>> requiredActivityMemories = Maps.newHashMap();
    final private Map<Activity, Set<MemoryModuleType<?>>> forgettingActivityMemories = Maps.newHashMap();
    private Set<Activity> coreActivities = Sets.newHashSet();
    final private Set<Activity> possibleActivities = Sets.newHashSet();
    ...
    public void stopAllTasks(ServerWorld world, E entity) {
        long l = ((Entity)entity).net_minecraft_world_World_getWorld().getTime();
    ...
    public static final class Profile<E extends LivingEntity> {
        final private Collection<? extends MemoryModuleType<?>> memoryModules;
        final private Collection<? extends SensorType<? extends Sensor<? super E>>> sensors;
        final private Codec<Brain<E>> codec;
    ...
    static final class MemoryEntry<U> {
        final private MemoryModuleType<U> type;
        final private Optional<? extends Memory<U>> data;
