...
extends MultiTickTask<BreezeEntity> {
    ...
    protected void run(ServerWorld serverWorld, BreezeEntity breezeEntity, long l) {
        ...
        Vec3d vec3d = null;
        if (bl && (vec3d2 = NoPenaltyTargeting.findFrom(breezeEntity, 5, 5, livingEntity.getPos())) != null && BreezeMovementUtil.canMoveTo(breezeEntity, vec3d2) && livingEntity.squaredDistanceTo(vec3d2.x, vec3d2.y, vec3d2.z) > livingEntity.squaredDistanceTo(breezeEntity)) {
            vec3d = vec3d2;
        }
        if (vec3d == null) {
