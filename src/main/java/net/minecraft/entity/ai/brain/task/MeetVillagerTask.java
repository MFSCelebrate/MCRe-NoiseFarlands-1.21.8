...
public class MeetVillagerTask {
    final static private float WALK_SPEED = 0.3f;
    ...
    public static SingleTickTask<LivingEntity> create() {
        return TaskTriggerer.task(context -> context.group(context.queryMemoryOptional(MemoryModuleType.WALK_TARGET), context.queryMemoryOptional(MemoryModuleType.LOOK_TARGET), context.queryMemoryValue(MemoryModuleType.MEETING_POINT), context.queryMemoryValue(MemoryModuleType.VISIBLE_MOBS), context.queryMemoryAbsent(MemoryModuleType.INTERACTION_TARGET)).apply((Applicative)context, (walkTarget, lookTarget, meetingPoint, visibleMobs, interactionTarget) -> (world, entity, time) -> {
            ...
            if (world.getRandom().nextInt(100) == 0 && world.getRegistryKey() == globalPos.dimension() && globalPos.pos().isWithinDistance(entity.getPos(), 4.0) && livingTargetCache.anyMatch(target -> EntityType.VILLAGER.equals(target.getType()))) {
