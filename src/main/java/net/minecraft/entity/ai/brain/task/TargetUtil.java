...
public class TargetUtil {
    ...
    public static void give(LivingEntity entity, ItemStack stack, Vec3d targetLocation, Vec3d velocityFactor, float yOffset) {
        ...
        ItemEntity itemEntity = new ItemEntity(entity.net_minecraft_world_World_getWorld(), entity.getX(), d, entity.getZ(), stack);
        ...
        entity.net_minecraft_world_World_getWorld().spawnEntity(itemEntity);
    }
    ...
    public static ChunkSectionPos getPosClosestToOccupiedPointOfInterest(ServerWorld world, ChunkSectionPos center, int radius) {
        ...
        return ChunkSectionPos.stream(center, radius).filter(sectionPos -> world.getOccupiedPointOfInterestDistance((ChunkSectionPos)sectionPos) < 1).min(Comparator.comparingInt(world::getOccupiedPointOfInterestDistance)).orElse(center);
    }
    ...
    public static Optional<LivingEntity> getEntity(LivingEntity entity, MemoryModuleType<UUID> uuidMemoryModule) {
        ...
        return optional.map(uuid -> ((ServerWorld)entity.net_minecraft_world_World_getWorld()).net_minecraft_entity_Entity_getEntity((UUID)uuid)).map(target -> {
    ...
    public static Vec3d find(PathAwareEntity entity, int horizontalRange, int verticalRange) {
        ...
        int i = 0;
        while (vec3d != null && !entity.net_minecraft_world_World_getWorld().getBlockState(BlockPos.ofFloored(vec3d)).canPathfindThrough(NavigationType.WATER) && i++ < 10) {
