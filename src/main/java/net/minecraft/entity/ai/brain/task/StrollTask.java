...
public class StrollTask {
    final static private int DEFAULT_HORIZONTAL_RADIUS = 10;
    final static private int DEFAULT_VERTICAL_RADIUS = 7;
    final static private int[][] RADII = new int[][]{{1, 1}, {3, 3}, {5, 5}, {6, 5}, {7, 7}, {10, 7}};
    ...
    private static Vec3d findTargetPos(PathAwareEntity entity) {
        ...
        Vec3d vec3d2 = null;
        for (int[] is : RADII) {
            vec3d2 = vec3d == null ? TargetUtil.find(entity, is[0], is[1]) : entity.getPos().add(entity.getPos().relativize(vec3d).normalize().multiply(is[0], is[1], is[0]));
            if (vec3d2 == null || entity.net_minecraft_world_World_getWorld().getFluidState(BlockPos.ofFloored(vec3d2)).isEmpty()) {
                return vec3d;
            }
            vec3d = vec3d2;
        }
