...
public class FindPointOfInterestTask {
    final static public int POI_SORTING_RADIUS = 48;
    ...
    public static Path findPathToPoi(MobEntity entity, Set<Pair<RegistryEntry<PointOfInterestType>, BlockPos>> pois) {
        ...
        for (Pair<RegistryEntry<PointOfInterestType>, BlockPos> pair : pois) {
            i = Math.max(1, ((PointOfInterestType)((RegistryEntry)pair.getFirst()).value()).searchDistance());
            set.add((BlockPos)pair.getSecond());
        }
        return entity.getNavigation().findPathTo(set, 1);
    }

    private static App method_46879(MemoryModuleType context, boolean bl, MutableLong mutableLong, Long2ObjectMap long2ObjectMap, Predicate predicate, BiPredicate biPredicate, Optional optional, TaskTriggerer.TaskContext taskContext) {
    ...
    static class RetryMarker {
        final static private int MIN_DELAY = 40;
        final static private int MAX_EXTRA_DELAY = 80;
        final static private int ATTEMPT_DURATION = 400;
