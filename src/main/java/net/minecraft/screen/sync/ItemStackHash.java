...
public interface ItemStackHash {
    final static public ItemStackHash EMPTY = new ItemStackHash(){
    ...
    final static public PacketCodec<RegistryByteBuf, ItemStackHash> PACKET_CODEC = PacketCodecs.optional(Impl.PACKET_CODEC).xmap(hash -> (ItemStackHash)DataFixUtils.orElse((Optional)hash, (Object)EMPTY), hash -> {
    ...
    {
        final static public PacketCodec<RegistryByteBuf, Impl> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.registryEntry(RegistryKeys.ITEM), Impl::item, PacketCodecs.VAR_INT, Impl::count, ComponentChangesHash.PACKET_CODEC, Impl::components, Impl::new);
