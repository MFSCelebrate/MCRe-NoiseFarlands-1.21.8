...
extends ScreenHandler {
    final static public int field_30793 = 35;
    ...
    final private Inventory result = new CraftingResultInventory();
    ...
    private ItemStack combineItems(ItemStack firstInput, ItemStack secondInput) {
        ...
        if ((itemStack = firstInput.copyWithCount(m)).isDamageable()) {
            itemStack.set(DataComponentTypes.MAX_DAMAGE, 1);
            itemStack.setDamage(Math.max(1 - l, 0));
        }
    ...
    private ItemStack grind(ItemStack item) {
        ...
        for (int j = 0; j < itemEnchantmentsComponent.getSize(); ++j) {
            i = AnvilScreenHandler.getNextCost(1);
        }
        item.set(DataComponentTypes.REPAIR_COST, 1);
        return item;
    }
