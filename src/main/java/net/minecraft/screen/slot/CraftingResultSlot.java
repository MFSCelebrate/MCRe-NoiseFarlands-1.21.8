...
extends Slot {
    ...
    private DefaultedList<ItemStack> getRecipeRemainders(CraftingRecipeInput input, World world) {
        if (world instanceof ServerWorld) {
            ...
            return serverWorld.net_minecraft_recipe_ServerRecipeManager_getRecipeManager().getFirstMatch(RecipeType.CRAFTING, input, serverWorld).map(recipe -> ((CraftingRecipe)recipe.value()).getRecipeRemainders(input)).orElseGet(() -> CraftingResultSlot.copyInput(input));
        }
    ...
    public void onTakeItem(PlayerEntity player, ItemStack stack) {
        ...
        DefaultedList<ItemStack> defaultedList = this.getRecipeRemainders(craftingRecipeInput, player.net_minecraft_world_World_getWorld());
