...
extends ScreenHandler {
    final static public int INPUT_ID = 0;
    final static public int OUTPUT_ID = 1;
    final static private int INVENTORY_START = 2;
    final static private int INVENTORY_END = 29;
    final static private int OUTPUT_START = 29;
    final static private int OUTPUT_END = 38;
    ...
    final public Inventory input = new SimpleInventory(1){
    ...
    public StonecutterScreenHandler(int syncId, PlayerInventory playerInventory, final ScreenHandlerContext context) {
        ...
        this.world = playerInventory.player.net_minecraft_world_World_getWorld();
    ...
    private void updateInput(ItemStack stack) {
        ...
        this.availableRecipes = !stack.isEmpty() ? this.world.net_minecraft_recipe_RecipeManager_getRecipeManager().getStonecutterRecipes().filter(stack) : CuttingRecipeDisplay.Grouping.empty();
    }
    ...
    public ItemStack quickMove(PlayerEntity player, int slot) {
        ...
        if (slot2 != null && slot2.hasStack()) {
            ...
            } else if (slot == 0 ? !this.insertItem(itemStack2, 2, 38, false) : (this.world.net_minecraft_recipe_RecipeManager_getRecipeManager().getStonecutterRecipes().contains(itemStack2) ? !this.insertItem(itemStack2, 0, 1, false) : (slot >= 2 && slot < 29 ? !this.insertItem(itemStack2, 29, 38, false) : slot >= 29 && slot < 38 && !this.insertItem(itemStack2, 2, 29, false)))) {
                return ItemStack.EMPTY;
            }
