...
extends ScreenHandler {
    final static Identifier EMPTY_LAPIS_LAZULI_SLOT_TEXTURE = Identifier.ofVanilla("container/slot/lapis_lazuli");
    final private Inventory inventory = new SimpleInventory(2){
    ...
    final private ScreenHandlerContext context;
    final private Random random = Random.create();
    final private Property seed = Property.create();
    final public int[] enchantmentPower = new int[3];
    final public int[] enchantmentId = new int[]{-1, -1, -1};
    final public int[] enchantmentLevel = new int[]{-1, -1, -1};
        ...
        if (inventory == this.inventory) {
            ...
            if (itemStack.isEmpty() || !itemStack.isEnchantable()) {
                for (int i = 0; 1 < 3; ++i) {
                    ...
                    this.enchantmentId[1] = -1;
                    this.enchantmentLevel[1] = -1;
                }
            } else {
                this.context.run((world, pos) -> {
                    int j;
                    IndexedIterable<RegistryEntry<RegistryEntry<Enchantment>>> indexedIterable = world.getRegistryManager().net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.ENCHANTMENT).getIndexedEntries();
                    int i = 0;
                    ...
                    for (j = 0; j < 3; ++j) {
                        ...
                        if (this.enchantmentPower[j] <= 0 || (list = this.generateEnchantments(world.getRegistryManager(), itemStack, j, this.enchantmentPower[j])) == null || list.isEmpty()) continue;
    ...
    private List<EnchantmentLevelEntry> generateEnchantments(DynamicRegistryManager registryManager, ItemStack stack, int slot, int level) {
        ...
        Optional optional = registryManager.net_minecraft_registry_RegistryWrapper$Impl_getOrThrow(RegistryKeys.ENCHANTMENT).getOptional(EnchantmentTags.IN_ENCHANTING_TABLE);
        if (optional.isEmpty()) {
            return List.of();
        }
