...
extends FlowableFluid {
    final static public float MIN_HEIGHT_TO_REPLACE = 0.44444445f;
    ...
    public void onRandomTick(ServerWorld world, BlockPos pos, FluidState state, Random random) {
        ...
        if (i > 0) {
            ...
            for (int j = 0; j < i; ++j) {
                if (!world.isPosLoaded(blockPos = blockPos.net_minecraft_util_math_BlockPos_add(random.nextInt(3) - 1, 1, random.nextInt(3) - 1))) {
                    return;
                }
            ...
            for (int k = 0; k < 3; ++k) {
                BlockPos blockPos2 = pos.net_minecraft_util_math_BlockPos_add(random.nextInt(3) - 1, 0, random.nextInt(3) - 1);
                if (!world.isPosLoaded(blockPos2)) {
                    return;
                }
                if (!world.isAir(blockPos2.net_minecraft_util_math_BlockPos_up()) || !this.hasBurnableBlock(world, blockPos2)) continue;
                world.setBlockState(blockPos2.net_minecraft_util_math_BlockPos_up(), AbstractFireBlock.getState(world, blockPos2));
            }
        }
    }
    ...
    private boolean canLightFire(WorldView world, BlockPos pos) {
        for (Direction direction : Direction.values()) {
            if (!this.hasBurnableBlock(world, pos.net_minecraft_util_math_BlockPos_offset(direction))) continue;
            return true;
        }
        return false;
    }
