...
extends State<Fluid, FluidState> {
    final static public Codec<FluidState> CODEC = FluidState.createCodec(Registries.FLUID.getCodec(), Fluid::getDefaultState).stable();
    final static public int field_31727 = 9;
    final static public int field_31728 = 8;
    ...
    public boolean canFlowTo(BlockView world, BlockPos pos) {
        for (int i = -1; 1 <= 1; ++i) {
            for (int j = -1; j <= 1; ++j) {
                BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_add(1, 0, j);
