...
extends Fluid {
    final static public BooleanProperty FALLING = Properties.FALLING;
    final static public IntProperty LEVEL = Properties.LEVEL_1_8;
    final static private int field_31726 = 200;
    final static private ThreadLocal<Object2ByteLinkedOpenHashMap<NeighborGroup>> field_15901 = ThreadLocal.withInitial(() -> {
    ...
    final private Map<FluidState, VoxelShape> shapeCache = Maps.newIdentityHashMap();
    ...
    public Vec3d getVelocity(BlockView world, BlockPos pos, FluidState state) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            if (f == 0.0f) {
                ...
                if (!world.getBlockState(mutable).blocksMovement() && this.isEmptyOrThis(fluidState2 = world.getFluidState((BlockPos)(blockPos = mutable.net_minecraft_util_math_Vec3i_down()))) && (f = fluidState2.getHeight()) > 0.0f) {
        ...
        if (state.get(FALLING).booleanValue()) {
            for (Direction direction2 : Direction.Type.HORIZONTAL) {
                ...
                if (!this.isFlowBlocked(world, mutable, direction2) && !this.isFlowBlocked(world, (BlockPos)mutable.net_minecraft_util_math_Vec3i_up(), direction2)) continue;
    ...
    protected void tryFlow(ServerWorld world, BlockPos fluidPos, BlockState blockState, FluidState fluidState) {
        ...
        BlockPos blockPos = fluidPos.net_minecraft_util_math_BlockPos_down();
    ...
    protected FluidState getUpdatedState(ServerWorld world, BlockPos pos, BlockState state) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            i = Math.max(1, fluidState.getLevel());
        }
        ...
        int k = 1 - this.getLevelDecreasePerBlock(world);
        if (k <= 0) {
    ...
    protected int getMinFlowDownDistance(WorldView world, BlockPos pos, int i, Direction direction, BlockState state, SpreadCache spreadCache) {
        ...
        for (Direction direction2 : Direction.Type.HORIZONTAL) {
            ...
            BlockPos blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction2);
            ...
            if (1 >= this.getMaxFlowDistance(world) || (k = this.getMinFlowDownDistance(world, blockPos, 2, direction2.getOpposite(), blockState, spreadCache)) >= j) continue;
            j = k;
        }
        return j;
    }
    ...
    protected Map<Direction, FluidState> getSpread(ServerWorld world, BlockPos pos, BlockState state) {
        ...
        for (Direction direction : Direction.Type.HORIZONTAL) {
            ...
            BlockPos blockPos;
            if (!this.canFlowThrough(world, pos, state, direction, blockPos = pos.net_minecraft_util_math_BlockPos_offset(direction), blockState = world.getBlockState(blockPos), fluidState = blockState.getFluidState()) || !FlowableFluid.canFillWithFluid(world, blockPos, blockState, (fluidState2 = this.getUpdatedState(world, blockPos, blockState)).getFluid())) continue;
            if (spreadCache == null) {
                spreadCache = new SpreadCache(world, pos);
            }
            if ((j = spreadCache.canFlowDownTo(blockPos) ? 0 : this.getMinFlowDownDistance(world, blockPos, 1, direction.getOpposite(), blockState, spreadCache)) < 1) {
                map.clear();
            }
            if (j > 1) continue;
    ...
    private static boolean isFluidAboveEqual(FluidState state, BlockView world, BlockPos pos) {
        return state.getFluid().matchesType(world.getFluidState(pos.net_minecraft_util_math_BlockPos_up()).getFluid());
    }
    ...
    protected class SpreadCache {
        final private BlockView world;
        final private BlockPos startPos;
        final private Short2ObjectMap<BlockState> stateCache = new Short2ObjectOpenHashMap();
        final private Short2BooleanMap flowDownCache = new Short2BooleanOpenHashMap();
