...
public record TextInputControl(int width, Text label, boolean labelVisible, String initial, int maxLength, Optional<Multiline> multiline) implements InputControl
{
    ...
    public record Multiline(Optional<Integer> maxLines, Optional<Integer> height) {
        final static public int MAX_HEIGHT = 512;
        final static public Codec<Multiline> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codecs.POSITIVE_INT.optionalFieldOf("max_lines").forGetter(Multiline::maxLines), (App)Codecs.rangedInt(1, 512).optionalFieldOf("height").forGetter(Multiline::height)).apply((Applicative)instance, Multiline::new));
    }
}
