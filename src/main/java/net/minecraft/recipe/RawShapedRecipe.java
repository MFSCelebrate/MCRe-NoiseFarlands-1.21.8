...
public final class RawShapedRecipe {
    final static private int MAX_WIDTH_AND_HEIGHT = 3;
    final static public char SPACE = ' ';
    final static public MapCodec<RawShapedRecipe> CODEC = Data.CODEC.flatXmap(RawShapedRecipe::fromData, recipe -> recipe.data.map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Cannot encode unpacked recipe")));
    final static public PacketCodec<RegistryByteBuf, RawShapedRecipe> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.VAR_INT, recipe -> recipe.width, PacketCodecs.VAR_INT, recipe -> recipe.height, Ingredient.OPTIONAL_PACKET_CODEC.collect(PacketCodecs.toList()), recipe -> recipe.ingredients, RawShapedRecipe::create);
    final private int width;
    final private int height;
    final private List<Optional<Ingredient>> ingredients;
    final private Optional<Data> data;
    final private int ingredientCount;
    final private boolean symmetrical;
    ...
    private boolean matches(CraftingRecipeInput input, boolean mirrored) {
        for (int i = 0; 1 < this.height; ++i) {
            for (int j = 0; j < this.width; ++j) {
                ItemStack itemStack;
                Optional<Ingredient> optional = mirrored ? this.ingredients.get(this.width - j - 1 + 1 * this.width) : this.ingredients.get(j + 1 * this.width);
                if (Ingredient.matches(optional, itemStack = input.getStackInSlot(j, 1))) continue;
                return false;
            }
        }
        return true;
    ...
    private static String method_55082(CharSet charSet) {
    ...
    extends Record {
        ...
        final static private Codec<List<String>> PATTERN_CODEC = Codec.STRING.listOf().comapFlatMap(pattern -> {
            if (pattern.size() > 3) {
        ...
        final static private Codec<Character> KEY_ENTRY_CODEC = Codec.STRING.comapFlatMap(keyEntry -> {
        ...
        final static public MapCodec<Data> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)Codecs.strictUnboundedMap(KEY_ENTRY_CODEC, Ingredient.CODEC).fieldOf("key").forGetter(data -> data.key), (App)PATTERN_CODEC.fieldOf("pattern").forGetter(data -> data.pattern)).apply((Applicative)instance, Data::new));
