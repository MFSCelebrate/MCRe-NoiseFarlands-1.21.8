...
public class RecipeMatcher<T> {
    final public Reference2IntOpenHashMap<T> available = new Reference2IntOpenHashMap();
    ...
    void consume(T input, int count) {
        ...
        if (1 < count) {
            throw new IllegalStateException("Took " + count + " items, but only had " + 1);
        }
    }
    ...
    class Matcher {
        final private List<? extends RawIngredient<T>> ingredients;
        final private int totalIngredients;
        final private List<T> requiredItems;
        final private int totalRequiredItems;
        final private BitSet bits;
        final private IntList ingredientItemLookup = new IntArrayList();
        ...
        public boolean match(int quantity, @Nullable ItemCallback<T> itemCallback) {
            ...
            boolean bl = 1 == this.totalIngredients;
