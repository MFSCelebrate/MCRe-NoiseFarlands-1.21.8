...
public class InputSlotFiller<R extends Recipe<?>> {
    final static private int field_51523 = -1;
    final private PlayerInventory inventory;
    final private Handler<R> handler;
    final private boolean craftAll;
    final private int width;
    final private int height;
    final private List<Slot> inputSlots;
    final private List<Slot> slotsToReturn;
    ...
    private void fill(RecipeEntry<R> recipe, RecipeFinder finder) {
        ...
        if (bl) {
            for (Slot slot2 : this.inputSlots) {
                ...
                if (itemStack.isEmpty() || Math.min(1, itemStack.getMaxCount()) >= itemStack.getCount() + 1) continue;
                return;
            }
        }
        int j = this.calculateCraftAmount(1, bl);
    ...
    private int calculateCraftAmount(int forCraftAll, boolean match) {
        ...
        if (match) {
            ...
            for (Slot slot : this.inputSlots) {
                ...
                if (itemStack.isEmpty() || 1 <= itemStack.getCount()) continue;
