...
Predicate<ItemStack>,
FabricIngredient {
    final static public PacketCodec<RegistryByteBuf, Ingredient> PACKET_CODEC = PacketCodecs.registryEntryList(RegistryKeys.ITEM).xmap(Ingredient::new, ingredient -> ingredient.entries);
    final static public PacketCodec<RegistryByteBuf, Optional<Ingredient>> OPTIONAL_PACKET_CODEC = PacketCodecs.registryEntryList(RegistryKeys.ITEM).xmap(entries -> entries.size() == 0 ? Optional.empty() : Optional.of(new Ingredient((RegistryEntryList<Item>)entries)), optional -> optional.map(ingredient -> ingredient.entries).orElse(RegistryEntryList.of(new RegistryEntry[0])));
    final static public Codec<RegistryEntryList<Item>> ENTRIES_CODEC = RegistryEntryListCodec.create(RegistryKeys.ITEM, Item.ENTRY_CODEC, false);
    final static public Codec<Ingredient> CODEC = Codecs.nonEmptyEntryList(ENTRIES_CODEC).xmap(Ingredient::new, ingredient -> ingredient.entries);
    final private RegistryEntryList<Item> entries;
    ...
    @Override
    public boolean acceptsItem(Object object) {
