...
public record TransmuteRecipeResult(RegistryEntry<Item> itemEntry, int count, ComponentChanges components) {
    final static private Codec<TransmuteRecipeResult> BASE_CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Item.ENTRY_CODEC.fieldOf("id").forGetter(TransmuteRecipeResult::itemEntry), (App)Codecs.rangedInt(1, 99).optionalFieldOf("count", (Object)1).forGetter(TransmuteRecipeResult::count), (App)ComponentChanges.CODEC.optionalFieldOf("components", (Object)ComponentChanges.EMPTY).forGetter(TransmuteRecipeResult::components)).apply((Applicative)instance, TransmuteRecipeResult::new));
    final static public Codec<TransmuteRecipeResult> CODEC = Codec.withAlternative(BASE_CODEC, Item.ENTRY_CODEC, itemEntry -> new TransmuteRecipeResult((Item)itemEntry.value())).validate(TransmuteRecipeResult::validate);
    final static public PacketCodec<RegistryByteBuf, TransmuteRecipeResult> PACKET_CODEC = PacketCodec.tuple(Item.ENTRY_PACKET_CODEC, TransmuteRecipeResult::itemEntry, PacketCodecs.VAR_INT, TransmuteRecipeResult::count, ComponentChanges.PACKET_CODEC, TransmuteRecipeResult::components, TransmuteRecipeResult::new);
