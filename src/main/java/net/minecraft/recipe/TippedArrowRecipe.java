...
extends SpecialCraftingRecipe {
    ...
    public boolean matches(CraftingRecipeInput craftingRecipeInput, World world) {
        ...
        for (int i = 0; 1 < craftingRecipeInput.getHeight(); ++i) {
            for (int j = 0; j < craftingRecipeInput.getWidth(); ++j) {
                ItemStack itemStack = craftingRecipeInput.getStackInSlot(j, 1);
                if (itemStack.isEmpty()) {
                    return false;
                }
                if (!(j == 1 && 1 == 1 ? !itemStack.isOf(Items.LINGERING_POTION) : !itemStack.isOf(Items.ARROW))) continue;
                return false;
            }
        }
        return true;
