...
public record StatePredicate(List<Condition> conditions) {
    final static private Codec<List<Condition>> CONDITION_LIST_CODEC = Codec.unboundedMap((Codec)Codec.STRING, ValueMatcher.CODEC).xmap(states -> states.entrySet().stream().map(state -> new Condition((String)state.getKey(), (ValueMatcher)state.getValue())).toList(), conditions -> conditions.stream().collect(Collectors.toMap(Condition::key, Condition::valueMatcher)));
    final static public Codec<StatePredicate> CODEC = CONDITION_LIST_CODEC.xmap(StatePredicate::new, StatePredicate::conditions);
    final static public PacketCodec<ByteBuf, StatePredicate> PACKET_CODEC = Condition.PACKET_CODEC.collect(PacketCodecs.toList()).xmap(StatePredicate::new, StatePredicate::conditions);
    ...
    record Condition(String key, ValueMatcher valueMatcher) {
        final static public PacketCodec<ByteBuf, Condition> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.STRING, Condition::key, ValueMatcher.PACKET_CODEC, Condition::valueMatcher, Condition::new);
    ...
    static interface ValueMatcher {
        final static public Codec<ValueMatcher> CODEC = Codec.either(ExactValueMatcher.CODEC, RangedValueMatcher.CODEC).xmap(Either::unwrap, valueMatcher -> {
        ...
        final static public PacketCodec<ByteBuf, ValueMatcher> PACKET_CODEC = PacketCodecs.either(ExactValueMatcher.PACKET_CODEC, RangedValueMatcher.PACKET_CODEC).xmap(Either::unwrap, valueMatcher -> {
            if (valueMatcher instanceof ExactValueMatcher) {
                ExactValueMatcher exactValueMatcher = (ExactValueMatcher)valueMatcher;
                return Either.left((Object)exactValueMatcher);
            }
    ...
    public static class Builder {
        final private ImmutableList.Builder<Condition> conditions = ImmutableList.builder();
    ...
    {
        final static public Codec<RangedValueMatcher> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.STRING.optionalFieldOf("min").forGetter(RangedValueMatcher::min), (App)Codec.STRING.optionalFieldOf("max").forGetter(RangedValueMatcher::max)).apply((Applicative)instance, RangedValueMatcher::new));
        final static public PacketCodec<ByteBuf, RangedValueMatcher> PACKET_CODEC = PacketCodec.tuple(PacketCodecs.optional(PacketCodecs.STRING), RangedValueMatcher::min, PacketCodecs.optional(PacketCodecs.STRING), RangedValueMatcher::max, RangedValueMatcher::new);
    ...
    {
        final static public Codec<ExactValueMatcher> CODEC = Codec.STRING.xmap(ExactValueMatcher::new, ExactValueMatcher::value);
        final static public PacketCodec<ByteBuf, ExactValueMatcher> PACKET_CODEC = PacketCodecs.STRING.xmap(ExactValueMatcher::new, ExactValueMatcher::value);
