...
{
    final static public ComponentsPredicate EMPTY = new ComponentsPredicate(ComponentMapPredicate.EMPTY, Map.of());
    final static public MapCodec<ComponentsPredicate> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)ComponentMapPredicate.CODEC.optionalFieldOf("components", (Object)ComponentMapPredicate.EMPTY).forGetter(ComponentsPredicate::exact), (App)ComponentPredicate.PREDICATES_MAP_CODEC.optionalFieldOf("predicates", Map.of()).forGetter(ComponentsPredicate::partial)).apply((Applicative)instance, ComponentsPredicate::new));
    final static public PacketCodec<RegistryByteBuf, ComponentsPredicate> PACKET_CODEC = PacketCodec.tuple(ComponentMapPredicate.PACKET_CODEC, ComponentsPredicate::exact, ComponentPredicate.PREDICATES_MAP_PACKET_CODEC, ComponentsPredicate::partial, ComponentsPredicate::new);
    ...
    @Override
    public boolean test(Object components) {
        return this.test((ComponentsAccess)components);
    }

    public static class Builder {
        private ComponentMapPredicate exact = ComponentMapPredicate.EMPTY;
        final private ImmutableMap.Builder<ComponentPredicate.Type<?>, ComponentPredicate> partial = ImmutableMap.builder();
