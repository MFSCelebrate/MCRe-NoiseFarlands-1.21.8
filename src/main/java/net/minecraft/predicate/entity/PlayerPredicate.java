...
public record PlayerPredicate(NumberRange.IntRange experienceLevel, GameModeList gameMode, List<StatMatcher<?>> stats, Object2BooleanMap<RegistryKey<Recipe<?>>> recipes, Map<Identifier, AdvancementPredicate> advancements, Optional<EntityPredicate> lookingAt, Optional<InputPredicate> input) implements EntitySubPredicate
{
    ...
    public boolean test(Entity entity, ServerWorld world, @Nullable Vec3d pos) {
        ...
        if (this.lookingAt.isPresent()) {
            ...
            EntityHitResult entityHitResult = ProjectileUtil.getEntityCollision(serverPlayerEntity.net_minecraft_server_world_ServerWorld_getWorld(), serverPlayerEntity, vec3d, vec3d3, new Box(vec3d, vec3d3).expand(1.0), hitEntity -> !hitEntity.isSpectator(), 0.0f);
    ...
    record StatMatcher<T>(StatType<T> type, RegistryEntry<T> value, NumberRange.IntRange range, Supplier<Stat<T>> stat) {
        final static public Codec<StatMatcher<?>> CODEC = Registries.STAT_TYPE.getCodec().dispatch(StatMatcher::type, StatMatcher::createCodec);
    ...
    extends Predicate<AdvancementProgress> {
        final static public Codec<AdvancementPredicate> CODEC = Codec.either(CompletedAdvancementPredicate.CODEC, AdvancementCriteriaPredicate.CODEC).xmap(Either::unwrap, predicate -> {
    ...
    public static class Builder {
        ...
        final private ImmutableList.Builder<StatMatcher<?>> stats = ImmutableList.builder();
        final private Object2BooleanMap<RegistryKey<Recipe<?>>> recipes = new Object2BooleanOpenHashMap();
        final private Map<Identifier, AdvancementPredicate> advancements = Maps.newHashMap();
    ...
    {
        final static public Codec<AdvancementCriteriaPredicate> CODEC = Codecs.object2BooleanMap(Codec.STRING).xmap(AdvancementCriteriaPredicate::new, AdvancementCriteriaPredicate::criteria);
    ...
    {
        final static public Codec<CompletedAdvancementPredicate> CODEC = Codec.BOOL.xmap(CompletedAdvancementPredicate::new, CompletedAdvancementPredicate::done);
