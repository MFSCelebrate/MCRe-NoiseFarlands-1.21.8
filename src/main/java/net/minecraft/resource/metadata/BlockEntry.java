...
public class BlockEntry {
    final static public Codec<BlockEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codecs.REGULAR_EXPRESSION.optionalFieldOf("namespace").forGetter(entry -> entry.namespace), (App)Codecs.REGULAR_EXPRESSION.optionalFieldOf("path").forGetter(entry -> entry.path)).apply((Applicative)instance, BlockEntry::new));
    final private Optional<Pattern> namespace;
    final private Predicate<String> namespacePredicate;
    final private Optional<Pattern> path;
    final private Predicate<String> pathPredicate;
    final private Predicate<Identifier> identifierPredicate;
