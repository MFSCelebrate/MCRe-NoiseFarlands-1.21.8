...
extends SimpleResourceReload<Summary> {
    ...
    final private Stopwatch reloadTimer = Stopwatch.createUnstarted();

    public static ResourceReload start(ResourceManager manager2, List<ResourceReloader> reloaders, Executor prepareExecutor, Executor applyExecutor, CompletableFuture<Unit> initialStage) {
        ProfiledResourceReload profiledResourceReload = new ProfiledResourceReload(reloaders);
        profiledResourceReload.start(prepareExecutor, applyExecutor, manager2, reloaders, (synchronizer, manager, reloader, prepare, apply) -> {
            AtomicLong atomicLong = new AtomicLong();
            AtomicLong atomicLong2 = new AtomicLong();
            AtomicLong atomicLong3 = new AtomicLong();
            AtomicLong atomicLong4 = new AtomicLong();
            CompletableFuture<Void> completableFuture = reloader.reload(synchronizer, manager, ProfiledResourceReload.getProfiledExecutor(prepare, atomicLong, atomicLong2, reloader.getName()), ProfiledResourceReload.getProfiledExecutor(apply, atomicLong3, atomicLong4, reloader.getName()));
