...
extends FileSystemProvider {
    final static public String SCHEME = "x-mc-link";
    ...
    public SeekableByteChannel newByteChannel(Path path, Set<? extends OpenOption> options, FileAttribute<?> ... attrs) throws IOException {
        ...
        Path path2 = ResourceFileSystemProvider.toResourcePath(path).net_minecraft_resource_fs_ResourcePath_toAbsolutePath().toPath();
        if (path2 == null) {
    ...
    public DirectoryStream<Path> newDirectoryStream(Path dir, final DirectoryStream.Filter<? super Path> filter) throws IOException {
        final ResourceFile.Directory directory = ResourceFileSystemProvider.toResourcePath(dir).net_minecraft_resource_fs_ResourcePath_toAbsolutePath().toDirectory();
        if (directory == null) {
    ...
    public FileStore getFileStore(Path path) {
        return ResourceFileSystemProvider.toResourcePath(path).net_minecraft_resource_fs_ResourceFileSystem_getFileSystem().getStore();
    }
    ...
    public <A extends BasicFileAttributes> A readAttributes(Path path, Class<A> type, LinkOption ... options) throws IOException {
        ResourcePath resourcePath = ResourceFileSystemProvider.toResourcePath(path).net_minecraft_resource_fs_ResourcePath_toAbsolutePath();
