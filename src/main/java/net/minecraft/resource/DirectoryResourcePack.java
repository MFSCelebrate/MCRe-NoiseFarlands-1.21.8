...
extends AbstractFileResourcePack {
    ...
    final static private Joiner SEPARATOR_JOINER = Joiner.on((String)"/");
    ...
    public static void findResources(String namespace, Path path, List<String> prefixSegments, ResourcePack.ResultConsumer consumer) {
        ...
        try {
            try (Stream<Path> stream = Files.find(path2, Integer.MAX_VALUE, DirectoryResourcePack::isRegularFile, new FileVisitOption[0]);){
        ...
        catch (NoSuchFileException | NotDirectoryException stream) {
            return;
        }
    ...
    public Set<String> getNamespaces(ResourceType type) {
        ...
        try {
            try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(path);){
                ...
                if (directoryStream == null) return set;
            }
            return set;
        }
        catch (NoSuchFileException | NotDirectoryException directoryStream) {
            return set;
        }
        catch (IOException iOException) {
