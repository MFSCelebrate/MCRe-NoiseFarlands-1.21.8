...
public record SetBlockPropertiesEnchantmentEffect(BlockStateComponent properties, Vec3i offset, Optional<RegistryEntry<GameEvent>> triggerGameEvent) implements EnchantmentEntityEffect
{
    ...
    public void apply(ServerWorld world, int level, EnchantmentEffectContext context, Entity user, Vec3d pos) {
        ...
        BlockPos blockPos = BlockPos.ofFloored(pos).net_minecraft_util_math_BlockPos_add(this.offset);
        BlockState blockState = user.net_minecraft_world_World_getWorld().getBlockState(blockPos);
        if (blockState != (blockState2 = this.properties.applyToState(blockState)) && user.net_minecraft_world_World_getWorld().setBlockState(blockPos, blockState2, Block.NOTIFY_ALL)) {
