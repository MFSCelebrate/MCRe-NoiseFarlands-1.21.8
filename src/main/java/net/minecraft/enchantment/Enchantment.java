...
public record Enchantment(Text description, Definition definition, RegistryEntryList<Enchantment> exclusiveSet, ComponentMap effects) {
    ...
    public record Cost(int base, int perLevelAboveFirst) {
        final static public Codec<Cost> CODEC = RecordCodecBuilder.create(instance -> instance.group((App)Codec.INT.fieldOf("base").forGetter(Cost::base), (App)Codec.INT.fieldOf("per_level_above_first").forGetter(Cost::perLevelAboveFirst)).apply((Applicative)instance, Cost::new));
    ...
    public static class Builder {
        ...
        final private Map<ComponentType<?>, List<?>> effectLists = new HashMap();
        final private ComponentMap.Builder effectMap = ComponentMap.builder();
        ...
        public <E> Builder addEffect(ComponentType<List<EnchantmentEffectEntry<E>>> effectType, E effect, LootCondition.Builder requirements) {
            this.getEffectsList(effectType).add(new EnchantmentEffectEntry<E>(effect, Optional.of(requirements.net_minecraft_loot_condition_LootCondition_build())));
            return this;
        }
        ...
        public <E> Builder addEffect(ComponentType<List<TargetedEnchantmentEffect<E>>> type, EnchantmentEffectTarget enchanted, EnchantmentEffectTarget affected, E effect, LootCondition.Builder requirements) {
            this.getEffectsList(type).add(new TargetedEnchantmentEffect<E>(enchanted, affected, effect, Optional.of(requirements.net_minecraft_loot_condition_LootCondition_build())));
            return this;
        }
