...
public class Batches {
    final static private int BATCH_SIZE = 50;
    final static public Decorator DEFAULT_DECORATOR = (instance, world) -> Stream.of(new GameTestState(instance, BlockRotation.NONE, world, TestAttemptConfig.once()));
    ...
    public static List<GameTestBatch> batch(Collection<RegistryEntry.Reference<TestInstance>> instances, Decorator decorator, ServerWorld world) {
        ...
        return map.entrySet().stream().flatMap(entry -> {
            ...
            return Streams.mapWithIndex(Lists.partition((List)list, 50).stream(), (states, index) -> Batches.create(states, registryEntry, (int)index));
        }).toList();
    }
