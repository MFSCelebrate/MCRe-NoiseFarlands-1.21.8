...
implements TestRunContext.TestStructureSpawner {
    final static private int MARGIN_X = 5;
    final static private int MARGIN_Z = 6;
    final private int testsPerRow;
    private int testsInCurrentRow;
    private Box box;
    final private BlockPos.Mutable mutablePos;
    final private BlockPos origin;
    final private boolean clearBeforeBatch;
    private float maxX = -1.0f;
    final private Collection<GameTestState> statesToClear = new ArrayList<GameTestState>();
    ...
    public Optional<GameTestState> spawnStructure(GameTestState gameTestState) {
        ...
        if (++this.testsInCurrentRow >= this.testsPerRow) {
            ...
            this.mutablePos.net_minecraft_util_math_BlockPos$Mutable_setX(this.origin.getX());
            this.box = new Box(this.mutablePos);
        }
