...
public class TestBootstrap {
    ...
    final static private String DEFAULT_UNIVERSE = "gametestserver";
    final static private String DEFAULT_WORLD = "gametestworld";
    final static private OptionParser PARSER = new OptionParser();
    final static private OptionSpec<String> UNIVERSE = PARSER.accepts("universe", "The path to where the test server world will be created. Any existing folder will be replaced.").withRequiredArg().defaultsTo((Object)"gametestserver", (Object[])new String[0]);
    final static private OptionSpec<File> REPORT = PARSER.accepts("report", "Exports results in a junit-like XML report at the given path.").withRequiredArg().ofType(File.class);
    final static private OptionSpec<String> TESTS = PARSER.accepts("tests", "Which test(s) to run (namespaced ID selector using wildcards). Empty means run all.").withRequiredArg();
    final static private OptionSpec<Boolean> VERIFY = PARSER.accepts("verify", "Runs the tests specified with `test` or `testNamespace` 100 times for each 90 degree rotation step").withRequiredArg().ofType(Boolean.class).defaultsTo((Object)false, (Object[])new Boolean[0]);
    final static private OptionSpec<String> PACKS = PARSER.accepts("packs", "A folder of datapacks to include in the world").withRequiredArg();
    final static private OptionSpec<Void> HELP = PARSER.accepts("help").forHelp();

    @UsesSystemOut(reason="Using System.err due to no bootstrap")
    public static void run(String[] args, Consumer<String> universeCallback) throws Exception {
        ...
        MinecraftServer.startServer(thread -> {
            TestServer testServer = TestServer.create(thread, session, resourcePackManager, TestBootstrap.get(optionSet, TESTS), optionSet.has(VERIFY));
    ...
    private static void copyPacks(String universe, String packDir) throws IOException {
        ...
        if (!Files.exists(path2 = Paths.get(packDir, new String[0]), new LinkOption[0])) return;
